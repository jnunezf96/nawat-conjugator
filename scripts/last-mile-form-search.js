...
const stubDocument = {
  ...
  createElement() { return { className: '', textContent: '', appendChild() {}, querySelector() { return null; } }; },
  addEventListener() {},
};
...
const filterFormSearchStemsToKnownVerbs = vm.runInContext('filterFormSearchStemsToKnownVerbs', sandbox);
const deriveVerbstemCandidatesFromForm = vm.runInContext('deriveVerbstemCandidatesFromForm', sandbox);
...
function checkLastMile(originalVerbToken) {
  ...
  for (const candidate of candidateVerbTokens) {
    ...
    for (const form of candidateForms) {
      ...
      if (matchesVerbstemSuggestion(originalVerbToken, filtered, {
        hasObjectPrefix: stemGuess.hasObjectPrefix,
        allowBoth,
      })) {
