...
- Lessons 42-43 have diagnostic clause/modification boundary metadata only.
...
- `src/core/nnc/adjectival` now records non-generative Lessons 40-41 adjectival NNC function boundary metadata and false-positive classifications for current `adjetivo` route surfaces and `nominalizationProfile.adjectivalFunction`.
...
- Andrews 39.9 characteristic-property embed continuation now consumes possessive `#3 salida` forms too: `numikkayu` omits the Nawat `-yu` matrix, promotes `nu-` to the outside object prefix, and reaches `nechmikkachikawa`.
...
- Current Lessons 42-43 metadata records the boundary between `adjetivo` route outputs, `nominalizationProfile`, ordinary NNC `formulaSlots`, translation adjectives, and confirmed modifier/head clause evidence. It does not generate forms or change nominal/adjectival output.
...
- Lesson 35 ownerhood, restricted/general-use preterit-agentive stems, and compound-embed nominalized stems are boundary-classified only; generation remains pending.
...
3. Gather confirmed Nawat/Pipil evidence before adding ownerhood, complete `z/liz` fixture coverage beyond the current `s/lis` action-nominal motor, or new patientive-family generation/fixtures.
...
5. Keep `core/nnc/adjectival` non-generative until confirmed Nawat/Pipil examples justify adjectival NNC function schema, diagnostics, generation, or UI.
6. Keep `core/clause/modification` non-generative until confirmed Nawat/Pipil modifier/head clause examples justify AST schema, diagnostics, generation, or UI.
7. Design a clause/modification AST for Lessons 42-43 only after confirmed examples and schema questions are settled.
...
- Current Lesson 44 metadata records the boundary between legacy adverbio output, adverb translations, particle labels, ordinary NNC/VNC outputs, and confirmed adverbial NNC/VNC or clause evidence. It does not generate forms or change adverbio output.
- Generated legacy adverbio rows may expose diagnostic `adverbialNuclearFrame` metadata for source VNC, source valency, and manner-surface scope. Rendering may show `Adverbial nuclear`, `Fuente VNC`, and `Alcance: adverbio heredado`; this remains display-only and does not add adverbial forms.
- Current route metadata keeps the legacy adverb output outside the linked output-composition surface.
- Lessons 45-47 relational NNCs have diagnostic boundary metadata only; they do not have fixture data or generation.
...
1. Keep `core/clause/adverbial` non-generative until confirmed Nawat/Pipil adverbial NNC/VNC or clause examples justify data, schema, generation, or UI.
2. Keep the current Lesson 44 adverbio surface separate from full adverbial NNC/VNC modeling.
3. Keep `core/nnc/relational` non-generative until confirmed Nawat/Pipil relational examples justify data, schema, generation, or UI.
