...
- Current Lessons 42-43 metadata records the boundary between `adjetivo` route outputs, `nominalizationProfile`, ordinary NNC `formulaSlots`, the opt-in adjectival-NNC function route, translation adjectives, and confirmed modifier/head clause evidence. It does not generate a modifier/head AST.
...
5. Keep `core/nnc/adjectival` limited to the current opt-in absolutive NNC function route until confirmed Nawat/Pipil examples justify broader adjectival NNC paradigms, modifier/head syntax, or UI.
