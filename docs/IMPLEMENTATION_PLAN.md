...
- Andrews 36.2-36.3 `agentivo` is treated as a customary-present agentive NNC route: the generated `ni` remains visible in the Nawat surface but belongs to the predicate stem, while the subject-number connector is exposed as `Ø`, `met`, or `wan`.
...
- Andrews 36.6 `instrumentivo` is treated as a two-source NNC route: absolutive output uses the customary-present impersonal/nonactive source, possessive output uses the imperfect active source, source reflexive `mu` maps to shuntline `ne`, and `ni/ya` source-tense material remains inside the predicate stem rather than the subject-number connector.
