...
- Keep Lessons 42-43 `core/clause/modification` as a non-generative adjectival-modification AST contract: it may compose supplied generated head/modifier outputs and record order/marker/scope/link metadata, but it must remain separate from ordinary NNC `formulaSlots`, parser/search detection, UI actions, and new word generation until confirmed Nawat/Pipil clause examples justify more.
- Keep Lesson 44 `core/clause/adverbial` as a non-generative adverbial nuclear-clause frame contract; `pasado-remoto-adverbio-activo` remains partial legacy adverbio surface support only.
- `adverbialNuclearClauseFrame` metadata/rendering may explain the source VNC, source valency, first-degree subject-pronoun adverbialization, and manner-surface scope for generated legacy adverbio rows, but it must remain tied to `#3 salida` output and must not generate new adverbial forms.
