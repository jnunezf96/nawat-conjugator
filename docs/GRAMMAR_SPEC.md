...
- `src/tests/nnc_adjectival.test.js` proves Andrews 40.1 opt-in adjectival NNC function generation, including the absolutive-state requirement, Andrews 41.1 intensified adjectival output from generated formula slots, and Andrews 41.2 compound-source metadata for generated `adjetivo` output, while keeping current `adjetivo` route output and `nominalizationProfile.adjectivalFunction` as unconfirmed full modification evidence.
