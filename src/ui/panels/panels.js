...
function getAndrewsSourceTargetRouteRegistryRoutes() {
    ...
    if (typeof buildAndrewsSourceGatedDerivationalRouteRegistry !== "function") {
        return null;
    }
    const registry = buildAndrewsSourceGatedDerivationalRouteRegistry();
...
function getAndrewsSourceTargetRouteRegistryMatches(routeIds = []) {
    ...
    return (Array.isArray(routeIds) ? routeIds : [])
        .map((routeId) => registry.routesById.get(routeId))
        .filter(Boolean);
}

function getAndrewsTenseSourceTargetRouteSpec(tenseValue = "", mode = TENSE_MODE.verbo) {
    ...
    if (typeof isPredicateNominalTense === "function" && isPredicateNominalTense(normalizedTense)) {
        return {
    ...
    if (
        normalizedMode === TENSE_MODE.verbo
        && typeof NONACTIVE_SUFFIX_ORDER !== "undefined"
        && Array.isArray(NONACTIVE_SUFFIX_ORDER)
        && NONACTIVE_SUFFIX_ORDER.includes(normalizedTense)
    ) {
        return {
    ...
    if (normalizedMode === TENSE_MODE.verbo && normalizedTense) {
        return {
    ...
    if (isNominalTenseMode(normalizedMode)) {
        return {
    ...
    if (normalizedMode === TENSE_MODE.particula) {
        return {
...
function getAndrewsTenseSourceTargetRouteAuthorityFrame(tenseValue = "", mode = TENSE_MODE.verbo) {
    ...
    const routeKinds = Array.from(new Set(registryMatches
        .map((route) => route.routeKind || route.operation || "")
        .filter(Boolean)));
    const routeFamilies = Array.from(new Set(registryMatches
        .map((route) => route.routeFamily || "")
        .filter(Boolean)));
    const operationalLayer = getAndrewsCnvCnnOperationalLayerForTense(tenseValue, mode);
    const operationalCoverageAudit = operationalLayer
        && typeof auditAndrewsCnvCnnOperationalLayerCoverage === "function"
        ? auditAndrewsCnvCnnOperationalLayerCoverage(operationalLayer.label || tenseValue)
        : null;
    const routeSuboperations = Array.isArray(operationalLayer?.operations)
        ? operationalLayer.operations
        : [];
    ...
    return {
        ...
        registryStatus: !routeIds.length
            ? "no-route-id"
            : (registryMatches.length ? "registry-match" : "registry-pending-or-unavailable"),
        ...
        generationGate: primaryRoute?.generationAllowed === true
            ? "andrews-route-generation-allowed"
            : "andrews-route-source-gated-or-diagnostic",
        operationalLayerKind: operationalLayer?.kind || "",
        routeSuboperationCount: operationalLayer?.operationCount || 0,
        routeSuboperationIds: Array.isArray(operationalLayer?.operationIds)
            ? Array.from(operationalLayer.operationIds)
            : [],
        routeSuboperationItems,
        routeSuboperationFamilies: Array.from(new Set(routeSuboperations
            .map((operation) => operation.family || "")
            .filter(Boolean))),
        routeSuboperationSections: Array.from(new Set(routeSuboperations
            .map((operation) => operation.andrewsSection || "")
            .filter(Boolean))),
        routeSuboperationSourceRequirementKeys: Array.isArray(operationalLayer?.sourceRequirementKeys)
            ? Array.from(operationalLayer.sourceRequirementKeys)
            : [],
        routeSuboperationTransformKeys: Array.isArray(operationalLayer?.transformKeys)
            ? Array.from(operationalLayer.transformKeys)
            : [],
        routeSuboperationBuildKeys: Array.isArray(operationalLayer?.buildKeys)
            ? Array.from(operationalLayer.buildKeys)
            : [],
        ...
        routeSuboperationMissingSectionCount: Array.isArray(operationalCoverageAudit?.missingSections)
            ? operationalCoverageAudit.missingSections.length
            : 0,
        routeSuboperationMissingSections: Array.isArray(operationalCoverageAudit?.missingSections)
            ? Array.from(operationalCoverageAudit.missingSections)
            : [],
...
function getAndrewsCnvCnnOperationalLayerForTense(tenseValue = "", mode = TENSE_MODE.verbo) {
    const normalizedTense = String(tenseValue || "").trim();
    if (!normalizedTense || typeof getAndrewsCnvCnnOperationalLayer !== "function") {
        return null;
    }
    const layer = getAndrewsCnvCnnOperationalLayer(normalizedTense);
...
function getAndrewsCnvCnnOperationalLayerDisplayText(sourceTargetRoute = null) {
    ...
    const sections = Array.isArray(sourceTargetRoute.routeSuboperationSections)
        ? sourceTargetRoute.routeSuboperationSections.slice(0, 3).filter(Boolean)
        : [];
...
function syncAndrewsTenseOperationalLayerElement(element = null, sourceTargetRoute = null) {
    ...
    if (!summary && typeof document !== "undefined" && typeof document.createElement === "function") {
    ...
    summary.title = Array.isArray(sourceTargetRoute.routeSuboperationIds)
        ? sourceTargetRoute.routeSuboperationIds.join(" | ")
        : "";
}
...
function syncAndrewsTenseBlockOperationalLayerElement(element = null, sourceTargetRoute = null) {
    ...
    if (!title || typeof document === "undefined" || typeof document.createElement !== "function") {
        if (
            title === null
            && element.dataset
    ...
    panel.dataset.andrewsRouteSuboperationIds = Array.isArray(sourceTargetRoute.routeSuboperationIds)
        ? sourceTargetRoute.routeSuboperationIds.join("|")
        : "";
    ...
    const summaryCoverage = document.createElement("span");
    summaryCoverage.className = "tense-block-operational-layer__summary-coverage";
    summaryCoverage.textContent = sourceTargetRoute.routeSuboperationCoverageComplete
        ? "cobertura Andrews"
        : `${sourceTargetRoute.routeSuboperationMissingSectionCount || 0} faltantes`;
...
function appendAndrewsOperationalLayerOperationRows(list = null, sourceTargetRoute = null, classPrefix = "tense-block-operational-layer") {
    if (!list || typeof document === "undefined" || typeof document.createElement !== "function") {
        return;
    }
    const items = Array.isArray(sourceTargetRoute.routeSuboperationItems)
        ? sourceTargetRoute.routeSuboperationItems
        : [];
...
function syncAndrewsTenseTabsOperationalLayerPanel(root = null, {
    ...
    if (!scope || typeof scope.querySelector !== "function" || typeof document === "undefined" || typeof document.createElement !== "function") {
        return null;
    }
    ...
    const activeTab = tabs.find((tab) => tab.classList?.contains("is-active"))
        || tabs.find((tab) => String(tab.getAttribute?.("aria-selected") || "") === "true")
        || tabs[0]
        || null;
    ...
    const descriptor = getAndrewsTenseAuthorityDomDescriptor(activeTab, { mode });
    ...
    coverageBadge.textContent = sourceTargetRoute.routeSuboperationCoverageComplete
        ? "cobertura Andrews"
        : `${sourceTargetRoute.routeSuboperationMissingSectionCount || 0} faltantes`;
...
function getAndrewsTenseAuthorityFrame(tenseValue = "", mode = TENSE_MODE.verbo) {
    const normalizedMode = String(mode || "").trim();
    const normalizedTense = String(tenseValue || "").trim();
    if (normalizedTense === "selection-required") {
        return {
    ...
    if (
        ...
        const coreFrame = cloneAndrewsTenseAuthorityFrame(
            getAndrewsCnvTenseLogicAuthorityFrame(normalizedTense)
        );
        if (coreFrame) {
            return coreFrame;
        }
...
function getAndrewsTenseGenerationGateValue(tenseValue = "", mode = TENSE_MODE.verbo) {
    return getAndrewsTenseGenerationGateFrame(
        getAndrewsTenseAuthorityFrame(tenseValue, mode)
    ).generationGate || "";
}

function isAndrewsCnvTenseGenerationGateAllowed(tenseValue = "", mode = TENSE_MODE.verbo) {
...
function getAndrewsTenseExecutorGateFrame(authorityFrame = null) {
    ...
    return {
        ...
        routeStage: generationAllowed
            ? "cnv-finite-output"
            : "andrews-cnv-tense-logic-gate",
        generationAllowed,
        formulaShellPolicy: generationAllowed
            ? "formula-shell-allowed"
            : "blocked-before-formula-shell",
        surfacePolicy: generationAllowed
            ? "orthography-bridge-required"
            : "blocked-before-surface",
        fallbackPolicy: generationAllowed
            ? "surface-output-not-grammar-authority"
            : "blocked-no-target-stem-fallback",
    };
}
...
function getAndrewsTenseTabSelectionAuthorityState({
    ...
    const normalizedMode = String(mode || "").trim() || TENSE_MODE.verbo;
    ...
    if (
        normalizedMode === TENSE_MODE.verbo
        && generationGate.generationGate !== "andrews-licensed-generation"
    ) {
    ...
    const outputAvailability = hasOutput === true
        ? "surface-available"
        : (hasOutput === false ? "surface-unavailable" : "surface-uncomputed");
    const blocked = blockedReasons.length > 0;
    const outputAvailabilityRole = blocked
        ? "selection-hard-gate"
        : (hasOutput === false
            ? "orthography-output-probe-not-grammar-gate"
            : "orthography-realization");
    return {
        frame,
        generationGate,
...
function buildAndrewsTenseTabClickAuthorityModel(selectionTargetFrame = null, diagnosticId = "") {
    const hasSelectionTarget = selectionTargetFrame
        && typeof selectionTargetFrame === "object"
        && selectionTargetFrame.kind === "andrews-tense-tab-selection-audit-target-frame";
    const sourceFrame = hasSelectionTarget
        ? {
            ...
            selectionBlockedReasons: String(selectionTargetFrame.selectionBlocked || "")
                .split("|")
...
function getAndrewsTenseTabClickAuthorityState(element = null) {
    ...
    const clickAuthorityModel = buildAndrewsTenseTabClickAuthorityModel(
        selectionModelTarget.targetFrame,
        selectionModelTarget.diagnosticId
    );
    const targetFrame = clickAuthorityModel.targetFrame || {};
    const blockedReasons = Array.isArray(targetFrame.blockedReasons)
        ? targetFrame.blockedReasons
        : [];
    return {
...
function buildAndrewsTenseTabSelectionAuditModel(state = {}, {
    ...
    const generationGate = state.generationGate && typeof state.generationGate === "object"
        ? state.generationGate
        : {};
    const sourceFrame = {
        ...
        blockedReasons: Array.isArray(state.blockedReasons)
            ? state.blockedReasons.map((entry) => String(entry || "").trim()).filter(Boolean)
            : [],
...
function getAndrewsTenseTabSelectionAuditModelTarget(element = null) {
    const model = element?.andrewsTenseTabSelectionAuditModel && typeof element.andrewsTenseTabSelectionAuditModel === "object"
        ? element.andrewsTenseTabSelectionAuditModel
        : null;
    const operationFrame = model?.operationFrame && typeof model.operationFrame === "object"
        ? model.operationFrame
        : null;
    if (
        !model
        || model.kind !== "andrews-tense-tab-selection-audit-model"
        || !operationFrame
        || operationFrame.kind !== "andrews-tense-tab-selection-audit-operation-frame"
        || operationFrame.status !== "authorized"
        || operationFrame.operation !== "audit-tense-tab-selection-from-state-frame"
    ) {
        return {
            targetFrame: null,
    ...
    const sourceFrame = operationFrame.sourceFrame && typeof operationFrame.sourceFrame === "object"
        ? operationFrame.sourceFrame
        : null;
    const targetFrame = operationFrame.targetFrame && typeof operationFrame.targetFrame === "object"
        ? operationFrame.targetFrame
        : null;
    if (
        !sourceFrame
        || sourceFrame.kind !== "andrews-tense-tab-selection-audit-source-frame"
        || !targetFrame
        || targetFrame.kind !== "andrews-tense-tab-selection-audit-target-frame"
    ) {
        return {
            targetFrame: null,
    ...
    const expected = {
        ...
        selectionSurfaceProbeRole: sourceFrame.outputAvailabilityRole || "",
        selectionNawatEvidenceRole: sourceFrame.outputAvailabilityRole || "",
        selectionBlocked: Array.isArray(sourceFrame.blockedReasons)
            ? sourceFrame.blockedReasons.join("|")
            : "",
    };
    const mismatched = Object.entries(expected)
        .some(([key, expectedValue]) => String(targetFrame[key] ?? "").trim() !== String(expectedValue ?? ""));
    if (
        mismatched
        || (targetFrame.blocked === true && (targetFrame.selected === true || targetFrame.ariaSelected === true))
        || targetFrame.blockedSelected !== (targetFrame.blocked === true && targetFrame.selected === true)
    ) {
        return {
            targetFrame: null,
...
function applyAndrewsTenseTabSelectionAuthorityDataset(element = null, options = {}) {
    ...
    const selectedNow = Boolean(classList?.contains("is-active"))
        || (typeof element.getAttribute === "function" && String(element.getAttribute("aria-selected") || "") === "true");
    const ariaSelectedNow = typeof element.getAttribute === "function"
        && String(element.getAttribute("aria-selected") || "") === "true";
    ...
    element.dataset.andrewsSelectionSelectedRole = state.blocked
        ? "blocked-gate-cannot-be-selected"
        : "ui-selection-state";
...
function applyAndrewsTenseAuthorityDataset(element = null, {
    ...
    const generationGate = getAndrewsTenseGenerationGateFrame(frame);
    const isCoreCnvMode = String(mode || "") === TENSE_MODE.verbo
        && frame.scope !== "andrews-output-gate";
    const coreFrame = isCoreCnvMode && typeof getAndrewsCnvTenseLogicAuthorityFrame === "function"
        ? getAndrewsCnvTenseLogicAuthorityFrame(tenseValue)
        : null;
    const coreGenerationGate = coreFrame && typeof getAndrewsCnvTenseLogicGenerationGateFrame === "function"
        ? getAndrewsCnvTenseLogicGenerationGateFrame(coreFrame)
        : null;
    ...
    if (status) {
        ...
        element.title = existingTitle && !existingTitle.includes(status)
            ? `${existingTitle} ${statusWithOperations}`
            : (existingTitle || statusWithOperations);
    }
    return frame;
}
...
function getAndrewsTenseAuthorityExpectedDataset(element = null, {
    ...
    const normalizedMode = String(descriptor.mode || "").trim() || TENSE_MODE.verbo;
    const normalizedTense = String(descriptor.tenseValue || "").trim();
    const canCompare = Boolean(normalizedTense)
        || isNominalTenseMode(normalizedMode)
        || normalizedMode === TENSE_MODE.particula;
    if (!canCompare) {
        return null;
    }
    const frame = getAndrewsTenseAuthorityFrame(normalizedTense, normalizedMode);
    const generationGate = getAndrewsTenseGenerationGateFrame(frame);
    const isCoreCnvMode = normalizedMode === TENSE_MODE.verbo
        && frame.scope !== "andrews-output-gate";
    const coreFrame = isCoreCnvMode && typeof getAndrewsCnvTenseLogicAuthorityFrame === "function"
        ? getAndrewsCnvTenseLogicAuthorityFrame(normalizedTense)
        : null;
...
function getAndrewsTenseAuthorityCanonicalMismatches(element = null, options = {}) {
    ...
    return Object.entries(expected.dataset)
        .filter(([key, expectedValue]) => String(dataset[key] || "") !== String(expectedValue || ""))
        .map(([key]) => `${key.replace(/[A-Z]/g, (match) => `-${match.toLowerCase()}`)}-mismatch`);
}
...
function getAndrewsTenseAuthorityExpectedClasses(element = null, options = {}) {
    ...
    const expectedState = new Map([
        [`${prefix}--andrews-authority`, scope === "andrews-licensed"],
        [`${prefix}--nawat-extension`, scope === "nawat-extension"],
        [`${prefix}--andrews-nominal-route`, scope === "andrews-nominal-route"],
        [`${prefix}--andrews-particle-boundary`, scope === "andrews-particle-boundary"],
        [`${prefix}--andrews-output-gate`, scope === "andrews-output-gate"],
        [`${prefix}--andrews-generation-gate`, generationGate.generationGate === "andrews-licensed-generation"],
        [`${prefix}--surface-evidence-only`, generationGate.outputRole === "surface-evidence-only"],
        [`${prefix}--andrews-unclassified`, scope === "unknown"],
        [`${prefix}--source-target-cnv-cnn`, sourceTargetRoute === "CNV->CNN"],
        [`${prefix}--source-target-cnn-cnn`, sourceTargetRoute === "CNN->CNN"],
        [`${prefix}--source-target-cnn-cnv`, sourceTargetRoute === "CNN->CNV"],
        [`${prefix}--source-target-cnv-cnv`, sourceTargetRoute === "CNV->CNV"],
        [`${prefix}--source-target-mixed`, routeClass === "mixed-compound-source-target-route"],
    ]);
    return {
        prefix,
        expectedState,
...
function getAndrewsTenseAuthorityClassMismatches(element = null, options = {}) {
    ...
    return Array.from(expected.expectedState.entries())
        .filter(([className, shouldHaveClass]) => classList.contains(className) !== shouldHaveClass)
        .map(([className, shouldHaveClass]) => (
            shouldHaveClass
                ? `${className}-class-missing`
                : `${className}-class-stale`
        ));
}
...
function getAndrewsTenseBlockOutputRowAuditRecord(row = null) {
    const model = row?.andrewsTenseBlockOutputRowAuditModel && typeof row.andrewsTenseBlockOutputRowAuditModel === "object"
        ? row.andrewsTenseBlockOutputRowAuditModel
        : null;
    const operationFrame = model?.operationFrame && typeof model.operationFrame === "object"
        ? model.operationFrame
        : null;
    if (
        !model
        || model.kind !== "andrews-tense-block-output-row-audit-model"
        || !operationFrame
        || operationFrame.kind !== "andrews-tense-block-output-row-audit-operation-frame"
        || operationFrame.status !== "authorized"
        || operationFrame.operation !== "audit-output-row-from-grammar-frame"
    ) {
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "andrews-tense-block-output-row-audit-source-frame"
        || !targetFrame
        || targetFrame.kind !== "andrews-tense-block-output-row-audit-target-frame"
    ) {
    ...
    const mismatched = Object.entries(expected)
        .some(([key, expectedValue]) => String(targetFrame[key] || "").trim() !== expectedValue);
    if (mismatched) {
...
function getAndrewsTenseBlockOutputAuditRecord(element = null) {
    ...
    const rowNodes = isBlock && typeof element?.querySelectorAll === "function"
        ? Array.from(element.querySelectorAll(".conjugation-row"))
        : [];
    const rowCount = rowNodes.length;
    const placeholderCount = isBlock && typeof element?.querySelectorAll === "function"
        ? Array.from(element.querySelectorAll(".tense-placeholder")).length
        : 0;
    const rowGrammarRecords = rowNodes.map((row) => getAndrewsTenseBlockOutputRowAuditRecord(row));
    const getDistinctRowValues = (key) => Array.from(new Set(
        rowGrammarRecords
    ...
    const grammarRouteContractMissingCount = rowGrammarRecords
        .filter((record) => (
            !String(record.grammarRouteFamily || "").trim()
            || !String(record.grammarRouteStage || "").trim()
            || !String(record.grammarGenerationAllowed || "").trim()
        )).length;
    const hardBlockedGates = new Set([
        "not-andrews-grammar-gate",
        "unclassified-andrews-frame-required",
        "route-selection-required",
    ]);
    const hardBlockedRouteStages = new Set([
        "andrews-cnv-tense-logic-gate",
    ]);
    const grammarGenerationAllowedCount = rowGrammarRecords
        .filter((record) => String(record.grammarGenerationAllowed || "") === "true").length;
    const grammarGenerationBlockedRowCount = rowGrammarRecords
        .filter((record) => String(record.grammarGenerationAllowed || "") === "false").length;
    const blockedGrammarRows = rowGrammarRecords
        .filter((record) => String(record.grammarGenerationAllowed || "") === "false");
    const generatedGrammarRows = rowGrammarRecords
        .filter((record) => String(record.grammarGenerationAllowed || "") === "true");
    const grammarBlockedResultOkCount = blockedGrammarRows
        .filter((record) => String(record.grammarResultOk || "").trim() === "true").length;
    const grammarGeneratedBlockedRouteContractCount = generatedGrammarRows
        .filter((record) => (
            hardBlockedRouteStages.has(String(record.grammarRouteStage || "").trim())
            || hardBlockedGates.has(String(record.grammarDiagnosticId || "").trim())
        )).length;
    const grammarGeneratedResultNotOkCount = generatedGrammarRows
        .filter((record) => String(record.grammarResultOk || "").trim() !== "true").length;
    const grammarLogicAuthorityMissingCount = generatedGrammarRows
        .filter((record) => String(record.grammarLogicAuthority || "").trim() !== "Andrews").length;
    const grammarSpellingEvidenceRoleMismatchCount = generatedGrammarRows
        .filter((record) => String(record.grammarSpellingEvidenceRole || "").trim() !== "orthography-realization-only").length;
    const grammarSourceContextAuthorityMismatchCount = generatedGrammarRows
        .filter((record) => {
            const value = String(record.grammarSourceContextTargetAuthority || "").trim();
            return value && !isAndrewsTargetAuthority(value);
        }).length;
    const grammarSourceEvidenceAuthorityMismatchCount = generatedGrammarRows
        .filter((record) => {
            const value = String(record.grammarSourceEvidenceTargetAuthority || "").trim();
            return value && !isAndrewsTargetAuthority(value);
        }).length;
    const grammarClassicalSpellingRoleMismatchCount = generatedGrammarRows
        .filter((record) => String(record.grammarClassicalSpellingRole || "").trim() !== "structural-only").length;
    const grammarOrthographyBoundaryMissingCount = generatedGrammarRows
        .filter((record) => String(record.grammarOrthographyBoundary || "").trim() !== "nawat-pipil-realization").length;
    const grammarSpellingAuthorityMismatchCount = generatedGrammarRows
        .filter((record) => !String(record.grammarSpellingAuthority || "").trim().startsWith("Nawat/Pipil orthography")).length;
    const grammarClassicalSurfaceImportNotBlockedCount = generatedGrammarRows
        .filter((record) => String(record.grammarClassicalSurfaceImport || "").trim() !== "blocked").length;
    ...
    const outputScope = !isBlock
        ...
        : (hardBlocked
            ? "blocked-output"
            : (effectiveGenerationGate === "andrews-licensed-generation"
                ? "andrews-generated-output"
                : (effectiveGenerationGate === "andrews-nominal-route-no-vnc-tns"
                    ? "andrews-nominal-output"
                    : (effectiveGenerationGate === "andrews-particle-boundary-no-vnc-tns"
                        ? "andrews-particle-output"
                        : (outputRole || "andrews-output-frame")))));
    return {
        isBlock,
        generationGate,
...
function getAndrewsTenseAuthorityDatasetAuditRecord(element = null) {
    ...
    const requiredKeys = [
        "andrewsTenseAuthority",
        "andrewsTenseSource",
        "andrewsTenseSlot",
        "andrewsTenseFamily",
        "andrewsTenseMode",
        "andrewsGrammarLogicAuthority",
        "andrewsClassicalSpellingRole",
        "andrewsOrthographyBoundary",
        "nawatPipilOutputBoundary",
        "andrewsOutputSpellingAuthority",
        "andrewsOrthographyRealizationPath",
        "andrewsLogicRole",
        "andrewsGenerationGate",
        "andrewsOutputRole",
        "nawatPipilEvidenceRole",
        "classicalOutputImport",
        "andrewsRouteAuthority",
        "andrewsRouteLogicAuthority",
        "andrewsSourceTargetRoute",
        "andrewsSourceTargetRouteClass",
        "andrewsSourceFormulaType",
        "andrewsTargetFormulaType",
        "andrewsRouteRegistryStatus",
        "andrewsRouteUiHost",
        "andrewsRouteGenerationGate",
        "andrewsRouteGenerationAllowed",
        "andrewsRouteClassicalSpellingRole",
    ...
    if (dataset.andrewsTenseMode === TENSE_MODE.sustantivo && dataset.andrewsTargetFormulaType === "CNV") {
        diagnostics.push("nominal-output-tab-has-verbal-target-route");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.sustantivo
        && (dataset.andrewsSourceTargetRoute === "CNN->CNV" || dataset.andrewsSourceTargetRoute === "CNV->CNV")
    ) {
    ...
    if (
        elementContract.contract
        && dataset.andrewsAuthorityElementContract
        && dataset.andrewsAuthorityElementContract !== elementContract.contract
    ) {
        diagnostics.push("andrews-authority-element-contract-mismatch");
    }
    if (
        elementContract.expectedTag
        && dataset.andrewsAuthorityExpectedTag
        && dataset.andrewsAuthorityExpectedTag !== elementContract.expectedTag
    ) {
        diagnostics.push("andrews-authority-expected-tag-mismatch");
    }
    if (
        elementContract.renderedTag
        && dataset.andrewsAuthorityRenderedTag
        && dataset.andrewsAuthorityRenderedTag !== elementContract.renderedTag
    ) {
        diagnostics.push("andrews-authority-rendered-tag-mismatch");
    }
    if (
        elementContract.contract === "button.tense-tab"
        && (dataset.andrewsSelectionGate || element?.andrewsTenseTabSelectionAuditModel)
    ) {
        const selectionModelTarget = getAndrewsTenseTabSelectionAuditModelTarget(element);
        ...
        const ariaSelected = typeof element?.getAttribute === "function"
            ? String(element.getAttribute("aria-selected") || "")
            : "";
        const isSelectionMarkedSelected = String(dataset.andrewsSelectionSelected || "") === "true"
            || isSelectionActive
            || ariaSelected === "true";
        ...
        if (
            targetFrame
            && dataset.andrewsSelectionGrammarGate !== targetFrame.selectionGrammarGate
        ) {
    ...
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreGenerationAuthority
        && dataset.andrewsTenseAuthority !== dataset.andrewsCoreGenerationAuthority
    ) {
        diagnostics.push("andrews-core-generation-authority-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreTenseSource
        && dataset.andrewsTenseSource !== dataset.andrewsCoreTenseSource
    ) {
        diagnostics.push("andrews-core-tense-source-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreTenseSlot
        && dataset.andrewsTenseSlot !== dataset.andrewsCoreTenseSlot
    ) {
        diagnostics.push("andrews-core-tense-slot-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreTenseFamily
        && dataset.andrewsTenseFamily !== dataset.andrewsCoreTenseFamily
    ) {
        diagnostics.push("andrews-core-tense-family-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreLogicRole
        && dataset.andrewsLogicRole !== dataset.andrewsCoreLogicRole
    ) {
        diagnostics.push("andrews-core-logic-role-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreGenerationGate
        && dataset.andrewsGenerationGate !== dataset.andrewsCoreGenerationGate
    ) {
        diagnostics.push("andrews-core-generation-gate-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreOutputRole
        && dataset.andrewsOutputRole !== dataset.andrewsCoreOutputRole
    ) {
        diagnostics.push("andrews-core-output-role-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreNawatEvidenceRole
        && dataset.nawatPipilEvidenceRole !== dataset.andrewsCoreNawatEvidenceRole
    ) {
        diagnostics.push("andrews-core-nawat-evidence-role-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreClassicalOutputImport
        && dataset.classicalOutputImport !== dataset.andrewsCoreClassicalOutputImport
    ) {
    ...
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsCoreGenerationGate
        && dataset.andrewsExecutorGenerationGate !== dataset.andrewsCoreGenerationGate
    ) {
        diagnostics.push("andrews-executor-generation-gate-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsExecutorGenerationGate === "andrews-licensed-generation"
        && dataset.andrewsExecutorRouteStage !== "cnv-finite-output"
    ) {
        diagnostics.push("andrews-executor-route-stage-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsExecutorGenerationGate
        && dataset.andrewsExecutorGenerationGate !== "andrews-licensed-generation"
        && dataset.andrewsExecutorRouteStage !== "andrews-cnv-tense-logic-gate"
    ) {
        diagnostics.push("andrews-executor-route-stage-mismatch");
    }
    if (
        dataset.andrewsTenseMode === TENSE_MODE.verbo
        && dataset.andrewsExecutorGenerationGate === "andrews-licensed-generation"
        && dataset.andrewsExecutorGenerationAllowed !== "true"
    ) {
    ...
    if (
        ...
        && dataset.andrewsExecutorGenerationGate
        && dataset.andrewsExecutorGenerationGate !== "andrews-licensed-generation"
        && dataset.andrewsExecutorGenerationAllowed !== "false"
    ) {
        diagnostics.push("andrews-executor-generation-allowed-mismatch");
    }
    if (
        ...
        && dataset.andrewsExecutorGenerationGate
        && dataset.andrewsExecutorGenerationGate !== "andrews-licensed-generation"
        && dataset.andrewsExecutorFormulaShellPolicy !== "blocked-before-formula-shell"
    ) {
    ...
    if (
        ...
        && dataset.andrewsExecutorGenerationGate
        && dataset.andrewsExecutorGenerationGate !== "andrews-licensed-generation"
        && dataset.andrewsExecutorFallbackPolicy !== "blocked-no-target-stem-fallback"
    ) {
...
function auditAndrewsTenseAuthorityAnnotatedDom(root = null) {
    ...
    const records = Array.from(scope.querySelectorAll(".tense-tab, .tense-block"))
        .map((element) => getAndrewsTenseAuthorityDatasetAuditRecord(element));
...
function summarizeAndrewsTenseBlockOutputAudit(root = null) {
    ...
    const records = Array.from(scope.querySelectorAll(".tense-block"))
        .map((element) => getAndrewsTenseBlockOutputAuditRecord(element));
...
function getAndrewsTenseTabSelectionAuditRecord(element = null) {
    ...
    const targetFrame = selectionModelTarget.targetFrame;
    if (!targetFrame) {
        return getEmptyAndrewsTenseTabSelectionAuditRecord(
            selectionModelTarget.diagnosticId || "andrews-selection-audit-operation-frame-missing"
        );
    }
    const authorityRecord = getAndrewsTenseAuthorityDatasetAuditRecord(element);
    authorityRecord.diagnostics
        .filter((diagnostic) => String(diagnostic || "").startsWith("andrews-selection-"))
        .forEach((diagnostic) => {
            if (!diagnostics.includes(diagnostic)) {
                diagnostics.push(diagnostic);
            }
...
function summarizeAndrewsTenseTabSelectionAudit(root = null) {
    ...
    const records = Array.from(scope.querySelectorAll(".tense-tab"))
        .map((element) => getAndrewsTenseTabSelectionAuditRecord(element));
    const countDiagnostic = (diagnosticId) => records
        .filter((record) => record.diagnostics.includes(diagnosticId)).length;
    const diagnosticCount = records.filter((record) => record.diagnostics.length > 0).length;
    ...
    return {
        ...
        disabledMismatchCount: countDiagnostic("andrews-selection-disabled-mismatch")
            + countDiagnostic("andrews-selection-native-disabled-mismatch")
            + countDiagnostic("andrews-selection-aria-disabled-mismatch"),
        activeMismatchCount: countDiagnostic("andrews-selection-selected-mismatch")
            + countDiagnostic("andrews-selection-blocked-tab-active")
            + countDiagnostic("andrews-selection-blocked-tab-aria-selected")
            + countDiagnostic("andrews-selection-blocked-tab-selected"),
        records,
    };
}
...
function getAndrewsTenseAuthorityDomDescriptor(element = null, {
    ...
    const dataset = element?.dataset || {};
    const classList = element?.classList || null;
    const fallbackMode = String(mode || "").trim()
        || String(dataset.tenseMode || "").trim()
        || String(dataset.andrewsTenseMode || "").trim()
        || (typeof getActiveTenseMode === "function" ? getActiveTenseMode() : "")
        || TENSE_MODE.verbo;
    const tenseValue = String(
        dataset.tenseValue
        || dataset.andrewsTenseValue
        || dataset.nonactiveSuffix
        || (classList?.contains("tense-block--selection-required") ? "selection-required" : "")
        || ""
    ).trim();
    return {
        tenseValue,
...
function syncAndrewsTenseAuthorityDomAudit(root = null, {
    ...
    if (annotateMissing) {
        Array.from(scope.querySelectorAll(".tense-tab, .tense-block")).forEach((element) => {
            ...
            const needsRepair = !needsInitialAnnotation
                && (record.missing.length > 0 || canonicalMismatches.length > 0 || classMismatches.length > 0);
    ...
    syncAndrewsTenseTabsOperationalLayerPanel(scope, { mode });
...
function getAndrewsFirstTenseHoverTitle(tenseValue = "", mode = TENSE_MODE.verbo) {
    const frame = getAndrewsTenseAuthorityFrame(tenseValue, mode);
...
function getAndrewsFormulaWorkbenchContainer() {
    return document.getElementById("formula-workbench");
}

function getAndrewsFormulaWorkbenchUiModel(activeId = AndrewsFormulaWorkbenchActiveCategoryId) {
    if (typeof getAndrewsFormulaWorkbenchModel !== "function") {
        return null;
    }
    const verbMeta = typeof getVerbInputMeta === "function" ? getVerbInputMeta() : null;
    const inputValue = verbMeta?.rawInput || verbMeta?.rawInputVerb || verbMeta?.parseInputVerb || verbMeta?.verb || "";
    return getAndrewsFormulaWorkbenchModel({ activeId, inputValue });
}
...
function getAndrewsFormulaWorkbenchUiStatusLabel(status = "") {
    if (typeof getAndrewsFormulaWorkbenchStatusLabel === "function") {
        return getAndrewsFormulaWorkbenchStatusLabel(status);
    }
...
function syncAndrewsFormulaWorkbenchCategoryToEngine(category = null) {
    ...
    if (category.engineMode === "ordinary-nnc") {
        ...
        if (
            typeof setActiveNawatTenseMode === "function"
            && typeof TENSE_MODE !== "undefined"
    ...
    if (category.engineMode === "vnc") {
        ...
        if (
            ...
            setActiveNawatTenseMode(TENSE_MODE.verbo);
        }
    }
}
...
function applyAndrewsFormulaWorkbenchSourceInput(value = "", {
    ...
    if (typeof syncComposerStateFromVerbInput === "function") {
        syncComposerStateFromVerbInput(nextValue);
    }
    if (!renderWorkbench) {
        return;
    }
...
function appendAndrewsFormulaWorkbenchOperationalLayer(parent, summary = null) {
    ...
    const summaryRefs = document.createElement("span");
    summaryRefs.className = "formula-workbench__operation-refs";
    summaryRefs.textContent = summary.complete === true
        ? "cobertura Andrews"
        : `${summary.missingSectionCount || 0} secciones faltantes`;
...
function appendAndrewsFormulaWorkbenchSlice(parent, slice = null) {
    ...
    sourceInput.addEventListener("input", () => {
        applyAndrewsFormulaWorkbenchSourceInput(sourceInput.value, { renderWorkbench: false });
    });
    sourceInput.addEventListener("keydown", (event) => {
        if (event.key === "Enter") {
            applyAndrewsFormulaWorkbenchSourceInput(sourceInput.value, { refreshEngine: true });
        }
    });
    ...
    appendAndrewsFormulaWorkbenchPill(
        result,
        "Contrato",
        getAndrewsFormulaWorkbenchUiStatusLabel(slice.generation?.status || "diagnostic-only"),
        `formula-workbench__pill--${slice.generation?.status || "diagnostic-only"}`
    );
    ...
    appendAndrewsFormulaWorkbenchPill(
        result,
        slice.generation?.allowed ? "Salida" : "Bloqueo",
        slice.generation?.surface || (slice.diagnostics?.[0]?.message || "sin salida"),
        slice.generation?.allowed ? "formula-workbench__pill--generated" : "formula-workbench__pill--unsupported"
    );
    ...
    if (boundary?.classicalStructuralOnly) {
        ...
        appendAndrewsFormulaWorkbenchPill(
            boundaryEl,
            "Andrews structural-only",
            Array.isArray(boundary.structuralExamples) ? boundary.structuralExamples.join(", ") : ""
        );
        appendAndrewsFormulaWorkbenchPill(
            boundaryEl,
            "Nawat/Pipil",
            boundary.nawatAuthority || "orthography bridge"
        );
    ...
    if (examples.length) {
        ...
        examples.forEach((example) => {
            ...
            if (example.nawatFormulaEcho && example.nawatFormulaEcho !== (example.structuralFormulaEcho || example.fullFormulaEcho)) {
                const nawatFormula = document.createElement("code");
...
function appendAndrewsFormulaWorkbenchDetail(parent, model = null) {
    ...
    appendAndrewsFormulaWorkbenchPill(
        meta,
        "Estado",
        getAndrewsFormulaWorkbenchUiStatusLabel(category.generationStatus),
        "formula-workbench__pill--status"
    );
...
function renderAndrewsFormulaWorkbench() {
    ...
    AndrewsFormulaWorkbenchActiveCategoryId = normalizeAndrewsFormulaWorkbenchUiCategoryId(
        AndrewsFormulaWorkbenchActiveCategoryId,
        model
    );
    ...
    (Array.isArray(model.statusSummary) ? model.statusSummary : []).forEach((entry) => {
        appendAndrewsFormulaWorkbenchPill(
            ...
            `formula-workbench__pill--${entry.status}`
        );
    });
    appendAndrewsFormulaWorkbenchPill(
        summary,
        "Rutas",
        String(model.routeRegistrySummary?.routeCount || 0),
        "formula-workbench__pill--route"
    );
...
function renderTenseTabs() {
    ...
    const buildTenseButton = (tenseValue, {
        ...
        applyAndrewsTenseAuthorityDataset(button, { tenseValue, mode: tenseMode });
        ...
        button.addEventListener("click", () => {
            ...
            if (
                ...
                && isOrdinaryNncGenerationModeEnabled()
                && typeof setOrdinaryNncGenerationModeEnabled === "function"
            ) {
                setOrdinaryNncGenerationModeEnabled(false);
                updateTenseModeTabs();
            }
    ...
    const buildAndrewsRouteDirectoryColumn = () => {
        ...
        const registryFrame = typeof buildAndrewsSourceGatedDerivationalRouteRegistry === "function"
            ? buildAndrewsSourceGatedDerivationalRouteRegistry()
            : null;
        const registryRoutes = Array.isArray(registryFrame?.routes)
            ? registryFrame.routes
            : [];
        ...
        const routeClassOrder = [
            "verbal-source-to-nominal-target",
            "nominal-source-to-nominal-target",
            "nominal-source-to-verbal-target",
            "verbal-source-to-verbal-target",
            "mixed-compound-source-target-route",
            "other-source-target-route",
            "unclassified-source-target-route",
        ];
        ...
        const getRouteFormulaTransition = (route) => (
            route?.formulaTransition
            || getAndrewsSourceTargetFormulaTransition(route?.sourceFormulaType, route?.targetFormulaType)
            || ""
        );
        const getRouteSourceTargetClass = (route) => getAndrewsSourceTargetRouteClass(
            getRouteFormulaTransition(route)
        );
        const getRouteSourceTargetHost = (route) => getAndrewsSourceTargetRouteUiHost(
            getRouteFormulaTransition(route)
        );
        ...
        const addRouteRow = (lesson, route, level, fallbackRef) => {
            ...
            const routeIdentity = [
                route.id || route.contractId || route.routeTemplateId || "",
                route.routeFamily || "",
                route.routeKind || "",
                formulaTransition,
                getRouteRef(lesson, route, fallbackRef),
            ].filter(Boolean).join("|");
            ...
            const searchText = [
                ...
                formulaTransition,
                routeClass,
                getRouteClassLabel(routeClass),
                routeHost,
                route.sourceFormulaType,
                route.targetFormulaType,
            ...
            routeRows.push({ key, lesson, route, level, ref, structuralInfo, searchText, formulaTransition, routeClass, routeHost });
        };
        ...
        registryRoutes.forEach((route) => {
            addRouteRow(
                { id: "registry", label: "Andrews route registry" },
                route,
                "registry",
                route.andrewsRefs?.[0] || route.id || route.routeKind || "Andrews route registry"
            );
        });
        if (!routeRows.length) {
            return null;
        ...
        const routeClasses = routeClassOrder
            .filter((routeClass) => routeClassCounts.has(routeClass))
            .concat(Array.from(routeClassCounts.keys()).filter((routeClass) => !routeClassOrder.includes(routeClass)).sort());
        ...
        let selectedRow = routeRows.find((row) => row.route.routeKind === "preterit-agentive-embedded-source-locative")
            ...
            || routeRows.find((row) => row.routeClass === "nominal-source-to-verbal-target")
            || routeRows.find((row) => row.routeClass === "verbal-source-to-verbal-target")
            || routeRows[0];
        ...
        const routeClassSelect = document.createElement("select");
        ...
        const allRouteClassesOption = document.createElement("option");
            ...
            return Array.isArray(route.puzzleStackTemplate?.actions)
                ? route.puzzleStackTemplate.actions
                    ...
                    .map((action) => ({
                        ...
                        absolutiveAllomorph: action.absolutiveAllomorph && typeof action.absolutiveAllomorph === "object"
                            ? action.absolutiveAllomorph
                            : null,
                    }))
                : [];
        };
        ...
        const appendAndrewsRoutePuzzleStackBuilder = (parent, row, steps) => {
            ...
            title.textContent = row.route.routeKind === "preterit-agentive-embedded-source-locative"
                ? "Andrews 46.3.1.a route builder"
                : "Route builder";
            ...
            const sourceEvidence = document.createElement("div");
            ...
            const nextOperation = document.createElement("div");
            ...
            const getActionAbsolutiveAllomorphLabel = (action) => {
                ...
                const appliesAfter = Array.isArray(frame.appliesAfter)
                    ? frame.appliesAfter.filter(Boolean).join("/")
                    : "";
                const previous = frame.previousNonZeroSegment
                    ? `no-cero ${frame.previousNonZeroSegment}`
                    : "";
                const realized = frame.realizedConnector
                    ? `-> ${frame.realizedConnector}`
                    : "";
                return [
                    `absolutivo ${frame.connectorFamily || "t/ti"}`,
                    frame.selector ? `selector ${frame.selector}` : "",
                    appliesAfter ? `aplica ${appliesAfter}` : "",
                    [previous, realized].filter(Boolean).join(" "),
                ].filter(Boolean).join(" · ");
            };
            const renderBuilder = () => {
                ...
                currentFormula.textContent = complete
                    ? (previousAction?.outputFormula || "")
                    : (currentAction.inputFormula || "");
                progress.textContent = `${Math.min(activeIndex, actions.length)} / ${actions.length}`;
                sourceEvidence.textContent = [
                    activeIndex === 0 ? `fuente: ${actions[0].inputFormula}` : "",
                    evidenceAction?.sourceEvidence ? `evidencia: ${evidenceAction.sourceEvidence}` : "",
                    evidenceAction?.routeBoundary ? `frontera: ${evidenceAction.routeBoundary}` : "",
                    getActionAbsolutiveAllomorphLabel(evidenceAction),
                ].filter(Boolean).join(" · ");
                nextOperation.textContent = complete
                    ? `Completo: ${previousAction?.outputFormula || ""}`
                    : `Siguiente: ${currentAction.operation || currentAction.selectablePiece || ""}`;
                ...
                actions.forEach((action, index) => {
                    ...
                    if (action.absolutiveAllomorph) {
                        ...
                        actionRow.dataset.absolutiveAllomorphAppliesAfter = Array.isArray(action.absolutiveAllomorph.appliesAfter)
                            ? action.absolutiveAllomorph.appliesAfter.filter(Boolean).join("|")
                            : "";
                    ...
                    actionMeta.textContent = index < activeIndex
                        ? [action.outputFormula, allomorphLabel].filter(Boolean).join(" · ")
                        : [action.operation, allomorphLabel, action.outputFormula].filter(Boolean).join(" -> ");
        ...
        const renderAndrewsRouteBrowserDetail = (row) => {
            ...
            [
                ...
                row.formulaTransition,
                getRouteClassLabel(row.routeClass),
                row.routeHost,
            ].filter(Boolean).forEach((chipText) => {
                const chip = document.createElement("span");
        ...
        const renderRows = () => {
            ...
            visibleRows.forEach((row) => {
                ...
                rowMeta.textContent = [
                    route.routeFamily,
                    row.formulaTransition || route.formulaTemplate,
                    getRouteClassLabel(row.routeClass),
