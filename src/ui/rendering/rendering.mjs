...
export function createUiRenderingGlobals(targetObject = globalThis) {
    ...
    function createNawatVerbNounConversionSwitchGroup({
      ...
      sectionLabel.textContent = "salida";
      const options = targetObject.document.createElement("div");
      options.className = "calc-guidance__route-switch-menu-options";
    ...
    function renderOutputGuidancePanel({
      ...
      const renderLineRows = (titleText, lines) => {
        ...
        const getLineSelectionStore = () => typeof targetObject.getNawatRouteStateStore === "function" ? targetObject.getNawatRouteStateStore() : typeof targetObject.window !== "undefined" ? targetObject.window : null;
        ...
        const getActiveLinkedPathSummary = () => lineSelectionStore?.activeLinkedGrammarPathSelectionSummary && lineSelectionStore.activeLinkedGrammarPathSelectionSummary.outputKind === "linked-grammar-path-selection-summary" ? lineSelectionStore.activeLinkedGrammarPathSelectionSummary : null;
        const getActiveLinkedPathExecution = () => lineSelectionStore?.activeLinkedGrammarPathExecution && lineSelectionStore.activeLinkedGrammarPathExecution.outputKind === "linked-grammar-path-chain-execution" ? lineSelectionStore.activeLinkedGrammarPathExecution : null;
        const getActiveLinkedPathPromotedSource = () => lineSelectionStore?.activeLinkedGrammarPathPromotedSource || null;
        const createSelectedLinkedPathSummary = () => {
          const summary = getActiveLinkedPathSummary();
          const execution = getActiveLinkedPathExecution();
          const promotedSource = getActiveLinkedPathPromotedSource();
          const appendableSelections = getNawatLinkedGrammarAppendableSelections(summary, {
            maxChoices: 6
          });
          const executionSourceOptions = typeof targetObject.getNawatLinkedGrammarPathExecutionSourceOptions === "function" ? targetObject.getNawatLinkedGrammarPathExecutionSourceOptions(execution) : [];
          const labels = [...buildNawatLinkedGrammarSelectedPathSubLabels(summary), ...buildNawatLinkedGrammarPathExecutionSubLabels(execution), ...buildNawatLinkedGrammarPromotedSourceSubLabels(promotedSource)];
          if (!labels.length) {
            return null;
          }
          const summaryRow = targetObject.document.createElement("div");
          summaryRow.className = "calc-guidance__linked-path-summary";
          summaryRow.dataset.linkedPathSelectionCount = String(summary.selectedSteps?.length || 0);
          const statusGroup = targetObject.document.createElement("div");
          statusGroup.className = "calc-guidance__linked-path-group calc-guidance__linked-path-group--status";
          const choiceGroup = targetObject.document.createElement("div");
          choiceGroup.className = "calc-guidance__linked-path-group calc-guidance__linked-path-group--choices";
          const actionGroup = targetObject.document.createElement("div");
          actionGroup.className = "calc-guidance__linked-path-group calc-guidance__linked-path-group--actions";
          labels.forEach((text, index) => {
            const chip = targetObject.document.createElement("div");
            chip.className = ["calc-guidance__chip", "calc-guidance__chip--linked-path", index === 0 ? "calc-guidance__chip--active" : ""].filter(Boolean).join(" ");
            const label = targetObject.document.createElement("span");
            label.className = "calc-guidance__chip-label";
          ...
          summaryRow.appendChild(statusGroup);
          if (summary?.selectedSteps?.length && typeof targetObject.executeActiveNawatLinkedGrammarPathSelections === "function") {
            const executeButton = targetObject.document.createElement("button");
            executeButton.type = "button";
            executeButton.className = "calc-guidance__chip calc-guidance__chip--button calc-guidance__chip--linked-execute";
            executeButton.dataset.linkedPathExecute = "true";
            const executeLabel = targetObject.document.createElement("span");
            executeLabel.className = "calc-guidance__chip-label";
            executeLabel.textContent = "generar trayecto";
            executeButton.appendChild(executeLabel);
            executeButton.addEventListener("click", () => {
              targetObject.executeActiveNawatLinkedGrammarPathSelections();
              executeButton.dataset.linkedPathExecuted = "true";
              renderLineRows(titleText, lines);
            });
            actionGroup.appendChild(executeButton);
          }
          if (summary?.selectedSteps?.length && typeof targetObject.removeLastActiveNawatLinkedGrammarPathSelection === "function") {
            const backButton = targetObject.document.createElement("button");
            backButton.type = "button";
            backButton.className = "calc-guidance__chip calc-guidance__chip--button calc-guidance__chip--linked-back";
            backButton.dataset.linkedPathBack = "true";
            const backLabel = targetObject.document.createElement("span");
            backLabel.className = "calc-guidance__chip-label";
            backLabel.textContent = "atrás";
            backButton.appendChild(backLabel);
            backButton.addEventListener("click", () => {
              targetObject.removeLastActiveNawatLinkedGrammarPathSelection();
              backButton.dataset.linkedPathBacktracked = "true";
              renderLineRows(titleText, lines);
            });
            actionGroup.appendChild(backButton);
          }
          if (typeof targetObject.appendActiveNawatLinkedGrammarPathSelection === "function") {
            appendableSelections.forEach((appendableSelection, choiceIndex) => {
              if (!appendableSelection?.selection) {
                return;
              }
              const appendButton = targetObject.document.createElement("button");
              appendButton.type = "button";
              appendButton.className = "calc-guidance__chip calc-guidance__chip--button calc-guidance__chip--linked-choice";
              appendButton.dataset.linkedPathAppendSummary = "true";
              appendButton.dataset.linkedPathChoiceIndex = String(choiceIndex);
              const appendLabel = targetObject.document.createElement("span");
              appendLabel.className = "calc-guidance__chip-label";
              appendLabel.textContent = "seguir";
              appendButton.appendChild(appendLabel);
              const appendSublabelText = [formatNawatLinkedGrammarCompactChoiceLabel(appendableSelection), appendableSelection.sourceVerb].filter(Boolean).join(" → ");
              if (appendSublabelText) {
                const appendSublabel = targetObject.document.createElement("span");
                appendSublabel.className = "calc-guidance__chip-sublabel";
                appendSublabel.textContent = appendSublabelText;
                appendButton.appendChild(appendSublabel);
              }
              appendButton.addEventListener("click", () => {
                targetObject.appendActiveNawatLinkedGrammarPathSelection(appendableSelection.selection);
                appendButton.dataset.linkedPathAppended = "true";
                renderLineRows(titleText, lines);
              });
              choiceGroup.appendChild(appendButton);
            });
          }
          if (execution?.steps?.length && executionSourceOptions.length > 1 && typeof targetObject.promoteActiveNawatLinkedGrammarPathExecutionStepSource === "function") {
            executionSourceOptions.slice(0, -1).forEach(sourceOption => {
              const stepNumber = Number(sourceOption.fromStepIndex) + 1;
              const stepButton = targetObject.document.createElement("button");
              stepButton.type = "button";
              stepButton.className = "calc-guidance__chip calc-guidance__chip--button calc-guidance__chip--linked-promote";
              stepButton.dataset.linkedPathPromoteStep = String(sourceOption.fromStepIndex);
              const stepLabel = targetObject.document.createElement("span");
              stepLabel.className = "calc-guidance__chip-label";
              stepLabel.textContent = `usar etapa ${stepNumber}`;
              stepButton.appendChild(stepLabel);
              const stepSublabelText = sourceOption.sourceVerb || sourceOption.generatedSurface || "";
              if (stepSublabelText) {
                const stepSublabel = targetObject.document.createElement("span");
                stepSublabel.className = "calc-guidance__chip-sublabel";
                stepSublabel.textContent = stepSublabelText;
                stepButton.appendChild(stepSublabel);
              }
              stepButton.addEventListener("click", () => {
                targetObject.promoteActiveNawatLinkedGrammarPathExecutionStepSource(sourceOption.fromStepIndex, {
                  syncInput: true
                });
                stepButton.dataset.linkedPathPromoted = "true";
                renderLineRows(titleText, lines);
              });
              actionGroup.appendChild(stepButton);
            });
          }
          if (execution?.steps?.length && typeof targetObject.promoteActiveNawatLinkedGrammarPathExecutionFinalSource === "function") {
            const promoteButton = targetObject.document.createElement("button");
            promoteButton.type = "button";
            promoteButton.className = "calc-guidance__chip calc-guidance__chip--button calc-guidance__chip--linked-promote";
            promoteButton.dataset.linkedPathPromote = "true";
            const promoteLabel = targetObject.document.createElement("span");
            promoteLabel.className = "calc-guidance__chip-label";
            promoteLabel.textContent = "usar salida";
            promoteButton.appendChild(promoteLabel);
            promoteButton.addEventListener("click", () => {
              targetObject.promoteActiveNawatLinkedGrammarPathExecutionFinalSource({
                syncInput: true
              });
              promoteButton.dataset.linkedPathPromoted = "true";
              renderLineRows(titleText, lines);
            });
            actionGroup.appendChild(promoteButton);
          }
          if (typeof targetObject.clearActiveNawatLinkedGrammarPathSelections === "function") {
            const clearButton = targetObject.document.createElement("button");
            clearButton.type = "button";
            clearButton.className = "calc-guidance__chip calc-guidance__chip--button calc-guidance__chip--linked-clear";
            clearButton.dataset.linkedPathClear = "true";
            const clearLabel = targetObject.document.createElement("span");
            clearLabel.className = "calc-guidance__chip-label";
            clearLabel.textContent = "borrar";
            clearButton.appendChild(clearLabel);
            clearButton.addEventListener("click", () => {
              targetObject.clearActiveNawatLinkedGrammarPathSelections();
              clearButton.dataset.linkedPathCleared = "true";
              renderLineRows(titleText, lines);
            });
            actionGroup.appendChild(clearButton);
          }
          if (choiceGroup.childNodes.length) {
            summaryRow.appendChild(choiceGroup);
          }
          if (actionGroup.childNodes.length) {
            summaryRow.appendChild(actionGroup);
          }
          return summaryRow;
        };
        ...
        lineToggles.setAttribute("aria-label", "Líneas de salida");
        lines.forEach(line => {
          const button = targetObject.document.createElement("button");
          button.type = "button";
        ...
        if (selectedLine) {
          ...
          (selectedLine.stations || []).forEach((station, index) => {
            ...
            const hasAction = typeof station.action === "function";
            const chip = targetObject.document.createElement(hasAction ? "button" : "div");
            if (hasAction) {
              chip.type = "button";
            }
            chip.className = ["calc-guidance__chip", "calc-guidance__chip--station", hasAction ? "calc-guidance__chip--button" : "", getGuidanceModeClass(station.mode), station.active ? "calc-guidance__chip--active" : ""].filter(Boolean).join(" ");
            if (station.linkedNextSource?.sourceVerb) {
              chip.dataset.linkedNextSource = station.linkedNextSource.sourceVerb;
            }
            if (station.linkedAppendableSelectionCount) {
              chip.dataset.linkedAppendableSelectionCount = String(station.linkedAppendableSelectionCount);
            }
            const linkedTitle = [station.linkedNextSourceLabel, station.linkedAppendableChoiceLabel].filter(Boolean).join(" · ");
            if (linkedTitle) {
              chip.title = linkedTitle;
            }
            ...
            if (typeof station.linkedAppendableAction === "function") {
              const choice = targetObject.document.createElement("button");
              choice.type = "button";
              choice.className = ["calc-guidance__chip", "calc-guidance__chip--button", "calc-guidance__chip--linked-choice", getGuidanceModeClass(station.mode)].filter(Boolean).join(" ");
              choice.dataset.linkedAppendableRoute = station.linkedAppendableSelection?.routeId || "";
              choice.dataset.linkedAppendableStage = station.linkedAppendableSelection?.stageKey || "";
              if (station.linkedAppendableChoiceLabel) {
                choice.title = station.linkedAppendableChoiceLabel;
              }
              const choiceLabel = targetObject.document.createElement("span");
              choiceLabel.className = "calc-guidance__chip-label";
              choiceLabel.textContent = "seguir";
              choice.appendChild(choiceLabel);
              if (station.linkedAppendableChoiceLabel) {
                const choiceSubLabel = targetObject.document.createElement("span");
                choiceSubLabel.className = "calc-guidance__chip-sublabel";
                choiceSubLabel.textContent = station.linkedAppendableChoiceLabel;
                choice.appendChild(choiceSubLabel);
              }
              choice.addEventListener("click", () => {
                station.linkedAppendableAction(choice);
                choice.dataset.linkedPathAppended = "true";
        ...
        const selectedPathSummary = createSelectedLinkedPathSummary();
        if (selectedPathSummary) {
          panel.appendChild(selectedPathSummary);
        }
      };
      ...
      const buildNawatLineModels = () => {
        ...
        const locativeMatrixRoot = hasLocativeContext ? String(routeStore?.activeLocativeMatrixRoot || "").trim() || "tajtani" : "tajtani";
        const activeRouteKey = activeRoute?.id || activeRoute?.legacyTenseValue || "";
        const activeRouteSourceVerb = String(activeRoute?.sourceVerb || resolvedVerb || "").trim();
        const activeRouteSourceObjectPrefix = activeRoute?.sourceObjectPrefix || "";
        const activeRouteTarget = activeRoute && typeof activeRoute === "object" ? activeRoute : null;
        const linkedStages = activeRouteKey && typeof targetObject.buildNawatLinkedGrammarPathStages === "function" ? targetObject.buildNawatLinkedGrammarPathStages(activeRoute, {
          sourceVerb: activeRouteSourceVerb,
          sourceObjectPrefix: activeRouteSourceObjectPrefix,
        ...
        return lineSpecs.filter(line => availableLineIds.has(line.id)).map(line => ({
          ...line,
          stations: attachNawatLinkedGrammarStagesToRailStations(line.stations.map(station => ({
            ...station,
            ...
            sourceVerb: activeRouteSourceVerb,
            sourceObjectPrefix: activeRouteSourceObjectPrefix,
            activateStation: typeof targetObject.activateNawatRouteStation === "function" ? targetObject.activateNawatRouteStation : null,
            previewNextSource: typeof targetObject.previewNawatLinkedGrammarPathNextSource === "function" ? targetObject.previewNawatLinkedGrammarPathNextSource : null,
            buildSelectionSummary: typeof targetObject.buildNawatLinkedGrammarPathSelectionSummary === "function" ? stage => targetObject.buildNawatLinkedGrammarPathSelectionSummary({
              sourceVerb: stage?.nextSource?.sourceVerb || "",
              sourceObjectPrefix: stage?.nextSource?.objectPrefix || ""
            }) : null,
            appendSelection: typeof targetObject.appendActiveNawatLinkedGrammarPathSelection === "function" ? (selection, options = {}) => targetObject.appendActiveNawatLinkedGrammarPathSelection(selection, {
      ...
      const renderNawatLineBoard = () => {
        renderLineRows("salida dinámica", buildNawatLineModels());
      };
    ...
    function renderNounConjugations({
      ...
      const createPatientivoPrelocativeDestinationPicker = (entry = {}) => {
        ...
        const sourceTenseValue = getPatientivoBlockSourceTenseValue(patientivoSource);
        const sourceCombinedMode = getPatientivoBlockSourceCombinedMode(patientivoSource);
        const promotedObjectPrefix = activePossessor ? String(targetObject.POSSESSIVE_TO_OBJECT_PREFIX[activePossessor] || "").trim() : "";
        const canPromotePossessor = !activePossessor || Boolean(promotedObjectPrefix);
        const canReachPrelocative = patientivoSource === "imperfectivo" && sourceCombinedMode === targetObject.COMBINED_MODE.active && NAWAT_PRELOCATIVE_PATIENTIVO_SOURCE_TENSES.has(sourceTenseValue) && canPromotePossessor && verb;
        ...
        const incorporatedRoot = stripPrelocativePatientivoConnector(patientivoSurface);
        if (!patientivoSurface || !incorporatedRoot) {
          return null;
        }
        const matrixRoot = "ni";
        const destinationSummary = targetObject.document.createElement("span");
        ...
        const focusLocativeLine = button => {
          const update = () => {
            ...
            if (lineSelectionStore) {
              ...
              lineSelectionStore.activeLocativePromotedObjectPrefix = promotedObjectPrefix;
              lineSelectionStore.activeLocativeSourcePossessorPrefix = activePossessor || "";
            }
            if (typeof targetObject.window !== "undefined" && targetObject.window) {
            ...
            if (!appliedToEntrada) {
              renderActiveConjugations({
                verb,
                objectPrefix: promotedObjectPrefix || (typeof targetObject.getCurrentObjectPrefix === "function" ? targetObject.getCurrentObjectPrefix() : ""),
                tense: "patientivo"
              });
            }
          };
        ...
        const transformLabel = promotedObjectPrefix ? `; poseedor ${activePossessor} > objeto ${promotedObjectPrefix}` : "";
        const button = targetObject.document.createElement("button");
        button.type = "button";
        ...
        button.title = `destino = raíz incorporada: ${incorporatedRoot || "—"}; raíz matriz: ${matrixRoot}; ${originLabel}${transformLabel}`;
      ...
      const renderPatientivoPrelocativeContinuation = ({
        ...
        if (!value || evaluation?.shouldMaskRow || patientivoSource !== "imperfectivo" || sourceCombinedMode !== targetObject.COMBINED_MODE.active || !NAWAT_PRELOCATIVE_PATIENTIVO_SOURCE_TENSES.has(sourceTenseValue)) {
          return false;
        }
        const sourceObjectPrefix = resolvePatientivoOriginSourceObjectPrefix(patientivoSource);
        const sourceSurface = getDirectPatientivoSourceSurface({
    ...
    Object.defineProperty(api, "NAWAT_PRELOCATIVE_PATIENTIVO_SOURCE_TENSES", {
        configurable: true,
        enumerable: true,
        get() { return NAWAT_PRELOCATIVE_PATIENTIVO_SOURCE_TENSES; },
    });
