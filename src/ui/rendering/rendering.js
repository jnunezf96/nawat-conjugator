...
function getActiveNawatDenominalAndrewsContractRouteRenderContextFromModel(options = {}) {
    const contextFns = [
        typeof getActiveNawatDenominalAndrewsContractRouteContext === "function"
            ? getActiveNawatDenominalAndrewsContractRouteContext
            : null,
        typeof globalThis !== "undefined" && typeof globalThis.getActiveNawatDenominalAndrewsContractRouteContext === "function"
...
function previewActiveNawatDenominalAndrewsContractRouteNextSourceForRendering({
    ...
    const context = ActiveNawatDenominalAndrewsContractRouteRenderContext
        || getActiveNawatDenominalAndrewsContractRouteRenderContextFromModel({ inputValue, targetInput, targetVerbStem });
    if (!context?.route) {
        return null;
    }
...
function getAndrewsFirstOutputBlockHoverTitle({
    ...
    if (resolvedMode === "verb" || resolvedMode === TENSE_MODE.verbo) {
        const tenseTitle = typeof getAndrewsFirstTenseHoverTitle === "function"
            ? getAndrewsFirstTenseHoverTitle(tenseValue, TENSE_MODE.verbo)
            : "Andrews dirige la arquitectura de CNV; Nawat/Pipil confirma la superficie.";
        return `Andrews Lecciones 4-7: bloque de cláusula nuclear verbal. ${tenseTitle}`;
    }
    if (resolvedMode === "noun" || resolvedMode === TENSE_MODE.sustantivo) {
        return "Andrews Lecciones 4 y 12-19: cláusula nuclear nominal; sin ranura tiempo ordinaria.";
    }
    if (resolvedMode === TENSE_MODE.adjetivo) {
        return "Andrews dirige la función adjectival nominal; Nawat/Pipil confirma la superficie.";
    }
    if (resolvedMode === TENSE_MODE.adverbio) {
        return "Andrews dirige la función adverbial; Nawat/Pipil confirma la superficie.";
    }
    return blockKind
        ? `Andrews dirige la arquitectura de ${blockKind}; Nawat/Pipil confirma la superficie.`
        : "Andrews dirige la arquitectura; Nawat/Pipil confirma la superficie.";
}
...
function getVisibleCnvFormulaAlignmentSlotFrame(sourceFrame = null, slotKey = "") {
    return (Array.isArray(sourceFrame?.slotFrames) ? sourceFrame.slotFrames : [])
...
function getVisibleCnvFormulaAlignmentSlotValue(sourceFrame = null, slotKey = "", fallback = "0", {
    ...
    const value = preferSurface
        ? (surfaceValue || formulaMorph)
        : (formulaMorph || surfaceValue);
...
function getVisibleCnvFormulaAlignmentObjectMorph(sourceFrame = null) {
    ...
    if (va) {
        return normalizeVisibleCnvFormulaAlignmentToken(
            String(va.surfaceValue || va.formulaMorph || ""),
            ""
        );
    }
    ...
    if (
        ...
        && va2Surface !== "0"
        && va2Surface !== "Ø"
        && va2Surface !== va2Formula
        && visible.endsWith(va2Formula)
    ) {
        return normalizeVisibleCnvFormulaAlignmentToken(
            `${visible.slice(0, -va2Formula.length)}${va2Surface}`,
            ""
        );
    }
    if (visible) {
    ...
    const va1Value = normalizeVisibleCnvFormulaAlignmentToken(
        String(va1?.surfaceValue || va1?.formulaMorph || ""),
        ""
    );
    const va2Value = normalizeVisibleCnvFormulaAlignmentToken(
        String(va2?.surfaceValue || va2?.formulaMorph || ""),
        "0"
    );
...
function resolveVisibleCnvFormulaAlignmentSurfaceSlots(sourceFrame = null, slots = {}) {
    ...
    const objectThenDirectional = objectSurface
        && directionalSurface
        && coreSurface.startsWith(`${objectSurface}${directionalSurface}`);
    ...
    const suffixCandidates = [
        fallbackConnector,
        ...(fallbackConnector === "ki-0" ? ["k-0"] : []),
        ...(fallbackConnector !== "0-0" ? ["0-0"] : []),
    ]
        .map((connector) => {
            const { num1, num2 } = splitVisibleCnvFormulaAlignmentConnector(connector);
            return { connector, num1, num2 };
        })
...
function buildVisibleCnvFormulaAlignmentTargetFrame(sourceFrame = null) {
    ...
    const objectFrame = getVisibleCnvFormulaAlignmentSlotFrame(sourceFrame, "va")
        || getVisibleCnvFormulaAlignmentSlotFrame(sourceFrame, "va1")
        || getVisibleCnvFormulaAlignmentSlotFrame(sourceFrame, "va2");
    ...
    prePredicateParts = surfaceSlots.prePredicateParts.length
        ? surfaceSlots.prePredicateParts
        : prePredicateParts;
...
function buildVisibleCnvFormulaAlignmentSourceFrame(record = null) {
    ...
    const slotFrames = paths
        .map((path, index) => ({
            ...
            linearPieces: Array.isArray(path?.linearPieces)
                ? path.linearPieces.map((piece) => String(piece ?? "").trim()).filter(Boolean)
                : [],
    ...
    return Object.freeze({
        ...
        slotFrames: Object.freeze(slotFrames.map((entry) => Object.freeze({
            ...entry,
            formulaFeatures: Object.freeze({ ...entry.formulaFeatures }),
...
function getVisibleCnvFormulaAlignmentMismatch(record = null, sourceFrame = null, operationFrame = null) {
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-visible-cnv-formula-alignment-operation-frame"
        || operationFrame.operationId !== "visible-cnv-formula-path-slot-alignment"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || operationFrame.formulaEchoAuthority !== false
        || operationFrame.surfaceTextAuthority !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
    ) {
...
function isVisibleCnvFormulaAlignmentSourceFrameComplete(sourceFrame = null) {
    ...
    const slotKeys = new Set((Array.isArray(sourceFrame.slotFrames) ? sourceFrame.slotFrames : [])
        .map((entry) => String(entry?.slotKey || "")));
    return ["pers1", "pers2", "base", "tns", "num1", "num2"]
        .every((slotKey) => slotKeys.has(slotKey));
}

function getVisibleCnvFormulaAlignmentPathRecords(path = null) {
    return (Array.isArray(path?.pathsBySurface) ? path.pathsBySurface : [])
        .filter((record) => isVisibleCnvFormulaAlignmentSourceFrameComplete(
            buildVisibleCnvFormulaAlignmentSourceFrame(record)
        ));
}

function alignVisibleCnvFormulaEchoToSurface(
    formulaEcho = "",
    surface = "",
    record = null,
    sourceFrame = null,
    ...
    const suffixCandidates = [
        { tense: tenseText, connector: connectorText },
        ...(connectorSurface === "ki" ? [{ tense: tenseText, connector: "k-0" }] : []),
        ...(connectorSurface ? [{ tense: tenseText, connector: "0-0" }] : []),
    ];
    const candidates = suffixCandidates
        .map((candidate) => {
            ...
            const base = suffixSurface
                ? coreSurface.slice(0, -suffixSurface.length)
                : coreSurface;
            if (!base) {
                return null;
            }
...
function getVisibleCnvFormulaSurfaceForms(source = null) {
    ...
    const pathForms = getVisibleCnvFormulaAlignmentPathRecords(path)
        .map((record) => String(record?.surface || "").trim())
        .filter((entry, index, list) => entry && entry !== "—" && list.indexOf(entry) === index);
    if (pathForms.length) {
        return pathForms;
    }
    const forms = typeof getConjugationSurfaceForms === "function"
        ? getConjugationSurfaceForms(source)
        : [];
    if (forms.length) {
        return forms;
    }
...
function normalizeGeneratedOutputVisibleCnvSlotValue(result = null, value = "") {
    ...
    return getVisibleCnvFormulaSurfaceForms(result).length
        ? normalizeVisibleCnvFormulaSlotEchoForSurfaceLine(normalized)
        : normalized;
}

function visibleCnvFormulaEchoesCoverSurfaceForms(echoes = [], surfaceForms = []) {
    const projectedSurfaces = (Array.isArray(echoes) ? echoes : [])
        .map((entry) => projectVisibleCnvFormulaEchoToSurface(
            typeof entry === "object" ? entry?.value : entry
        ))
        .filter(Boolean);
    const normalizedSurfaceForms = (Array.isArray(surfaceForms) ? surfaceForms : [])
...
function getCanonicalVisibleCnvFormulaEchoEntries(source = null) {
    return getFormulaSurfacePairsForGeneratedOutput(source)
        .filter((pair) => pair?.formulaRealizationRecord?.kind === "grammar-formula-realization-record")
        .map((pair) => ({
...
function getVisibleCnvFormulaConnectorRealizations(source = null) {
    ...
    return [...directRealizations, ...pathRealizations]
        .map((entry) => normalizeVisibleCnvFormulaSurfaceZero(entry).trim())
        .filter((entry, index, list) => entry && list.indexOf(entry) === index);
}
...
function getVisibleCnvFormulaPathRecordEntry(record = null, slotKey = "") {
    return (Array.isArray(record?.paths) ? record.paths : [])
...
function formatVisibleCnvFormulaEchoForPath(
    formulaEcho = "",
    record = null,
    sourceFrame = null,
    operationFrame = null
...
function getVisibleCnvFormulaEchoEntries(formulaEcho = "", source = null) {
    ...
    const slotEcho = getVisibleCnvFormulaSurfaceForms(source).length
        ? normalizeVisibleCnvFormulaSlotEchoForSurfaceLine(formula)
        : formula;
    const slotEntries = [{
        value: surfaceForms.length === 1
            ? alignVisibleCnvFormulaEchoToSurface(slotEcho, surfaceForms[0])
            : slotEcho,
    ...
    const pathEntries = (Array.isArray(path?.pathsBySurface) ? path.pathsBySurface : [])
        .filter((record) => surfaceSet.has(String(record?.surface || "").trim()))
        .map((record) => {
            ...
            return {
                value: alignVisibleCnvFormulaEchoToSurface(
                    pathFormula,
                    String(record?.surface || "").trim(),
                    record,
                    sourceFrame,
                    operationFrame
                ),
...
function formatVisibleCnvFormulaSurfacePairEcho(result = null, pair = null) {
    ...
    const record = (Array.isArray(path?.pathsBySurface) ? path.pathsBySurface : [])
        .find((entry) => normalizeAndrewsCnvCnnNominalSurfaceKey(entry?.surface || "") === surfaceKey);
    if (
        pair?.formulaRealizationRecord?.kind === "grammar-formula-realization-record"
        && !record
    ) {
        return formula;
    }
    ...
    const sourceFrame = buildVisibleCnvFormulaAlignmentSourceFrame(record);
    const operationFrame = buildVisibleCnvFormulaAlignmentOperationFrame(sourceFrame);
    return alignVisibleCnvFormulaEchoToSurface(
        formatVisibleCnvFormulaEchoForPath(formula, record, sourceFrame, operationFrame),
        pair?.surface || "",
        record,
        sourceFrame,
        operationFrame
...
function getLesson4DiagramNodeMeta(node = null, shell = null) {
    ...
    if (node.key === "predicate") {
        return String(shell?.formulaType || "").toUpperCase() === "NNC"
            ? "núcleo nominal"
            : "núcleo verbal + tiempo";
    }
    if (node.key === "subject") {
...
function appendLesson4DiagramNode(parent = null, node = null, shell = null, depth = 0) {
    ...
    const keyClass = node.key && node.key !== node.role
        ? `lesson4-inspector__diagram-node--${node.key}`
        : "";
    nodeEl.className = [
        "lesson4-inspector__diagram-node",
        roleClass,
        keyClass,
        node.role === "predicate-position" && node.slot === "Ø" ? "is-vacant" : "",
    ].filter(Boolean).join(" ");
    ...
    if (node.slot) {
        row.appendChild(createLesson4InspectorChip(
            formatVisibleAndrewsSlotToken(node.slot),
...
function appendLesson4NuclearClauseInspector(panel = null, shell = null) {
    ...
    chips.append(
        ...
        createLesson4InspectorChip("clasificación"),
    ...
    const structureFacts = document.createElement("div");
    ...
    const predicateStructure = formulaType === "NNC"
        ? "núcleo nominal = estado + base"
        : "núcleo verbal = valencia + base + tiempo";
    ...
    const activeFormula = lesson4.activeFormula && typeof lesson4.activeFormula === "object"
        ...
        : null;
    const predicatePosition = [
        activeFormula?.predicatePositionLabel || "",
        activeFormula?.predicatePositionStatusLabel || "",
    ].filter(Boolean).join(": ");
    ...
    if (pronounFrame) {
        const pronounRule = [
            pronounFrame.form === "affixal-only" ? "afijal" : "",
            pronounFrame.onlyReferringElements === true ? "referente único" : "",
        ].filter(Boolean).join(" · ");
        if (pronounRule) {
            facts.appendChild(createLesson4InspectorLine("pronombre", pronounRule));
        }
        const categories = Array.isArray(pronounFrame.categories)
            ? pronounFrame.categories
                .map((entry) => getLesson4InspectorPronounCategoryLabel(entry))
                .filter(Boolean)
            : [];
        ...
        facts.appendChild(createLesson4InspectorLine(
            "casos",
            "nominativo: sujeto · objetivo: predicado CNV · posesivo: predicado CNN"
        ));
    }
...
function activateCnnOutputModeForContinuation({ clearRoute = true } = {}) {
    if (typeof TENSE_MODE === "undefined" || !TENSE_MODE?.sustantivo) {
        return;
    }
    if (typeof setActiveNawatTenseMode === "function") {
        setActiveNawatTenseMode(TENSE_MODE.sustantivo, { syncOutput: true });
    ...
    if (typeof setActiveTenseMode === "function") {
        setActiveTenseMode(TENSE_MODE.sustantivo, {
            modeSystem: typeof TENSE_MODE_SYSTEM !== "undefined"
                ? (TENSE_MODE_SYSTEM.function || TENSE_MODE_SYSTEM.european || "function")
                : "function",
...
function buildVerbDerivedNominalizationProfileSubLabels(profile = null, { isNawat = false } = {}) {
    ...
    const operationalFrame = getAndrewsCnvCnnNominalRenderingFrame({ nominalizationProfile: profile });
    ...
    if (Array.isArray(operationalFrame?.formulaSurfacePairs) && operationalFrame.formulaSurfacePairs.length) {
        labels.push(`formas: ${operationalFrame.formulaSurfacePairs
            .map((entry) => `${entry.surface}: ${entry.sourceFormulaEcho || "SOURCE"} -> ${entry.targetFormulaEcho}`)
            .join(" / ")}`);
    }
...
function buildDenominalFamilyProfileSubLabels(profile = null) {
    ...
    const sourceContextRequiredCount = Number(
        contractRoutePreview?.finiteRouteSourceContextRequiredCount
        || contractRoutePreview?.finiteRouteSourceEvidenceRequiredCount
        || 0
    );
...
function buildNawatDenominalSourceEvidenceSubLabels(sourceEvidence = null) {
    ...
    if (
        evidence.boundaries?.sourceContextFromSelectedGeneratedStage === true
        || evidence.boundaries?.sourceEvidenceFromSelectedGeneratedStage === true
    ) {
        labels.push("Contexto: etapa generada");
    }
    if (
        evidence.boundaries?.sourceContextFromAndrewsContractRoute === true
        || evidence.boundaries?.sourceEvidenceFromAndrewsContractRoute === true
    ) {
...
function buildNawatLinkedGrammarStageSubLabels(stage = null, {
    ...
    const sourceContext = nextSource.sourceContext && typeof nextSource.sourceContext === "object"
        ...
        : (stage?.sourceContext && typeof stage.sourceContext === "object"
            ? stage.sourceContext
            : (nextSource.sourceEvidence && typeof nextSource.sourceEvidence === "object"
    ...
    return [
        ...
        Number.isFinite(candidateRouteCount) && candidateRouteCount > 0
            ...
            : "",
        ...buildNawatDenominalSourceEvidenceSubLabels(sourceContext),
    ].filter(Boolean);
}
...
function getAndrewsCnvCnnNominalRenderingFrame(frameLike = null) {
    ...
    const candidates = [
        frameLike?.nominalizationProfile?.operationalSuboperationFrame,
        frameLike?.result?.nominalizationProfile?.operationalSuboperationFrame,
        frameLike?.frames?.nominalizationProfile?.operationalSuboperationFrame,
        frameLike?.grammarFrame?.nominalizationProfile?.operationalSuboperationFrame,
        frameLike?.operationalSuboperationFrame,
        frameLike,
    ];
    return candidates.find((candidate) => (
        candidate
        && typeof candidate === "object"
        && candidate.kind === "andrews-cnv-cnn-operational-suboperation-frame"
    )) || null;
}
...
function getAndrewsCnvCnnNominalFormulaSurfacePairs(frame = null) {
    return Array.isArray(frame?.formulaSurfacePairs)
...
function getAndrewsCnvCnnNominalFormulaSurfacePairForSurface(frame = null, surface = "") {
    ...
    return getAndrewsCnvCnnNominalFormulaSurfacePairs(frame)
        .find((entry) => normalizeAndrewsCnvCnnNominalSurfaceKey(entry.surface) === surfaceKey)
        || null;
}
...
function getGeneratedOutputCanonicalFormulaSlots(result = null) {
    return getGeneratedOutputCanonicalFormulaRecords(result)
        .map((record) => record.formulaSlots)
        .find((slots) => slots && typeof slots === "object")
        || null;
}
...
function getGeneratedOutputStructuredContinuationFormulaSlots(result = null) {
    ...
    const formulaSlots = source.nuclearClauseShell?.formulaSlots
        || source.nuclearClauseShell?.slots
...
function getGeneratedOutputStructuredContinuationFormulaEcho(result = null, formulaSlots = null) {
    ...
    if (formulaSlots && typeof formulaSlots === "object" && typeof buildOrdinaryNncFormulaEchoFromSlots === "function") {
        return String(buildOrdinaryNncFormulaEchoFromSlots(formulaSlots) || "").trim();
    }
    return "";
}
...
function getGeneratedOutputStructuredContinuationPredicateStem(result = null) {
    ...
    const predicateSlot = formulaSlots.predicateStem
        || formulaSlots.predicate
        || formulaSlots.stem
        || null;
    ...
    return [
        predicateSlot.stem,
        predicateSlot.root,
        predicateSlot.formulaValue,
    ].map(normalizeGeneratedOutputStructuredContinuationStem).find(Boolean) || "";
}
...
function getGeneratedOutputCanonicalRealizationSurfaceForms(result = null) {
    return getGeneratedOutputCanonicalRealizationRecords(result)
        .flatMap(getCanonicalFormulaRealizationSurfaceForms)
        .filter((entry, index, list) => entry && list.indexOf(entry) === index);
}

function getGeneratedOutputSelectedRealizationVariant(result = null, variantIndex = 0) {
    const selectedIndex = Number.isInteger(Number(variantIndex)) && Number(variantIndex) >= 0
        ? Number(variantIndex)
        : 0;
    let currentIndex = 0;
    for (const record of getGeneratedOutputCanonicalRealizationRecords(result)) {
        ...
        for (const surface of surfaces) {
            ...
            const variantId = [
                formulaRealizationRecordId || formulaRecordId,
                `variant:${localIndex}`,
            ].filter(Boolean).join("#");
            return {
...
function getGeneratedOutputCanonicalFormulaRecordById(result = null, recordId = "") {
    ...
    return getGeneratedOutputCanonicalFormulaRecords(result)
        .find((record) => [
            record.id,
            record.formulaRecordId,
            record.formula,
            record.formulaText,
        ].map((entry) => String(entry || "").trim()).includes(id))
        || null;
}
...
function getGeneratedOutputCanonicalRealizationRecordById(result = null, recordId = "") {
    ...
    return getGeneratedOutputCanonicalRealizationRecords(result)
        .find((record) => [
            record.id,
            record.formulaRealizationRecordId,
        ].map((entry) => String(entry || "").trim()).includes(id))
        || null;
}
...
function buildGeneratedOutputTypedContinuationFrame(result = null, selectedVariant = null, {
    ...
    const grammarFrame = getConjugationResultFrame(result);
    const formulaRecord = getGeneratedOutputCanonicalFormulaRecordById(result, selectedVariant?.formulaRecordId || "")
        || getGeneratedOutputCanonicalFormulaRecord(result);
    const formulaRealizationRecord = getGeneratedOutputCanonicalRealizationRecordById(
        result,
        selectedVariant?.formulaRealizationRecordId || ""
    ) || getGeneratedOutputCanonicalRealizationRecords(result)[0] || null;
...
function getCanonicalFormulaSurfacePairsForGeneratedOutput(result = null) {
    ...
    formulaRecords.forEach((record) => {
        [
            ...
            .forEach((id) => formulaById.set(id, record));
    });
    return realizationRecords.flatMap((realizationRecord) => {
        const formulaRecord = formulaById.get(String(realizationRecord.formulaRecordId || "").trim())
            || formulaRecords[0]
            || null;
        ...
        const sourceFormula = String(
            formulaRecord?.sourceFrame?.formula
            || formulaRecord?.sourceFrame?.formulaText
            || ""
        ).trim();
        const routeFormula = String(
            formulaRecord?.sourceFrame?.label
            || formulaRecord?.sourceFrame?.formula
            || formulaRecord?.sourceFrame?.formulaText
            || formula
        ).trim();
...
function getGeneratedOutputRouteContractIdentityParts(result = null) {
    ...
    return [
        routeContract.routeId ? `route-id:${routeContract.routeId}` : "",
        routeContract.contractId ? `contract-id:${routeContract.contractId}` : "",
        routeContract.routeFamily ? `route-family:${routeContract.routeFamily}` : "",
        routeContract.routeStage ? `route-stage:${routeContract.routeStage}` : "",
        routeContract.outputKind ? `output-kind:${routeContract.outputKind}` : "",
        routeContract.unitKind ? `unit-kind:${routeContract.unitKind}` : "",
        routeContract.sourceContract?.sourceCategory ? `source-category:${routeContract.sourceContract.sourceCategory}` : "",
        routeContract.sourceContract?.sourceClauseKind ? `source-clause:${routeContract.sourceContract.sourceClauseKind}` : "",
        routeContract.sourceContract?.sourceVerb ? `source-verb:${routeContract.sourceContract.sourceVerb}` : "",
        routeContract.sourceContract?.sourceTenseValue ? `source-tense:${routeContract.sourceContract.sourceTenseValue}` : "",
        routeContract.sourceContract?.sourceCombinedMode ? `source-mode:${routeContract.sourceContract.sourceCombinedMode}` : "",
        routeContract.targetContract?.outputKind ? `target-output:${routeContract.targetContract.outputKind}` : "",
        routeContract.targetContract?.functionKind ? `target-function:${routeContract.targetContract.functionKind}` : "",
        routeContract.targetContract?.generationRoute ? `target-route:${routeContract.targetContract.generationRoute}` : "",
    ].filter(Boolean);
}
...
function getGeneratedOutputCanonicalContinuationIdentityParts(result = null) {
    ...
    const formulaParts = sources.flatMap((source) => getGeneratedOutputCanonicalFormulaRecords(source)).flatMap((record) => {
        ...
        return [
            ...
            sourceFrameId ? `formula-source:${sourceFrameId}` : "",
            routeId ? `formula-route:${routeId}` : "",
        ].filter(Boolean);
    });
    const realizationParts = sources.flatMap((source) => getGeneratedOutputCanonicalRealizationRecords(source)).flatMap((record) => {
        ...
        return [
            ...
            formulaRecordId ? `realization-formula:${formulaRecordId}` : "",
        ].filter(Boolean);
    });
    return [
        ...sources.flatMap((source) => getGeneratedOutputRouteContractIdentityParts(source)),
        ...formulaParts,
        ...realizationParts,
    ].filter(Boolean);
}
...
function getGeneratedOutputContinuationIdentityKey(result = null, context = {}) {
    ...
    const contextParts = [
        contextObject.namespace ? `namespace:${contextObject.namespace}` : "",
        contextObject.targetTense ? `target-tense:${contextObject.targetTense}` : "",
        contextObject.sourceUnit ? `source-unit:${contextObject.sourceUnit}` : "",
        contextObject.predicateNominalSourceTense ? `predicate-source-tense:${contextObject.predicateNominalSourceTense}` : "",
        contextObject.sourceObjectPrefix ? `source-object:${contextObject.sourceObjectPrefix}` : "",
        contextObject.sourceVariantId ? `source-variant:${contextObject.sourceVariantId}` : "",
        contextObject.targetVariantId ? `target-variant:${contextObject.targetVariantId}` : "",
    ].filter(Boolean);
...
function applyGeneratedOutputContinuationIdentityDataset(element = null, result = null, context = {}) {
    ...
    element.dataset.continuationIdentitySource = getGeneratedOutputCanonicalContinuationIdentityParts(result).length
        ? "canonical-formula-realization-record"
        : "route-contract";
    return identityKey;
}
...
function getDenominalAndrewsRouteContinuationIdentityKey(route = null, context = {}) {
    ...
    const contextObject = context && typeof context === "object" ? context : {};
    const parts = [
        "denominal-andrews-route-continuation",
        contextObject.namespace ? `namespace:${contextObject.namespace}` : "",
        contextObject.routeRecordId ? `route-record:${contextObject.routeRecordId}` : "",
        contextObject.targetTense ? `target-tense:${contextObject.targetTense}` : "",
        contextObject.objectPrefix ? `object-prefix:${contextObject.objectPrefix}` : "",
        route.contractId ? `contract:${route.contractId}` : "",
        route.routeTemplateId ? `template:${route.routeTemplateId}` : "",
        route.executableRuleId ? `rule:${route.executableRuleId}` : "",
        route.routeId ? `route-id:${route.routeId}` : "",
        route.sourceStem ? `source-stem:${route.sourceStem}` : "",
        route.targetVerbStem ? `target-stem:${route.targetVerbStem}` : "",
        route.targetInputValue ? `target-input:${route.targetInputValue}` : "",
        !route.targetInputValue && route.targetInput ? `target-input:${route.targetInput}` : "",
        route.targetStemClass ? `target-class:${route.targetStemClass}` : "",
        route.sourceEvidence?.id ? `source-evidence:${route.sourceEvidence.id}` : "",
    ].map((entry) => String(entry || "").trim()).filter(Boolean);
...
function getFormulaSurfacePairsForGeneratedOutput(result = null) {
    ...
    return pairs
        .map((entry) => {
            ...
            const formulaRecord = entry.formulaRecord && typeof entry.formulaRecord === "object"
                ? entry.formulaRecord
                : null;
            const realizationRecord = entry.formulaRealizationRecord && typeof entry.formulaRealizationRecord === "object"
                ? entry.formulaRealizationRecord
                : null;
            ...
            const sourceFormulaEcho = String(
                formulaRecord?.sourceFrame?.formula
                || formulaRecord?.sourceFrame?.formulaText
                || entry.sourceFormulaEcho
                || ""
            ).trim();
            const routeFormula = String(
                formulaRecord?.sourceFrame?.label
            ...
            return {
                ...
                sourceToTargetFormulaEcho: canonicalFormula
                    ? `${routeFormula} -> ${canonicalFormula}`
                    : String(entry.sourceToTargetFormulaEcho || "").trim(),
                andrewsToConjugatorFormulaEcho: canonicalFormula
                    ? `${routeFormula} -> ${canonicalFormula}`
                    : String(entry.andrewsToConjugatorFormulaEcho || entry.sourceToTargetFormulaEcho || "").trim(),
                formulaRecord,
                formulaRealizationRecord: realizationRecord,
            };
        })
        ...
        .filter((entry, index, list) => list.findIndex((candidate) => (
            ...
            && candidate.targetFormulaEcho === entry.targetFormulaEcho
            && String(candidate.formulaRealizationRecord?.id || "") === String(entry.formulaRealizationRecord?.id || "")
        )) === index);
}
...
function getFormulaSurfacePairForGeneratedOutput(result = null, surface = "") {
    ...
    if (requestedSurface) {
        const requestedPair = pairs.find((entry) => (
            normalizeAndrewsCnvCnnNominalSurfaceKey(entry.surface) === requestedSurface
        ));
        if (requestedPair) {
...
function applyAndrewsCnvCnnNominalRenderingDataset(element = null, frameLike = null, options = {}) {
    ...
    const missingRequirements = Array.isArray(frame.missingRequirements)
        ? frame.missingRequirements.map((entry) => String(entry || "").trim()).filter(Boolean)
        : [];
    const diagnostics = Array.isArray(frame.diagnostics)
        ? frame.diagnostics.map((entry) => String(entry || "").trim()).filter(Boolean)
        : [];
    ...
    element.dataset.andrewsCnvCnnNominalFormulaSurfacePairs = formulaSurfacePairs
    ...
    element.dataset.andrewsCnvCnnNominalSpellingAuthority = String(
        orthography.spellingAuthority || "Nawat/Pipil orthography bridge"
    ).trim();
    element.dataset.andrewsCnvCnnNominalClassicalSpellingRole = String(
        orthography.classicalSpellingRole || "structural-only"
    ).trim();
    element.dataset.andrewsCnvCnnNominalClassicalSurfaceImport = orthography.noClassicalSurfaceImport === false
        ? "allowed"
        : "blocked";
    return frame;
}
...
function getAndrewsCnvCnnNominalRenderedSurface(frameLike = null) {
    ...
    const formulaPairSurfaces = getAndrewsCnvCnnNominalFormulaSurfacePairs(frame)
    ...
    const forms = Array.isArray(frame.surfaceForms)
        ? frame.surfaceForms.map((entry) => String(entry || "").trim()).filter(Boolean)
        : [];
    return forms[0] || "";
}
...
function appendConjugationConversionSurfaceLines(surfaceText = null, surfaceDisplay = "", frameLike = null) {
    ...
    const groupedSurfaceDisplay = typeof formatConjugationDisplay === "function"
        ? formatConjugationDisplay(resolvedSurfaceDisplay)
        : resolvedSurfaceDisplay;
    return String(groupedSurfaceDisplay || "")
        .split(/\n+/)
        .map((line) => line.trim())
        .filter(Boolean)
        .flatMap((line) => {
            ...
            return parts.map((surface, index) => {
                if (index) {
                    surfaceText.appendChild(document.createTextNode(" / "));
                }
                const lineElement = document.createElement("span");
                lineElement.className = "conjugation-conversion-surface-line";
                applyAndrewsCnvCnnNominalRenderingDataset(lineElement, frameLike, { surface });
...
function resolveAndrewsCnvCnnRouteRecordContinuationGroupMeta(routeRecordId = "") {
    switch (String(routeRecordId || "").trim()) {
        case "cnv-predicate-to-cnn-nounstem-nominalization":
            return { key: "registro-cnv-cnn-nominalizada", routeRecordId: "cnv-predicate-to-cnn-nounstem-nominalization", mode: "sustantivo", eyebrow: "Registro", title: "CNV → CNN nominalizada" };
        case "cnv-core-to-cnn-nounstem-deverbal":
            return { key: "registro-cnv-cnn-deverbal", routeRecordId: "cnv-core-to-cnn-nounstem-deverbal", mode: "sustantivo", eyebrow: "Registro", title: "núcleo CNV → CNN deverbal" };
        case "cnn-nounstem-to-cnv-verbstem-denominal":
            return { key: "registro-cnn-cnv-denominal", routeRecordId: "cnn-nounstem-to-cnv-verbstem-denominal", mode: "verbo", eyebrow: "Registro", title: "CNN → CNV denominal" };
        case "cnv-verbstem-to-cnv-verbstem-deverbal":
            return { key: "registro-cnv-cnv-deverbal", routeRecordId: "cnv-verbstem-to-cnv-verbstem-deverbal", mode: "verbo", eyebrow: "Registro", title: "CNV → CNV deverbal" };
        case "cnv-to-cnn-to-cnv-loop":
            return { key: "registro-cnv-cnn-cnv", routeRecordId: "cnv-to-cnn-to-cnv-loop", mode: "verbo", eyebrow: "Registro", title: "CNV → CNN → CNV" };
        case "cnn-to-cnv-to-cnn-active-action-loop":
            return { key: "registro-cnn-cnv-cnn-accion", routeRecordId: "cnn-to-cnv-to-cnn-active-action-loop", mode: "sustantivo", eyebrow: "Registro", title: "CNN → CNV → CNN acción" };
        case "cnn-to-cnv-to-cnv-deverbal-chain":
            return { key: "registro-cnn-cnv-cnv", routeRecordId: "cnn-to-cnv-to-cnv-deverbal-chain", mode: "verbo", eyebrow: "Registro", title: "CNN → CNV → CNV" };
        default:
            return null;
    }
}
...
function getVisibleContinuationChipRouteExpectation(chip = null) {
    ...
    if (
        ...
        return expected({
            datasetKey: [
                "vncAdjectivalFunctionContinuation",
                "nominalizedVncAdjectivalFunctionContinuation",
                "preteritAgentiveNominalCompoundContinuation",
                "customaryAgentiveNominalCompoundContinuation",
                "preteritAgentiveComplementContinuation",
                "preteritAgentiveAdverbialContinuation",
            ].find(has) || "",
    ...
    if (
        ...
        return expected({
            datasetKey: [
                "ordinaryNncAdjectivalFunctionContinuation",
                "patientivoAdjectivalFunctionContinuation",
                "intensifiedAdjectivalFunctionContinuation",
                "compoundSourceAdjectivalFunctionContinuation",
                "activeActionNominalCompoundContinuation",
                "patientivoNominalCompoundContinuation",
                "actionNounSourceSubjectPossessor",
                "instrumentivoSourceSubjectPossessor",
            ].find(has) || "",
            routeRecordId: "cnv-core-to-cnn-nounstem-deverbal",
    ...
    if (
        ...
        return expected({
            datasetKey: [
                "activeActionCompoundEmbedContinuation",
                "customaryAgentiveCompoundEmbedContinuation",
                "preteritAgentiveCompoundEmbedContinuation",
                "patientivoCompoundEmbedContinuation",
                "patientivoCharacteristicPropertyEmbedContinuation",
                "preteritAgentiveOwnerhoodContinuation",
                "denominalCompoundAdjectivalFunctionContinuation",
            ].find(has) || "",
            routeRecordId: "cnv-to-cnn-to-cnv-loop",
...
function auditVisibleContinuationRouteRecordGroups(root = null) {
    ...
    const missingRouteRecordGroups = groups
        .filter((group) => !isAndrewsCnvCnnRouteRecordContinuationGroup(group))
        .map((group) => ({
...
function resolveContinuationActionGroupMeta(action = null) {
    ...
    if (classList.contains("calc-guidance__chip--mode-adjetivo")) {
        return { key: "registro-pendiente-adjetivo", mode: "sustantivo", eyebrow: "Registro pendiente", title: "continuación sin registro Andrews" };
    }
    if (classList.contains("calc-guidance__chip--mode-sustantivo")) {
        return { key: "registro-pendiente-sustantivo", mode: "sustantivo", eyebrow: "Registro pendiente", title: "continuación sin registro Andrews" };
    }
    return { key: "registro-pendiente-verbo", mode: "verbo", eyebrow: "Registro pendiente", title: "continuación sin registro Andrews" };
}
...
function getAndrewsCnvCnnRouteSourceRequirementIdsForRendering(frameLike = null) {
    const sourceRequirements = Array.isArray(frameLike?.sourceRequirement?.requirements)
        ? frameLike.sourceRequirement.requirements
        : [];
    const routeDiagnostics = Array.isArray(frameLike?.routeDiagnostics)
        ? frameLike.routeDiagnostics
        : [];
    return [
        ...sourceRequirements.map((entry) => String(entry?.id || entry || "").trim()),
        ...routeDiagnostics.map((entry) => String(entry?.id || entry?.code || "").trim()),
    ].filter(Boolean);
}
...
function getAndrewsCnvCnnRouteActionContractForRendering(frameLike = null) {
    ...
    const embedded = frame?.morphBoundaryFrame?.andrewsRouteActionContract
        || frame?.routeContract?.sourceContract?.andrewsRouteActionContract
        || frame?.routeContract?.targetContract?.andrewsRouteActionContract
        || frameLike.andrewsRouteActionContract
        || null;
    if (embedded?.routeRecordId) {
        return embedded;
    }
    if (typeof buildAndrewsCnvCnnBackAndForthRouteActionContract === "function") {
        return buildAndrewsCnvCnnBackAndForthRouteActionContract(frameLike, {
...
function buildVerbNominalContinuationRouteActionContractForRendering({
    ...
    if (
        ...
        || typeof buildAndrewsCnvCnnBackAndForthRouteActionContract !== "function"
    ) {
        return null;
    }
    ...
    const routeContract = frame?.routeContract && typeof frame.routeContract === "object"
        ? frame.routeContract
        : null;
    ...
    return buildAndrewsCnvCnnBackAndForthRouteActionContract(preview, {
        routeRecordId,
...
function formatAndrewsCnvCnnRouteTransitionForRendering(transition = "") {
    ...
    return value
        .replace(/\bCNV predicate\b/g, "predicado CNV")
        .replace(/\bCNV core\b/g, "núcleo CNV")
        .replace(/\bCNV verbstem\b/g, "tronco CNV")
        .replace(/\bCNN deverbal nounstem\b/g, "tronco deverbal CNN")
        .replace(/\bCNN active-action nounstem\b/g, "tronco de acción activa CNN")
        .replace(/\bCNN nounstem or root-ranked noun source\b/g, "fuente nominal CNN o raíz nominal")
        .replace(/\bCNN nounstem through ti\/hui denominal verbstem\b/g, "tronco CNN vía verbalización ti/hui")
        .replace(/\bCNN nounstem\b/g, "tronco CNN")
        .replace(/\bCNN denominal verbstem\b/g, "tronco denominal CNV")
        .replace(/\bCNV denominal\/deverbal verbstem\b/g, "tronco denominal/deverbal CNV")
        .replace(/\bCNV denominal verbstem\b/g, "tronco denominal CNV")
        .replace(/\bCNV deverbal verbstem\b/g, "tronco deverbal CNV")
        .replace(/->/g, "→");
}
...
function appendAndrewsCnvCnnRouteActionTitle(element = null, contract = null) {
    ...
    const obstacleGateIds = Array.isArray(contract.obstacleGateIds)
        ? contract.obstacleGateIds.map((entry) => String(entry || "").trim()).filter(Boolean)
        : [];
    const details = [
        `registro Andrews: ${transition || contract.routeRecordId}`,
        obstacleGateIds.length ? `compuertas Andrews: ${obstacleGateIds.join(", ")}` : "",
    ].filter(Boolean);
    ...
    if (typeof element.setAttribute === "function") {
        ...
        const nextLabel = [currentLabel, transition ? `registro ${transition}` : "", obstacleGateIds.length ? `${obstacleGateIds.length} compuertas Andrews` : ""]
            .filter(Boolean)
            .join(" · ");
        if (nextLabel) {
...
function clearAndrewsCnvCnnRouteActionDataset(element = null) {
    ...
    [
        ...
        "andrewsRouteRecordTransition",
        "andrewsRouteRecordKind",
        "andrewsRouteSourceUnit",
        "andrewsRouteTargetUnit",
        "andrewsRouteObstacleGates",
        "andrewsRouteObstacleGateCount",
        "andrewsRouteRankingAllowed",
        "andrewsRouteFunctionUseValenceGate",
        "andrewsRouteFunctionUseValenceReason",
        "andrewsRouteActionAllowed",
        "andrewsRouteDiagnosticOnly",
        "andrewsRouteHardGateBlocked",
        "andrewsRouteActionBlocked",
        "functionUseValenceHardGate",
        "andrewsUiDatasetKey",
        "andrewsUiExpectedRouteRecordId",
        "andrewsUiOperation",
    ].forEach((key) => {
...
function applyAndrewsCnvCnnRouteActionDataset(element = null, frameLike = null) {
    ...
    const functionUseValenceGate = contract.functionUseValenceGate && typeof contract.functionUseValenceGate === "object"
        ? contract.functionUseValenceGate
        : null;
    const generationAllowed = routeRankingAllowed && (typeof contract.generationAllowed === "boolean"
        ? contract.generationAllowed
        : !(element.disabled === true || element.classList?.contains?.("is-unavailable")));
...
function getFunctionUseContinuationRouteOwnershipOptions(contract = null) {
    ...
    const sourceRouteFrame = source.sourceRouteFrame
        || source.routeFrame
        || source.incorporationRouteFrame
        || source.sourceContract?.sourceRouteFrame
        || source.sourceContract?.routeFrame
        || source.targetContract?.sourceRouteFrame
        || source.targetContract?.routeFrame
        || source.entryRouteContract?.sourceRouteFrame
        || source.entryRouteContract?.routeFrame
        || grammarFrame?.routeContract?.sourceContract?.sourceRouteFrame
        || grammarFrame?.routeContract?.sourceContract?.routeFrame
        || grammarFrame?.routeContract?.targetContract?.sourceRouteFrame
        || grammarFrame?.routeContract?.targetContract?.routeFrame
        || grammarFrame?.sourceContract?.sourceRouteFrame
        || grammarFrame?.sourceContract?.routeFrame
        || grammarFrame?.targetContract?.sourceRouteFrame
        || grammarFrame?.targetContract?.routeFrame
        || grammarFrame?.participantFrame?.sourceRouteFrame
        || grammarFrame?.participantFrame?.routeFrame
        || source.adjectivalNncFunctionFrame?.sourceRouteFrame
        || source.adjectivalNncFunctionFrame?.routeFrame
        || source.adverbialNuclearFrame?.sourceRouteFrame
        || source.adverbialNuclearFrame?.routeFrame
        || null;
    const objectSlotOwnership = source.objectSlotOwnership
        || sourceRouteFrame?.objectSlotOwnership
        || source.sourceContract?.objectSlotOwnership
        || source.targetContract?.objectSlotOwnership
        || source.entryRouteContract?.objectSlotOwnership
        || source.entryRouteContract?.sourceContract?.objectSlotOwnership
        || source.entryRouteContract?.targetContract?.objectSlotOwnership
        || grammarFrame?.participantFrame?.objectSlotOwnership
        || grammarFrame?.sourceContract?.objectSlotOwnership
        || grammarFrame?.targetContract?.objectSlotOwnership
        || grammarFrame?.routeContract?.sourceContract?.objectSlotOwnership
        || grammarFrame?.routeContract?.targetContract?.objectSlotOwnership
        || null;
    const functionUseValenceGate = source.functionUseValenceGate
        || source.sourceContract?.functionUseValenceGate
        || source.targetContract?.functionUseValenceGate
        || source.entryRouteContract?.functionUseValenceGate
        || source.entryRouteContract?.sourceContract?.functionUseValenceGate
        || source.entryRouteContract?.targetContract?.functionUseValenceGate
        || grammarFrame?.routeContract?.sourceContract?.functionUseValenceGate
        || grammarFrame?.routeContract?.targetContract?.functionUseValenceGate
        || grammarFrame?.routeContract?.functionUseValenceGate
        || null;
    return {
        grammarFrame,
        sourceRouteFrame,
...
function applyAndrewsCnvCnnRouteActionHardGateState(element = null, contract = null) {
    ...
    const functionUseValenceGate = contract.functionUseValenceGate && typeof contract.functionUseValenceGate === "object"
        ? contract.functionUseValenceGate
        : null;
    const blocked = contract.routeRankingAllowed === false
        || functionUseValenceGate?.status === "blocked"
        || functionUseValenceGate?.routeRankingAllowed === false
        || functionUseValenceGate?.generationAllowed === false;
...
function renderDenominalAndrewsContractRouteContinuationForValue({
    ...
    const renderObjectPrefixChoice = ({
        ...
        const sourceRequirementIds = Array.isArray(route.sourceRequirement?.requirements)
            ...
            : [];
        const sourceContextRequired = route.finiteGenerationRequiresSourceContext === true
            || route.sourceRequirement?.finiteGenerationRequiresSourceContext === true;
        const sourceEvidenceRequired = sourceContextRequired
        ...
        const resolvedSourceEvidenceSatisfied = sourceEvidenceSatisfied
            || resolvedSourceContextSatisfied
            || route.sourceRequirement?.validationStatus === "source-evidence-satisfied";
        ...
        objectButton.className = [
            ...
            resolvedSourceEvidenceSatisfied ? "is-source-satisfied" : "",
            sourceEvidencePending ? "is-source-pending" : "",
            sourceEvidencePending ? "is-andrews-source-diagnostic" : "",
        ].filter(Boolean).join(" ");
        ...
        objectButton.dataset.sourceRequirementGate = sourceEvidencePending
        ...
        objectSubLabel.textContent = [
            ...
            sourceFinalPatternLabel,
            "registro CNN -> CNV",
            objectRouteActionContract?.obstacleGateIds?.length ? `compuertas ${objectRouteActionContract.obstacleGateIds.length}` : "",
            "fuente Andrews",
            resolvedTargetTense,
        ].filter(Boolean).join(" · ");
        ...
        objectButton.title = [
            ...
            objectRouteActionContract?.requiredBoundary ? `límite Andrews: ${objectRouteActionContract.requiredBoundary}` : "",
            objectRouteActionContract?.obstacleGateIds?.length ? `compuertas Andrews: ${objectRouteActionContract.obstacleGateIds.join(", ")}` : "",
    ...
    routes.forEach((route) => {
        ...
        const alreadyRendered = Array.from(actions.querySelectorAll("[data-denominal-andrews-contract-route-continuation]"))
            .some((button) => button.dataset.continuationIdentityKey === continuationIdentityKey);
        if (alreadyRendered) {
            return;
        }
        ...
        const sourceEvidenceSatisfied = sourceContextSatisfied
            || route.sourceRequirement?.validationStatus === "source-evidence-satisfied";
        ...
        continueSubLabel.textContent = [
            ...
            "registro CNN -> CNV",
            continueRouteActionContract?.obstacleGateIds?.length ? `compuertas ${continueRouteActionContract.obstacleGateIds.length}` : "",
            sourceEvidencePending ? "contexto Andrews pendiente" : "",
        ...
        continueButton.title = [
            ...
            `entrada verbal: ${targetInput}`,
            `tiempo: ${resolvedTargetTense}`,
            sourceEvidencePending ? "contexto Andrews pendiente; no bloquea generacion" : "",
            sourceEvidenceSatisfied ? "fuente Andrews satisfecha por etapa generada" : "",
            objectPrefixRequired ? "requiere objeto verbal" : "",
            continueRouteActionContract?.requiredBoundary ? `límite Andrews: ${continueRouteActionContract.requiredBoundary}` : "",
            continueRouteActionContract?.obstacleGateIds?.length ? `compuertas Andrews: ${continueRouteActionContract.obstacleGateIds.join(", ")}` : "",
            sourceFinalPatternLabel,
...
function buildGeneratedOutputSlotPredicateValue(predicateSlot = null) {
    const stem = normalizeGeneratedOutputSlotChipValue(
        predicateSlot?.displayStem || predicateSlot?.stem || predicateSlot?.formula || predicateSlot?.sourceFormula || predicateSlot?.surface || "",
        ""
    );
    if (!stem) {
        return "";
...
function isLesson46PreteritAgentiveLocativeResult(result = null) {
    return Boolean(
        ...
        && (
            result.kind === "lesson-46-3-1-a-preterit-agentive-locative-nnc"
            || result.andrewsSection === "46.3.1.a"
        )
    );
}

function buildGeneratedOutputVisibleCnvPredicateValue(result = null, predicateSlot = null) {
    return normalizeGeneratedOutputVisibleCnvSlotValue(
        result,
        buildGeneratedOutputSlotPredicateValue(predicateSlot)
    );
}
...
function getGeneratedOutputVisibleSurfaceForms(result = null) {
    const forms = typeof getConjugationSurfaceForms === "function"
        ? getConjugationSurfaceForms(result)
        : [];
    if (forms.length) {
        return forms;
    }
...
function buildGeneratedOutputVisibleCnvSubjectValue(result = null, subjectSlot = null) {
    return normalizeGeneratedOutputVisibleCnvSlotValue(
        result,
        buildGeneratedOutputVncSubjectValue(subjectSlot)
    );
}
...
function buildGeneratedOutputFormulaSlotsFromCnvPath(result = null) {
    ...
    const connector = Array.isArray(path?.surfaceNumberConnectorRealizations) && path.surfaceNumberConnectorRealizations[0]
        ? normalizeVisibleCnvFormulaSurfaceZero(path.surfaceNumberConnectorRealizations[0])
        : `${getPathFormulaValue("num1") || "0"}-${getPathFormulaValue("num2") || "0"}`;
...
function buildGeneratedOutputSlotChips(result = null, { includeFormula = true } = {}) {
    ...
    const formulaType = getGeneratedOutputSlotFormulaType(result, primarySlots)
        || getFormulaSurfacePairFormulaType(formulaSurfacePair);
    const shellFormulaEcho = normalizeGeneratedOutputSlotChipValue(
    ...
    const formulaEcho = normalizeGeneratedOutputSlotChipValue(
        formulaType === "NNC"
            ? (shellFormulaEcho || formulaSurfacePair?.targetFormulaEcho || "")
            : (formulaSurfacePair?.targetFormulaEcho || shellFormulaEcho || ""),
        ""
    );
    ...
    if (includeFormula && formulaEcho && formulaType === "VNC") {
        const formulaChipEntries = formulaSurfacePairs.length
            ? formulaSurfacePairs.map((pair) => ({
        ...
        formulaChipEntries.forEach((entry) => {
            ...
            const title = [
                ...
                entry.andrewsToConjugatorFormulaEcho || entry.sourceToTargetFormulaEcho ? `formula: ${entry.andrewsToConjugatorFormulaEcho || entry.sourceToTargetFormulaEcho}` : "",
        ...
        const useShellFormulaChip = formulaType === "NNC"
            && shellFormulaEcho
            && shellFormulaEcho !== String(formulaSurfacePair?.targetFormulaEcho || "").trim();
        const formulaChipPairs = !useShellFormulaChip && formulaSurfacePairs.length
            ? formulaSurfacePairs
            : (formulaSurfacePair ? [formulaSurfacePair] : []);
    ...
    if (formulaType === "VNC") {
        ...
        if (hasStructuredSlotAuthority && (directionalSlot?.isPresent || (directionalSlot?.prefix && directionalSlot.prefix !== "Ø"))) {
            pushChip(
                "directional",
                ANDREWS_RENDERING_TERMS.directional,
                normalizeGeneratedOutputSlotChipValue(
                    directionalSlot?.displayPrefix || directionalSlot?.prefix || "",
                    ""
                )
            );
        }
        ...
        if (formulaSurfacePairs.length) {
            const surfaceDisplay = surfaceForms.length
                ? surfaceForms.join(" / ")
                : formulaSurfacePairs.map((pair) => pair.surface).filter(Boolean).join(" / ");
        ...
        const predicateState = predicateSlot?.stateLabel
            || (isLesson46PreteritAgentiveLocativeResult(result) ? "adverbializado -0-" : "")
            || predicateSlot?.state
            || result.nncBasic?.categoryProfile?.predicateState?.label
            || result.nominalizationProfile?.predicateState?.value
            || "";
        const predicateStemLabel = isLesson46PreteritAgentiveLocativeResult(result)
            ? "STEM"
            : ANDREWS_RENDERING_TERMS.predicateStem;
...
function renderGeneratedOutputSlotChips(container = null, result = null, options = {}) {
    ...
    chips.forEach((chip) => {
        ...
        const formulaPair = chip.formulaPair && typeof chip.formulaPair === "object"
            ? chip.formulaPair
            : null;
        if (formulaPair) {
            ...
            chipEl.dataset.andrewsToConjugatorFormulaEcho = String(
                formulaPair.andrewsToConjugatorFormulaEcho
                || formulaPair.sourceToTargetFormulaEcho
                || ""
            ).trim();
        }
        if (chip.title) {
...
function buildGrammarFrameRouteAuditModelForRendering(frame = null) {
    ...
    const sourceContext = authorityFrame.sourceContext && typeof authorityFrame.sourceContext === "object"
        ? authorityFrame.sourceContext
        : (authorityFrame.sourceEvidence && typeof authorityFrame.sourceEvidence === "object"
    ...
    const sourceEvidence = authorityFrame.sourceEvidence && typeof authorityFrame.sourceEvidence === "object"
        ? authorityFrame.sourceEvidence
        : sourceContext;
    const spellingAuthority = String(
        orthographyFrame.spellingAuthority
        || authorityFrame.spellingAuthority
        || "Nawat/Pipil orthography bridge"
    ).trim();
    const sourceFrame = {
        ...
        orthographyFrame: {
            ...
            spellingAuthority,
            classicalSurfaceImport: orthographyFrame.noClassicalSurfaceImport === false
                ? "allowed"
                : "blocked",
        },
        diagnosticFrame: {
...
function applyGrammarFrameRouteDataset(element = null, frameLike = null) {
    ...
    const sourceEvidence = authorityFrame.sourceEvidence && typeof authorityFrame.sourceEvidence === "object"
        ? authorityFrame.sourceEvidence
        : sourceContext;
    const sourceContextBoundaries = sourceContext.boundaries && typeof sourceContext.boundaries === "object"
        ? sourceContext.boundaries
        : {};
    const authorityRefs = Array.isArray(authorityFrame.andrewsRefs)
        ...
        : [];
    const orthographyRefs = Array.isArray(authorityFrame.orthographyRefs)
        ? authorityFrame.orthographyRefs.map((entry) => String(entry || "").trim()).filter(Boolean)
        : [];
    const sourceEvidenceBoundaries = sourceEvidence.boundaries && typeof sourceEvidence.boundaries === "object"
        ...
        : {};
    const sourceContextFlags = Object.keys(sourceContextBoundaries)
        .filter((key) => sourceContextBoundaries[key] === true)
        .sort();
    ...
    element.dataset.grammarSpellingAuthority = String(
        orthographyFrame.spellingAuthority
        || authorityFrame.spellingAuthority
        || "Nawat/Pipil orthography bridge"
    ).trim();
    element.dataset.grammarClassicalSurfaceImport = orthographyFrame.noClassicalSurfaceImport === false
        ? "allowed"
        : "blocked";
    ...
    if (formulaSurfacePair) {
        ...
        element.dataset.formulaSurfacePairs = getFormulaSurfacePairsForGeneratedOutput(frameLike)
            .map((entry) => `${entry.surface}=>${entry.andrewsFormulaEcho || entry.sourceFormulaEcho || ""}=>${entry.conjugatorFormulaEcho || entry.targetFormulaEcho}`)
            .join("|");
    }
    applyAndrewsCnvCnnRouteActionDataset(element, frameLike);
...
function renderOutputSelectionRequiredPlaceholder({
    ...
    mode = typeof getActiveTenseMode === "function" ? getActiveTenseMode() : TENSE_MODE.verbo,
    ...
    if (typeof applyAndrewsTenseAuthorityDataset === "function") {
        applyAndrewsTenseAuthorityDataset(block, {
            tenseValue,
            mode,
            blockKind,
        });
...
function renderParticleModeConjugations({
    ...
    if (groupedInventory.length) {
        groupedInventory.forEach((group) => {
            ...
            if (typeof applyAndrewsTenseAuthorityDataset === "function") {
                applyAndrewsTenseAuthorityDataset(groupBlock, {
    ...
    if (typeof applyAndrewsTenseAuthorityDataset === "function") {
        applyAndrewsTenseAuthorityDataset(boundaryBlock, {
            tenseValue: "particle-boundary",
            mode: TENSE_MODE.particula,
            blockKind: "Particula inventario",
        });
...
function renderOrdinaryNncConjugations({
    ...
    if (typeof applyAndrewsTenseAuthorityDataset === "function") {
        applyAndrewsTenseAuthorityDataset(controlsBlock, {
            mode: TENSE_MODE.sustantivo,
            blockKind: "controles CNN ordinaria",
        });
    }
    ...
    const nounClassToggle = buildToggleControl({
        options: [
            { id: "t", label: "t", title: "conector t: (...V)t" },
            { id: "ti", label: "ti", title: "conector ti: (...C)ti" },
            { id: "in", label: "in", title: "conector in: (...C)in" },
            { id: "zero", label: "Ø", title: "conector Ø: (...C/V)" },
        ].map((entry) => ({
            ...entry,
            title: fixtureNounClass && entry.id !== fixtureNounClass
                ? `${entry.title}; ficha registrada: ${getOrdinaryNncNounClassLabel(fixtureNounClass)}`
                : entry.title,
        })),
        ...
        onSelect: (id) => {
            setOrdinaryNncGenerationState({ nounClass: id || "zero" });
            rerender();
        },
        ...
        options: [
            {
                id: "inanimate",
                label: "Inanim",
                title: hasFixtureAnimacy && fixtureAnimacy !== "inanimate"
                    ? "ficha registrada: animado"
                    : "solo sujeto Ø; plural distributivo",
            },
            {
                id: "animate",
                label: "Anim",
                title: hasFixtureAnimacy && fixtureAnimacy !== "animate"
                    ? "ficha registrada: inanimado"
    ...
    const rowFormulaEcho = rowFormulaPair?.targetFormulaEcho || (typeof buildOrdinaryNncFormulaEchoFromSlots === "function"
    ...
    const renderOrdinaryNncAdjectivalFunctionContinuation = () => {
        if (
            ...
            || typeof buildStructuredOrdinaryAdjectivalNncFunctionOutput !== "function"
        ...
        const contract = buildStructuredOrdinaryAdjectivalNncFunctionOutput({
...
function renderActiveConjugations({
    ...
    const activeFormalTenseMode = typeof getActiveNawatTenseModeForCurrentSelection === "function"
        ...
        : activeTenseMode;
    const isFormalCnvAdjectivalTense = activeFormalTenseMode === TENSE_MODE.verbo
        && typeof isFormalCnvFunctionTense === "function"
        && isFormalCnvFunctionTense(selectedFunctionTense)
        && selectedFunctionTense !== "pasado-remoto-adverbio-activo";
    const isFormalCnvAdverbialTense = activeFormalTenseMode === TENSE_MODE.verbo
        && selectedFunctionTense === "pasado-remoto-adverbio-activo";
    const isFormalCnnAdjectivalTense = activeFormalTenseMode === TENSE_MODE.sustantivo
        && typeof isFormalCnnFunctionTense === "function"
        && isFormalCnnFunctionTense(selectedFunctionTense);
    const adjectivalFunctionOverride = (
        activeTenseMode === TENSE_MODE.adjetivo
        || isFormalCnvAdjectivalTense
        || isFormalCnnAdjectivalTense
    )
    ...
    if (isOrdinaryNncGenerationModeEnabled() && !tenseOverride) {
...
function buildVerbTenseBlock({
    ...
    if (typeof applyAndrewsTenseAuthorityDataset === "function") {
        applyAndrewsTenseAuthorityDataset(tenseBlock, {
            tenseValue,
            mode: TENSE_MODE.verbo,
            blockKind: "CNV",
        });
    ...
    const appendVerbToPatientivoRowContinuation = ({
        ...
        const actions = ensureVerbRowConversionActions({ value, sourceDisplay, frameLike: evaluation?.result });
        if (!actions) {
            return false;
        }
        const continueButton = document.createElement("button");
    ...
    const appendVerbToNominalRowContinuations = ({
        ...
        specs.forEach((spec) => {
            ...
            continueButton.addEventListener("click", () => {
                activateCnnOutputModeForContinuation({ clearRoute: true });
    ...
    const appendVncAdjectivalFunctionRowContinuation = ({
        ...
        if (
            ...
            || typeof buildVncAdjectivalNncOperationFrame !== "function"
            || typeof applyAdjectivalNncFunctionToVerbEntry !== "function"
        ) {
            return false;
        }
        ...
        const contracts = forms.map((form, index) => {
            ...
            const operationFrame = buildVncAdjectivalNncOperationFrame({
                sourceContinuationFrame,
            ...
            return buildVncAdjectivalNncFunctionOutput({
                vncSurface: form,
                sourceVerb: verb,
                sourceTenseValue: tenseValue,
                sourceCombinedMode,
    ...
    rowNodes.forEach((row) => {
        ...
        const entry = {
            ...
            ...(typeof getUnifiedVerbOutputGrammarDatasetMetadata === "function"
                ? getUnifiedVerbOutputGrammarDatasetMetadata(row.dataset)
                : {}),
            ...(typeof getVisibleConjugationFormulaSurfaceExportMetadata === "function"
...
function renderVerbConjugationsCore({
    ...
    const resolvedOutputTenseValue = resolveVerbTenseValue({ modeKey, tenseValue });
    if (
        modeKey !== "universal"
        && typeof isAndrewsCnvTenseGenerationGateAllowed === "function"
        && !isAndrewsCnvTenseGenerationGateAllowed(resolvedOutputTenseValue, TENSE_MODE.verbo)
    ) {
        clearUnifiedVerbOutputDataset();
        renderOutputSelectionRequiredPlaceholder({
            containerId,
        ...
        if (typeof syncAndrewsTenseAuthorityDomAudit === "function") {
            syncAndrewsTenseAuthorityDomAudit(container, { mode: TENSE_MODE.verbo });
        }
        return;
    }
...
function renderLocativoTemporalConjugations({
    ...
    const { grid } = createObjectSectionGrid(container);
    const sourceColumns = modeFilter == null && !isPreteritAgentiveLocativeNnc
        ? createSourceModeColumns(grid, isNawat)
        : null;
...
function buildNounTabRenderContext({
    ...
    const showObjectToggle = !isPreteritAgentiveLocativeNnc
        && nounObjectSlotStates.some((slot) => slot.showToggle);
...
function renderNounConjugations({
    ...
    const getActionNominalSourceEntriesFromEvaluation = (evaluation = {}) => {
        ...
        return getActionNominalSurfacesFromEvaluation(evaluation)
            .map((surface, index) => {
                ...
                const sourceVariantId = selectedVariant?.variantId
                    || (hasConjugationResultFrame(result) ? `structured-result-frame:${index}` : `legacy-display:${index}`);
                return {
                    surface,
                    sourceVariantId,
                    selectedVariant,
    ...
    const getPreteritAgentiveGeneralUseStemsFromEvaluation = (evaluation = {}) => {
        ...
        const predicateState = String(
            (hasResultFrame ? structuredPredicateSlot?.state : slots.predicate?.state)
            || result?.nominalizationProfile?.predicateState?.value
            || ""
        ).trim();
        const possessorPrefix = String(
            (hasResultFrame ? structuredPredicateSlot?.stateSlot?.possessorPrefix : slots.predicate?.stateSlot?.possessorPrefix)
    ...
    const renderActiveActionCompoundEmbedContinuation = ({
        ...
        const contracts = actionNominalEntries.flatMap((sourceEntry) => (
            matrixInventory.map((matrixSpec) => ({
    ...
    const renderActiveActionNominalCompoundContinuation = ({
        ...
        const contracts = actionNominalEntries.flatMap((sourceEntry) => (
            matrixInventory.map((matrixSpec) => ({
                ...buildActiveActionNominalCompoundContinuationContract({
                    actionNominalSurface: sourceEntry.surface,
                    sourceSurface: sourceEntry.surface,
                    sourceFormulaSlots,
                    sourceFormulaEcho,
        ...
        const applyPreteritAgentiveNominalContract = (contract) => {
            if (typeof applyActiveActionNominalCompoundToOrdinaryNncEntry === "function") {
                applyActiveActionNominalCompoundToOrdinaryNncEntry({
                    ...
                    sourceFormulaEcho: contract.sourceFormulaEcho || sourceFormulaEcho || "",
                    routeRecordId: "cnv-predicate-to-cnn-nounstem-nominalization",
                    routeDataset: { preteritAgentiveNominalCompoundContinuation: "true" },
    ...
    const renderCompoundSourceAdjectivalFunctionContinuation = ({
        ...
        if (
            ...
            || typeof buildCompoundSourceAdjectivalNncOperationFrame !== "function"
            || typeof applyAdjectivalNncFunctionToVerbEntry !== "function"
        ) {
            return false;
        }
        ...
        const contracts = forms.map((form, index) => {
            ...
            const operationFrame = buildCompoundSourceAdjectivalNncOperationFrame({
                sourceContinuationFrame,
                sourceCompoundFrame,
            ...
            return buildCompoundSourceAdjectivalNncFunctionOutput({
    ...
    const renderDenominalCompoundAdjectivalFunctionContinuation = ({
        ...
        const contracts = forms.map((form, index) => {
            ...
            const operationFrame = buildDenominalCompoundAdjectivalNncOperationFrame({
                sourceContinuationFrame,
            ...
            return buildDenominalCompoundAdjectivalNncFunctionOutput({
    ...
    const renderIntensifiedAdjectivalFunctionContinuation = ({
        ...
        if (
            ...
            || typeof buildIntensifiedAdjectivalNncOperationFrame !== "function"
            || typeof applyAdjectivalNncFunctionToVerbEntry !== "function"
        ) {
            return false;
        }
        ...
        const contractEntries = forms.map((form, index) => {
            ...
            const operationFrame = buildIntensifiedAdjectivalNncOperationFrame({
                sourceContinuationFrame,
                role: "predicate-surface",
            });
            const contract = buildIntensifiedAdjectivalNncOutput({
                sourceSurface: form,
                sourceFormulaSlots,
                sourceFormulaEcho,
                sourceContinuationFrame,
    ...
    const renderPatientivoAdjectivalFunctionContinuation = ({
        ...
        if (
            ...
            || typeof buildPatientivoAdjectivalNncOperationFrame !== "function"
        ) {
            return false;
        }
        const forms = getConjugationSurfaceForms(evaluation?.result)
        ...
        const contracts = forms.map((form, index) => {
            ...
            const operationFrame = buildPatientivoAdjectivalNncOperationFrame({
                sourceContinuationFrame,
                patientivoSource,
                role: "predicate-surface",
            });
            ...
            return buildPatientivoAdjectivalNncFunctionOutput({
    ...
    const renderNominalizedVncAdjectivalFunctionContinuation = ({
        ...
        if (
            ...
            || typeof buildNominalizedVncAdjectivalNncOperationFrame !== "function"
        ) {
            return false;
        }
        ...
        const contracts = forms.map((form, index) => {
            ...
            const operationFrame = buildNominalizedVncAdjectivalNncOperationFrame({
                sourceContinuationFrame,
            ...
            return buildNominalizedVncAdjectivalNncFunctionOutput({
    ...
    const renderCalificativoInstrumentivoSourceSubjectGeneralUseContinuation = ({
        ...
        const sourceSubjectFrame = typeof buildAndrewsSourceSubjectFrame === "function"
            ? buildAndrewsSourceSubjectFrame({
                subjectPrefix: selection?.subjectPrefix || "",
                subjectSuffix: selection?.subjectSuffix || "",
                sourceUnit: "CNV",
                sourceTense: resolvedNominalControlCombinedMode === COMBINED_MODE.nonactive
                    ? "distant-past-passive"
                    : "distant-past-active",
            })
            : null;
        const sourceSubjectPossessorOperationFrame = typeof buildSourceSubjectPossessorOperationFrame === "function"
            ? buildSourceSubjectPossessorOperationFrame({
                ...
                andrewsRef: resolvedNominalControlCombinedMode === COMBINED_MODE.nonactive
                    ? "Andrews 36.10"
                    : "Andrews 36.11",
            })
            : null;
        ...
        const continuationIdentityKey = getGeneratedOutputContinuationIdentityKey(
        ...
        if (
            ...
            || !sourceIdentityKey
            || !targetIdentityKey
            || sourceIdentityKey === targetIdentityKey
    ...
    const renderInstrumentivoSourceSubjectPossessiveContinuation = ({
        ...
        if (possessivePrefix) {
            ...
            const continuationIdentityKey = getGeneratedOutputContinuationIdentityKey(
                [evaluation?.result, possessiveEvaluation?.result],
                identityContext
            );
            if (
    ...
    const renderRowsForList = (entry = {}) => {
        ...
        const objectSlotSelectionModels = isPreteritAgentiveLocativeNnc
            ? []
            : buildNounObjectSlotModelsForState();
        const possessorSelections = getPossessorSelectionsForId(activePossessor);
        iterateNounObjectSlotSelections(objectSlotSelectionModels, (selectedBySlot) => {
            const objectPrefix = selectedBySlot.object || "";
            const indirectObjectMarker = selectedBySlot.object2 || "";
            ...
            selections.forEach((subjectEntry) => {
                ...
                possessorSelections.forEach((possessorPrefix) => {
                    ...
                    const personLabel = document.createElement("div");
                    personLabel.className = "person-label";
                    personLabel.textContent = isPreteritAgentiveLocativeNnc
                        ? "CNN"
                        : (displaySelection
                    ...
                    const personSub = document.createElement("div");
                    personSub.className = "person-sub";
                    const basePersonSub = isPreteritAgentiveLocativeNnc
                        ? ""
                        : (displayPersonSubLabel
                    ...
                    const objectLabel = objectMarkers.length
                        ...
                        : (suppressZeroObjectLabel
                            ? ""
                            : getNounZeroObjectComboLabel(isNawat, { isImpersonalDummy: isDummyImpersonalRow }));
                    let possessorLabel = isPreteritAgentiveLocativeNnc
                        ? ""
                        : getPossessorLabel(possessorPrefix, isNawat);
                    label.appendChild(personLabel);
                    label.appendChild(personSub);
...
async function installDeveloperHooksIfEnabled() {
    ...
    try {
        ...
        const hooksModuleUrl = new URL("./scripts/dev_hooks.mjs", window.location.href).href;
