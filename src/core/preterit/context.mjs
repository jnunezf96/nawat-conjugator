...
export function createPreteritContext(targetObject = globalThis) {
    ...
    function getPretUniversalCoreVowelCount(verb) {
      ...
      if (lastLIndex >= 0 && lastLIndex < verb.length - 1) {
        return targetObject.getTotalVowelCount(verb.slice(lastLIndex + 1));
      }
      return targetObject.getTotalVowelCount(verb);
    }
    function getUniversalReplacementStem(verb, options = {}) {
      const baseSpec = typeof targetObject.buildPretPerfectiveReplacementBaseSpec === "function" ? targetObject.buildPretPerfectiveReplacementBaseSpec(verb, options) : null;
      return typeof targetObject.realizePretBaseSpec === "function" ? targetObject.realizePretBaseSpec(baseSpec, "") : "";
    }
    ...
    function applyPretUniversalDeletionShift(stem, options = {}) {
      const specs = [];
      if (typeof targetObject.buildPretDeletionShiftBaseSpec === "function") {
        if (stem.endsWith("kw")) {
          specs.push(targetObject.buildPretDeletionShiftBaseSpec(stem, "identity", options));
        } else if (stem.endsWith("w")) {
          specs.push(targetObject.buildPretDeletionShiftBaseSpec(stem, "w-keep", options));
          specs.push(targetObject.buildPretDeletionShiftBaseSpec(stem, "w-to-j", options));
      ...
      const realized = specs.map(spec => typeof targetObject.realizePretBaseSpec === "function" ? targetObject.realizePretBaseSpec(spec, "") : "").filter(Boolean);
    ...
    const PRET_UNIVERSAL_RULE_TIER_ORDER = Object.freeze(["override", "path", "monosyllable", "forced", "shape", "default"]);
    const PRET_UNIVERSAL_DEFAULT_RULE_LABEL = "default class rules";
    const PRET_UNIVERSAL_DEFAULT_RULE_TIER = "default";
    const PRET_UNIVERSAL_CLASS_ORDER_INDEX = Object.freeze(PRET_UNIVERSAL_CLASS_ORDER.reduce((acc, classKey, index) => {
      acc[classKey] = index;
      return acc;
    }, {}));
    ...
    function getPretDenominalSourceContext({
      ...
      if (!normalizedVerb || normalizedVerb.length <= normalizedMatrix.length || !normalizedVerb.endsWith(normalizedMatrix)) {
        return defaults;
      }
      ...
      const sourceEndsWithVowel = Boolean(lastLetter && targetObject.isVerbLetterVowel(lastLetter));
      const sourceEndsWithConsonant = Boolean(lastLetter && targetObject.isVerbLetterConsonant(lastLetter));
      return {
    ...
    function sortPretUniversalClassKeys(values) {
      const entries = Array.isArray(values) ? values : Array.from(values || []);
      return Array.from(new Set(entries.filter(Boolean))).sort((left, right) => {
        const leftIndex = Object.prototype.hasOwnProperty.call(PRET_UNIVERSAL_CLASS_ORDER_INDEX, left) ? PRET_UNIVERSAL_CLASS_ORDER_INDEX[left] : Number.MAX_SAFE_INTEGER;
        const rightIndex = Object.prototype.hasOwnProperty.call(PRET_UNIVERSAL_CLASS_ORDER_INDEX, right) ? PRET_UNIVERSAL_CLASS_ORDER_INDEX[right] : Number.MAX_SAFE_INTEGER;
    ...
    function inferPretUniversalRuleTier(ruleLabel = "") {
      ...
      if (label === PRET_UNIVERSAL_DEFAULT_RULE_LABEL || label === "unpronounceable root") {
        return PRET_UNIVERSAL_DEFAULT_RULE_TIER;
      }
      return PRET_UNIVERSAL_DEFAULT_RULE_TIER;
    }
    ...
    function clonePretUniversalContext(context) {
      ...
      if (context.rightEdgeDescriptor && typeof context.rightEdgeDescriptor === "object") {
        clone.rightEdgeDescriptor = {
          ...context.rightEdgeDescriptor
        };
      }
      if (context.verbOverride && typeof context.verbOverride === "object") {
        clone.verbOverride = {
          ...context.verbOverride
        };
    ...
    function formatPretRightEdgeProfile(syllables = []) {
      return (Array.isArray(syllables) ? syllables : []).map(syllable => String(syllable?.form || "")).filter(Boolean).join("|");
    }
    function formatPretJuncture(syllables = []) {
      const forms = (Array.isArray(syllables) ? syllables : []).map(syllable => String(syllable?.form || "")).filter(Boolean);
      if (!forms.length) {
        return "";
      }
    ...
    function normalizePretRightEdgeQuery(query = {}) {
      ...
      return buildPretOrderedRightEdgeQuery({
        ...
        rightEdgeProfileSuffixes: Array.isArray(query.rightEdgeProfileSuffixes) ? query.rightEdgeProfileSuffixes : query.rightEdgeProfileSuffix ? [query.rightEdgeProfileSuffix] : [],
    ...
    function pretRightEdgeDescriptorMatchesQuery(descriptor, query = {}) {
      ...
      const rightEdgeProfileSuffixes = Array.isArray(orderedQuery.rightEdgeProfileSuffixes) ? orderedQuery.rightEdgeProfileSuffixes : [];
      if (rightEdgeProfileSuffixes.length && !rightEdgeProfileSuffixes.some(suffix => descriptor.rightEdgeProfile === suffix || descriptor.rightEdgeProfile.endsWith(`|${suffix}`))) {
        return false;
      }
      ...
      const finalOnsets = Array.isArray(orderedQuery.finalOnsets) ? orderedQuery.finalOnsets : orderedQuery.finalOnset ? [orderedQuery.finalOnset] : [];
      if (finalOnsets.length && !finalOnsets.includes(descriptor.finalOnset)) {
        return false;
      }
      const finalNuclei = Array.isArray(orderedQuery.finalNuclei) ? orderedQuery.finalNuclei : orderedQuery.finalNucleus ? [orderedQuery.finalNucleus] : [];
      if (finalNuclei.length && !finalNuclei.includes(descriptor.finalNucleus)) {
        return false;
      }
      const previousForms = Array.isArray(orderedQuery.previousForms) ? orderedQuery.previousForms : orderedQuery.previousForm ? [orderedQuery.previousForm] : [];
      if (previousForms.length && !previousForms.includes(descriptor.previousForm)) {
        return false;
      }
      const previousOnsets = Array.isArray(orderedQuery.previousOnsets) ? orderedQuery.previousOnsets : orderedQuery.previousOnset ? [orderedQuery.previousOnset] : [];
      if (previousOnsets.length && !previousOnsets.includes(descriptor.previousOnset)) {
        return false;
      }
      const previousNuclei = Array.isArray(orderedQuery.previousNuclei) ? orderedQuery.previousNuclei : orderedQuery.previousNucleus ? [orderedQuery.previousNucleus] : [];
      if (previousNuclei.length && !previousNuclei.includes(descriptor.previousNucleus)) {
        return false;
      }
      const previousCodas = Array.isArray(orderedQuery.previousCodas) ? orderedQuery.previousCodas : orderedQuery.previousCoda ? [orderedQuery.previousCoda] : [];
      ...
      const antepenultimateForms = Array.isArray(orderedQuery.antepenultimateForms) ? orderedQuery.antepenultimateForms : orderedQuery.antepenultimateForm ? [orderedQuery.antepenultimateForm] : [];
    ...
    function pretContextHasAnyRightEdge(context, queries = []) {
      return (Array.isArray(queries) ? queries : []).some(query => pretContextHasRightEdge(context, query));
    }
    ...
    function normalizePretDescriptorQuery(query = {}) {
      ...
      return buildPretOrderedDescriptorQuery({
        ...
        junctures: Array.isArray(query.junctures) ? query.junctures : query.juncture ? [query.juncture] : [],
        endingFamily: query.endingFamily || "",
        modifierSet: Object.prototype.hasOwnProperty.call(query, "modifierSet") ? Array.isArray(query.modifierSet) ? query.modifierSet : [] : null,
    ...
    function makePretDescriptorQuery(endingFamily = "", rightEdgeProfileOrProfiles = null, options = {}) {
      const rightEdgeProfiles = Array.isArray(rightEdgeProfileOrProfiles) ? rightEdgeProfileOrProfiles.filter(Boolean) : rightEdgeProfileOrProfiles ? [String(rightEdgeProfileOrProfiles)] : [];
      const modifierSet = Object.prototype.hasOwnProperty.call(options, "modifierSet") ? Array.isArray(options.modifierSet) ? options.modifierSet.filter(Boolean) : [] : null;
      ...
      const junctures = Array.isArray(options.junctures) ? options.junctures.filter(Boolean) : options.juncture ? [String(options.juncture)] : buildPretJuncturesFromProfiles(rightEdgeProfiles);
      return buildPretOrderedDescriptorQuery({
        rightEdgeProfiles,
        junctures,
        endingFamily,
        ...
        excludeModifiers: Array.isArray(options.excludeModifiers) ? options.excludeModifiers.filter(Boolean) : []
      });
    }
    function pretDescriptorMatchesQuery(descriptor, query = {}) {
      ...
      const endingFamilies = Array.isArray(query.endingFamilies) ? query.endingFamilies : orderedQuery.endingFamily ? [orderedQuery.endingFamily] : [];
      if (endingFamilies.length && !endingFamilies.includes(descriptor.endingFamily)) {
        return false;
      }
      const profiles = Array.isArray(orderedQuery.rightEdgeProfiles) ? orderedQuery.rightEdgeProfiles : orderedQuery.rightEdgeProfile ? [orderedQuery.rightEdgeProfile] : [];
    ...
    function pretDescriptorListHasQuery(descriptors = [], query = {}) {
      return (Array.isArray(descriptors) ? descriptors : []).some(descriptor => pretDescriptorMatchesQuery(descriptor, query));
    }
    function pretDescriptorListHasAnyQuery(descriptors = [], queries = []) {
      return (Array.isArray(queries) ? queries : []).some(query => pretDescriptorListHasQuery(descriptors, query));
    }
    function formatPretDescriptorLabel(descriptor, options = {}) {
      if (!descriptor || typeof descriptor !== "object") {
        return "";
      }
      const activeProfile = options.activeRightEdgeProfile && Array.isArray(descriptor.rightEdgeProfiles) && descriptor.rightEdgeProfiles.includes(options.activeRightEdgeProfile) ? options.activeRightEdgeProfile : descriptor.rightEdgeProfiles?.[0] || "";
      const parts = [];
      ...
      const modifiers = Array.isArray(descriptor.modifiers) ? descriptor.modifiers.filter(modifier => modifier !== "aggregate") : [];
      return parts.concat(modifiers).join(" · ");
    }
    const PRET_DESCRIPTOR_QUERIES = Object.freeze({
      shape: Object.freeze({
        cvv: makePretDescriptorQuery("Ø+*", "CV|V", {
          modifierSet: []
        }),
        vv: makePretDescriptorQuery("Ø+*", "V|V", {
          modifierSet: []
        }),
        vlv: makePretDescriptorQuery("Ø+*", "Vl|V", {
          modifierSet: []
        }),
        cvlv: makePretDescriptorQuery("Ø+*", "CVl|V", {
          modifierSet: []
        }),
        ca: makePretDescriptorQuery("*+a", "CV", {
          modifierSet: []
        }),
        ti: makePretDescriptorQuery("t+i", "CV", {
          modifierSet: []
        }),
        vna: makePretDescriptorQuery("n+a", "V|CV", {
          modifierSet: []
        }),
        cvna: makePretDescriptorQuery("n+a", "CV|CV", {
          modifierSet: []
        }),
        cvcvna: makePretDescriptorQuery("n+a", "CV|CV|CV", {
          modifierSet: []
        }),
        cvlvna: makePretDescriptorQuery("n+a", "CVl|V|CV", {
          modifierSet: []
        }),
        vlcvna: makePretDescriptorQuery("n+a", "Vl|CV|CV", {
          modifierSet: []
        }),
        cvccvna: makePretDescriptorQuery("n+a", "CV|C|CV|CV", {
          modifierSet: []
        }),
        cvcvcvna: makePretDescriptorQuery("n+a", "CV|CV|CV|CV", {
          modifierSet: []
        }),
        cvccvcvna: makePretDescriptorQuery("n+a", "CV|C|CV|CV|CV", {
          modifierSet: []
        }),
        vjcvna: makePretDescriptorQuery("n+a", ["Vj|CV|CV", "V|C|CV|CV"], {
          modifierSet: []
        }),
        longNa: makePretDescriptorQuery("n+a", "...|CV", {
          modifierSet: ["long"]
        }),
        cvta: makePretDescriptorQuery("t+a", "CV|CV", {
          modifierSet: []
        }),
        cvtza: makePretDescriptorQuery("tz+a", "CV|CV", {
          modifierSet: []
        }),
        vjcvtza: makePretDescriptorQuery("tz+a", ["Vj|CV|CV", "V|C|CV|CV"], {
          modifierSet: []
        }),
        cvnia: makePretDescriptorQuery("n+ia", "CV|CV|V", {
          modifierSet: []
        }),
        cvcvnia: makePretDescriptorQuery("n+ia", "CV|CV|CV|V", {
          modifierSet: []
        }),
        cvlvnia: makePretDescriptorQuery("n+ia", "CVl|V|CV|V", {
          modifierSet: []
        }),
        vjcvnia: makePretDescriptorQuery("n+ia", ["Vj|CV|CV|V", "V|C|CV|CV|V"], {
          modifierSet: []
        }),
        cvlvni: makePretDescriptorQuery("n+i", "CVl|V|CV", {
          modifierSet: []
        }),
        vjcvni: makePretDescriptorQuery("n+i", ["Vj|CV|CV", "V|C|CV|CV"], {
          modifierSet: []
        }),
        cvcvni: makePretDescriptorQuery("n+i", "CV|CV|CV", {
          modifierSet: []
        }),
        cvcvcvni: makePretDescriptorQuery("n+i", "CV|CV|CV|CV", {
          modifierSet: []
        }),
        cvccvcvni: makePretDescriptorQuery("n+i", "CV|C|CV|CV|CV", {
          modifierSet: []
        }),
        cvcvcvcvni: makePretDescriptorQuery("n+i", "CV|CV|CV|CV|CV", {
          modifierSet: []
        }),
        cvvni: makePretDescriptorQuery("n+i", "CV|V|CV", {
          modifierSet: []
        }),
        cvni: makePretDescriptorQuery("n+i", "CV|CV", {
          modifierSet: []
        }),
        cvniU: makePretDescriptorQuery("n+i", "CV|CV", {
          modifierSet: ["leadNucleus=u"]
        }),
        longNi: makePretDescriptorQuery("n+i", "...|CV", {
          modifierSet: ["long"]
        }),
        vnV: makePretDescriptorQuery("n+*", "V|CV", {
          modifierSet: []
        }),
        cvnV: makePretDescriptorQuery("n+*", "CV|CV", {
          modifierSet: []
        }),
        cvsV: makePretDescriptorQuery("s+*", "CV|CV", {
          modifierSet: []
        }),
        cvpV: makePretDescriptorQuery("p+*", "CV|CV", {
          modifierSet: []
        }),
        cvmV: makePretDescriptorQuery("m+*", "CV|CV", {
          modifierSet: []
        }),
        cvma: makePretDescriptorQuery("m+a", "CV|CV", {
          modifierSet: []
        }),
        vjcvma: makePretDescriptorQuery("m+a", ["Vj|CV|CV", "V|C|CV|CV"], {
          modifierSet: []
        }),
        vwi: makePretDescriptorQuery("w+i", "V|CV", {
          modifierSet: []
        }),
        cvwi: makePretDescriptorQuery("w+i", "CV|CV", {
          modifierSet: []
        }),
        ccvwi: makePretDescriptorQuery("w+i", "C|CV", {
          modifierSet: []
        }),
        vccvwiShort: makePretDescriptorQuery("w+i", "V|C|CV", {
          modifierSet: []
        }),
        vcvwi: makePretDescriptorQuery("w+i", "V|CV|CV", {
          modifierSet: []
        }),
        vlvwi: makePretDescriptorQuery("w+i", "Vl|V|CV", {
          modifierSet: []
        }),
        cvcvwi: makePretDescriptorQuery("w+i", "CV|CV|CV", {
          modifierSet: []
        }),
        cvlvwi: makePretDescriptorQuery("w+i", "CVl|V|CV", {
          modifierSet: []
        }),
        vlcvwi: makePretDescriptorQuery("w+i", "Vl|CV|CV", {
          modifierSet: []
        }),
        cvcvcvwi: makePretDescriptorQuery("w+i", "CV|CV|CV|CV", {
          modifierSet: []
        }),
        vccvwi: makePretDescriptorQuery("w+i", "V|C|CV|CV", {
          modifierSet: []
        }),
        cvjcvwi: makePretDescriptorQuery("w+i", "CVj|CV|CV", {
          modifierSet: []
        }),
        cvcvlvwi: makePretDescriptorQuery("w+i", "CV|CVl|V|CV", {
          modifierSet: []
        }),
        cvccvwi: makePretDescriptorQuery("w+i", "CV|C|CV|CV", {
          modifierSet: []
        }),
        cvccvcvwi: makePretDescriptorQuery("w+i", "CV|C|CV|CV|CV", {
          modifierSet: []
        }),
        cvlcvcvwi: makePretDescriptorQuery("w+i", "CVl|CV|CV|CV", {
          modifierSet: []
        }),
        vjcvwi: makePretDescriptorQuery("w+i", ["Vj|CV|CV", "V|C|CV|CV"], {
          modifierSet: []
        }),
        vjcvcvwi: makePretDescriptorQuery("w+i", "Vj|CV|CV|CV", {
          modifierSet: []
        }),
        cvvjcvwi: makePretDescriptorQuery("w+i", "CV|Vj|CV|CV", {
          modifierSet: []
        }),
        longWi: makePretDescriptorQuery("w+i", "...|CV", {
          modifierSet: ["long"]
        }),
        cvkwi: makePretDescriptorQuery("kw+i", "CV|CV", {
          modifierSet: []
        }),
        vcvcu: makePretDescriptorQuery("*+u", "V|CV|CV", {
          modifierSet: []
        }),
        vwa: makePretDescriptorQuery("w+a", "V|CV", {
          modifierSet: []
        }),
        vjwa: makePretDescriptorQuery("w+a", "Vj|CV", {
          modifierSet: []
        }),
        cvwa: makePretDescriptorQuery("w+a", "CV|CV", {
          modifierSet: []
        }),
        vccvwa: makePretDescriptorQuery("w+a", "V|C|CV", {
          modifierSet: []
        }),
        cvwaA: makePretDescriptorQuery("w+a", "CV|CV", {
          modifierSet: ["leadNucleus=a"]
        }),
        cvwaI: makePretDescriptorQuery("w+a", "CV|CV", {
          modifierSet: ["leadNucleus=i"]
        }),
        cewa: makePretDescriptorQuery("w+a", "CV|CV", {
          modifierSet: ["leadNucleus=e"]
        }),
        vccawa: makePretDescriptorQuery("w+a", "V|C|CV|CV", {
          modifierSet: ["bridgeNucleus=a"]
        }),
        cuwa: makePretDescriptorQuery("w+a", "CV|CV", {
          modifierSet: ["leadNucleus=u"]
        }),
        vwaI: makePretDescriptorQuery("w+a", "V|CV", {
          modifierSet: ["leadNucleus=i"]
        }),
        vjcvwa: makePretDescriptorQuery("w+a", ["Vj|CV|CV", "V|C|CV|CV"], {
          modifierSet: []
        }),
        cvjcvwa: makePretDescriptorQuery("w+a", "CVj|CV|CV", {
          modifierSet: []
        }),
        cvcawa: makePretDescriptorQuery("w+a", "CV|CV|CV", {
          modifierSet: ["bridgeNucleus=a"]
        }),
        cvcvwa: makePretDescriptorQuery("w+a", "CV|CV|CV", {
          modifierSet: []
        }),
        cvcvewa: makePretDescriptorQuery("w+a", "CV|CV|CV", {
          modifierSet: ["bridgeNucleus=e"]
        }),
        vjcewa: makePretDescriptorQuery("w+a", "Vj|CV|CV", {
          modifierSet: ["bridgeNucleus=e"]
        }),
        cvlewa: makePretDescriptorQuery("w+a", "CVl|V|CV", {
          modifierSet: ["bridgeNucleus=e"]
        }),
        cvlawa: makePretDescriptorQuery("w+a", "CVl|V|CV", {
          modifierSet: ["bridgeNucleus=a"]
        }),
        vlvwa: makePretDescriptorQuery("w+a", "Vl|V|CV", {
          modifierSet: []
        }),
        cvlvwa: makePretDescriptorQuery("w+a", "CVl|V|CV", {
          modifierSet: []
        }),
        vlcvwa: makePretDescriptorQuery("w+a", "Vl|CV|CV", {
          modifierSet: []
        }),
        cvccvwa: makePretDescriptorQuery("w+a", "CV|C|CV|CV", {
          modifierSet: []
        }),
        cvcvcvwa: makePretDescriptorQuery("w+a", "CV|CV|CV|CV", {
          modifierSet: []
        }),
        cvccvcvwa: makePretDescriptorQuery("w+a", "CV|C|CV|CV|CV", {
          modifierSet: []
        }),
        cvlcvcvwa: makePretDescriptorQuery("w+a", "CVl|CV|CV|CV", {
          modifierSet: []
        }),
        vccvcvwa: makePretDescriptorQuery("w+a", "V|C|CV|CV|CV", {
          modifierSet: []
        }),
        longWa: makePretDescriptorQuery("w+a", "...|CV", {
          modifierSet: ["long"]
        })
      }),
      aggregate: Object.freeze({
        wiPattern: makePretDescriptorQuery("w+i", null, {
          modifierSet: ["aggregate"]
        }),
        waPattern: makePretDescriptorQuery("w+a", null, {
          modifierSet: ["aggregate"]
        }),
        ewaPattern: makePretDescriptorQuery("w+a", null, {
          modifierSet: ["aggregate", "bridgeNucleus=e"]
        }),
        lwaPattern: makePretDescriptorQuery("w+a", null, {
          modifierSet: ["aggregate", "l-bridge"]
        }),
    ...
    const PRET_DERIVED_SHAPE_DESCRIPTOR_SPECS = Object.freeze([Object.freeze({
      descriptorKey: "cvniU",
      baseKey: "cvni",
      leadingNucleus: "u"
    }), Object.freeze({
    ...
    const PRET_AGGREGATE_DESCRIPTOR_SPECS = Object.freeze([Object.freeze({
      descriptor: PRET_DESCRIPTOR_QUERIES.aggregate.wiPattern,
      sourceQueries: Object.freeze([PRET_DESCRIPTOR_QUERIES.shape.vwi, PRET_DESCRIPTOR_QUERIES.shape.cvwi, PRET_DESCRIPTOR_QUERIES.shape.vcvwi, PRET_DESCRIPTOR_QUERIES.shape.vlvwi, PRET_DESCRIPTOR_QUERIES.shape.cvcvwi, PRET_DESCRIPTOR_QUERIES.shape.cvlvwi, PRET_DESCRIPTOR_QUERIES.shape.cvcvcvwi, PRET_DESCRIPTOR_QUERIES.shape.vccvwi, PRET_DESCRIPTOR_QUERIES.shape.cvjcvwi, PRET_DESCRIPTOR_QUERIES.shape.cvcvlvwi, PRET_DESCRIPTOR_QUERIES.shape.cvccvwi, PRET_DESCRIPTOR_QUERIES.shape.cvccvcvwi, PRET_DESCRIPTOR_QUERIES.shape.cvlcvcvwi, PRET_DESCRIPTOR_QUERIES.shape.vjcvwi, PRET_DESCRIPTOR_QUERIES.shape.vjcvcvwi, PRET_DESCRIPTOR_QUERIES.shape.cvvjcvwi, PRET_DESCRIPTOR_QUERIES.shape.longWi])
    }), Object.freeze({
      descriptor: PRET_DESCRIPTOR_QUERIES.aggregate.waPattern,
      sourceQueries: Object.freeze([PRET_DESCRIPTOR_QUERIES.shape.cvwa, PRET_DESCRIPTOR_QUERIES.shape.cvcvwa, PRET_DESCRIPTOR_QUERIES.shape.vccvwa, PRET_DESCRIPTOR_QUERIES.shape.cvccvwa, PRET_DESCRIPTOR_QUERIES.shape.cvcvcvwa, PRET_DESCRIPTOR_QUERIES.shape.cvccvcvwa, PRET_DESCRIPTOR_QUERIES.shape.cvlcvcvwa, PRET_DESCRIPTOR_QUERIES.shape.vccvcvwa, PRET_DESCRIPTOR_QUERIES.shape.longWa])
    }), Object.freeze({
      descriptor: PRET_DESCRIPTOR_QUERIES.aggregate.ewaPattern,
      sourceQueries: Object.freeze([PRET_DESCRIPTOR_QUERIES.shape.cewa, PRET_DESCRIPTOR_QUERIES.shape.cvcvewa, PRET_DESCRIPTOR_QUERIES.shape.vjcewa, PRET_DESCRIPTOR_QUERIES.shape.cvlewa])
    }), Object.freeze({
      descriptor: PRET_DESCRIPTOR_QUERIES.aggregate.lwaPattern,
      sourceQueries: Object.freeze([PRET_DESCRIPTOR_QUERIES.shape.vlvwa, PRET_DESCRIPTOR_QUERIES.shape.cvlvwa])
    }), Object.freeze({
      descriptor: PRET_DESCRIPTOR_QUERIES.aggregate.kawa,
      matcher: (shapeDescriptors = [], runtimeState = {}) => pretDescriptorListHasQuery(shapeDescriptors, PRET_DESCRIPTOR_QUERIES.shape.cvwaA) && runtimeState.leadingOnset === "k"
    })]);
    ...
    function pretRightEdgeProfileMatchesSyllableWindow(profile = "", syllables = [], startIndex = 0) {
      ...
      if (profileParts[0] === "...") {
        ...
        return suffixParts.every((part, index) => windowForms[windowForms.length - suffixParts.length + index] === part);
      }
      if (windowForms.length !== profileParts.length) {
        return false;
      }
    ...
    function pretShapeDescriptorMatchesSyllableWindow(query = {}, syllables = [], startIndex = 0) {
      ...
      const rightEdgeProfiles = Array.isArray(normalizedQuery.rightEdgeProfiles) ? normalizedQuery.rightEdgeProfiles : [];
      if (rightEdgeProfiles.length && !rightEdgeProfiles.some(profile => pretRightEdgeProfileMatchesSyllableWindow(profile, windowSyllables, 0))) {
        return false;
      }
    ...
    const PRET_SHAPE_SOURCE_OPTIONS = Object.freeze({
      cvta: Object.freeze({
        includeRawReduplicatedSource: true
      }),
      vcvcu: Object.freeze({
        includeSupportiveSource: false
      }),
      vnV: Object.freeze({
        extraSources: Object.freeze([Object.freeze({
          source: "syllables",
          startIndex: 1,
          prefixForms: Object.freeze(["V", "Vj"])
        })])
      }),
      cvnV: Object.freeze({
        extraSources: Object.freeze([Object.freeze({
          source: "syllables",
          startIndex: 1,
          prefixForms: Object.freeze(["CV", "CVj"])
        })])
      }),
      cvsV: Object.freeze({
        extraSources: Object.freeze([Object.freeze({
    ...
    function pretShapeDescriptorMatchesSources(shapeKey = "", query = {}, {
      ...
      if (options.includeRawReduplicatedSource === true && baseIsReduplicated && pretShapeDescriptorMatchesSyllableWindow(query, rawSyllables, 0)) {
        return true;
      }
      if (options.includeSupportiveSource !== false && supportiveSyllables && pretShapeDescriptorMatchesSyllableWindow(query, supportiveSyllables, 0)) {
        return true;
      }
      ...
      for (const extraSource of extraSources) {
        ...
        if (sourceMatches(availableSources[sourceName], Number.isInteger(extraSource?.startIndex) ? extraSource.startIndex : 0, Array.isArray(extraSource?.prefixForms) ? extraSource.prefixForms : [])) {
          return true;
        }
      }
      return false;
    ...
    function buildPretActiveShapeDescriptors(shapeQueries = PRET_DESCRIPTOR_QUERIES.shape, {
      ...
      Object.entries(shapeQueries).forEach(([shapeKey, query]) => {
        ...
        if (pretShapeDescriptorMatchesSources(shapeKey, query, {
          syllables,
          analysisSyllables,
          supportiveSyllables,
          rawSyllables,
      ...
      if (hasShape(shapeQueries.vna) || hasShape(shapeQueries.cvna)) {
        return Object.freeze(activeDescriptors.filter(descriptor => !pretDescriptorMatchesQuery(descriptor, shapeQueries.vnV) && !pretDescriptorMatchesQuery(descriptor, shapeQueries.cvnV)));
      }
    ...
    function buildPretAggregateDescriptors(shapeDescriptors = [], runtimeState = {}) {
      ...
      PRET_AGGREGATE_DESCRIPTOR_SPECS.forEach(spec => {
        const isActive = typeof spec.matcher === "function" ? spec.matcher(shapeDescriptors, runtimeState) : pretDescriptorListHasAnyQuery(shapeDescriptors, spec.sourceQueries || []);
    ...
    function pretContextHasShapeEndingFamily(context, endingFamily) {
      return pretContextHasShapeDescriptor(context, {
        endingFamily
      });
    }
    function buildPretUniversalContext(verb, analysisVerb, isTransitive, options = {}) {
      ...
      const exactBaseVerb = derivationType === targetObject.DERIVATION_TYPE.direct ? options.exactBaseVerb : "";
      ...
      const sourceVerb = exactBaseVerb || targetObject.getDerivationRuleBase(analysisVerb || verb, {
        analysisVerb,
        hasSlashMarker,
        hasSuffixSeparator,
        hasLeadingDash,
      ...
      const rawSyllables = targetObject.getSyllables(sourceVerb, {
        analysis: true,
        assumeFinalV: true
      });
      const getLooseRedupRoot = syls => {
        ...
        const first = syls[0];
        const second = syls[1];
        if (!targetObject.REDUP_PREFIX_FORMS.has(first?.form) || !second?.nucleus) {
          return sourceVerb;
        }
        if (targetObject.getSyllableBaseKey(first) !== targetObject.getSyllableBaseKey(second)) {
          return sourceVerb;
        }
        return syls.slice(1).map(syllable => syllable.text).join("");
      };
      const strictNonRedupRoot = targetObject.getNonReduplicatedRoot(sourceVerb);
      const nonRedupRoot = strictNonRedupRoot !== sourceVerb ? strictNonRedupRoot : getLooseRedupRoot(rawSyllables);
      const baseIsReduplicated = sourceVerb !== nonRedupRoot;
      const parsedRootPlusYaBase = typeof options.rootPlusYaBase === "string" ? options.rootPlusYaBase : "";
      const parsedRootPlusYaPronounceable = typeof options.rootPlusYaBasePronounceable === "string" ? options.rootPlusYaBasePronounceable : "";
      const denominalRootPlusYaBase = !isTransitive && denominalSource.isDenominalMatrixInput && denominalSource.denominalMatrixStem === "ya" && denominalSource.denominalSourceStem ? denominalSource.denominalSourceStem : "";
      const computedRootPlusYaBase = !isTransitive && parsedRootPlusYaBase ? parsedRootPlusYaBase : denominalRootPlusYaBase || targetObject.getRootPlusYaBase(rootPlusYaSource, {
        isTransitive,
        isYawi,
        isWeya
      });
      ...
      const redupRoot = isRootPlusYa ? targetObject.getNonReduplicatedRoot(rootPlusYaBase) : analysisRoot;
      const isReduplicatedRootPlusYa = isRootPlusYa && redupRoot !== rootPlusYaBase;
      const lexicalVerbOverride = targetObject.getPretUniversalVerbOverride(analysisRoot, isTransitive);
      const verbOverride = (() => {
      ...
      const letters = analysisRoot ? targetObject.splitVerbLetters(analysisRoot) : [];
      const startsWithConsonant = letters.length > 0 && targetObject.isVerbLetterConsonant(letters[0]);
      const startsWithConsonantCluster = startsWithConsonant && letters.length > 1 && targetObject.isVerbLetterConsonant(letters[1]);
      ...
      const allowSupportiveMatch = startsWithConsonant && !hasNonspecificValence && (hasOptionalSupportiveI || !hasBoundaryMarker || startsWithConsonantCluster || startsWithL || startsWithJ);
      const supportiveInitialI = allowSupportiveMatch;
      const supportiveRoot = allowSupportiveMatch ? `i${analysisRoot}` : "";
      const syllables = targetObject.getSyllables(analysisRoot, {
        analysis: true,
        assumeFinalV: true
      });
      const supportiveSyllables = supportiveRoot ? targetObject.getSyllables(supportiveRoot, {
        analysis: true,
        assumeFinalV: true
      }) : null;
      const analysisSyllables = baseIsReduplicated ? rawSyllables : syllables;
      const letterCount = targetObject.getVerbLetterCount(analysisRoot);
      const syllableForms = syllables.length ? syllables.map(syllable => syllable.form) : null;
      const syllableCount = syllableForms ? syllableForms.length : getPretUniversalCoreVowelCount(analysisRoot);
      const vowelCount = targetObject.getTrailingVowelCountFromSyllables(syllables);
      ...
      const resolvedForceClassBEnding = forceClassBEnding && !hasShapeDescriptor(shapeQuery.vccvwi) && !hasShapeDescriptor(shapeQuery.vccvwiShort) && !hasShapeDescriptor(shapeQuery.vccvwa);
      ...
      const endsInOpenSyllable = targetObject.isOpenSyllable(lastSyllable);
      const endsInOpenSyllableNonU = endsInOpenSyllable && lastNucleus !== "u";
      const isItaVerb = targetObject.isItaSyllableSequence(syllables);
      const forceClassAForKWV = hasRightEdge({
        finalForm: "CV",
        finalOnset: "kw"
      }) && !hasRightEdge({
        finalForm: "CV",
    ...
    const PRET_UNIVERSAL_EARLY_TIER_RULES = Object.freeze([{
      ...
      tier: "path",
      when: context => context.isDenominalMatrixInput && context.isDenominalTiMatrix && context.denominalSourceEndsWithConsonant,
      classes: ["B"]
    }, {
      id: "denominal_ti_source_vowel",
      label: "denominal TI with vowel source",
      tier: "path",
      when: context => context.isDenominalMatrixInput && context.isDenominalTiMatrix && context.denominalSourceEndsWithVowel,
      classes: ["A", "B"]
    }, {
      id: "denominal_wi_source_consonant",
      label: "denominal WI with consonant source",
      tier: "path",
      when: context => context.isDenominalMatrixInput && context.isDenominalWiMatrix && context.denominalSourceEndsWithConsonant,
      classes: ["B"]
    }, {
      id: "denominal_wi_source_vowel",
      label: "denominal WI with vowel source",
      tier: "path",
      when: context => context.isDenominalMatrixInput && context.isDenominalWiMatrix && context.denominalSourceEndsWithVowel,
      classes: ["A", "B"]
    }, {
      id: "monosyllable_transitive_v_e",
      label: "monosyllable transitive V (e)",
      tier: "monosyllable",
      when: context => context.isMonosyllable && context.isTransitive && context.lastSyllableForm === "V" && context.lastNucleus === "e",
      classes: ["D"]
    }, {
      ...
      when: context => {
        ...
        const allowClusterWiWaShape = pretContextHasAnyShapeDescriptor(context, [PRET_DESCRIPTOR_QUERIES.shape.ccvwi, PRET_DESCRIPTOR_QUERIES.shape.vccvwiShort, PRET_DESCRIPTOR_QUERIES.shape.vccvwa]);
      ...
      label: "descriptor tz+*",
      tier: "forced",
      when: context => pretContextHasRightEdge(context, {
        finalOnset: "tz"
    ...
    const PRET_UNIVERSAL_LV_TIER_RULES = Object.freeze([{
      ...
      tier: "forced",
      when: context => !context.isTransitive && context.lastNucleus === "i" && pretContextHasAnyRightEdge(context, [{
      ...
      tier: "forced",
      when: context => pretContextHasAnyRightEdge(context, [{
        rightEdgeProfileSuffixes: ["Vl|V", "CVl|V"]
      }]),
      classes: ["A"]
    }, {
      ...
      label: "descriptor l+* · [Vl|V|CVl|V] · i",
      tier: "shape",
      when: context => context.lastNucleus === "i" && pretContextHasAnyRightEdge(context, [{
        rightEdgeProfileSuffixes: ["Vl|V", "CVl|V"]
      }]),
      classes: ["A", "B"]
    }]);
    ...
    const PRET_UNIVERSAL_SHAPE_NA_INTRANSITIVE_TIER_RULES = Object.freeze([{
      ...
      tier: "shape",
      when: context => !context.isTransitive && pretContextHasRightEdge(context, PRET_RIGHT_EDGE_RULE_QUERIES.naCV_CV_CV),
      classes: ["A"]
    }, {
      id: "descriptor_na_four_syllable_intransitive",
      label: "descriptor n+a, four-syllable right edge (intransitive)",
      tier: "shape",
      when: context => !context.isTransitive && pretContextHasRightEdge(context, PRET_RIGHT_EDGE_RULE_QUERIES.naCV_CV_CV_CV),
      classes: ["A"]
    }, {
    ...
    const PRET_UNIVERSAL_SHAPE_WA_ENTRY_TIER_RULES = Object.freeze([{
      ...
      tier: "shape",
      when: context => context.isTransitive && pretContextHasShapeDescriptor(context, PRET_DESCRIPTOR_QUERIES.shape.vwa) && !pretContextHasShapeDescriptor(context, PRET_DESCRIPTOR_QUERIES.shape.vwaI),
      classes: ["A"]
    }]);
    const PRET_UNIVERSAL_SHAPE_WA_REST_TIER_RULES = Object.freeze([{
      ...
      tier: "shape",
      when: context => !context.isTransitive && pretContextHasAnyShapeDescriptor(context, [PRET_DESCRIPTOR_QUERIES.shape.vccvwiShort, PRET_DESCRIPTOR_QUERIES.shape.ccvwi]),
      classes: ["A"]
    }, {
      ...
      tier: "shape",
      when: context => !context.isTransitive && pretContextHasRightEdge(context, PRET_RIGHT_EDGE_RULE_QUERIES.naDepthAtLeastFour),
      classes: ["A"]
    }, {
    ...
    const PRET_UNIVERSAL_DEFAULT_CLASS_RULES = Object.freeze([{
      ...
      when: ctx => ctx.endsInOpenSyllableNonU && ctx.vowelCount === 2 && pretContextHasRightEdge(ctx, {
        finalForm: "V",
        finalNucleus: "a",
        previousHasCoda: false,
        previousNuclei: ["i", "u"]
    ...
    const PRET_UNIVERSAL_RULE_TIER_BY_LABEL = Object.freeze(buildPretUniversalRuleTierByLabelMap());
    ...
    function resolvePretUniversalRuleMatch(rule, context, flags = {}) {
      ...
      if (typeof rule.resolveCandidates === "function") {
        candidates = normalizePretUniversalRuleCandidates(rule.resolveCandidates(context, flags));
        if (!candidates.size) {
          return null;
        }
      } else {
        const matched = typeof rule.when === "function" ? rule.when(context, flags) : false;
        if (!matched) {
          return null;
        }
      ...
      const resolvedLabel = typeof rule.resolveLabel === "function" ? rule.resolveLabel(context, flags) : rule.label;
      return {
        id: rule.id,
        label: resolvedLabel,
        tier: rule.tier,
    ...
    function evaluatePretUniversalClassSelectionPipeline(context, flags = {}) {
      const gateMatch = evaluatePretUniversalRuleTable(PRET_UNIVERSAL_CLASS_GATE_RULES, context, flags);
      if (gateMatch) {
        return gateMatch;
      }
      ...
      return {
        ...
        tier: PRET_UNIVERSAL_DEFAULT_RULE_TIER,
        candidates: collectPretUniversalRuleTableCandidates(PRET_UNIVERSAL_DEFAULT_CLASS_RULES, context, flags)
      };
    }
    ...
    function resolvePretUniversalClassSelection(context, options = {}) {
      const traceState = options.trace && typeof options.trace === "object" ? options.trace : null;
      ...
      recordPretUniversalSelectionTrace(traceState, {
        ruleLabel,
        ruleTier,
        ruleTierIndex
      });
    ...
    function getPretUniversalClassCandidates(context, trace = null) {
      ...
      // > general class rules.
      return resolvePretUniversalClassSelection(context, {
        trace
      }).candidates;
    }
    function getPretUniversalShapeLabels(context) {
      ...
      return shapeDescriptors.map(descriptor => formatPretDescriptorLabel(descriptor, {
    ...
    function buildPretUniversalRuleSummary(context) {
      ...
      if (typeof targetObject.getPretUniversalVariantsByClass === "function") {
    ...
    Object.defineProperty(api, "PRET_UNIVERSAL_CLASS_ORDER", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_CLASS_ORDER; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_RULE_TIER_ORDER", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_RULE_TIER_ORDER; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_DEFAULT_RULE_LABEL", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_DEFAULT_RULE_LABEL; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_DEFAULT_RULE_TIER", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_DEFAULT_RULE_TIER; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_CLASS_ORDER_INDEX", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_CLASS_ORDER_INDEX; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_CLASS_GATE_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_CLASS_GATE_RULES; },
    });
    Object.defineProperty(api, "PRET_DENOMINAL_MATRIX_STEM_SET", {
        configurable: true,
        enumerable: true,
        get() { return PRET_DENOMINAL_MATRIX_STEM_SET; },
    });
    api.normalizePretDenominalStem = normalizePretDenominalStem;
    api.getPretDenominalSourceContext = getPretDenominalSourceContext;
    api.getPretUniversalClassOrder = getPretUniversalClassOrder;
    api.sortPretUniversalClassKeys = sortPretUniversalClassKeys;
    api.formatPretUniversalClassList = formatPretUniversalClassList;
    api.inferPretUniversalRuleTier = inferPretUniversalRuleTier;
    Object.defineProperty(api, "PRET_UNIVERSAL_CONTEXT_CACHE_LIMIT", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_CONTEXT_CACHE_LIMIT; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_CONTEXT_CACHE", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_CONTEXT_CACHE; },
    });
    api.resetPretUniversalContextCache = resetPretUniversalContextCache;
    api.getPretUniversalContextCacheSize = getPretUniversalContextCacheSize;
    api.encodePretContextCacheValue = encodePretContextCacheValue;
    api.buildPretUniversalContextCacheKey = buildPretUniversalContextCacheKey;
    api.clonePretUniversalContext = clonePretUniversalContext;
    api.formatPretRightEdgeProfile = formatPretRightEdgeProfile;
    api.formatPretJuncture = formatPretJuncture;
    api.formatPretEndingFamily = formatPretEndingFamily;
    api.buildPretOrderedRightEdgeDescriptor = buildPretOrderedRightEdgeDescriptor;
    api.buildPretRightEdgeDescriptor = buildPretRightEdgeDescriptor;
    api.buildPretOrderedRightEdgeQuery = buildPretOrderedRightEdgeQuery;
    api.normalizePretRightEdgeQuery = normalizePretRightEdgeQuery;
    api.pretRightEdgeDescriptorMatchesQuery = pretRightEdgeDescriptorMatchesQuery;
    api.pretContextHasRightEdge = pretContextHasRightEdge;
    api.pretContextHasAnyRightEdge = pretContextHasAnyRightEdge;
    api.freezePretDescriptorList = freezePretDescriptorList;
    api.buildPretJuncturesFromProfiles = buildPretJuncturesFromProfiles;
    api.buildPretOrderedDescriptorQuery = buildPretOrderedDescriptorQuery;
    api.normalizePretDescriptorQuery = normalizePretDescriptorQuery;
    api.makePretDescriptorQuery = makePretDescriptorQuery;
    api.pretDescriptorMatchesQuery = pretDescriptorMatchesQuery;
    api.pretDescriptorListHasQuery = pretDescriptorListHasQuery;
    api.pretDescriptorListHasAnyQuery = pretDescriptorListHasAnyQuery;
    api.formatPretDescriptorLabel = formatPretDescriptorLabel;
    Object.defineProperty(api, "PRET_DESCRIPTOR_QUERIES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_DESCRIPTOR_QUERIES; },
    });
    Object.defineProperty(api, "PRET_DERIVED_SHAPE_DESCRIPTOR_SPECS", {
        configurable: true,
        enumerable: true,
        get() { return PRET_DERIVED_SHAPE_DESCRIPTOR_SPECS; },
    });
    Object.defineProperty(api, "PRET_RIGHT_EDGE_RULE_QUERIES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_RIGHT_EDGE_RULE_QUERIES; },
    });
    Object.defineProperty(api, "PRET_AGGREGATE_DESCRIPTOR_SPECS", {
        configurable: true,
        enumerable: true,
        get() { return PRET_AGGREGATE_DESCRIPTOR_SPECS; },
    });
    api.pretEndingFamilyMatchesPattern = pretEndingFamilyMatchesPattern;
    api.pretRightEdgeProfileMatchesSyllableWindow = pretRightEdgeProfileMatchesSyllableWindow;
    api.pretShapeModifiersMatchSyllableWindow = pretShapeModifiersMatchSyllableWindow;
    api.pretShapeDescriptorMatchesSyllableWindow = pretShapeDescriptorMatchesSyllableWindow;
    Object.defineProperty(api, "PRET_SHAPE_SOURCE_OPTIONS", {
        configurable: true,
        enumerable: true,
        get() { return PRET_SHAPE_SOURCE_OPTIONS; },
    });
    api.pretShapeDescriptorMatchesSources = pretShapeDescriptorMatchesSources;
    api.pretShapeHasRedupPrefix = pretShapeHasRedupPrefix;
    api.buildPretActiveShapeDescriptors = buildPretActiveShapeDescriptors;
    api.buildPretAggregateDescriptors = buildPretAggregateDescriptors;
    api.buildPretDescriptorState = buildPretDescriptorState;
    api.pretContextGetDescriptorState = pretContextGetDescriptorState;
    api.pretContextHasShapeDescriptor = pretContextHasShapeDescriptor;
    api.pretContextHasAnyShapeDescriptor = pretContextHasAnyShapeDescriptor;
    api.pretContextHasAggregateDescriptor = pretContextHasAggregateDescriptor;
    api.pretContextHasShapeEndingFamily = pretContextHasShapeEndingFamily;
    api.buildPretUniversalContext = buildPretUniversalContext;
    api.getRootPlusYaClassCandidates = getRootPlusYaClassCandidates;
    Object.defineProperty(api, "PRET_UNIVERSAL_EARLY_TIER_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_EARLY_TIER_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_LV_TIER_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_LV_TIER_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_SHAPE_CORE_TIER_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_SHAPE_CORE_TIER_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_SHAPE_NA_INTRANSITIVE_TIER_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_SHAPE_NA_INTRANSITIVE_TIER_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_SHAPE_NA_TRANSITIVE_TIER_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_SHAPE_NA_TRANSITIVE_TIER_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_SHAPE_TA_PV_TIER_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_SHAPE_TA_PV_TIER_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_SHAPE_MA_KWI_NI_TIER_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_SHAPE_MA_KWI_NI_TIER_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_SHAPE_WA_ENTRY_TIER_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_SHAPE_WA_ENTRY_TIER_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_SHAPE_WA_REST_TIER_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_SHAPE_WA_REST_TIER_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_DEFAULT_CLASS_RULES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_DEFAULT_CLASS_RULES; },
    });
    Object.defineProperty(api, "PRET_UNIVERSAL_CLASS_TIER_TABLES", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_CLASS_TIER_TABLES; },
    });
    api.buildPretUniversalRuleTierByLabelMap = buildPretUniversalRuleTierByLabelMap;
    Object.defineProperty(api, "PRET_UNIVERSAL_RULE_TIER_BY_LABEL", {
        configurable: true,
        enumerable: true,
        get() { return PRET_UNIVERSAL_RULE_TIER_BY_LABEL; },
    });
    api.toPretUniversalCandidateSet = toPretUniversalCandidateSet;
    api.normalizePretUniversalRuleCandidates = normalizePretUniversalRuleCandidates;
    api.resolvePretUniversalRuleMatch = resolvePretUniversalRuleMatch;
    api.evaluatePretUniversalRuleTable = evaluatePretUniversalRuleTable;
    api.collectPretUniversalRuleTableCandidates = collectPretUniversalRuleTableCandidates;
    api.collectPretUniversalRuleMatches = collectPretUniversalRuleMatches;
    api.findPretUniversalRuleMatch = findPretUniversalRuleMatch;
    api.buildPretUniversalClassComputationFlags = buildPretUniversalClassComputationFlags;
    api.evaluatePretUniversalClassSelectionPipeline = evaluatePretUniversalClassSelectionPipeline;
    api.recordPretUniversalSelectionTrace = recordPretUniversalSelectionTrace;
    api.resolvePretUniversalClassSelection = resolvePretUniversalClassSelection;
    api.getPretUniversalClassCandidates = getPretUniversalClassCandidates;
    api.getPretUniversalShapeLabels = getPretUniversalShapeLabels;
    api.buildPretUniversalRuleSummary = buildPretUniversalRuleSummary;
    return api;
}
