...
function buildPretBaseOperationFrame({
    ...
    if (
        ...
        || sourceFrame.kind !== PRET_BASE_SOURCE_FRAME_KIND
        || !sourceFrame.surfaceBase
        || !transformKind
    ) {
        return null;
    }
    return Object.freeze({
        ...
        transformKind,
        sourceFrame: Object.freeze({ ...sourceFrame }),
...
function getPretContactLetters(value = "") {
    ...
    return typeof splitVerbLetters === "function" ? splitVerbLetters(normalized) : normalized.split("");
}
...
function buildPretTransformBaseSpecFromOperationFrame(operationFrame = null) {
    ...
    if (
        ...
        || operationFrame.kind !== PRET_BASE_OPERATION_FRAME_KIND
        || !sourceBase
        || !transformKind
    ) {
        return null;
...
function buildPretBaseSourceFrameFromRootPlusYaSourceFrame(rootPlusYaFrame = null) {
    if (
        !rootPlusYaFrame
        || typeof rootPlusYaFrame !== "object"
        || rootPlusYaFrame.kind !== "preterit-root-plus-ya-source-frame"
        || !rootPlusYaFrame.sourceVerbFrame
        || rootPlusYaFrame.sourceVerbFrame.kind !== "preterit-root-plus-ya-source-segment-frame"
    ) {
        return null;
    }
...
function getPretOperationFrameSourceBase(operationFrame = null) {
    return typeof operationFrame?.sourceFrame?.surfaceBase === "string"
        ? normalizePretBaseFrameText(operationFrame.sourceFrame.surfaceBase)
        : "";
}
...
function evaluatePretBaseOperationFrame(operationFrame = null, expected = {}) {
    ...
    if (
        operationFrame.kind !== PRET_BASE_OPERATION_FRAME_KIND
        || !sourceBase
        || (expected.transformKind && transformKind !== expected.transformKind)
    ) {
    ...
    if (transformKind === PRET_STEM_TRANSFORM_KIND.replaceSuffix) {
        ...
        return {
            ok: true,
            base: sourceBase.endsWith(sourceSuffix)
                ? `${sourceBase.slice(0, -sourceSuffix.length)}${replacement}`
                : `${sourceBase}${replacement}`,
            diagnostics: [],
            operationFrame,
        };
    }
    ...
    if (transformKind === PRET_STEM_TRANSFORM_KIND.deletionShift) {
        ...
        if (operationFrame.deletionVariant === "w-keep") {
            return { ok: true, base: sourceBase, diagnostics: [], operationFrame };
        }
    ...
    if (transformKind === PRET_STEM_TRANSFORM_KIND.coalesceFinalI) {
        return {
            ok: true,
            base: shouldCoalesceFinalI(sourceBase)
...
function getPretVariantSurfaceRuleMeta(variant = null) {
    ...
    return variant.surfaceRuleMeta && typeof variant.surfaceRuleMeta === "object"
        ? variant.surfaceRuleMeta
        : null;
}
...
function buildPretUniversalClassC(context) {
    ...
    const allowShapeCVV = context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvv);
    const allowShapeVV = !context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vv);
    ...
    const classCFrameResult = buildPretClassCBaseSpecsFromSourceFrame(
        context.classCSourceFrame,
        context
    );
...
function buildPretUniversalClassD(context) {
    ...
    if (context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vwaI)) {
        const classDFrameResult = buildPretClassDBaseSpecsFromSourceFrame(
            context.classDSourceFrame,
            context
        );
...
function buildPretRootPlusYaClassABaseSpecsFromSourceFrame(rootPlusYaFrame = null, context = {}) {
    ...
    return baseSpec
        ? { ok: true, baseSpecs: [baseSpec], diagnostics: [] }
...
function getPretClassAYyaFrameText(frame = null, role = "") {
    ...
    const candidates = [
        frame.sourceVerbFrame,
        frame.retainedBaseFrame,
        frame.suffixFrame,
        frame.previousYFrame,
        frame.finalOnsetFrame,
        frame.finalNucleusFrame,
    ];
...
function buildPretClassAYyaBaseSpecsFromSourceFrame(sourceFrame = null, context = {}) {
    if (
        !sourceFrame
        || typeof sourceFrame !== "object"
        || sourceFrame.kind !== "preterit-class-a-yya-source-frame"
    ) {
    ...
    if (
        ...
        || finalOnset !== "y"
        || finalNucleus !== "a"
        || `${retainedBase}${suffix}` !== sourceVerb
    ) {
...
function getPretClassAItaFrameText(frame = null, role = "") {
    ...
    const candidates = [
        frame.sourceVerbFrame,
        frame.retainedBaseFrame,
        frame.sourceSuffixFrame,
        frame.replacementFrame,
        frame.firstNucleusFrame,
        frame.finalOnsetFrame,
        frame.finalNucleusFrame,
    ];
    const segment = candidates.find((entry) => entry?.role === role) || null;
...
function buildPretClassAItaBaseSpecsFromSourceFrame(sourceFrame = null, context = {}) {
    if (
        !sourceFrame
        || typeof sourceFrame !== "object"
        || sourceFrame.kind !== "preterit-class-a-ita-source-frame"
    ) {
    ...
    if (
        ...
        || finalOnset !== "t"
        || finalNucleus !== "a"
        || `${retainedBase}${sourceSuffix}` !== sourceVerb
    ) {
...
function getPretClassADeletionFrameText(frame = null, role = "") {
    ...
    const candidates = [
        frame.sourceVerbFrame,
        frame.deletedBaseFrame,
        frame.finalVowelFrame,
        frame.finalBaseSegmentFrame,
    ];
    const segment = candidates.find((entry) => entry?.role === role) || null;
    return normalizePretBaseFrameText(segment?.text || "");
}
...
function buildPretClassAFinalVowelDeletionBaseSpecsFromSourceFrame(sourceFrame = null, context = {}) {
    if (
        !sourceFrame
        || typeof sourceFrame !== "object"
        || sourceFrame.kind !== "preterit-class-a-final-vowel-deletion-source-frame"
    ) {
    ...
    return shiftSpecs.length
        ? { ok: true, baseSpecs: shiftSpecs, diagnostics: [] }
...
function getPretClassBFrameText(frame = null, role = "") {
    ...
    const candidates = [
        frame.sourceVerbFrame,
        frame.vowelCountFrame,
        frame.syllableCountFrame,
        frame.finalFormFrame,
        frame.finalOnsetFrame,
        frame.finalNucleusFrame,
    ];
    const segment = candidates.find((entry) => entry?.role === role) || null;
...
function buildPretClassBBaseSpecsFromSourceFrame(sourceFrame = null, _context = {}) {
    if (
        !sourceFrame
        || typeof sourceFrame !== "object"
        || sourceFrame.kind !== "preterit-class-b-source-frame"
    ) {
        return blockPretClassBBaseSpecs("preterit-class-b-missing-source-frame");
    }
    if (
        !sourceFrame.operationFrame
        || typeof sourceFrame.operationFrame !== "object"
        || sourceFrame.operationFrame.kind !== "preterit-class-b-literal-base-operation-frame"
        || sourceFrame.operationFrame.operation !== "select-source-verb-as-class-b-base"
    ) {
    ...
    const syllables = typeof getSyllables === "function"
        ? getSyllables(sourceVerb, { analysis: true, assumeFinalV: true })
        : [];
    const edge = typeof buildPretRightEdgeDescriptor === "function"
        ? buildPretRightEdgeDescriptor(syllables)
        : {};
    const vowelCount = typeof getTrailingVowelCountFromSyllables === "function"
        ? getTrailingVowelCountFromSyllables(syllables)
        : getPretUniversalCoreVowelCount(sourceVerb);
    ...
    if (
        !syllables.length
        || frameVowelCount !== vowelCount
        || frameSyllableCount !== syllables.length
        || finalForm !== normalizePretBaseFrameText(edge.finalForm || "")
        || finalOnset !== normalizePretBaseFrameText(edge.finalOnset || "")
        || finalNucleus !== normalizePretBaseFrameText(edge.finalNucleus || "")
    ) {
    ...
    return baseSpec
        ? { ok: true, baseSpecs: [baseSpec], sourceVerb, diagnostics: [] }
...
function buildPretClassCBaseSpecsFromSourceFrame(sourceFrame = null, context = {}) {
    if (
        !sourceFrame
        || typeof sourceFrame !== "object"
        || sourceFrame.kind !== "preterit-class-c-source-frame"
    ) {
...
function buildPretClassDBaseSpecsFromSourceFrame(sourceFrame = null, context = {}) {
    if (
        !sourceFrame
        || typeof sourceFrame !== "object"
        || sourceFrame.kind !== "preterit-class-d-source-frame"
    ) {
...
function createPretDescriptorTierMatcher(descriptors = []) {
    return Object.freeze({
        has(query) {
            return pretDescriptorListHasQuery(descriptors, query);
        },
        hasEndingFamily(endingFamily) {
            return pretDescriptorListHasQuery(descriptors, { endingFamily });
        },
        hasModifier(modifier, options = {}) {
            return pretDescriptorListHasQuery(descriptors, {
...
function hasAuthoritativePretClassASlashAkiZeroFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassASlashAkiFrameMismatch === "function"
...
function hasAuthoritativePretClassAKwvForceFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassAKwvFrameMismatch === "function"
...
function hasAuthoritativePretClassAKvAllowFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassAKvAllowFrameMismatch === "function"
...
function hasAuthoritativePretClassAChiAllowFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassAChiAllowFrameMismatch === "function"
...
function hasAuthoritativePretClassATaRedupFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassATaRedupFrameMismatch === "function"
...
function hasAuthoritativePretClassAPaTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassAPaTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassAMTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassAMTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassAPiIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassAPiIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassAPiCvTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassAPiCvTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassACvwiTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassACvwiTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassACvcvwiTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassACvcvwiTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassACvwaiTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassACvwaiTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassACvewaTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassACvewaTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassACvawaTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassACvawaTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassAPaCvIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassAPaCvIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassANaCvIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassANaCvIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassBVnaIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassBVnaIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassBTaIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassBTaIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassBTaTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassBTaTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassBKwiCvIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassBKwiCvIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassBVcvcuIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassBVcvcuIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassBVlcvwiIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassBVlcvwiIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassBCvniuIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassBCvniuIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassACvvniIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassACvvniIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassACvsvIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassACvsvIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassACvwiIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassACvwiIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassACvcvwiIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassACvcvwiIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassBVjwaIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassBVjwaIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassBCuwaIntransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassBCuwaIntransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassANiCvTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassANiCvTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassANaCvTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassANaCvTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassANaCvcvcvTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassANaCvcvcvTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassANaCvlvcvTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassANaCvlvcvTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassANaVlcvcvTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassANaVlcvcvTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassANaVjcvcvTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassANaVjcvcvTransitiveFrameMismatch === "function"
...
function hasAuthoritativePretClassATzaTransitiveFrame(context = null) {
    return Boolean(
        context
        && typeof getPretClassATzaTransitiveFrameMismatch === "function"
...
function buildPretUniversalClassA(context) {
    ...
    const isAllowedStem = (base) => allowUnpronounceableStems || isSyllableSequencePronounceable(base);
    const patterns = createPretDescriptorMatcher(context);
    const hasRightEdge = (query = {}) => pretContextHasRightEdge(context, query);
    const hasAnyRightEdge = (queries = []) => pretContextHasAnyRightEdge(context, queries);
    ...
    if (!context.isTransitive && context.fromRootPlusYa) {
        ...
        const baseSpecFrameResult = buildPretRootPlusYaClassABaseSpecsFromSourceFrame(
            context.rootPlusYaSourceFrame,
            context
        );
    ...
    if (!context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvniU)) {
        return null;
    }
    const isIntransitiveWiKiOnly = !context.isTransitive && (
        patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vwi)
        || (pretContextHasRightEdge(context, PRET_RIGHT_EDGE_RULE_QUERIES.wiV_CV_CV) && !context.supportiveInitialI)
        || patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vjcvwi)
        || patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vlvwi)
        || patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvlvwi)
    );
    const allowIntransitiveChiClassA = hasAuthoritativePretClassAChiAllowFrame(context);
    const isShapeLVIKiOnly = !hasAnyRightEdge([{ rightEdgeProfileSuffixes: ["Vl|V", "CVl|V"] }])
        && context.lastNucleus === "i"
        && (patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vlv) || patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvlv));
    const isIntransitiveLWaKiOnly = !context.isTransitive && patterns.hasAggregate(PRET_DESCRIPTOR_QUERIES.aggregate.lwaPattern);
    const isEwaKiOnly = context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cewa);
    const isTransitiveVwaKiOnly = context.isTransitive
        && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vwa)
        && !patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vwaI);
    const isEwaAllowZero = context.isTransitive
        && patterns.hasAggregate(PRET_DESCRIPTOR_QUERIES.aggregate.ewaPattern)
        && !patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cewa);
    const isTransitiveCawa = context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvwaA);
    const isTransitiveCVwaI = context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvwaI);
    ...
    const isTransitiveAwaAllowZero = context.isTransitive
        && (patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvcawa) || patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvlawa));
    const isTransitiveCVwi = context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvwi);
    const hasMTransitiveRightEdge = context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ finalOnset: "m", finalNuclei: ["a", "i"] });
    const allowMTransitiveClassA = hasAuthoritativePretClassAMTransitiveFrame(context);
    const classAMTransitivePolicyFrame = allowMTransitiveClassA
        ? context.classAMTransitiveOperationFrame
        : null;
    const hasPiIntransitiveRightEdge = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "p+i" });
    const allowPiIntransitiveClassA = hasAuthoritativePretClassAPiIntransitiveFrame(context);
    const classAPiIntransitivePolicyFrame = allowPiIntransitiveClassA
        ? context.classAPiIntransitiveOperationFrame
        : null;
    const hasPiCvTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "p+i", rightEdgeProfiles: ["CV|CV"] });
    const allowPiCvTransitiveClassA = hasAuthoritativePretClassAPiCvTransitiveFrame(context);
    const classAPiCvTransitivePolicyFrame = allowPiCvTransitiveClassA
        ? context.classAPiCvTransitiveOperationFrame
        : null;
    const hasCvwiTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && context.isReduplicated !== true
        && hasRightEdge({ endingFamily: "w+i", rightEdgeProfiles: ["CV|CV"] });
    const allowCvwiTransitiveClassA = hasAuthoritativePretClassACvwiTransitiveFrame(context);
    const classACvwiTransitivePolicyFrame = allowCvwiTransitiveClassA
        ? context.classACvwiTransitiveOperationFrame
        : null;
    const hasCvcvwiTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && context.isReduplicated !== true
    ...
    const classACvcvwiTransitivePolicyFrame = allowCvcvwiTransitiveClassA
        ? context.classACvcvwiTransitiveOperationFrame
        : null;
    const hasCvwaiTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({
            endingFamily: "w+a",
    ...
    const classACvwaiTransitivePolicyFrame = allowCvwaiTransitiveClassA
        ? context.classACvwaiTransitiveOperationFrame
        : null;
    const hasCvewaTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({
            endingFamily: "w+a",
            rightEdgeProfiles: ["CV|CV"],
    ...
    const classACvewaTransitivePolicyFrame = allowCvewaTransitiveClassA
        ? context.classACvewaTransitiveOperationFrame
        : null;
    const hasCvawaTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && context.isReduplicated !== true
    ...
    const classACvawaTransitivePolicyFrame = allowCvawaTransitiveClassA
        ? context.classACvawaTransitiveOperationFrame
        : null;
    const hasPaTransitiveRightEdge = context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "p+a" });
    const allowPaTransitiveClassA = hasAuthoritativePretClassAPaTransitiveFrame(context);
    const classAPaTransitivePolicyFrame = allowPaTransitiveClassA
        ? context.classAPaTransitiveOperationFrame
        : null;
    const hasPaCvIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "p+a", rightEdgeProfiles: ["CV|CV"] });
    const allowPaCvIntransitiveClassA = hasAuthoritativePretClassAPaCvIntransitiveFrame(context);
    const classAPaCvIntransitivePolicyFrame = allowPaCvIntransitiveClassA
        ? context.classAPaCvIntransitiveOperationFrame
        : null;
    const hasNaCvIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "n+a", rightEdgeProfiles: ["CV|CV"] });
    const allowNaCvIntransitiveClassA = hasAuthoritativePretClassANaCvIntransitiveFrame(context);
    const classANaCvIntransitivePolicyFrame = allowNaCvIntransitiveClassA
        ? context.classANaCvIntransitiveOperationFrame
        : null;
    const hasCvvniIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "n+i", rightEdgeProfiles: ["CV|V|CV"] });
    const allowCvvniIntransitiveClassA = hasAuthoritativePretClassACvvniIntransitiveFrame(context);
    const classACvvniIntransitivePolicyFrame = allowCvvniIntransitiveClassA
        ? context.classACvvniIntransitiveOperationFrame
        : null;
    const hasCvsvIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "s+i", rightEdgeProfiles: ["CV|CV"] });
    const allowCvsvIntransitiveClassA = hasAuthoritativePretClassACvsvIntransitiveFrame(context);
    const classACvsvIntransitivePolicyFrame = allowCvsvIntransitiveClassA
        ? context.classACvsvIntransitiveOperationFrame
        : null;
    const hasCvwiIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "w+i", rightEdgeProfiles: ["CV|CV"] });
    const allowCvwiIntransitiveClassA = hasAuthoritativePretClassACvwiIntransitiveFrame(context);
    const classACvwiIntransitivePolicyFrame = allowCvwiIntransitiveClassA
        ? context.classACvwiIntransitiveOperationFrame
        : null;
    const hasCvcvwiIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "w+i", rightEdgeProfiles: ["CV|CV|CV"] });
    const allowCvcvwiIntransitiveClassA = hasAuthoritativePretClassACvcvwiIntransitiveFrame(context);
    const classACvcvwiIntransitivePolicyFrame = allowCvcvwiIntransitiveClassA
        ? context.classACvcvwiIntransitiveOperationFrame
        : null;
    const hasNiCvTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "n+i", rightEdgeProfiles: ["CV|CV"] });
    const allowNiCvTransitiveClassA = hasAuthoritativePretClassANiCvTransitiveFrame(context);
    const classANiCvTransitivePolicyFrame = allowNiCvTransitiveClassA
        ? context.classANiCvTransitiveOperationFrame
        : null;
    const hasNaCvTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "n+a", rightEdgeProfiles: ["CV|CV"] });
    const allowNaCvTransitiveClassA = hasAuthoritativePretClassANaCvTransitiveFrame(context);
    const classANaCvTransitivePolicyFrame = allowNaCvTransitiveClassA
        ? context.classANaCvTransitiveOperationFrame
        : null;
    const isTransitiveShapeCVCVna = context.isTransitive
    ...
    const classANaCvcvcvTransitivePolicyFrame = allowNaCvcvcvTransitiveClassA
        ? context.classANaCvcvcvTransitiveOperationFrame
        : null;
    const hasNaCvlvcvTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "n+a", rightEdgeProfiles: ["CVl|V|CV"] });
    const allowNaCvlvcvTransitiveClassA = hasAuthoritativePretClassANaCvlvcvTransitiveFrame(context);
    const classANaCvlvcvTransitivePolicyFrame = allowNaCvlvcvTransitiveClassA
        ? context.classANaCvlvcvTransitiveOperationFrame
        : null;
    const hasNaVlcvcvTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vlcvna);
    const allowNaVlcvcvTransitiveClassA = hasAuthoritativePretClassANaVlcvcvTransitiveFrame(context);
    const classANaVlcvcvTransitivePolicyFrame = allowNaVlcvcvTransitiveClassA
        ? context.classANaVlcvcvTransitiveOperationFrame
        : null;
    const hasNaVjcvcvTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vjcvna);
    const allowNaVjcvcvTransitiveClassA = hasAuthoritativePretClassANaVjcvcvTransitiveFrame(context);
    const classANaVjcvcvTransitivePolicyFrame = allowNaVjcvcvTransitiveClassA
        ? context.classANaVjcvcvTransitiveOperationFrame
        : null;
    const hasTzaTransitiveShape = context.isTransitive
        && !context.isMonosyllable
        && (
            patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvtza)
            || patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vjcvtza)
        );
    const allowTzaTransitiveClassA = hasAuthoritativePretClassATzaTransitiveFrame(context);
    const classATzaTransitivePolicyFrame = allowTzaTransitiveClassA
        ? context.classATzaTransitiveOperationFrame
        : null;
    const isTransitiveCVnaAllowZero = context.isTransitive
        && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvna)
        && (context.isReduplicated || context.isBitransitive);
    const isTransitiveShapeNi = context.isTransitive
        && !patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvnV)
        && patterns.hasShapeEndingFamily("n+i");
    const isTransitiveTaRedupCVCV = hasAuthoritativePretClassATaRedupFrame(context);
    const allowSlashAkiZero = !context.isTransitive
        && context.hasSlashMarker
        && hasAuthoritativePretClassASlashAkiZeroFrame(context);
    ...
    if (patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvsV)) {
    ...
    if (hasPiCvTransitiveShape) {
        ...
        allowZeroSuffix = false;
        allowKiSuffix = true;
    } else if (context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvpV)) {
    ...
    const allowIntransitiveKV = finalOnset === "kw"
        ? forceClassAForKWV
        : (finalOnset === "k"
            ? hasAuthoritativePretClassAKvAllowFrame(context)
    ...
    if (
        context.isTransitive
        && hasRightEdge({ endingFamily: "k+a" })
        && !hasRightEdge({ endingFamily: "k+a", previousForm: "CV", previousNucleus: "a" })
    ) {
        return null;
    }
    if (!context.isTransitive && hasRightEdge({ endingFamily: "k+a", previousForm: "V" })) {
        return null;
    }
    if (!context.isTransitive && hasRightEdge({ endingFamily: "k+a", previousForm: "CV" })) {
        return null;
    }
    if (context.isTransitive && hasRightEdge({ endingFamily: "tz+a" })) {
    ...
    if (
        hasRightEdge({ finalOnset: "tz" })
        && !hasAnyRightEdge([{ rightEdgeProfileSuffixes: ["V|C|CV", "CV|C|CV"] }])
    ) {
        allowZeroSuffix = false;
        allowKiSuffix = true;
    }
    ...
    if (!context.isTransitive && hasRightEdge({ endingFamily: "n+a" })) {
        if (context.totalVowels <= 2 && !hasNaCvIntransitiveShape && !patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvna)) {
            return null;
        }
    ...
    if (hasRightEdge({ endingFamily: "y+a" })) {
        allowZeroSuffix = false;
    }
    if (!context.isTransitive && hasRightEdge({ finalOnset: "n", previousForm: "CV" })) {
        allowZeroSuffix = false;
    }
    if (!context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvlvni)) {
        allowZeroSuffix = false;
    }
    if (context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vnV)) {
        allowZeroSuffix = false;
        allowKiSuffix = true;
    }
    if (context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvnV)) {
        if (patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvna)) {
            if (!allowNaCvTransitiveClassA) {
                return null;
            }
            allowZeroSuffix = classANaCvTransitivePolicyFrame?.targetFrame?.allowZeroSuffix === true;
            allowKiSuffix = classANaCvTransitivePolicyFrame?.targetFrame?.allowKiSuffix === true;
        } else if (patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvni)) {
            if (!allowNiCvTransitiveClassA) {
                return null;
            }
            allowZeroSuffix = classANiCvTransitivePolicyFrame?.targetFrame?.allowZeroSuffix === true;
    ...
    if (context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vjcvna)) {
        allowZeroSuffix = true;
        allowKiSuffix = true;
    }
    if (context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vlcvna)) {
        allowZeroSuffix = false;
        allowKiSuffix = true;
    }
    ...
    if (!forceClassAForKWV) {
        ...
        if (
            (context.isMonosyllable && finalOnset !== "t" && !isDenominalWiVowelSourceClassA) ||
            (
                !context.isTransitive
    ...
    if (context.isTransitive && (patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vjcvwa) || patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vlcvwa))) {
        allowZeroSuffix = true;
        allowKiSuffix = true;
    }
    if (context.isTransitive && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvjcvwa)) {
        allowZeroSuffix = false;
        allowKiSuffix = true;
    }
    if (hasAnyRightEdge([{ rightEdgeProfileSuffixes: ["Vl|V", "CVl|V"] }])) {
        allowZeroSuffix = true;
        allowKiSuffix = false;
    }
    if (!context.isTransitive && !hasAnyRightEdge([{ rightEdgeProfileSuffixes: ["Vl|V", "CVl|V"] }])) {
        allowZeroSuffix = false;
        allowKiSuffix = true;
    }
    ...
    const hasItaClassASourceShape = context.isTransitive
        && context.isItaVerb
        && Array.isArray(context.syllableForms)
        && context.syllableForms.length === 2
        && context.syllableForms[0] === "V"
        && context.syllableForms[1] === "CV"
        && context.rightEdgeDescriptor?.previousNucleus === "i"
        && context.rightEdgeDescriptor?.finalOnset === "t"
        && context.rightEdgeDescriptor?.finalNucleus === "a";
    if (hasItaClassASourceShape) {
        const itaFrameResult = buildPretClassAItaBaseSpecsFromSourceFrame(
            context.itaClassASourceFrame,
            context
        );
    ...
    const hasDirectYyaRightEdge = !context.isTransitive
        && context.rightEdgeDescriptor?.finalOnset === "y"
        && context.rightEdgeDescriptor?.finalNucleus === "a"
        && context.rightEdgeDescriptor?.previousForm === "C"
        && context.rightEdgeDescriptor?.previousOnset === "y";
    if (hasDirectYyaRightEdge) {
        const yyaFrameResult = buildPretClassAYyaBaseSpecsFromSourceFrame(
            context.yyaClassASourceFrame,
            context
        );
    ...
    const deletionFrameResult = buildPretClassAFinalVowelDeletionBaseSpecsFromSourceFrame(
        context.classAFinalVowelDeletionSourceFrame,
        context
    );
    if (!deletionFrameResult.ok) {
        return null;
    }
    let deletedVariants = deletionFrameResult.baseSpecs
        .map((baseSpec) => buildPretVariant("", "", { baseSpec }))
        .filter(Boolean);
    ...
    if (
        classACvawaTransitivePolicyFrame?.targetFrame?.allowJBaseVariant === false
        || (context.isTransitive
    ...
    deletedVariants.forEach((variant) => {
        ...
        if (allowKiSuffix) {
            addUniquePretVariant(variants, "", "ki", {
                ...
                routePolicyFrame: classAMTransitivePolicyFrame
                    || classAPiIntransitivePolicyFrame
                    || classAPaTransitivePolicyFrame
                    || classAPiCvTransitivePolicyFrame
                    || classACvwiTransitivePolicyFrame
                    || classACvcvwiTransitivePolicyFrame
                    || classACvwaiTransitivePolicyFrame
                    || classACvewaTransitivePolicyFrame
                    || classACvawaTransitivePolicyFrame
                    || classAPaCvIntransitivePolicyFrame
                    || classANaCvIntransitivePolicyFrame
                    || classACvvniIntransitivePolicyFrame
                    || classACvsvIntransitivePolicyFrame
                    || classANiCvTransitivePolicyFrame
                    || classANaCvTransitivePolicyFrame
                    || classANaCvcvcvTransitivePolicyFrame
                    || classANaCvlvcvTransitivePolicyFrame
                    || classANaVlcvcvTransitivePolicyFrame
                    || classANaVjcvcvTransitivePolicyFrame
                    || classATzaTransitivePolicyFrame
...
function buildPretUniversalClassB(context) {
    ...
    const patterns = createPretDescriptorMatcher(context);
    const hasRightEdge = (query = {}) => pretContextHasRightEdge(context, query);
    const hasTaIntransitiveRightEdge = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "t+a" });
    const allowTaIntransitiveClassB = hasAuthoritativePretClassBTaIntransitiveFrame(context);
    const classBTaIntransitivePolicyFrame = allowTaIntransitiveClassB
        ? context.classBTaIntransitiveOperationFrame
        : null;
    const hasTaTransitiveRightEdge = context.isTransitive
        && !context.isMonosyllable
        && context.isItaVerb !== true
        && hasRightEdge({ endingFamily: "t+a" });
    const allowTaTransitiveClassB = hasAuthoritativePretClassBTaTransitiveFrame(context);
    const classBTaTransitivePolicyFrame = allowTaTransitiveClassB
        ? context.classBTaTransitiveOperationFrame
        : null;
    const hasPiIntransitiveRightEdge = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "p+i" });
    const allowPiIntransitiveClassB = hasAuthoritativePretClassAPiIntransitiveFrame(context);
    const classAPiIntransitivePolicyFrame = allowPiIntransitiveClassB
        ? context.classAPiIntransitiveOperationFrame
        : null;
    const hasPaCvIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
    ...
    const classAPaCvIntransitivePolicyFrame = allowPaCvIntransitiveClassB
        ? context.classAPaCvIntransitiveOperationFrame
        : null;
    const hasNaCvIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
    ...
    const classANaCvIntransitivePolicyFrame = allowNaCvIntransitiveClassB
        ? context.classANaCvIntransitiveOperationFrame
        : null;
    const hasVnaIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "n+a", rightEdgeProfiles: ["V|CV"] });
    const allowVnaIntransitiveClassB = hasAuthoritativePretClassBVnaIntransitiveFrame(context);
    const classBVnaIntransitivePolicyFrame = allowVnaIntransitiveClassB
        ? context.classBVnaIntransitiveOperationFrame
        : null;
    const hasKwiCvIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "kw+i", rightEdgeProfiles: ["CV|CV"] });
    const allowKwiCvIntransitiveClassB = hasAuthoritativePretClassBKwiCvIntransitiveFrame(context);
    const classBKwiCvIntransitivePolicyFrame = allowKwiCvIntransitiveClassB
        ? context.classBKwiCvIntransitiveOperationFrame
        : null;
    const hasVcvcuIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ rightEdgeProfiles: ["V|CV|CV"], finalNucleus: "u" });
    const allowVcvcuIntransitiveClassB = hasAuthoritativePretClassBVcvcuIntransitiveFrame(context);
    const classBVcvcuIntransitivePolicyFrame = allowVcvcuIntransitiveClassB
        ? context.classBVcvcuIntransitiveOperationFrame
        : null;
    const hasVlcvwiIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "w+i", rightEdgeProfiles: ["Vl|CV|CV"] });
    const allowVlcvwiIntransitiveClassB = hasAuthoritativePretClassBVlcvwiIntransitiveFrame(context);
    const classBVlcvwiIntransitivePolicyFrame = allowVlcvwiIntransitiveClassB
        ? context.classBVlcvwiIntransitiveOperationFrame
        : null;
    const hasCvniuIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "n+i", rightEdgeProfiles: ["CV|CV"], previousNucleus: "u" });
    const allowCvniuIntransitiveClassB = hasAuthoritativePretClassBCvniuIntransitiveFrame(context);
    const classBCvniuIntransitivePolicyFrame = allowCvniuIntransitiveClassB
        ? context.classBCvniuIntransitiveOperationFrame
        : null;
    const hasCvvniIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "n+i", rightEdgeProfiles: ["CV|V|CV"] });
    const allowCvvniIntransitiveClassB = hasAuthoritativePretClassACvvniIntransitiveFrame(context);
    const classACvvniIntransitivePolicyFrame = allowCvvniIntransitiveClassB
        ? context.classACvvniIntransitiveOperationFrame
        : null;
    const hasCvsvIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
    ...
    const classACvsvIntransitivePolicyFrame = allowCvsvIntransitiveClassB
        ? context.classACvsvIntransitiveOperationFrame
        : null;
    const hasCvwiTransitiveShape = context.isTransitive
        && !context.isMonosyllable
    ...
    const classACvwiTransitivePolicyFrame = allowCvwiTransitiveClassB
        ? context.classACvwiTransitiveOperationFrame
        : null;
    const hasCvcvwiTransitiveShape = context.isTransitive
        && !context.isMonosyllable
    ...
    const classACvcvwiTransitivePolicyFrame = allowCvcvwiTransitiveClassB
        ? context.classACvcvwiTransitiveOperationFrame
        : null;
    const hasCvwiIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
    ...
    const classACvwiIntransitivePolicyFrame = allowCvwiIntransitiveClassB
        ? context.classACvwiIntransitiveOperationFrame
        : null;
    const hasCvcvwiIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
    ...
    const classACvcvwiIntransitivePolicyFrame = allowCvcvwiIntransitiveClassB
        ? context.classACvcvwiIntransitiveOperationFrame
        : null;
    const hasVjwaIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({ endingFamily: "w+a", rightEdgeProfiles: ["Vj|CV"] });
    const allowVjwaIntransitiveClassB = hasAuthoritativePretClassBVjwaIntransitiveFrame(context);
    const classBVjwaIntransitivePolicyFrame = allowVjwaIntransitiveClassB
        ? context.classBVjwaIntransitiveOperationFrame
        : null;
    const hasCuwaIntransitiveShape = !context.isTransitive
        && !context.isMonosyllable
        && hasRightEdge({
            endingFamily: "w+a",
            rightEdgeProfiles: ["CV|CV"],
    ...
    const classBCuwaIntransitivePolicyFrame = allowCuwaIntransitiveClassB
        ? context.classBCuwaIntransitiveOperationFrame
        : null;
    if (!context.isTransitive && context.fromRootPlusYa) {
        const rootPlusYaFrameResult = buildPretRootPlusYaClassBBaseSpecsFromSourceFrame(
            context.rootPlusYaSourceFrame,
            context,
        );
        ...
        if (isSlashDenominalRootPlusYaMatrix(context)) {
            ...
            return [buildPretVariant("", "k", { baseSpec: rootPlusYaFrameResult.deletedYaBaseSpec })];
        }
        if (context.isWeya) {
            return [buildPretVariant("", "k", { baseSpec: rootPlusYaFrameResult.sourceVerbBaseSpec })];
        }
        ...
        const slashEmbeddedYaBase = context.hasSlashMarker
            ? rootPlusYaFrameResult.deletedYaBase
            : "";
    ...
    if (!context.isTransitive && pretContextHasRightEdge(context, PRET_RIGHT_EDGE_RULE_QUERIES.naCV_CV_CV_CV)) {
        return null;
    }
    if (!context.isTransitive && (patterns.hasAggregate(PRET_DESCRIPTOR_QUERIES.aggregate.waPattern) || hasCuwaIntransitiveShape)) {
        const classBFrameResult = buildPretClassBBaseSpecsFromSourceFrame(
            context.classBSourceFrame,
            context,
        );
        ...
        return [buildPretVariant("", "k", { baseSpec: classBFrameResult.baseSpecs[0] })].filter(Boolean);
    }
    ...
    const variants = [buildPretVariant("", classBTargetSuffix, {
        baseSpec: classBFrameResult.baseSpecs[0],
        routePolicyFrame: classBTaIntransitivePolicyFrame
            || classBTaTransitivePolicyFrame
            || classAPiIntransitivePolicyFrame
            || classAPaCvIntransitivePolicyFrame
            || classANaCvIntransitivePolicyFrame
            || classBVnaIntransitivePolicyFrame
            || classBKwiCvIntransitivePolicyFrame
            || classBVcvcuIntransitivePolicyFrame
            || classBVlcvwiIntransitivePolicyFrame
            || classBCvniuIntransitivePolicyFrame
            || classACvvniIntransitivePolicyFrame
            || classACvsvIntransitivePolicyFrame
            || classACvwiTransitivePolicyFrame
            || classACvcvwiTransitivePolicyFrame
            || classACvwiIntransitivePolicyFrame
            || classACvcvwiIntransitivePolicyFrame
            || classBVjwaIntransitivePolicyFrame
            || classBCuwaIntransitivePolicyFrame,
    })].filter(Boolean);
...
function getPretUniversalVariants(verb, tense, isTransitive, analysisVerb = verb, options = {}) {
    ...
    if (
        !isTransitive
        && ["k", "kw"].includes(context?.rightEdgeDescriptor?.finalOnset || "")
        && (context?.rightEdgeDescriptor?.finalNucleus || "") !== "u"
    ) {
        context.allowIntransitiveKV = true;
    }
...
function getPronounceableClassBFallback(context) {
    ...
    const allowUnpronounceableStems = context.allowUnpronounceableStems === true;
    if (!allowUnpronounceableStems && !isSyllableSequencePronounceable(classBFrameResult.sourceVerb)) {
        return null;
    }
    return [buildPretVariant("", "k", { baseSpec: classBFrameResult.baseSpecs[0] })];
}
...
function evaluatePretPrefixBaseContactFrame(contactFrame = null) {
    if (
        !contactFrame
        || typeof contactFrame !== "object"
        || contactFrame.kind !== PRET_PREFIX_BASE_CONTACT_FRAME_KIND
    ...
    const prefixLetters = Array.isArray(contactFrame.prefixFrame.letters)
        ? contactFrame.prefixFrame.letters.slice()
        : getPretContactLetters(contactFrame.prefixFrame.text || "");
    const baseLetters = Array.isArray(contactFrame.baseFrame.letters)
        ? contactFrame.baseFrame.letters.slice()
        : getPretContactLetters(contactFrame.baseFrame.text || "");
    if (
        joinPretContactLetters(prefixLetters) !== normalizePretBaseFrameText(contactFrame.prefixFrame.text || "")
        || joinPretContactLetters(baseLetters) !== normalizePretBaseFrameText(contactFrame.baseFrame.text || "")
    ) {
    ...
    if (
        operationFrame.suppressBareKBeforeK === true
        && joinPretContactLetters(prefixLetters) === "k"
        && (baseLetters[0] === "k" || baseLetters[0] === "kw")
    ) {
        prefixLetters.length = 0;
    }
    if (
        operationFrame.composedObjectPrefix === true
        && prefixLetters[prefixLetters.length - 1] === "k"
        && (baseLetters[0] === "k" || baseLetters[0] === "kw")
    ) {
        if (baseLetters[0] === "kw") {
            prefixLetters.pop();
    ...
    if (
        prefixLetters.length
        && baseLetters.length
        && isVerbLetterVowel(baseLetters[0])
        && prefixLetters[prefixLetters.length - 1] === "n"
        && prefixLetters[prefixLetters.length - 2] !== "h"
        && isVerbLetterVowel(prefixLetters[prefixLetters.length - 2] || "")
    ) {
...
function attachPretUniversalVariantAssemblyGrammarContract(output = null, {
    ...
    const diagnostics = Array.isArray(diagnosticsOverride)
        ? diagnosticsOverride
        : ok
        ? []
...
function buildPretUniversalResultDetailedFromVariants(assemblyFrame) {
    if (
        !assemblyFrame
        || typeof assemblyFrame !== "object"
        || assemblyFrame.kind !== PRET_VARIANT_ASSEMBLY_FRAME_KIND
    ) {
    ...
    if (
        !sourceFrame
        || typeof sourceFrame !== "object"
        || sourceFrame.kind !== PRET_VARIANT_ASSEMBLY_SOURCE_FRAME_KIND
        || !Array.isArray(sourceFrame.variants)
    ) {
    ...
    if (
        !operationFrame
        || typeof operationFrame !== "object"
        || operationFrame.kind !== PRET_VARIANT_ASSEMBLY_OPERATION_FRAME_KIND
    ) {
    ...
    variants.forEach((variant) => {
        ...
        if (!entry) {
            entry = { suffixes: new Set(), order: [], verbCore, surfaceRuleMeta };
    ...
    order.forEach((baseKey) => {
        ...
        const { verbCore, surfaceRuleMeta } = entry;
...
function getKVClassPolicy({
    ...
    const allowClassBWithA = !allowAllClasses
        ...
        && !isTransitive
        && (
            (pretContextHasRightEdge(context, PRET_RIGHT_EDGE_RULE_QUERIES.waCV_CV_CV) && !context.isReduplicated)
    ...
    const isKOnlyNoU = ["k", "kw"].includes(rightEdgeDescriptor.finalOnset || "")
        && (rightEdgeDescriptor.finalNucleus || "") !== "u";
...
function resolvePretClassPolicy({
    ...
    const ruleContext = {
        ...
        isCVliPattern: Boolean(
            ...
            && !context.isTransitive
            && (
                patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvlv)
                || patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.vlv)
                || pretContextHasAnyRightEdge(context, [{ rightEdgeProfileSuffixes: ["Vl|V", "CVl|V"] }])
            )
            && context.lastNucleus === "i"
        ),
        isCVpVPattern: Boolean(
            context
            && patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvpV)
            && !context.isTransitive
        ),
        isIntransitivePiPattern: Boolean(
            ...
            && !context.isMonosyllable
            && hasAuthoritativePretClassAPiIntransitiveFrame(context)
        ),
        isCVVniPattern: Boolean(
            context
            && pretContextHasRightEdge(context, { endingFamily: "n+i", rightEdgeProfiles: ["CV|V|CV"] })
            && !context.isTransitive
            && hasAuthoritativePretClassACvvniIntransitiveFrame(context)
        ),
    };
    const rules = [
        {
...
function buildPretUniversalResultWithProvenance({
    ...
    if (classKey === "B") {
        ...
        if (hasAuthoritativePretClassAKwvForceFrame(context)) {
            if (hasClassAVariants) {
                ...
                const { result: resultKWV, forms: formsKWV } = buildPretUniversalResultDetailedFromVariants(buildPretVariantAssemblyFrame({
        ...
        if (!isTransitive && !context.fromRootPlusYa) {
            const mvSource = context.analysisVerb || context.verb || "";
            const isMVEnding = pretContextHasRightEdge(context, {
                finalOnset: "m",
                finalNuclei: ["a", "i"],
            }) || /m[ai]$/.test(mvSource);
            const patterns = createPretDescriptorMatcher(context);
            const allowClassBWithA = (
                (patterns.hasShape(PRET_DESCRIPTOR_QUERIES.shape.cvsV) && context.lastNucleus === "i")
                || pretContextHasRightEdge(context, PRET_RIGHT_EDGE_RULE_QUERIES.waCV_CV_CV)
            ...
            if (candidates.has("A") && hasClassAVariants && !isMVEnding && !allowClassBWithA) {
                ...
                const { result: resultFallbackA, forms: formsFallbackA } = buildPretUniversalResultDetailedFromVariants(buildPretVariantAssemblyFrame({
                    variants: classAVariants,
                    subjectPrefix,
                    objectPrefix,
                    subjectSuffix,
    ...
    const { result, forms } = buildPretUniversalResultDetailedFromVariants(buildPretVariantAssemblyFrame({
        variants,
        subjectPrefix,
        objectPrefix,
        subjectSuffix,
