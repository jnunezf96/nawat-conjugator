export function createPreteritApi(targetObject = globalThis) {
    ...
    function resolvePretUniversalContextBundle({
        ...
        const summary = includeSummary && typeof targetObject.buildPretUniversalRuleSummary === "function"
            ? targetObject.buildPretUniversalRuleSummary(context)
            : null;
        return {
    ...
    function buildClassBasedResultWithProvenance({
        ...
        const resolvedFinalYaProxyBase = (
            !rootPlusYaBasePronounceable
            && targetObject.PRETERITO_CLASS_TENSES.has(tense)
            && !isTransitive
        )
            ? targetObject.resolveFinalYaPerfectiveAlternateBase(analysisVerb || verb, {
                isTransitive,
                isYawi,
                isWeya,
    ...
    return {
        buildClassBasedProvenance,
        resolvePretUniversalContextBundle,
        buildPretUniversalMarkerOptions,
        buildPretMarkerOptionsFromFlags,
        buildPretUniversalContextOptions,
