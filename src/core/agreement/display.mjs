...
export function createAgreementDisplayApi(targetObject = globalThis) {
    ...
    function getDefaultOutputToggleSelection({
        ...
        if (context === "verb-primary-object") {
            ...
            return resolveDefaultToggleId(values, preferredId ?? defaultPreferredId, [
                defaultPreferredId,
                targetObject.OBJECT_TOGGLE_ALL,
                ...fallbackIds,
            ]);
        }
        ...
        if (context === "noun-subject") {
            const zeroSubjectId = Array.isArray(subjectOptions)
                ...
                : null;
            return resolveDefaultToggleId(values, preferredId ?? zeroSubjectId ?? targetObject.SUBJECT_TOGGLE_ALL, [
                zeroSubjectId,
                targetObject.SUBJECT_TOGGLE_ALL,
                ...fallbackIds,
            ]);
        }
    ...
    function getCurrentObjectPrefix() {
        ...
        const selectionState = targetObject.getCurrentResolvedConjugationSelectionState();
        const tenseValue = selectionState.group === targetObject.CONJUGATION_GROUPS.universal
        ...
        const isNonactiveMode =
            targetObject.getActiveTenseMode() === targetObject.TENSE_MODE.verbo
            && targetObject.getCombinedMode() === targetObject.COMBINED_MODE.nonactive;
        const objectStateKey = targetObject.getObjectStateKey({ groupKey, tenseValue, mode, isNonactive: isNonactiveMode });
        const stored = targetObject.getToggleStateValue(targetObject.ObjectToggleState, objectStateKey);
    ...
    function getObjectPrefixesForTransitividad(parsedVerb = null) {
        ...
        return availableSlots > 0
            ? targetObject.OBJECT_PREFIXES.map((opt) => opt.value)
            : [""];
    }
    ...
    function getNounObjectLabel(prefix, isNawat = false) {
        if (!prefix) {
            return targetObject.getLocalizedLabel(
                targetObject.NOUN_OBJECT_LABELS.intransitive,
                isNawat,
                "a él/ella/eso"
            );
        }
    ...
    function getSubjectSubLabel(selection, options = {}) {
        ...
        const isNonanimateContext = options.isNonanimateContext === true
            || (mode === "noun" && targetObject.NONANIMATE_NOUN_TENSES.has(tenseValue));
        if (!selection) {
            return "";
        }
        if (!isNawat && isNonanimateContext && targetObject.isNonanimateSubject(selection.subjectPrefix, selection.subjectSuffix)) {
    ...
    return {
        VALENCE3_PLUS_SLOT_ROLE_KEYS,
        VALENCE3_PLUS_SLOT_ROLE_FALLBACKS,
        resolveDefaultToggleId,
        getDefaultOutputToggleSelection,
        getPreferredObjectPrefix,
        getPreferredNounObjectPrefix,
        getCurrentObjectPrefix,
        getKnownTenseSuffixes,
