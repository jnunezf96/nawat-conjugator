export function createPhonologyApi(targetObject = globalThis) {
    function splitVerbLetters(verb) {
        const letters = [];
        for (let index = 0; index < verb.length; index += 1) {
            const pair = verb.slice(index, index + 2);
            if (targetObject.DIGRAPH_SET.has(pair)) {
                letters.push(pair);
                index += 1;
            } else {
                letters.push(verb[index]);
            }
        }
        return letters;
    }
    ...
    function isVerbLetterVowel(letter) {
        return letter.length === 1 && targetObject.VOWELS.includes(letter);
    }
    ...
    function applySyllableAnalysisTargetOptions(rawTarget, options = {}) {
        ...
        const hasFinalVowel = targetObject.VOWELS.includes(lastChar);
    ...
    function getNonReduplicatedRoot(verb) {
        ...
        for (let index = 0; index < syllables.length - 1; index += 1) {
            const first = syllables[index];
            const second = syllables[index + 1];
            if (!first || !second) {
                continue;
            }
            const isStandardRedup = (
                targetObject.REDUP_PREFIX_FORMS.has(first.form)
                && second.nucleus
            ...
            if (isStandardRedup || isLRedup) {
                return syllables.slice(index + 1).map((syllable) => syllable.text).join("");
            }
        }
        return verb;
    }
    ...
    function areSyllablesPronounceable(syllables) {
        ...
        for (let index = 0; index < syllables.length; index += 1) {
            ...
            if (!targetObject.SYLLABLE_FORM_SET.has(form)) {
                return false;
            }
            if (
    ...
    function getBareRootFromYaStem(stem = "") {
        ...
        if (stem === "weya") {
            return targetObject.getSuppletiveWeyaRootPlusYaBase() || stem.slice(0, -2);
        }
        let current = stem;
        let guard = 0;
    ...
    function analyzeFinalYaStructure(verb, options = {}) {
        ...
        if (options.isWeya) {
            const bareRoot = targetObject.getSuppletiveWeyaRootPlusYaBase() || "";
...
export function installPhonologyGlobals(targetObject = globalThis) {
    const api = createPhonologyApi(targetObject);
