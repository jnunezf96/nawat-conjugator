...
export function createClauseGlobals(targetObject = globalThis) {
    ...
    function buildVerbalNuclearClauseFormulaEchoFromSlots(formulaSlots = null) {
      ...
      const predicateDisplay = rawPredicateDisplay.startsWith("(") && rawPredicateDisplay.endsWith(")") ? rawPredicateDisplay.slice(1, -1) : rawPredicateDisplay;
    ...
    function buildNuclearClauseShellMetadata(options = {}) {
      ...
      const shell = {
        ...
        clauseKind,
        displayLabel: formulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.unknown ? "Clausula nuclear" : `Clausula ${formulaType}`,
...
export function installClauseGlobals(targetObject = globalThis) {
    const api = createClauseGlobals(targetObject);
