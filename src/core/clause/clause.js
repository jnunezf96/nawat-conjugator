...
const NUCLEAR_CLAUSE_LESSON4_USAGE_OPTIONS = Object.freeze([
    Object.freeze({
...
const NUCLEAR_CLAUSE_LESSON4_VOCABLE_SCOPE_FRAME = Object.freeze({
    ...
    useRoles: Object.freeze([
        NUCLEAR_CLAUSE_LESSON4_USAGE_ROLE.simpleSentence,
        NUCLEAR_CLAUSE_LESSON4_USAGE_ROLE.mainClause,
        NUCLEAR_CLAUSE_LESSON4_USAGE_ROLE.dependentClause,
        NUCLEAR_CLAUSE_LESSON4_USAGE_ROLE.conjoinedClause,
    ]),
});
...
const NUCLEAR_CLAUSE_LESSON4_POSITION_COMPLEXITY_FRAME = Object.freeze({
    ...
    positions: Object.freeze({
        person: Object.freeze({ complexity: "dyadic", subpositions: Object.freeze(["pers1", "pers2"]) }),
        number: Object.freeze({ complexity: "dyadic", subpositions: Object.freeze(["num1", "num2"]) }),
        tense: Object.freeze({ complexity: "monadic", slot: "tns", occursIn: Object.freeze([NUCLEAR_CLAUSE_FORMULA_TYPE.vnc]) }),
...
const NUCLEAR_CLAUSE_LESSON4_LAYER_PROFILES = Object.freeze({
    [NUCLEAR_CLAUSE_FORMULA_TYPE.vnc]: Object.freeze([
        Object.freeze({ level: 1, key: "verbstem", label: "verbstem", labelEs: "tronco verbal", role: "foundation" }),
        Object.freeze({ level: 2, key: "verbcore", label: "verbcore = valence + stem", labelEs: "núcleo verbal = valencia + base", role: "core" }),
        Object.freeze({ level: 3, key: "predicate", label: "predicate = verbcore + tense", labelEs: "predicado = núcleo verbal + tiempo", role: "predicate" }),
        Object.freeze({ level: 4, key: "vnc", label: "VNC = subject + predicate", labelEs: "CNV = sujeto + predicado", role: "nuclear-clause" }),
    ]),
    [NUCLEAR_CLAUSE_FORMULA_TYPE.nnc]: Object.freeze([
        Object.freeze({ level: 1, key: "nounstem", label: "nounstem", labelEs: "tronco nominal", role: "foundation" }),
        Object.freeze({ level: 2, key: "nouncore", label: "nouncore = predicate = state + stem", labelEs: "núcleo nominal = predicado = estado + base", role: "predicate" }),
        Object.freeze({ level: 3, key: "nnc", label: "NNC = subject + predicate", labelEs: "CNN = sujeto + predicado", role: "nuclear-clause" }),
    ]),
});
...
const NUCLEAR_CLAUSE_LESSON4_SUBSECTION_INVENTORY = Object.freeze([
    Object.freeze({
...
const NUCLEAR_CLAUSE_POSSESSIVE_PREFIX_FEATURES = Object.freeze({
    "": Object.freeze({ person: 0, number: "none" }),
    nu: Object.freeze({ person: 1, number: "sg" }),
    mu: Object.freeze({ person: 2, number: "sg" }),
    i: Object.freeze({ person: 3, number: "sg" }),
    tu: Object.freeze({ person: 1, number: "pl" }),
    anmu: Object.freeze({ person: 2, number: "pl" }),
    in: Object.freeze({ person: 3, number: "pl" }),
});
...
function getNuclearClauseTerminologyForFormulaType(formulaType = "") {
    ...
    const terminology = typeof getNuclearClauseTerminology === "function"
        ? getNuclearClauseTerminology()
        : NUCLEAR_CLAUSE_TERMINOLOGY_FALLBACK;
    const fallback = normalizedFormulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.vnc
        ? NUCLEAR_CLAUSE_TERMINOLOGY_FALLBACK.vnc
        : (normalizedFormulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.nnc
            ? NUCLEAR_CLAUSE_TERMINOLOGY_FALLBACK.nnc
            : NUCLEAR_CLAUSE_TERMINOLOGY_FALLBACK.nc);
    const source = normalizedFormulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.vnc
        ? terminology?.vnc
        : (normalizedFormulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.nnc
            ? terminology?.nnc
            : terminology?.nc);
    return {
        ...
        legacyFormulaType: normalizedFormulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.vnc
            || normalizedFormulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.nnc
...
function normalizeNuclearClausePredicatePositionStatus(value = "") {
    const normalized = String(value || "").trim().toLowerCase();
    if ([
        NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.dyadic,
        "diadic",
        "diadica",
        "diádica",
        "va1-va2",
...
function getNuclearClauseLesson4UsageOptionLabel(role = "") {
    ...
    return NUCLEAR_CLAUSE_LESSON4_USAGE_OPTIONS
        .find((entry) => entry.role === normalizedRole)?.labelEs || "sin uso sintáctico fijado";
}
...
function getNuclearClauseLesson4FormulaInventory() {
    const buildSet = (formulaType) => ([
        NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.dyadic,
        NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.monadic,
        NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.vacant,
    ].map((predicatePositionStatus) => buildNuclearClauseLesson4FormulaRecord({
        formulaType,
        predicatePositionStatus,
    })));
    return {
...
function getNuclearClauseOrganizationalLayers(formulaType = "") {
    const normalizedFormulaType = normalizeNuclearClauseFormulaType(formulaType);
    return (NUCLEAR_CLAUSE_LESSON4_LAYER_PROFILES[normalizedFormulaType] || [])
        .map((entry) => ({ ...entry }));
}

function cloneNuclearClausePositionComplexityFrame() {
    const positions = {};
    Object.entries(NUCLEAR_CLAUSE_LESSON4_POSITION_COMPLEXITY_FRAME.positions)
        .forEach(([key, value]) => {
            positions[key] = {
                ...value,
                ...
                slotsByStatus: value.slotsByStatus ? { ...value.slotsByStatus } : undefined,
            };
...
function getNuclearClausePersonalPronounFrame() {
    return {
        ...
        cases: Object.fromEntries(
            Object.entries(NUCLEAR_CLAUSE_PERSONAL_PRONOUN_FRAME.cases)
                .map(([key, value]) => [key, {
                    ...value,
...
function buildNuclearClauseLesson4PursuitFrame() {
    ...
    return {
        ...
        remainingGaps: [
            ...
            "Personal-pronoun reference for 3a person remains context-required.",
            "Lessons 5-16 still govern detailed CNV/CNN filler paradigms.",
        ],
        closestPass: false,
    };
}
...
function buildNuclearClauseLesson4UseFrame({ usageRole = "" } = {}) {
    ...
    return {
        ...
        diagnosticStatus: activeRole === NUCLEAR_CLAUSE_LESSON4_USAGE_ROLE.unspecified
            ? "context-required"
            : "classified",
        diagnosticId: activeRole === NUCLEAR_CLAUSE_LESSON4_USAGE_ROLE.unspecified
            ? "lesson4-nuclear-clause-use-unspecified"
...
function buildNuclearClauseLesson4PredicatePositionControlFrame({
    ...
    const entries = (
        NUCLEAR_CLAUSE_LESSON4_STAGE_3_FORMULAS[normalizedFormulaType]
        ? [
            NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.dyadic,
            NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.monadic,
            NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.vacant,
        ...
        const roleLabel = normalizedFormulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.vnc
            ? (status === NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.dyadic
                ? "valencia doble"
                : (status === NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.monadic
                    ? "valencia simple"
                    : "sin valencia explícita"))
            : (status === NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.dyadic
                ? "estado doble"
                : (status === NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.monadic
                    ? "estado simple"
    ...
    return {
        ...
        diagnosticId: statusSource === "explicit"
            ? "lesson4-predicate-position-explicit"
            : "lesson4-predicate-position-inferred",
        options: entries,
    };
}
...
function getNuclearClauseSubjectPronounFeatures(slot = null) {
    ...
    const info = typeof getPers1Pers2Info === "function"
...
function getNuclearClauseObjectPronounFeatures(slot = null) {
    ...
    const info = typeof getObj1PersonInfo === "function"
        ? getObj1PersonInfo(prefix)
        : null;
    return info
        ? {
            person: info.person,
...
function getNuclearClausePossessivePrefixFromSlot(predicateSlot = null) {
    const stateSlot = predicateSlot?.stateSlot && typeof predicateSlot.stateSlot === "object"
        ? predicateSlot.stateSlot
        : null;
    return String(
        stateSlot?.possessorPrefix
        || stateSlot?.possessor
        || stateSlot?.prefix
        || stateSlot?.possessivePrefix
        || predicateSlot?.possessorPrefix
        || predicateSlot?.possessor
        || ""
    );
}
...
function buildNuclearClausePersonalPronounFillerRecord({
    ...
    return {
        caseKey,
        caseLabelEs: caseKey === "nominative"
            ? "nominativo"
            : (caseKey === "objective" ? "objetivo" : "posesivo"),
        slot: String(slot || ""),
        role: String(role || ""),
...
function buildNuclearClauseLesson4PersonalPronounResolutionFrame({
    ...
    return {
        ...
        animacyHumannessResolution: {
            status: hasThirdPersonContext ? "context-required" : "participant-anchored",
            noteEs: hasThirdPersonContext
                ? "La animacidad/humanidad de la 3a persona no sale de la forma sola."
                : "1a/2a persona se ancla en participantes del habla.",
        },
...
function buildNuclearClauseLesson4Frame({
    ...
    const useFrame = buildNuclearClauseLesson4UseFrame({ usageRole });
    ...
    return {
        ...
        formulaStages: [
            ...
            },
            {
                ...
                role: normalizedFormulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.vnc
                    ? "predicado = valencia + base + tiempo"
                    : "predicado = estado + base",
            },
            activeFormula,
        ],
        ...
        diagnostics: [
            {
                ...
                messageEs: useFrame.diagnosticStatus === "classified"
                    ? `Uso de CN: ${useFrame.activeRoleLabelEs}.`
                    : "Uso de CN pendiente: oración simple, cláusula principal, dependiente o coordinada.",
            },
            {
...
function inferVerbalPredicatePositionStatus({
    ...
    const explicitStatus = normalizeNuclearClausePredicatePositionStatus(
        predicate?.valenceStructure
        || predicate?.valenceStatus
        || predicate?.predicatePositionStatus
        || object?.valenceStructure
        || object?.predicatePositionStatus
        || ""
    );
    if (explicitStatus !== NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.unknown) {
        return explicitStatus;
    }
    const hasSecondaryValence = Boolean(
        object2?.prefix || object2?.obj2 || object?.obj2
        || object3?.prefix || object3?.obj3 || object?.obj3
    );
...
function getVerbalPredicatePositionStatusSource({
    ...
    return explicitStatus !== NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.unknown
        ? "explicit"
        : "inferred-from-visible-fillers";
}
...
function inferNominalPredicatePositionStatus({
    ...
    const explicitStatus = normalizeNuclearClausePredicatePositionStatus(
        predicateSource.stateStructure
        || predicateSource.stateStatus
        || predicateSource.predicatePositionStatus
        || stateSlot?.stateStructure
        || stateSlot?.predicatePositionStatus
        || ""
    );
    if (explicitStatus !== NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.unknown) {
        return explicitStatus;
    ...
    if (
        stateSlot?.isVacant === false
        || stateSlot?.hasPossessor
        || stateSlot?.statePosition === "possessor"
        || String(predicateSource.state || predicateState || "").trim().toLowerCase() === "possessive"
    ) {
        return NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.monadic;
    }
    return NUCLEAR_CLAUSE_PREDICATE_POSITION_STATUS.vacant;
...
function buildVerbalNuclearClauseFormulaEchoFromSlots(formulaSlots = null) {
    ...
    const predicateDisplay = (
        normalizedPredicateDisplay.includes("(")
        && normalizedPredicateDisplay.includes(")")
    )
...
function buildNuclearClauseShellMetadata(options = {}) {
    ...
    const payload = clauseKind === NUCLEAR_CLAUSE_KIND.verbal
        ...
        : (clauseKind === NUCLEAR_CLAUSE_KIND.nominal
            ...
            : {
                ...
                clauseUseFrame: buildNuclearClauseLesson4UseFrame({ usageRole: options.usageRole || "" }),
                diagramTree: null,
    ...
    const shell = {
        ...
        displayLabel: formulaType === NUCLEAR_CLAUSE_FORMULA_TYPE.unknown
            ? getNuclearClauseDisplayLabel(formulaType)
            : getNuclearClauseDisplayLabel(formulaType),
        generationAllowed: false,
        ...payload,
