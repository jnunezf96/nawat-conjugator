...
const NNC_LESSON12_SUBJECT_POSITION_FRAME = Object.freeze({
    ...
    num1Num2Rule: Object.freeze({
        ...
        nawatPluralDyadBridge: Object.freeze([
            Object.freeze({
                classicalDyad: "m-eh",
...
const NNC_LESSON12_REMAINING_GAPS = Object.freeze([
    ...
    "Current ordinary NNC output follows Andrews slot logic and cannot import Classical num1-num2 surfaces directly; Nawat/Pipil realization still passes through the orthography bridge.",
...
const NNC_LESSON13_REMAINING_GAPS = Object.freeze([
    ...
    "Monadic possessive-state NNCs with ne, te, and nonhuman tla/ta are Andrews-licensed when the source stem is present; Classical letters still realize through the orthography bridge.",
    "Current specific possessor prefixes are orthography-bridge realizations of Andrews st1/st2 logic, while full allomorphy and traditional-spelling caveats remain diagnostic metadata.",
...
const NNC_LESSON14_REMAINING_GAPS = Object.freeze([
    ...
    "Affinity and distributive/varietal nounstem derivation, plural-subject alternatives, and class/subclass num1 choices remain diagnostic or source-gated.",
    "Constituent-analysis ambiguities around uh, ti, tli, o, m, and traditional spelling remain diagnostic metadata until vocabulary and Andrews source logic plus orthography-bridge support settle them.",
]);
...
const NNC_LESSON15_REMAINING_GAPS = Object.freeze([
    "Possessive plural assimilation, suppletive possessive stems, te/tla secondary stems, possessor reduplication, and ti-subclass reclassification remain diagnostic until Andrews source models plus the orthography bridge can license concrete forms.",
...
const NNC_LESSON16_REMAINING_GAPS = Object.freeze([
    "A complete Andrews pronominal-NNC route contract and orthography-bridge mapping are still needed before personal, interrogative, demonstrative, indefinite, or quantitive paradigms can generate.",
...
function getOrdinaryNncFormulaSchema() {
    return typeof getAndrewsFormulaSlotSchema === "function"
        ? getAndrewsFormulaSlotSchema("ordinary-nnc-shell")
        : null;
}

function getOrdinaryNncFormulaSlotDefinition(slotId = "") {
    return typeof getAndrewsFormulaSlotDefinition === "function"
        ? getAndrewsFormulaSlotDefinition("ordinary-nnc-shell", slotId)
        : null;
}

function renderOrdinaryNncFormulaTemplate() {
    return typeof renderAndrewsFormulaTemplate === "function"
        ? renderAndrewsFormulaTemplate("ordinary-nnc-shell")
...
function getOrdinaryNncOrganicPossessionFrameMismatch({
    ...
    if (
        ...
        || operationFrame.operationId !== "ordinary-nnc-organic-possession-yu-realization"
        || operationFrame.routeFamily !== "ordinary-nnc"
        || operationFrame.operationApplied !== "append-organic-possession-yu-from-source-frame"
...
function buildOrdinaryNncOrganicPossessionProfile({
    ...
    const mismatch = getOrdinaryNncOrganicPossessionFrameMismatch({ sourceFrame, operationFrame });
    if (mismatch) {
        return null;
    }
    return {
...
function buildOrdinaryNncOrganicPossessionFrame({
    ...
    const profile = buildOrdinaryNncOrganicPossessionProfile({ sourceFrame, operationFrame });
...
function buildOrdinaryNncAbsolutiveSingularOperationFrame(sourceFrame = null) {
    ...
    const targetSurface = sourceFrame.supported === true
        ? `${sourceFrame.sourceStem || ""}${sourceFrame.nounClassSurface || ""}`
        : "";
    const targetFrame = Object.freeze({
        kind: "ordinary-nnc-absolutive-singular-target-frame",
        surface: targetSurface,
        segmentFrames: Object.freeze([
            Object.freeze({
...
function buildOrdinaryNncAbsolutiveSingularSurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-absolutive-singular-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-absolutive-singular-realization"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ...
    if (
        ...
        || !rebuiltOperationFrame?.targetFrame
        || JSON.stringify(operationFrame.targetFrame) !== JSON.stringify(rebuiltOperationFrame.targetFrame)
    ) {
        return "";
    }
...
function buildOrdinaryNncOpenStemPossessiveSourceFrame({
    ...
    const sourceFrame = {
        ...
        sourceStemRole: normalizedSourceStemEvidence === "static-nnc-structured-possessive-stem"
            ? "ordinary-nnc-structured-possessive-predicate-stem"
            : "ordinary-nnc-structural-predicate-stem",
...
function buildOrdinaryNncOpenStemPossessiveOperationFrame(sourceFrame = null) {
    ...
    const targetStemSurface = sourceFrame.supported === true
        ? (sourceFrame.requiresReduplicatedStem === true
            ? buildOrdinaryNncReduplicatedSurfaceLegacy(sourceFrame.sourceStem || "")
            : String(sourceFrame.sourceStem || ""))
        : "";
    const targetPossessorSurface = sourceFrame.supported === true
        && String(sourceFrame.possessorPrefix || "") === "in"
        && /^[aeiou]/.test(targetStemSurface)
        ? "inh"
        : String(sourceFrame.possessorPrefix || "");
    const targetSurface = sourceFrame.supported === true
        ? `${targetPossessorSurface}${targetStemSurface}${sourceFrame.pluralPossessorSuffix || ""}`
        : "";
    const soundSpellingFrames = sourceFrame.possessorPrefix === "in" && targetPossessorSurface === "inh"
    ...
    const targetFrame = Object.freeze({
        ...
        segmentFrames: Object.freeze([
            ...
            Object.freeze({
                kind: "ordinary-nnc-result-text-segment-frame",
                role: sourceFrame.requiresReduplicatedStem === true
                    ? "predicate-possessive-reduplicated"
                    : "predicate",
                slot: "STEM",
...
function buildOrdinaryNncOpenStemPossessiveSurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-open-stem-possessive-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-open-stem-possessive-realization"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function getOrdinaryNncSurfaceSegmentRecordsFromCell(cell = null, {
    ...
    return (Array.isArray(cell.surfaceForms) ? cell.surfaceForms : [])
        .map((surface, index) => ({
...
function getOrdinaryNncFixtureStateSurfaceSegmentRecords(fixture = null, state = "", {
    ...
    if (state === ORDINARY_NNC_STATE.possessive) {
        const possessorPrefix = possessor?.prefix || "";
        const byNumber = stateData.numberFormsByPossessor?.[number]?.[possessorPrefix];
        const recordsByNumber = getOrdinaryNncSurfaceSegmentRecordsFromCell(byNumber, { pluralType });
        if (recordsByNumber.length) {
            return recordsByNumber;
        }
        const byPossessor = stateData.surfaceByPossessor?.[possessorPrefix];
        return getOrdinaryNncSurfaceSegmentRecordsFromCell(byPossessor, { pluralType });
    }
    const byNumber = stateData.numberForms?.[number];
    const recordsByNumber = getOrdinaryNncSurfaceSegmentRecordsFromCell(byNumber, { pluralType });
    if (recordsByNumber.length) {
    ...
    return getOrdinaryNncSurfaceSegmentRecordsFromCell(stateData, { pluralType });
}
...
function buildOrdinaryNncStructuredPossessiveSurfaceResults({
    ...
    const possessiveStem = structuredPossessiveStem
        || (normalizedNounClass === "zero" && fixture.openStem !== true ? String(fixture.stem || "").trim() : "");
    if (!possessiveStem) {
        return [];
    }
    const sourceFrame = buildOrdinaryNncOpenStemPossessiveSourceFrame({
        ...
        sourceStemEvidence: structuredPossessiveStem
            ? "static-nnc-structured-possessive-stem"
            : "ordinary-nnc-zero-class-formula-stem",
        possessor,
        animacy,
    });
    const operationFrame = buildOrdinaryNncOpenStemPossessiveOperationFrame(sourceFrame);
...
function buildOrdinaryNncDistributiveReduplicationSourceFrame({
    ...
    const syllables = normalizedSourceSurface && typeof splitVerbSyllables === "function"
        ? splitVerbSyllables(normalizedSourceSurface)
        : [];
    const syllableFrames = (Array.isArray(syllables) ? syllables : [])
        .map((syllable, index) => ({
    ...
    if (
        !priorOperationFrame
        || priorOperationFrame.kind !== "andrews-typed-operation-frame"
        || priorOperationFrame.operationId !== "ordinary-nnc-absolutive-singular-realization"
    ) {
    ...
    if (
        priorSourceFrame
        && priorOperationFrame
        && (!priorSurface || !priorTargetSurface || priorSurface !== priorTargetSurface)
    ) {
...
function buildOrdinaryNncDistributiveReduplicationOperationFrame(sourceFrame = null) {
    ...
    const targetSurface = sourceFrame.supported === true
        ? `${sourceFrame.reduplicant || ""}${sourceFrame.sourceSurface || ""}`
        : "";
    const targetFrame = Object.freeze({
...
function buildOrdinaryNncDistributiveReduplicatedSurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-distributive-reduplication-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-distributive-reduplication"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function buildOrdinaryNncAnimateDistributiveSourceFrame({
    ...
    const syllables = normalizedSourceStem && typeof splitVerbSyllables === "function"
        ? splitVerbSyllables(normalizedSourceStem)
        : [];
    const syllableFrames = (Array.isArray(syllables) ? syllables : [])
        .map((syllable, index) => ({
...
function buildOrdinaryNncAnimateDistributiveOperationFrame(sourceFrame = null) {
    ...
    const targetStemSurface = sourceFrame.supported === true
        ? `${sourceFrame.reduplicant || ""}${sourceFrame.sourceStem || ""}`
        : "";
    const targetSurface = sourceFrame.supported === true
        ? `${sourceFrame.subjectPrefix || ""}${targetStemSurface}${sourceFrame.numberConnectorSurface || ""}`
        : "";
    const targetFrame = Object.freeze({
...
function buildOrdinaryNncAnimateDistributiveSurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-animate-distributive-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-animate-distributive-realization"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function buildOrdinaryNncAnimateCountPluralOperationFrame(sourceFrame = null) {
    ...
    const targetSubjectSurface = sourceFrame.supported === true
        && String(sourceFrame.subjectPrefix || "").endsWith("n")
        && /^[aeiou]/.test(String(sourceFrame.sourceSurface || ""))
        ? `${sourceFrame.subjectPrefix || ""}h`
        : String(sourceFrame.subjectPrefix || "");
    const targetSurface = sourceFrame.supported === true
        ? `${targetSubjectSurface}${sourceFrame.sourceSurface || ""}${sourceFrame.numberConnectorSurface || ""}`
        : "";
    const soundSpellingFrames = targetSubjectSurface !== String(sourceFrame.subjectPrefix || "")
        ? Object.freeze([Object.freeze({
            ruleId: "n-open-transition-nh",
            source: "n",
            sourceSurface: "n",
...
function buildOrdinaryNncAnimateCountPluralSurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-animate-count-plural-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-animate-count-plural-realization"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function buildOrdinaryNncAnimateSubjectPrefixOperationFrame(sourceFrame = null) {
    ...
    const targetSurface = sourceFrame.supported === true
        ? `${targetSubjectSurface}${sourceSurface}`
        : "";
...
function buildOrdinaryNncAnimateSubjectPrefixedSurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-animate-subject-prefix-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-animate-subject-prefix-realization"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function buildOrdinaryNncNonanimatePossessiveDistributiveOperationFrame(sourceFrame = null) {
    ...
    const targetPossessorSurface = sourceFrame.supported === true
        ...
        : String(sourceFrame.possessorPrefix || "");
    const targetSurface = sourceFrame.supported === true
        ? `${targetPossessorSurface}${targetStemSurface}`
        : "";
    const soundSpellingFrames = sourceFrame.possessorPrefix === "in" && targetPossessorSurface === "inh"
        ? Object.freeze([Object.freeze({
            ruleId: "n-open-transition-nh",
...
function buildOrdinaryNncNonanimatePossessiveDistributiveSurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-nonanimate-possessive-distributive-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-nonanimate-possessive-distributive-realization"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function buildOrdinaryNncPossessiveDistributiveOperationFrame(sourceFrame = null) {
    ...
    const targetStemSurface = sourceFrame.supported === true
        ...
        : "";
    const targetSurface = sourceFrame.supported === true
        ? `${sourceFrame.possessorPrefix || ""}${targetStemSurface}${sourceFrame.pluralPossessorSuffix || ""}`
        : "";
    const targetFrame = Object.freeze({
...
function buildOrdinaryNncPossessiveDistributiveSurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-possessive-distributive-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-possessive-distributive-realization"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function buildOrdinaryNncAnimatePossessiveCountPluralSourceFrame({
    ...
    const pluralPossessorSuffix = normalizedAnimacy === "animate" && isOrdinaryNncPluralPossessor(resolvedPossessor) ? "wan" : "";
    const requiresReduplicatedStem = normalizedAnimacy === "animate" && possessorPrefix === "in";
    const syllables = requiresReduplicatedStem && normalizedSourceStem && typeof splitVerbSyllables === "function"
        ? splitVerbSyllables(normalizedSourceStem)
        : [];
    const syllableFrames = (Array.isArray(syllables) ? syllables : [])
        .map((syllable, index) => ({
...
function buildOrdinaryNncAnimatePossessiveCountPluralOperationFrame(sourceFrame = null) {
    ...
    const targetStemSurface = sourceFrame.supported === true
        ? `${sourceFrame.requiresReduplicatedStem ? sourceFrame.reduplicant || "" : ""}${sourceFrame.sourceStem || ""}`
        : "";
    const targetPossessorSurface = sourceFrame.supported === true
        && String(sourceFrame.possessorPrefix || "") === "in"
        && /^[aeiou]/.test(targetStemSurface)
        ? "inh"
        : String(sourceFrame.possessorPrefix || "");
    const possessiveBaseSurface = sourceFrame.supported === true
        ? `${targetPossessorSurface}${targetStemSurface}${sourceFrame.pluralPossessorSuffix || ""}`
        : "";
    ...
    const possessorSoundSpellingFrames = sourceFrame.possessorPrefix === "in" && targetPossessorSurface === "inh"
        ...
        : [];
    const targetSurface = sourceFrame.supported === true
        ? `${targetSubjectSurface}${possessiveBaseSurface}`
        : "";
    const targetFrame = Object.freeze({
        ...
        segmentFrames: Object.freeze([
            ...
            Object.freeze({
                kind: "ordinary-nnc-result-text-segment-frame",
                role: sourceFrame.requiresReduplicatedStem
                    ? "predicate-possessive-reduplicated"
                    : "predicate",
                slot: "STEM",
                formulaValue: sourceFrame.sourceStem || "",
...
function buildOrdinaryNncAnimatePossessiveCountPluralSurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-animate-possessive-count-plural-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-animate-possessive-count-plural-realization"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function buildOrdinaryNncAnimatePossessivePluralIdentitySourceFrame({
    ...
    if (
        !priorOperationFrame
        || priorOperationFrame.kind !== "andrews-typed-operation-frame"
        || priorOperationFrame.operationId !== "ordinary-nnc-open-stem-possessive-realization"
    ) {
    ...
    if (
        ...
        && String(priorSourceFrame.sourceStem || "") !== normalizedSourceStem
    ) {
    ...
    if (
        priorSourceFrame
        && possessorPrefix
        && String(priorSourceFrame.possessorPrefix || "") !== possessorPrefix
    ) {
    ...
    const sourceFrame = {
        ...
        priorOperationTargetFrameKind: priorOperationFrame?.targetFrame?.kind || "",
        priorOperationTargetSurface: priorTargetSurface,
        priorTargetSegmentFrames: Object.freeze((Array.isArray(priorOperationFrame?.targetFrame?.segmentFrames)
            ? priorOperationFrame.targetFrame.segmentFrames
            : []).map(Object.freeze)),
        supported: diagnostics.length === 0,
        diagnostics: Object.freeze(diagnostics),
        consumesRenderedInput: false,
...
function buildOrdinaryNncAnimatePossessivePluralIdentityOperationFrame(sourceFrame = null) {
    ...
    const targetSurface = sourceFrame.supported === true
        ? String(sourceFrame.priorOperationTargetSurface || "")
        : "";
    const priorSegmentFrames = Array.isArray(sourceFrame.priorTargetSegmentFrames)
        ? sourceFrame.priorTargetSegmentFrames
        : [];
    const targetFrame = Object.freeze({
...
function buildOrdinaryNncAnimatePossessivePluralIdentitySurfaceFromFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-animate-possessive-plural-identity-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-animate-possessive-plural-identity"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function buildOrdinaryNncDerivedPluralForms(singularForms = [], {
    ...
    if (state === ORDINARY_NNC_STATE.possessive && animacy === "animate") {
        const resolvedPossessor = resolveOrdinaryNncPossessor(possessor);
        const useTypedPossessiveDistributive = effectivePluralType === ORDINARY_NNC_PLURAL_TYPE.distributive
            && isOrdinaryNncPluralPossessor(resolvedPossessor)
            && String(resolvedPossessor?.prefix || "") !== "in"
            && String(sourceStem || "").trim();
        const pluralEntries = useTypedPossessiveDistributive
            ...
            : (() => {
                if (
                    effectivePluralType !== ORDINARY_NNC_PLURAL_TYPE.count
                    && String(resolvedPossessor?.prefix || "") !== "in"
                ) {
        ...
        const pluralResults = pluralEntries.map((entry) => {
            ...
            const subjectSourceFrame = entry.sourceFrame
                && entry.operationFrame?.supported === true
                && priorTargetSurface
                && priorTargetSurface === String(entry.surface || "")
                ? buildOrdinaryNncAnimateSubjectPrefixSourceFrame({
                    sourceStem,
        ...
        return {
            ...
            surfaceSegmentRecords: pluralResults
                .map((entry, index) => ({
                    kind: "ordinary-nnc-derived-plural-surface-segment-record",
                    index,
                    surface: entry.surface || "",
    ...
    const priorAbsolutiveSingularSourceFrame = state === ORDINARY_NNC_STATE.absolutive
        && normalizedSourceStem
        && normalizedNounClass
        ? buildOrdinaryNncAbsolutiveSingularSourceFrame({
            sourceStem: normalizedSourceStem,
            nounClass: normalizedNounClass,
            state: ORDINARY_NNC_STATE.absolutive,
            number: "singular",
        ...
        : null;
    const priorAbsolutiveSingularOperationFrame = buildOrdinaryNncAbsolutiveSingularOperationFrame(
        priorAbsolutiveSingularSourceFrame
    );
    ...
    const distributiveSourceEntries = state === ORDINARY_NNC_STATE.absolutive
        ? (priorAbsolutiveSingularSurface
            ? [{
            ...
            : [])
        : singularForms.map((form) => ({ sourceSurface: form }));
    const distributiveResults = distributiveSourceEntries
        .map((entry) => {
            if (state === ORDINARY_NNC_STATE.possessive) {
                const typedPossessiveDistributiveStem = normalizedSourcePossessiveStem
                    || (normalizedNounClass === "zero" ? normalizedSourceStem : "");
                if (animacy === "inanimate" && typedPossessiveDistributiveStem) {
                    const sourceFrame = buildOrdinaryNncNonanimatePossessiveDistributiveSourceFrame({
                        ...
                        sourceStemEvidence: normalizedSourcePossessiveStem
                            ? "static-nnc-structured-possessive-stem"
                            : "ordinary-nnc-formula-stem",
                        possessor,
                        state,
                        animacy,
                        pluralType: effectivePluralType,
    ...
    return {
        ...
        surfaceSegmentRecords: distributiveResults
            .map((entry, index) => ({
                kind: "ordinary-nnc-derived-plural-surface-segment-record",
                index,
                surface: entry.surface || "",
...
function buildOrdinaryNncNum1Num2Dyad({
    ...
    return {
        ...
        compactSurface: num1,
        source: normalizedNumber === "plural"
            ? "plural-stem-route-keeps-zero-subject-number-dyad"
            : "singular-common-class-connector",
    };
}
...
function getOrdinaryNncRequestFormulaSlots({
    ...
    const candidates = [
        formulaSlots,
        clauseFrame?.formulaSlots,
        nuclearClauseFrame?.formulaSlots,
        routeContract?.formulaSlots,
        routeContract?.clauseFrame?.formulaSlots,
        routeContract?.nuclearClauseFrame?.formulaSlots,
        routeContract?.morphBoundaryFrame?.formulaSlots,
    ];
...
function getOrdinaryNncRequestFormulaSlotStem(formulaSlots = null) {
    ...
    return normalizeOrdinaryNncText(
        predicate?.stem
...
function getOrdinaryNncRequestFormulaSlotNounClass(formulaSlots = null) {
    ...
    return normalizeOrdinaryNncNum1Num2Class(
        connector?.nounClass
        || connector?.nounStemClass
...
function buildOrdinaryNncNum1Num2({
    ...
    const dyad = buildOrdinaryNncNum1Num2Dyad({ nounClass: connectorClass, number, pluralType });
    ...
    const blockedInterpretations = Array.isArray(slotDefinition.blockedInterpretations)
        ? Array.from(slotDefinition.blockedInterpretations)
...
function buildOrdinaryNncFormulaEchoFromSlots(formulaSlots = null, {
    ...
    const expandedConnector = String(numberConnector.displayDyad || "").trim()
        || `${formatOrdinaryNncFormulaDyadSegment(numberConnector.num1)}-${formatOrdinaryNncFormulaDyadSegment(numberConnector.num2)}`;
    if (typeof renderAndrewsFormulaEchoFromSchema === "function") {
        return renderAndrewsFormulaEchoFromSchema("ordinary-nnc-shell", {
            pers1Pers2: {
            ...
            num1Num2: {
                ...(expanded
                    ? { displayDyad: expandedConnector, displayConnector: compactConnector }
...
function buildOrdinaryNncExpandedFormulaEchoFromSlots(formulaSlots = null) {
    return buildOrdinaryNncFormulaEchoFromSlots(formulaSlots, { expanded: true });
}

function buildOrdinaryNncCompactFormulaEchoFromSlots(formulaSlots = null) {
    return buildOrdinaryNncFormulaEchoFromSlots(formulaSlots, { expanded: false });
}
...
function buildOrdinaryNncFormulaSurfacePairs({
    ...
    const targetFormulaEcho = String(
        clauseFrame?.fullFormulaEcho
        || clauseFrame?.formulaEcho
        || ""
    ).trim();
    if (!targetFormulaEcho) {
        return Object.freeze([]);
    }
    const normalizedSourceStem = String(
        sourceStem
        || clauseFrame?.predicate?.stem
        || clauseFrame?.formulaSlots?.predicateStem?.stem
        || "STEM"
    ).trim() || "STEM";
    const sourceLabel = `NNC(${normalizedSourceStem})`;
    const formulaRecord = typeof buildGrammarFormulaRecord === "function"
    ...
    const baseSegmentFrames = [
        ...
        },
        {
            ...
            surface: clauseFrame?.formulaSlots?.predicateStem?.surface
                || clauseFrame?.formulaSlots?.predicateStem?.stem
                || clauseFrame?.predicate?.stem
                || "",
            orthographyBridge: "Nawat/Pipil orthography bridge",
        },
        {
            slot: "num1-num2",
            role: "subject-number-connector",
            formulaValue: clauseFrame?.formulaSlots?.num1Num2?.displayDyad
                || clauseFrame?.formulaSlots?.num1Num2?.connector
                || "",
            surface: clauseFrame?.formulaSlots?.num1Num2?.surface
                || clauseFrame?.formulaSlots?.num1Num2?.compactSurface
                || "",
            orthographyBridge: "Nawat/Pipil orthography bridge",
        },
    ];
    const seen = new Set();
    return Object.freeze((Array.isArray(surfaceForms) ? surfaceForms : [])
        ...
        .map((surface, surfaceIndex) => {
            const surfaceSegmentRecord = (Array.isArray(surfaceSegmentRecords) ? surfaceSegmentRecords : [])
                .find((record) => String(record?.surface || "").trim() === surface) || null;
            const recordSegmentFrames = Array.isArray(surfaceSegmentRecord?.segmentFrames)
                ? surfaceSegmentRecord.segmentFrames
                : [];
            const candidateSegmentFrames = recordSegmentFrames.length
                ? recordSegmentFrames
                : baseSegmentFrames;
            const candidateSegmentDerivedSurface = candidateSegmentFrames
                .map((segment) => String(segment?.surface || "").trim())
                .join("");
            const formulaRealizationRecord = typeof buildGrammarFormulaRealizationRecord === "function"
...
function buildOrdinaryNncFormulaSurfacePairFields({
    ...
    return {
        ...
        sourceToTargetFormulaEcho: formulaSurfacePairs
            ...
            .join(" | "),
        andrewsToConjugatorFormulaEcho: formulaSurfacePairs
            .map((entry) => entry.andrewsToConjugatorFormulaEcho)
            .join(" | "),
    };
}

function normalizeOrdinaryNncResultTextRecords(formulaSurfacePairs = []) {
    return (Array.isArray(formulaSurfacePairs) ? formulaSurfacePairs : [])
        .map((pair, index) => {
            const realizationRecord = pair?.formulaRealizationRecord || null;
            if (
                !realizationRecord
                || realizationRecord.kind !== "grammar-formula-realization-record"
                || !Array.isArray(realizationRecord.segmentFrames)
            ) {
                return null;
            }
            const segmentFrames = realizationRecord.segmentFrames
                ...
                .filter((segment) => segment.surface);
            const segmentSurface = segmentFrames
...
function getOrdinaryNncResultTextSourceSignature(sourceFrame = null) {
    ...
    return JSON.stringify({
        records: (Array.isArray(sourceFrame.resultRecordFrames)
            ? sourceFrame.resultRecordFrames
            : []
        ).map((record) => ({
...
function buildOrdinaryNncResultTextSourceFrame({
    ...
    const surfaceForms = resultRecordFrames
        .map((record) => record.surface)
        .filter((surface, index, list) => surface && list.indexOf(surface) === index);
    const diagnostics = [];
...
function buildOrdinaryNncResultTextOperationFrame(sourceFrame = null) {
    ...
    const targetSurfaceForms = (Array.isArray(sourceFrame.surfaceForms) ? sourceFrame.surfaceForms : [])
        .map((surface) => String(surface || "").trim())
        .filter(Boolean);
    const targetFrame = Object.freeze({
...
function validateOrdinaryNncResultTextOperationFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "ordinary-nnc-result-text-source-frame"
        || String(sourceFrame.sourceSignature || "") !== String(rebuiltSourceFrame.sourceSignature || "")
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "ordinary-nnc-result-text-render"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function resolveOrdinaryNncResultTextFromFrames({
    ...
    if (sourceFrame?.supported === true) {
        return {
            ...
            diagnostic: typedResultText ? null : buildOrdinaryNncDiagnostic(
                ORDINARY_NNC_DIAGNOSTIC_IDS.resultTextMissingTypedOperationFrame,
                "Ordinary NNC result text requires a matching typed source and operation frame."
            ),
    ...
    return {
        supported: true,
        resultText: (Array.isArray(fallbackSurfaceForms) ? fallbackSurfaceForms : [])
            .map((surface) => String(surface || "").trim())
            .filter(Boolean)
            .join(" / "),
        diagnostic: null,
...
function generateOrdinaryNncParadigm({
    ...
    const legacyFormulaInput = parseOrdinaryNncPredicateFormulaInput(stem, { diagnosticOnly: true });
    ...
    const legacyConflictsWithSlots = Boolean(
        hasLegacyFormulaString
        && legacyFormulaInput
        && requestFormulaSlots
        && (
            (structuralStem && legacyFormulaInput.stem && structuralStem !== legacyFormulaInput.stem)
            || (structuralNounClass && legacyFormulaInput.nounClass && structuralNounClass !== legacyFormulaInput.nounClass)
        )
    );
    if ((hasLegacyFormulaString && !requestFormulaSlots) || legacyConflictsWithSlots) {
        const diagnosticId = legacyConflictsWithSlots
            ? ORDINARY_NNC_DIAGNOSTIC_IDS.legacyFormulaStringConflictsWithSlots
            : ORDINARY_NNC_DIAGNOSTIC_IDS.legacyFormulaStringBlocked;
        const diagnosticMessage = legacyConflictsWithSlots
            ? "Ordinary NNC generation rejected a formula-looking string because it conflicts with the supplied Andrews formula slots."
            : "Ordinary NNC generation no longer lets formula-looking strings authorize or infer grammar; pass Andrews formula slots or a route contract instead.";
    ...
    if (normalizedPossessionKind === ORDINARY_NNC_POSSESSION_KIND.organic) {
        ...
        const organicSourceFrame = organicPossessionSourceFrame
            || buildOrdinaryNncOrganicPossessionSourceFrame({
        ...
        if (!organicSourceFrame) {
            return {
                ...buildOrdinaryNncUnsupportedResult({
                    ...
                    openStem: true,
                    diagnostic: buildOrdinaryNncDiagnostic(
                        ORDINARY_NNC_DIAGNOSTIC_IDS.organicPossessionMissingSourceFrame,
                        "Organic possession NNC generation requires an Andrews formula-slot source frame; stem strings cannot authorize the target."
                    ),
                }),
                possessionKind: normalizedPossessionKind,
                source: {
        ...
        const organicOperationFrame = organicPossessionOperationFrame
            || buildOrdinaryNncOrganicPossessionOperationFrame(organicSourceFrame);
        ...
        if (organicFrameMismatch) {
            const diagnosticId = organicFrameMismatch.includes("operation")
                ? ORDINARY_NNC_DIAGNOSTIC_IDS.organicPossessionMissingOperationFrame
                : ORDINARY_NNC_DIAGNOSTIC_IDS.organicPossessionContradictoryFrame;
            return {
                ...buildOrdinaryNncUnsupportedResult({
    ...
    const structuredAbsolutiveSurfaceResults = (
        isOrdinaryNncThirdSingularSubject(resolvedSubject)
        || (normalizedState === ORDINARY_NNC_STATE.absolutive && normalizedNumber === "singular" && fixtureAnimacy === "animate")
    )
        ? buildOrdinaryNncStructuredAbsolutiveSingularSurfaceResults({
            fixture,
            state: normalizedState,
            number: normalizedNumber,
            nounClass: fixtureClass,
        ...
        : [];
    const structuredSurfaceResults = structuredPossessiveSurfaceResults.length
        ? structuredPossessiveSurfaceResults
        : structuredAbsolutiveSurfaceResults;
    const surfaceForms = structuredSurfaceResults.length
        ? []
    ...
    const sourceSoundSpellingFrames = structuredSurfaceResults.length
        ? collectOrdinaryNncSurfaceSoundSpellingFrames(...structuredSurfaceResults)
        : getOrdinaryNncFixtureStateSoundSpellingFrames(fixture, normalizedState, {
            number: normalizedNumber,
            possessor: resolvedPossessor,
            pluralType: normalizedPluralType,
        });
    const sourceSurfaceSegmentRecords = structuredSurfaceResults.length
        ? structuredSurfaceResults.map((entry, index) => ({
            kind: structuredPossessiveSurfaceResults.length
                ? "ordinary-nnc-structured-possessive-surface-segment-record"
                : "ordinary-nnc-structured-absolutive-singular-surface-segment-record",
            index,
            surface: entry.surface || "",
            sourceFrame: entry.sourceFrame || null,
            operationFrame: entry.operationFrame || null,
        ...
        : getOrdinaryNncFixtureStateSurfaceSegmentRecords(fixture, normalizedState, {
            number: normalizedNumber,
            possessor: resolvedPossessor,
            pluralType: normalizedPluralType,
        });
    const singularSourceCell = normalizedNumber === "plural"
        ? getOrdinaryNncFixtureStateCell(fixture, normalizedState, {
            number: "singular",
            possessor: resolvedPossessor,
        })
        : null;
    const sourcePossessiveStem = normalizedState === ORDINARY_NNC_STATE.possessive
        ? getOrdinaryNncPossessiveStemFromCell(singularSourceCell)
        : "";
    const singularForms = normalizedNumber === "plural"
        ...
        : [];
    const derivedPlural = normalizedNumber === "plural" && !surfaceForms.length && !structuredSurfaceResults.length
        ...
        : null;
    const subjectAppliedSurfaceResults = structuredSurfaceResults.length
        && fixtureAnimacy === "animate"
        && !isOrdinaryNncThirdSingularSubject(resolvedSubject)
        ? structuredSurfaceResults.map((entry) => {
            const priorTargetSurface = String(entry.operationFrame?.targetFrame?.surface || "");
            const sourceFrame = entry.sourceFrame
                && entry.operationFrame?.supported === true
                && priorTargetSurface
                && priorTargetSurface === String(entry.surface || "")
                ? buildOrdinaryNncAnimateSubjectPrefixSourceFrame({
                    sourceStem: fixture.stem || normalizedStem,
                    sourceSurface: priorTargetSurface,
                    sourceSurfaceRole: structuredPossessiveSurfaceResults.length
                        ? "ordinary-nnc-prior-typed-possessive-singular-source-form"
                        : "ordinary-nnc-prior-typed-absolutive-singular-source-form",
                    priorSourceFrame: entry.sourceFrame,
                    priorOperationFrame: entry.operationFrame,
                    subject: resolvedSubject,
                    state: normalizedState,
        ...
        : (structuredSurfaceResults.length
            ? structuredSurfaceResults
            : (surfaceForms.length
            ...
            : []));
    const subjectAppliedSurfaceSegmentRecords = subjectAppliedSurfaceResults
        .map((entry, index) => ({
        ...
        .filter((entry) => entry.surface && Array.isArray(entry.segmentFrames) && entry.segmentFrames.length);
    const resolvedSurfaceForms = structuredSurfaceResults.length
...
function getOrdinaryNncFormulaWorkbenchSlotValue(slot = null, formulaSlots = {}) {
    ...
    if (typeof getAndrewsFormulaEchoSlotValue === "function") {
        return getAndrewsFormulaEchoSlotValue(slot, formulaSlots);
    }
...
function buildOrdinaryNncFormulaWorkbenchSlotRecords(schema = null, formulaSlots = {}) {
    ...
    return slots.map((slot) => {
        ...
        const compactValue = slot.key === "num1Num2" || slot.id === "num1-num2"
            ? String(sourceSlot.compactDisplay || sourceSlot.connector || sourceSlot.surface || "Ø")
            : value;
        ...
        return {
            ...
            renderedValue: missing
                ? ""
                : `${slot.leading || ""}${slot.wrapper === "parentheses" ? `(${value})` : value}${slot.trailing || ""}`,
            compactRenderedValue: missing
                ? ""
                : `${slot.leading || ""}${slot.wrapper === "parentheses" ? `(${compactValue})` : compactValue}${slot.trailing || ""}`,
...
function normalizeOrdinaryNncFormulaWorkbenchInput(value = "") {
    ...
    const normalizedStem = analogueInput?.stem
        || (hasLegacyFormulaString ? "" : normalizeOrdinaryNncText(rawInput).replace(/[()#]/g, ""));
    return {
        rawInput,
        stem: normalizedStem,
...
function buildOrdinaryNncFormulaWorkbenchExample({
    ...
    return {
        ...
        fullFormulaEcho: result?.clauseFrame?.fullFormulaEcho
            || (formulaSlots ? buildOrdinaryNncExpandedFormulaEchoFromSlots(formulaSlots) : ""),
        compactFormulaEcho: result?.clauseFrame?.compactFormulaEcho
            || result?.clauseFrame?.formulaEcho
            || (formulaSlots ? buildOrdinaryNncCompactFormulaEchoFromSlots(formulaSlots) : ""),
...
function buildOrdinaryNncFormulaWorkbenchExamples() {
    return [
        buildOrdinaryNncFormulaWorkbenchExample({
            id: "zero-common-kal",
            label: "zero/common",
            request: { stem: "kal", state: "absolutive", number: "singular" },
        }),
        buildOrdinaryNncFormulaWorkbenchExample({
            id: "t-class-siwa",
            label: "t class",
            request: { stem: "siwa", state: "absolutive", nounClass: "t", number: "singular" },
        }),
        buildOrdinaryNncFormulaWorkbenchExample({
            id: "ti-class-xilun",
            label: "ti class",
            request: { stem: "xilun", state: "absolutive", nounClass: "ti", number: "singular" },
        }),
        buildOrdinaryNncFormulaWorkbenchExample({
            id: "in-class-tekpan",
            label: "in class",
            request: { stem: "tekpan", state: "absolutive", nounClass: "in", number: "singular" },
        }),
        buildOrdinaryNncFormulaWorkbenchExample({
            id: "animate-count-mistun",
            label: "animate plural count",
            request: { stem: "mistun", state: "absolutive", number: "plural", pluralType: "count" },
        }),
    ];
}
...
function buildOrdinaryNncFormulaWorkbenchSlice({
    ...
    const sourceRequirementCheck = typeof evaluateAndrewsFormulaSourceRequirements === "function"
        ? evaluateAndrewsFormulaSourceRequirements("ordinary-nnc-shell", {
        ...
        : {
            ...
            missingRequirements: parsedInput.stem ? [] : [{ id: "ordinary-nnc-predicate-stem" }],
            satisfiedRequirements: parsedInput.stem ? [{ id: "ordinary-nnc-predicate-stem" }] : [],
        };
    const formulaAuthority = typeof evaluateAndrewsFormulaGenerationAuthority === "function"
        ? evaluateAndrewsFormulaGenerationAuthority("ordinary-nnc-shell", {
            inputValue: parsedInput.stem,
            slotValues: baseFormulaSlots,
            sourceValues: {
                predicateStem: parsedInput.stem,
        ...
        : {
            ...
            gate: parsedInput.stem && sourceRequirementCheck.ok
                ? "andrews-formula-authorized-generation"
                : "andrews-formula-generation-blocked",
    ...
    const compactFormulaEcho = result?.clauseFrame?.compactFormulaEcho
        || result?.clauseFrame?.formulaEcho
        || buildOrdinaryNncCompactFormulaEchoFromSlots(formulaSlots)
        || "";
    const fullFormulaEcho = result?.clauseFrame?.fullFormulaEcho
        || buildOrdinaryNncExpandedFormulaEchoFromSlots(formulaSlots)
        || "";
    const slotBlockDiagnostics = ["tense", "stem-suffix"]
        .map((interpretation) => (
            typeof diagnoseAndrewsFormulaSlotInterpretation === "function"
                ? diagnoseAndrewsFormulaSlotInterpretation("ordinary-nnc-shell", "num1-num2", interpretation).diagnostic
                : null
        ))
        .filter(Boolean);
    ...
    const generationStatus = !parsedInput.stem
        ? "blocked"
        : (generationAllowed ? "generated" : "unsupported");
    return {
        ...
        evidenceRefs: [
            ...
            ...(Array.isArray(result?.source?.sourceRefs) ? result.source.sourceRefs : []),
        ].filter(Boolean),
        generation: {
            ...
            formulaAuthorityBlockedReasons: Array.isArray(formulaAuthority.blockedReasons)
                ? Array.from(formulaAuthority.blockedReasons)
                : [],
        ...
        diagnostics: [
            ...(Array.isArray(sourceRequirementCheck.diagnostics) ? sourceRequirementCheck.diagnostics : []),
            ...resultDiagnostics,
            ...slotBlockDiagnostics,
        ],
    };
}

const POSSESSIVE_STATE_NNC_SPECIFIC_POSSESSOR_SPECS = Object.freeze({
    nu: Object.freeze({ id: "1sg", personSubKey: "1sg", andrewsStructural: "n-o", nawatFormula: "n-u", nawatRealization: "nu", gloss: "my", source: "Andrews 13.6 + current Nawat possessor prefix" }),
    tu: Object.freeze({ id: "1pl", personSubKey: "1pl", andrewsStructural: "t-o", nawatFormula: "t-u", nawatRealization: "tu", gloss: "our", source: "Andrews 13.6 + current Nawat possessor prefix" }),
    mu: Object.freeze({ id: "2sg", personSubKey: "2sg", andrewsStructural: "m-o", nawatFormula: "m-u", nawatRealization: "mu", gloss: "your (sg)", source: "Andrews 13.6 + current Nawat possessor prefix" }),
    anmu: Object.freeze({ id: "2pl", personSubKey: "2pl", andrewsStructural: "am-o", nawatFormula: "an-mu", nawatRealization: "anmu", gloss: "your (pl)", source: "Andrews 13.6 + current Nawat possessor prefix" }),
    i: Object.freeze({ id: "3sg", personSubKey: "3sg", andrewsStructural: "i-0", nawatFormula: "i-Ø", nawatRealization: "i", gloss: "his/her/its", source: "Andrews 13.6 + current Nawat possessor prefix" }),
    in: Object.freeze({ id: "3pl", personSubKey: "3pl", andrewsStructural: "i-n", nawatFormula: "i-n", nawatRealization: "in", gloss: "their", source: "Andrews 13.6 + current Nawat possessor prefix" }),
});

const POSSESSIVE_STATE_NNC_MONADIC_POSSESSOR_SPECS = Object.freeze({
    te: Object.freeze({ id: "human-nonspecific", andrewsStructural: "te", nawatRealization: "te", gloss: "someone's", source: "Andrews 13.4", status: "andrews-logic-generated" }),
    ta: Object.freeze({ id: "nonhuman-nonspecific", andrewsStructural: "tla", nawatRealization: "ta", gloss: "something's", source: "Andrews 13.4; Classical tla maps structurally to Nawat ta", status: "andrews-logic-generated" }),
    ne: Object.freeze({ id: "reciprocative", andrewsStructural: "ne", nawatRealization: "ne", gloss: "one another's", source: "Andrews 13.4", status: "andrews-logic-generated" }),
});

function getPossessiveStateNncFormulaSchema() {
    return typeof getAndrewsFormulaSlotSchema === "function"
        ? getAndrewsFormulaSlotSchema("possessive-state-nnc")
        : null;
}

function getPossessiveStateNncFormulaSlotDefinition(slotId = "") {
    return typeof getAndrewsFormulaSlotDefinition === "function"
        ? getAndrewsFormulaSlotDefinition("possessive-state-nnc", slotId)
        : null;
}
...
function getPossessiveStateNncSpecificPossessorSpec(prefix = "nu") {
    ...
    return POSSESSIVE_STATE_NNC_SPECIFIC_POSSESSOR_SPECS[normalized]
        || POSSESSIVE_STATE_NNC_SPECIFIC_POSSESSOR_SPECS.nu;
}

function getPossessiveStateNncMonadicPossessorSpec(key = "te") {
    const normalized = String(key || "te").trim();
    return POSSESSIVE_STATE_NNC_MONADIC_POSSESSOR_SPECS[normalized]
        || POSSESSIVE_STATE_NNC_MONADIC_POSSESSOR_SPECS.te;
}
...
function buildPossessiveStateNncStateSlot({
    ...
    const spec = isMonadic
        ? getPossessiveStateNncMonadicPossessorSpec(possessor)
        : getPossessiveStateNncSpecificPossessorSpec(possessor);
    ...
    return {
        ...
        blockedInterpretations: Array.isArray(slotDefinition.blockedInterpretations)
            ? Array.from(slotDefinition.blockedInterpretations)
            : ["subject-connector", "tense"],
        notSubjectConnector: true,
...
function buildPossessiveStateNncFormulaSlots({
    ...
    return {
        ...
        num1Num2: {
            ...
            referenceNumber: connector.referenceNumber,
            pluralType: connector.pluralType,
            blockedInterpretations: Array.isArray(connectorSlotDefinition.blockedInterpretations)
                ? Array.from(connectorSlotDefinition.blockedInterpretations)
                : ["tense", "stem-suffix", "nounstem", "predicate-state"],
...
function renderPossessiveStateNncFormulaTemplate(formulaSlots = null) {
    if (typeof renderAndrewsFormulaTemplate !== "function") {
        return "";
    }
    return renderAndrewsFormulaTemplate("possessive-state-nnc", {
        slotTokens: {
...
function buildPossessiveStateNncFormulaEchoFromSlots(formulaSlots = null, {
    ...
    const displayState = mode === "compact"
        ? (stateSlot.compactDisplay || stateSlot.surface || stateSlot.nawatDisplay || stateSlot.nawatRealization || stateSlot.display || "")
        : (mode === "nawat"
            ? (stateSlot.nawatDisplay || stateSlot.nawatRealization || stateSlot.compactDisplay || stateSlot.display || "")
            : (stateSlot.structuralDisplay || stateSlot.andrewsStructural || stateSlot.display || ""));
    const numberConnector = formulaSlots.num1Num2 || {};
    ...
    if (typeof renderAndrewsFormulaEchoFromSchema === "function") {
        return renderAndrewsFormulaEchoFromSchema("possessive-state-nnc", echoSlots);
    }
    const subject = formulaSlots.pers1Pers2 || {};
    const prefix = String(subject.displayPrefix || subject.prefix || "Ø") || "Ø";
    const suffix = String(subject.displaySuffix || subject.suffix || "Ø") || "Ø";
...
function buildPossessiveStateNncFormulaWorkbenchSlotRecords(schema = null, formulaSlots = {}) {
    ...
    return slots.map((slot) => {
        const sourceSlot = formulaSlots?.[slot.key] || formulaSlots?.[slot.id] || formulaSlots?.[slot.path] || {};
        const structuralValue = slot.key === "possessiveState"
            ? String(sourceSlot.structuralDisplay || sourceSlot.andrewsStructural || sourceSlot.display || "")
            : getOrdinaryNncFormulaWorkbenchSlotValue(slot, formulaSlots);
        const nawatValue = slot.key === "possessiveState"
            ? String(sourceSlot.nawatDisplay || sourceSlot.nawatRealization || sourceSlot.compactDisplay || structuralValue)
            : (slot.key === "num1Num2" || slot.id === "num1-num2"
                ? String(sourceSlot.displayDyad || structuralValue || "Ø")
                : structuralValue);
        const compactValue = slot.key === "num1Num2" || slot.id === "num1-num2"
            ? String(sourceSlot.compactDisplay || sourceSlot.connector || sourceSlot.surface || "Ø")
            : (slot.key === "possessiveState"
                ? String(sourceSlot.compactDisplay || nawatValue || structuralValue)
                : structuralValue);
        const missing = slot.requiredForEcho === true && !String(structuralValue || "").trim();
        const renderedValue = missing
            ? ""
            : `${slot.leading || ""}${slot.wrapper === "parentheses" ? `(${structuralValue})` : structuralValue}${slot.trailing || ""}`;
        return {
            id: slot.id || "",
            key: slot.key || "",
            label: slot.label || "",
            ...
            layer: slot.layer || "",
            position: slot.position || "",
            token: slot.key === "possessiveState"
                ? (sourceSlot.stateSlot || slot.token || slot.id || "")
                : (typeof renderAndrewsFormulaSlotTemplate === "function"
                    ? renderAndrewsFormulaSlotTemplate(slot)
                    : (slot.wrapper === "parentheses" ? `(${slot.token || ""})` : (slot.token || ""))),
            ...
            modelFields: [
                ...
                ...(slot.key === "possessiveState" ? [
                    { label: "Andrews", value: structuralValue },
                    { label: "Nawat", value: nawatValue },
                    { label: "position", value: sourceSlot.statePosition || "" },
                ] : []),
                ...(slot.key === "num1Num2" ? [
                    { label: "dyad", value: structuralValue },
                    { label: "not", value: "tense/state" },
                ] : []),
...
function getPossessiveStateNncMonadicFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "possessive-state-nnc-monadic-realization"
        || operationFrame.routeFamily !== "possessive-state-nnc"
        || operationFrame.operationApplied !== "realize-monadic-possessive-state-from-formula-frame"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
...
function buildPossessiveStateNncFormulaWorkbenchExample({
    ...
    const formulaSlots = sourceFormulaSlots && typeof sourceFormulaSlots === "object"
        ? sourceFormulaSlots
        : (normalizedKind === "specific" ? buildPossessiveStateNncFormulaSlots({
        stem: normalizedStem,
        number,
        nounClass,
    ...
    const structuralFormulaEcho = buildPossessiveStateNncFormulaEchoFromSlots(formulaSlots, { mode: "structural", expanded: true });
    const nawatFormulaEcho = buildPossessiveStateNncFormulaEchoFromSlots(formulaSlots, { mode: "nawat", expanded: true });
    const compactFormulaEcho = buildPossessiveStateNncFormulaEchoFromSlots(formulaSlots, { mode: "compact", expanded: false });
    const result = normalizedKind === "specific"
        ? generateOrdinaryNncParadigm({
            stem: normalizedStem,
            state: ORDINARY_NNC_STATE.possessive,
            possessor,
        ...
        : null;
    const resolvedSourceFrame = normalizedKind === "monadic"
        ? (sourceFrame || buildPossessiveStateNncMonadicSourceFrame({ formulaSlots }))
        : null;
    const resolvedOperationFrame = normalizedKind === "monadic"
        ? (operationFrame || buildPossessiveStateNncMonadicOperationFrame(resolvedSourceFrame))
        : null;
    const monadicMismatch = normalizedKind === "monadic"
        ? getPossessiveStateNncMonadicFrameMismatch({
            sourceFrame: resolvedSourceFrame,
            operationFrame: resolvedOperationFrame,
        })
        : "";
    const monadicSurface = normalizedKind === "monadic" && !monadicMismatch
        ? String(resolvedOperationFrame?.targetFrame?.surface || "")
        : "";
    const supported = normalizedKind === "specific"
        ? result?.supported === true
        : Boolean(monadicSurface);
    const surface = normalizedKind === "specific"
        ? (supported ? (result?.result || "") : "")
        : monadicSurface;
    const surfaceForms = normalizedKind === "specific"
        ? (supported && Array.isArray(result?.surfaceForms) ? Array.from(result.surfaceForms) : [])
        : (monadicSurface ? [monadicSurface] : []);
    const monadicDiagnostic = (() => {
        ...
        if (monadicMismatch.includes("operation")) {
            return buildOrdinaryNncDiagnostic(
                ORDINARY_NNC_DIAGNOSTIC_IDS.possessiveStateMonadicMissingOperationFrame,
                monadicMismatch
            );
        }
        if (monadicMismatch.includes("contradictory")) {
            return buildOrdinaryNncDiagnostic(
                ORDINARY_NNC_DIAGNOSTIC_IDS.possessiveStateMonadicContradictoryFrame,
                monadicMismatch
            );
        }
        return buildOrdinaryNncDiagnostic(
            ORDINARY_NNC_DIAGNOSTIC_IDS.possessiveStateMonadicMissingSourceFrame,
            monadicMismatch
        );
    })();
    return {
        ...
        stem: normalizedKind === "monadic"
            ? String(formulaSlots?.predicateStem?.stem || normalizedStem)
            : normalizedStem,
        state: ORDINARY_NNC_STATE.possessive,
        ...
        supported,
        status: supported
            ? (normalizedKind === "monadic" ? "andrews-logic-generated" : "generated")
            : "blocked",
        formulaSlots,
        slotModel: {
...
function buildPossessiveStateNncFormulaWorkbenchSlice({
    ...
    const sourceRequirementCheck = typeof evaluateAndrewsFormulaSourceRequirements === "function"
        ? evaluateAndrewsFormulaSourceRequirements("possessive-state-nnc", {
        ...
        : {
            ...
            generationContract: schema?.generationContract || null,
            sourceRequirements: schema?.sourceRequirements || [],
            missingRequirements: normalizedStem ? [] : [{ id: "possessive-nnc-predicate-stem" }],
            satisfiedRequirements: normalizedStem ? [{ id: "possessive-nnc-predicate-stem" }] : [],
        };
    const formulaAuthority = typeof evaluateAndrewsFormulaGenerationAuthority === "function"
        ? evaluateAndrewsFormulaGenerationAuthority("possessive-state-nnc", {
            inputValue: normalizedStem,
            slotValues: formulaSlots,
            sourceValues: {
                predicateStem: normalizedStem,
        ...
        : {
            ...
            gate: normalizedStem && sourceRequirementCheck.ok
                ? "andrews-formula-authorized-generation"
                : "andrews-formula-generation-blocked",
    ...
    const result = normalizedStem
        ...
        : null;
    const resultSlots = result?.supported && result?.clauseFrame?.formulaSlots
        ? buildPossessiveStateNncFormulaSlots({
        ...
        : formulaSlots;
    const structuralFormulaEcho = buildPossessiveStateNncFormulaEchoFromSlots(resultSlots, { mode: "structural", expanded: true });
    const nawatFormulaEcho = buildPossessiveStateNncFormulaEchoFromSlots(resultSlots, { mode: "nawat", expanded: true });
    const compactFormulaEcho = buildPossessiveStateNncFormulaEchoFromSlots(resultSlots, { mode: "compact", expanded: false });
    const resultDiagnostics = Array.isArray(result?.diagnostics) ? result.diagnostics : [];
    const slotBlockDiagnostics = ["subject-connector", "tense"]
        .map((interpretation) => (
            typeof diagnoseAndrewsFormulaSlotInterpretation === "function"
                ? diagnoseAndrewsFormulaSlotInterpretation("possessive-state-nnc", "st1-st2", interpretation).diagnostic
                : null
        ))
        .filter(Boolean);
    const numBlockDiagnostics = ["predicate-state", "tense"]
        .map((interpretation) => (
            typeof diagnoseAndrewsFormulaSlotInterpretation === "function"
                ? diagnoseAndrewsFormulaSlotInterpretation("possessive-state-nnc", "num1-num2", interpretation).diagnostic
                : null
        ))
        .filter(Boolean);
    const generationAllowed = Boolean(formulaAuthority.allowed && result?.supported === true);
    const generationStatus = !normalizedStem
        ? "blocked"
        : (generationAllowed ? "generated" : "unsupported");
    return {
        ...
        formulaFamilies: [
            {
                ...
                formula: typeof renderAndrewsFormulaTemplate === "function"
                    ? renderAndrewsFormulaTemplate("possessive-state-nnc", { slotTokens: { possessiveState: "st" } })
            ...
            },
            {
                ...
                label: "dyadic",
                formula: typeof renderAndrewsFormulaTemplate === "function"
                    ? renderAndrewsFormulaTemplate("possessive-state-nnc", { slotTokens: { possessiveState: "st1-st2" } })
...
function getSubjectNumberConnectorFormulaSchema() {
    return typeof getAndrewsFormulaSlotSchema === "function"
        ? getAndrewsFormulaSlotSchema(SUBJECT_NUMBER_CONNECTOR_SCHEMA_ID)
        : null;
}
...
function buildSubjectNumberConnectorFormulaWorkbenchSlotRecords(schema = null, {
    ...
    return slots.map((slot) => {
        ...
        const nawatValue = slot.key === "num1Num2" || slot.id === "num1-num2"
            ? numberNawat
            : structuralValue;
        const compactValue = slot.key === "num1Num2" || slot.id === "num1-num2"
            ? numberCompact
        ...
        return {
            ...
            modelFields: [
                ...
                { label: "path", value: slot.path || "" },
                ...(slot.key === "pers1Pers2" ? [
                    { label: "dyad", value: subjectDyad },
                    { label: "outside", value: "predicate" },
                    { label: "not", value: "state/tense/object" },
                ] : []),
                ...(slot.key === "predicateSide" ? [
                    { label: "boundary", value: "connectors stay outside" },
                    { label: "context", value: "stem/state/valence/tns differ by shell" },
                ] : []),
                ...(slot.key === "num1Num2" ? [
                    { label: "dyad", value: numberStructural },
                    { label: "Nawat", value: numberNawat },
                    { label: "not", value: "tense/object/state/stem suffix" },
                ] : []),
            ].filter((entry) => entry.value),
            blockedInterpretations: Array.isArray(slot.blockedInterpretations)
                ? Array.from(slot.blockedInterpretations)
...
function buildSubjectNumberConnectorVncExample() {
    const vncSlice = typeof buildVncValenceFormulaWorkbenchSlice === "function"
        ? buildVncValenceFormulaWorkbenchSlice({ inputValue: "ki-piya" })
        : null;
...
function buildSubjectNumberConnectorDiagnosticExample() {
    const diagnostic = typeof diagnoseAndrewsFormulaSlotInterpretation === "function"
        ? diagnoseAndrewsFormulaSlotInterpretation(SUBJECT_NUMBER_CONNECTOR_SCHEMA_ID, "num1-num2", "tense").diagnostic
        : {
...
function buildSubjectNumberConnectorFormulaWorkbenchExamples(stem = "kal") {
    ...
    return [
        ...
        buildSubjectNumberConnectorVncExample(),
        buildSubjectNumberConnectorDiagnosticExample(),
    ];
}
...
function buildSubjectNumberConnectorFormulaWorkbenchSlice({
    ...
    const slotBlockDiagnostics = [
        [SUBJECT_NUMBER_CONNECTOR_SCHEMA_ID, "num1-num2", "tense"],
        [SUBJECT_NUMBER_CONNECTOR_SCHEMA_ID, "num1-num2", "stem-suffix"],
        [SUBJECT_NUMBER_CONNECTOR_SCHEMA_ID, "num1-num2", "object-valence"],
        [SUBJECT_NUMBER_CONNECTOR_SCHEMA_ID, "num1-num2", "predicate-state"],
        ["vnc-shell", "tns", "subject-number-connector"],
    ].map(([schemaId, slotId, interpretation]) => (
        typeof diagnoseAndrewsFormulaSlotInterpretation === "function"
            ? diagnoseAndrewsFormulaSlotInterpretation(schemaId, slotId, interpretation).diagnostic
            : null
    )).filter(Boolean);
    return {
        ...
        formulaFamilies: [
            ...
            },
            {
                ...
                label: "CNN posesiva",
                formula: typeof renderAndrewsFormulaTemplate === "function"
                    ? renderAndrewsFormulaTemplate("possessive-state-nnc")
                    : "#pers1-pers2+st1-st2(STEM)num1-num2#",
            },
            {
                ...
                label: "CNV",
                formula: typeof renderAndrewsFormulaTemplate === "function"
                    ? renderAndrewsFormulaTemplate("vnc-shell")
...
function buildAndrewsSourceSubjectFrame({
    ...
    const frame = {
        ...
        sourceFormulaRecord: sourceFormulaRecord && typeof sourceFormulaRecord === "object"
            ? sourceFormulaRecord
            : null,
...
function isAndrewsSourceSubjectFrame(frame = null) {
    return Boolean(
        frame
        && typeof frame === "object"
        && frame.kind === "andrews-source-subject-frame"
        && frame.subject
        && typeof frame.subject === "object"
    );
}
...
function getAndrewsSourceSubjectFramePossessorPrefix(frame = null) {
    ...
    return resolveNawatPossessorPrefixFromSourceSubject(
        frame.subject.prefix || "",
        frame.subject.suffix || ""
    );
}
...
function buildSourceSubjectPossessorOperationFrame({
    ...
    const frame = isAndrewsSourceSubjectFrame(sourceSubjectFrame)
        ? sourceSubjectFrame
        : null;
    const resolvedOperationId = String(operationId || "").trim()
        || (
            String(nominalKind || "") === "instrumentivo"
                ? "andrews-36-6-instrumentive-source-subject-to-possessor"
                : "andrews-36-11-active-action-source-subject-to-possessor"
        );
    const resolvedTargetPossessivePrefix = String(targetPossessivePrefix || "").trim()
        || getAndrewsSourceSubjectFramePossessorPrefix(frame);
    return Object.freeze({
        version: 1,
        ...
        andrewsRef: String(andrewsRef || "").trim()
            || (String(nominalKind || "") === "instrumentivo" ? "Andrews 36.6" : "Andrews 36.11"),
...
function getSourceSubjectPossessorOperationFrameMismatch({
    ...
    if (
        operationFrame.kind !== "andrews-source-subject-possessor-operation-frame"
        || operationFrame.operationFrame?.kind !== "andrews-typed-operation-frame"
    ) {
        return "missing-operation-frame";
    }
    ...
    if (
        String(operationFrame.sourceSubject?.prefix || "") !== String(sourceSubjectFrame.subject?.prefix || "")
        || String(operationFrame.sourceSubject?.suffix || "") !== String(sourceSubjectFrame.subject?.suffix || "")
    ) {
    ...
    if (operationFrame.sourceSubjectFrame && operationFrame.sourceSubjectFrame !== sourceSubjectFrame) {
        ...
        if (
            !isAndrewsSourceSubjectFrame(embedded)
            || String(embedded.subject?.prefix || "") !== String(sourceSubjectFrame.subject?.prefix || "")
            || String(embedded.subject?.suffix || "") !== String(sourceSubjectFrame.subject?.suffix || "")
        ) {
            return "contradictory-source-frame";
        }
    }
...
function resolvePredicateNominalAbsolutiveTtiConnector(value = "", fallback = "t", options = {}) {
    ...
    if (typeof getTClassSuffixForStem === "function") {
        return getTClassSuffixForStem(value) || String(fallback || "t");
    }
...
function getPredicateNominalPreviousNonZeroSegmentFromStemSpec(stemSpec = null) {
    ...
    if (stemSpec.kind === "base-suffix") {
        return getPredicateNominalPreviousNonZeroSegment(
            stemSpec.resultSuffix || stemSpec.sourceBase || ""
        );
    }
...
function buildPredicateNominalConnectorOperationFrame({
    ...
    const resolvedOperationId = String(operationId || "").trim()
        || "andrews-36-6-note-2-predicate-nominal-absolutive-tti-connector";
    return Object.freeze({
        version: 1,
        ...
        nominalKind: String(nominalKind || VERB_DERIVED_NOMINAL_KIND.predicadoNominal).trim(),
        sourceUnit: "vnc-predicate",
        sourceTense: normalizePredicateNominalSourceTense(sourceTense),
...
function getPredicateNominalConnectorOperationFrameMismatch({
    ...
    if (
        operationFrame.kind !== "andrews-predicate-nominal-connector-operation-frame"
        || operationFrame.operationFrame?.kind !== "andrews-typed-operation-frame"
    ) {
        return "missing-operation-frame";
    }
    ...
    const expectedKind = String(nominalKind || VERB_DERIVED_NOMINAL_KIND.predicadoNominal).trim();
    if (expectedKind && String(operationFrame.nominalKind || "") !== expectedKind) {
        return "contradictory-operation-frame";
    }
    const expectedSourceTense = normalizePredicateNominalSourceTense(sourceTense);
...
function buildPredicateNominalFormulaDisplayStem(baseStem = "", sourceTenseSuffix = "", surfaceObjectPrefix = "") {
    ...
    const displayStem = objectPrefix && stem && !stem.startsWith(objectPrefix)
        ? (typeof buildOutputPrefixedChain === "function"
            ? buildOutputPrefixedChain({ obj1: objectPrefix, tronco: stem })
            : `${objectPrefix}${stem}`)
        : stem;
...
function getPredicateNominalResult({
    ...
    const hasResolvedNonactiveSourceStem = isNonactive
        && verbMeta?.predicateNominalSourceStemResolved === true;
    ...
    forwardStemContexts.forEach((stemContext) => {
        ...
        sourceStemEntries.forEach((sourceStemEntry) => {
            ...
            const predicateStemSpec = sourceTenseSuffix
                ...
                : placedStemSpec;
            const resolvedConnector = resolveTtiConnector
                ? resolvePredicateNominalAbsolutiveTtiConnectorFromFrame({
                    operationFrame: predicateNominalConnectorOperationFrame,
                    predicateStemSpec,
            ...
            entries.push(buildVerbDerivedNominalEntry({
                ...
                metadata: {
                    formulaDisplayStem: sourceTenseSuffix === "s"
                        ? buildPredicateNominalFormulaDisplayStem(
                            nominalSurface.verb,
                            sourceTenseSuffix,
                            nominalSurface.objectPrefix
                        )
                        : "",
                    sourceTenseSuffix,
                    absolutiveConnectorFamily: resolveTtiConnector
                        ? "t/ti"
                        : "",
                    absolutiveConnectorSelector: sourceTenseSuffix
                        ? "typed-predicate-stem-frame-previous-non-zero-segment"
                        : "",
                    absolutiveConnectorInput: "",
                    absolutiveConnectorOperationFrame: resolveTtiConnector
    ...
    return attachVerbDerivedNominalGrammarContract({
        ...
        predicateNominalSource: {
            ...
            connectorSelector: resolveTtiConnector
                ? "typed-predicate-stem-frame-previous-non-zero-segment"
                : "",
        },
        predicateNominalConnectorOperationFrame: resolveTtiConnector
            ? predicateNominalConnectorOperationFrame
            : null,
    }, {
        kind: resolvedNominalKind,
...
function getInstrumentivoResult({
    ...
    if (mode === INSTRUMENTIVO_MODE.posesivo && !explicitPossessivePrefix) {
        resolvedSourceSubjectPossessorFrame = isAndrewsSourceSubjectFrame(sourceSubjectFrame)
            ? sourceSubjectFrame
            : null;
        ...
        const operationMismatch = getSourceSubjectPossessorOperationFrameMismatch({
            ...
            nominalKind: VERB_DERIVED_NOMINAL_KIND.instrumentivo,
        ...
        if (operationMismatch) {
            return buildVerbDerivedNominalBlockedResult({
                kind: VERB_DERIVED_NOMINAL_KIND.instrumentivo,
                rawVerb,
                diagnosticId: operationMismatch === "contradictory-source-frame" || operationMismatch === "contradictory-target-frame" || operationMismatch === "contradictory-operation-frame"
                    ? "instrumentivo-source-subject-contradictory-frame"
                    : "instrumentivo-source-subject-typed-operation-frame-required",
            });
        }
...
function getCalificativoInstrumentivoResult({
    ...
    if (useGeneralActionStem && !explicitPossessivePrefix) {
        ...
        const operationMismatch = getSourceSubjectPossessorOperationFrameMismatch({
            ...
            sourceSubjectFrame: resolvedSourceSubjectPossessorFrame,
            targetPossessivePrefix: resolvedPossessivePrefix,
            nominalKind: VERB_DERIVED_NOMINAL_KIND.calificativoInstrumentivo,
        ...
        if (operationMismatch) {
            return buildVerbDerivedNominalBlockedResult({
                ...
                rawVerb,
                diagnosticId: operationMismatch === "contradictory-source-frame" || operationMismatch === "contradictory-target-frame" || operationMismatch === "contradictory-operation-frame"
                    ? "calificativo-instrumentivo-source-subject-contradictory-frame"
                    : "calificativo-instrumentivo-source-subject-typed-operation-frame-required",
            });
        }
        resolvedSourceSubjectPossessorOperationFrame = sourceSubjectPossessorOperationFrame;
    }
