...
export function createSearchRuntimeApi(targetObject = globalThis) {
    ...
    function getOrdinaryNncSearchCandidateInfo(rawValue, options = {}) {
      const resolver = typeof targetObject.resolveOrdinaryNncFixture === "function" ? targetObject.resolveOrdinaryNncFixture : null;
      if (!resolver) {
        return null;
      }
    ...
    function isNominalTenseMode(mode = targetObject.getActiveTenseMode()) {
      return mode === targetObject.TENSE_MODE.sustantivo || mode === targetObject.TENSE_MODE.adjetivo || mode === targetObject.TENSE_MODE.adverbio;
    }
    function getCurrentSearchTense() {
      const selectionState = targetObject.buildConjugationSelectionState();
      return selectionState.group === targetObject.CONJUGATION_GROUPS.universal ? selectionState.universalTenseValue : selectionState.tenseValue;
    }
    function getSearchModeGroups(mode) {
      return isNominalTenseMode(mode) ? [targetObject.CONJUGATION_GROUPS.tense] : [targetObject.CONJUGATION_GROUPS.tense, targetObject.CONJUGATION_GROUPS.universal];
    }
    ...
    function buildSearchPlanAcrossModes() {
      ...
      const activeGroup = isNominalTenseMode(activeMode) ? targetObject.CONJUGATION_GROUPS.tense : activeSelectionState.group;
      const currentTense = activeGroup === targetObject.CONJUGATION_GROUPS.universal ? activeSelectionState.universalTenseValue : activeSelectionState.tenseValue;
      addGroup(activeMode, activeGroup, currentTense);
      getSearchModeGroups(activeMode).filter(group => group !== activeGroup).forEach(group => addGroup(activeMode, group));
      [targetObject.TENSE_MODE.verbo, targetObject.TENSE_MODE.sustantivo, targetObject.TENSE_MODE.adjetivo, targetObject.TENSE_MODE.adverbio].filter(Boolean).filter(mode => mode !== activeMode).forEach(mode => {
    ...
    function isNonanimateNounTense(tenseValue) {
      ...
      return tenseValue === "sustantivo-verbal" || tenseValue === "instrumentivo" || tenseValue === "calificativo-instrumentivo";
    }
    function getNonanimateSubjectId() {
      const match = targetObject.SUBJECT_COMBINATIONS.find(entry => entry.subjectPrefix === "" && entry.subjectSuffix === "");
    ...
    const NOUN_OBJECT_SLOT_SCHEMA = Object.freeze([Object.freeze({
      id: "object",
      stateSuffix: "",
    ...
    function getNominalAvailableObjectSlots(verbMeta, tenseValue = "", options = {}) {
      ...
      const shouldUseNonactiveSlots = resolvedCombinedMode === targetObject.COMBINED_MODE.nonactive && (targetObject.isPotencialHabitualTense(tenseValue) || tenseValue === "calificativo-instrumentivo");
      const valency = targetObject.getValencyFromDirectActive(verbMeta);
      const baseObjectSlots = shouldUseNonactiveSlots ? Number(valency?.nonactiveSlots || 0) : Number(valency?.activeSlots || 0);
    ...
    function getAllowedNounObjectPrefixesFromMeta(verbMeta, tenseValue, options = {}) {
      ...
      const isAdjetivoWrapper = targetObject.isPotencialActiveTense(tenseValue) || targetObject.isPotencialHabitualTense(tenseValue);
      const resolvedCombinedMode = options.combinedMode || targetObject.getCombinedMode();
      if (isCalificativoInstrumentivo) {
        return isTransitiveVerb && allowsObjectPrefix ? nounTransitivePrefixes : [""];
      }
      if (isLocativoTemporal) {
        ...
        if (resolvedCombinedMode === targetObject.COMBINED_MODE.nonactive) {
          return [...Array.from(targetObject.PASSIVE_IMPERSONAL_DIRECT_OBJECTS), ...nounTransitivePrefixes];
        }
        return nounTransitivePrefixes;
      }
      if (!isTransitiveVerb || !allowsObjectPrefix) {
    ...
    function getNounObjectSlotPlansFromMeta(verbMeta, tenseValue, options = {}) {
      ...
      const primaryPrefixes = Array.from(new Set(Array.isArray(allowedPrimaryPrefixes) && allowedPrimaryPrefixes.length ? allowedPrimaryPrefixes : [""]));
      const slotPlans = NOUN_OBJECT_SLOT_SCHEMA.slice(0, slotSummary.availableObjectSlots).map((slotMeta, index) => {
        const isPrimary = index === 0;
        const isAddedSlot = index >= slotSummary.directObjectSlots;
        const seedValues = isPrimary ? primaryPrefixes : nounTransitivePrefixes;
        const toggleValues = Array.from(new Set(seedValues.filter(value => value !== undefined && value !== null))).map(value => String(value || ""));
    ...
    function buildNounObjectSlotToggleStates({
      ...
      const slotStates = slotPlanBundle.slotPlans.map(slotPlan => {
        ...
        const isStoredValid = storedValue !== undefined && (slotPlan.toggleValues.includes(storedValue) || showToggle && storedValue === targetObject.OBJECT_TOGGLE_ALL);
        ...
        targetObject.setToggleStateValue(targetObject.ObjectToggleState, stateKey, activeId, {
          syncLock: false
        });
        return {
          ...slotPlan,
    ...
    function buildNounObjectSlotSelectionModels(slotStates = [], options = {}) {
      const includeSlot = typeof options.includeSlot === "function" ? options.includeSlot : () => true;
      return (Array.isArray(slotStates) ? slotStates : []).filter(slotState => includeSlot(slotState)).map(slotState => {
        const values = slotState.activeId === targetObject.OBJECT_TOGGLE_ALL ? slotState.toggleValues : [slotState.activeId];
        return {
          ...slotState,
    ...
    function iterateNounObjectSlotSelections(slotSelectionModels = [], onSelection = null) {
      ...
      const visit = (slotIndex, selectedBySlot) => {
        ...
        const values = Array.isArray(slotModel.values) && slotModel.values.length ? slotModel.values : [""];
    ...
    function getSearchNonactiveSuffixPlan(verbMeta) {
      ...
      const ordered = current && suffixes.includes(current) ? [current, ...suffixes.filter(suffix => suffix !== current)] : suffixes;
      return ordered.filter((value, index) => ordered.indexOf(value) === index);
    }
    function getSearchCombinedModePlan() {
      const current = targetObject.getCombinedMode();
      const other = current === targetObject.COMBINED_MODE.nonactive ? targetObject.COMBINED_MODE.active : targetObject.COMBINED_MODE.nonactive;
      return [current, other];
    }
    function getNonTodosSubjectOptionIds() {
      return targetObject.getSubjectToggleOptions().filter(entry => entry.id !== targetObject.SUBJECT_TOGGLE_ALL).map(entry => entry.id);
    }
    function buildVerbSearchPlans({
      tenseValue,
      group,
      isNonactive,
      verbMeta
      ...
      const objectPrefixGroups = isNonactive ? targetObject.getNonactiveObjectPrefixGroups(resolvedMeta).groups : targetObject.buildObjectPrefixGroups(objectPrefixes);
    ...
    function buildNounSearchPlans({
      tenseValue,
      verbMeta
    }) {
      const nominalSearchCombinedMode = targetObject.getResolvedNominalCombinedModeForTense(tenseValue, targetObject.getCombinedMode());
      ...
      const isPotencialHabitualIntransitive = isPotencialHabitual && nominalSearchCombinedMode === targetObject.COMBINED_MODE.nonactive && targetObject.getBaseObjectSlots(verbMeta) <= 0;
      const usePotencialHabitualNonactiveSubjects = isPotencialHabitual && nominalSearchCombinedMode === targetObject.COMBINED_MODE.nonactive && !isPotencialHabitualIntransitive;
      ...
      const subjectOptionIds = subjectOptions.filter(entry => entry.id !== targetObject.SUBJECT_TOGGLE_ALL).map(entry => entry.id);
      ...
      const objectSlotPlans = slotPlanBundle.slotPlans.map(slotPlan => {
        ...
        const options = buildSearchOptionPlan(slotPlan.toggleValues, stored, slotPlan.preferredId);
        return {
          slotId: slotPlan.id,
    ...
    api.rememberNonSearchValue = rememberNonSearchValue;
    api.getSearchInputBase = getSearchInputBase;
    api.isComposerTemplateOnlyBaseValue = isComposerTemplateOnlyBaseValue;
    api.getSearchQueryInfo = getSearchQueryInfo;
    ...
    api.getCurrentSearchTense = getCurrentSearchTense;
    api.getSearchModeGroups = getSearchModeGroups;
    api.getSearchGroupTenseOrder = getSearchGroupTenseOrder;
    api.buildSearchPlanAcrossModes = buildSearchPlanAcrossModes;
    api.getNounPossessorKey = getNounPossessorKey;
    api.getDefaultPossessorForTense = getDefaultPossessorForTense;
    api.getSearchPossessorPlan = getSearchPossessorPlan;
    api.isNonanimateNounTense = isNonanimateNounTense;
    api.getNonanimateSubjectId = getNonanimateSubjectId;
    api.buildSearchOptionPlan = buildSearchOptionPlan;
    Object.defineProperty(api, "NOUN_OBJECT_SLOT_SCHEMA", {
        configurable: true,
        enumerable: true,
        get() { return NOUN_OBJECT_SLOT_SCHEMA; },
    });
    api.getNounDerivationTypeFromMeta = getNounDerivationTypeFromMeta;
    api.getNominalAvailableObjectSlots = getNominalAvailableObjectSlots;
    api.getNounObjectSlotSummary = getNounObjectSlotSummary;
    api.getNounObjectSlotStateKey = getNounObjectSlotStateKey;
    api.getPreferredNounObjectSlotPrefix = getPreferredNounObjectSlotPrefix;
    api.getAllowedNounObjectPrefixesFromMeta = getAllowedNounObjectPrefixesFromMeta;
    api.getNounObjectSlotPlansFromMeta = getNounObjectSlotPlansFromMeta;
    api.buildNounObjectSlotToggleStates = buildNounObjectSlotToggleStates;
    api.buildNounObjectSlotSelectionModels = buildNounObjectSlotSelectionModels;
    api.iterateNounObjectSlotSelections = iterateNounObjectSlotSelections;
    api.getSearchNonactiveSuffixPlan = getSearchNonactiveSuffixPlan;
    api.getSearchCombinedModePlan = getSearchCombinedModePlan;
    api.getNonTodosSubjectOptionIds = getNonTodosSubjectOptionIds;
    api.buildVerbSearchPlans = buildVerbSearchPlans;
    api.buildNounSearchPlans = buildNounSearchPlans;
    return api;
}
