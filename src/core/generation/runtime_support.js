...
function normalizeGenerateRuntimeContractSurface(value = "") {
    return typeof normalizeGrammarSurfaceValue === "function"
        ? normalizeGrammarSurfaceValue(value)
...
function attachGenerateRuntimeBlockedContract(result = null, {
    ...
    if (!result || typeof result !== "object" || typeof buildGrammarFrame !== "function") {
        return result;
    }
    ...
    const diagnostics = normalizeGenerateRuntimeDiagnostics(
        result.diagnostics,
        Array.isArray(result.diagnostics) && result.diagnostics.length ? null : fallbackDiagnostic,
        { routeFamily, routeStage }
    );
...
function buildNoStemMaskResult({
    ...
    return attachGenerateRuntimeBlockedContract(
        { result: "—", surfaceForms: [], isReflexive },
        {
            routeFamily,
            routeStage,
            renderVerb,
...
function applyGenerateForwardDerivations({
    ...
    const selectedForwardMetaByType = {
        ...
        [DERIVATION_TYPE.applicative]: null,
    };
