...
function normalizeOrdinaryNncOutputSet(value = "") {
    const normalized = String(value || "")
        ...
        .replace(/[_\s]+/g, "-");
    if ([
        "lesson32-pil-child-nnc-side",
        "lesson-32-pil-child-nnc-side",
        "lesson32-p294-pil-child-nnc-side",
        "andrews-p294-pil-child-nnc-side",
        "p294-pil-child-nnc-side",
        "pil-child-nnc-side",
    ].includes(normalized)) {
...
function getOrdinaryNncOutputSet(ordinaryNnc = null) {
    ...
    return normalizeOrdinaryNncOutputSet(
        ordinaryNnc.outputSet
        || ordinaryNnc.generationSet
        || ordinaryNnc.ordinaryNncOutputSet
        || ordinaryNnc.nncSideOutputSet
        || ""
    );
}
...
function isLesson32PilChildNncSideOrdinaryNncRequest(ordinaryNnc = null) {
    ...
    return getOrdinaryNncOutputSet(ordinaryNnc) === ORDINARY_NNC_OUTPUT_SET.lesson32PilChildNncSide
        || ordinaryNnc.lesson32PilChildNncSide === true
        || ordinaryNnc.pilChildNncSide === true
        || ordinaryNnc.pilChildNncSideGeneration === true
        || ordinaryNnc.generatePilChildNncSide === true;
}
...
const LESSON6_DIRECT_NAWAT_DYAD_SPLIT_FRAMES = Object.freeze({
    "n-ech": Object.freeze({
        ...
        functionalVa1: "n-0",
        functionalVa2: "ech",
        val1Features: Object.freeze({ person: "n", number: "0" }),
        val2Features: Object.freeze({ objective: "ech" }),
    ...
    "t-ech": Object.freeze({
        ...
        functionalVa1: "t-0",
        functionalVa2: "ech",
        val1Features: Object.freeze({ person: "t", number: "0" }),
        val2Features: Object.freeze({ objective: "ech" }),
    ...
    "m-etz": Object.freeze({
        ...
        functionalVa1: "m-0",
        functionalVa2: "etz",
        val1Features: Object.freeze({ person: "m", number: "0" }),
        val2Features: Object.freeze({ objective: "etz" }),
    ...
    "m-etz-in": Object.freeze({
        ...
        functionalVa1: "m-in",
        functionalVa2: "etz",
        val1Features: Object.freeze({ person: "m", number: "in" }),
        val2Features: Object.freeze({ objective: "etz" }),
    ...
    "k-0": Object.freeze({
        ...
        functionalVa1: "k-0",
        functionalVa2: "0",
        val1Features: Object.freeze({ person: "k", objective: "0" }),
        val2Features: Object.freeze({ number: "0" }),
    ...
    "ki-0": Object.freeze({
        ...
        functionalVa1: "ki-0",
        functionalVa2: "0",
        val1Features: Object.freeze({ person: "ki", objective: "0" }),
        val2Features: Object.freeze({ number: "0" }),
    ...
    "k-in": Object.freeze({
        ...
        surfaceLinearMorph: "",
        val1Features: Object.freeze({ person: "k", objective: "0" }),
        val2Features: Object.freeze({ number: "in" }),
...
function cloneLesson6DirectNawatDyadSplitFrame(frame = null) {
    ...
    return {
        ...frame,
        val1Features: frame.val1Features ? { ...frame.val1Features } : null,
        val2Features: frame.val2Features ? { ...frame.val2Features } : null,
...
function buildLesson6DirectNawatDyadSourceFrame({
    ...
    const signature = [
        normalizedDirectDyad,
        targetDyadFrame.va1 || "",
        targetDyadFrame.va2 || "",
        targetDyadFrame.functionalVa1 || "",
        targetDyadFrame.functionalVa2 || "",
        Array.isArray(targetDyadFrame.linearPieces) ? targetDyadFrame.linearPieces.join(".") : "",
    ].join("|");
...
function getLesson6DirectNawatDyadFrameMismatch({
    ...
    if (
        ...
        || operationFrame.operationId !== "andrews-6-direct-nawat-dyad-split"
        || operationFrame.routeFamily !== "vnc-valence"
        || operationFrame.operationApplied !== "split-direct-nawat-dyad-into-va1-va2-slots"
...
function splitLesson6DirectNawatDyad(value = "", {
    ...
    void value;
    const mismatch = getLesson6DirectNawatDyadFrameMismatch({ sourceFrame, operationFrame });
    if (mismatch) {
        return {
            va: "",
            va1: "",
...
function buildGeneratedClassPerfectiveFormulaSourceFrame({
    ...
    const baseCandidateEntries = getCnvFormulaSourceStemVariants(sourceStem)
        .filter((variant) => variant.relation !== "source-final-vowel-removed");
    const baseCandidates = baseCandidateEntries.map((variant) => variant.value);
    const preferredVariant = (
        String(objectPrefix || "").trim()
        && String(sourceStem || "").trim().startsWith("i")
    )
        ? baseCandidateEntries.find((variant) => variant.relation === "source-final-a-perfective-j")
        : null;
...
function getGeneratedClassPerfectiveFormulaFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "generated-class-perfective-formula-profile-realization"
        || operationFrame.routeFamily !== "vnc-formula"
        || operationFrame.operationApplied !== "derive-class-perfective-formula-profile-from-source-frame"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
...
function buildGeneratedClassPerfectiveFormulaProfile({
    ...
    void sourceStem;
    const mismatch = getGeneratedClassPerfectiveFormulaFrameMismatch({ sourceFrame, operationFrame });
    if (mismatch) {
        return null;
    }
    return {
...
function buildCnvFormulaLesson7SurfaceSlotsSourceFrame({
    ...
    return Object.freeze({
        ...
        formulaSlots: slots,
        targetSurfaceSlots: Object.freeze({ ...targetSurfaceSlots }),
        surfacePathIndex,
        sourceSignature,
        consumesRenderedInput: false,
        displayStringsAuthorizeGrammar: false,
...
function buildCnvFormulaLesson7SurfaceSlotsOperationFrame(sourceFrame = null) {
    ...
    return Object.freeze({
        ...
        sourceFrameKind: sourceFrame.kind,
        sourceSignature: sourceFrame.sourceSignature || "",
        targetSurfaceSlots: Object.freeze({ ...(sourceFrame.targetSurfaceSlots || {}) }),
...
function getCnvFormulaLesson7SurfaceSlotsFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "cnv-formula-lesson-7-surface-slot-realization"
        || operationFrame.routeFamily !== "vnc-formula"
...
function getCnvFormulaLesson7SurfaceSlots(formulaSlots = null, surface = "", segments = [], options = {}) {
    ...
    const mismatch = getCnvFormulaLesson7SurfaceSlotsFrameMismatch({ sourceFrame, operationFrame });
    if (mismatch) {
        return null;
    }
    return { ...(operationFrame.targetSurfaceSlots || {}) };
}
...
function buildCnvFormulaSurfacePathRecord({
    ...
    const resolvedSourceFrame = surfaceSlotsSourceFrame
        || buildCnvFormulaLesson7SurfaceSlotsSourceFrame({
            formulaSlots,
            surface,
            segments,
            surfacePathIndex,
        });
    const resolvedOperationFrame = surfaceSlotsOperationFrame
        || buildCnvFormulaLesson7SurfaceSlotsOperationFrame(resolvedSourceFrame);
...
function getCnvFormulaSurfacePathSlot(paths = [], slotKey = "") {
    return (Array.isArray(paths) ? paths : [])
        .find((path) => String(path?.formulaSlotKey || "") === slotKey)
        || null;
}
...
function getCnvFormulaPathSegmentLetters(path = null) {
    ...
    const features = path.formulaFeatures && typeof path.formulaFeatures === "object"
        ? path.formulaFeatures
        : {};
    const pieces = [];
    if (slotKey === "va1") {
    ...
    return pieces
        ...
        .filter((piece) => piece && piece !== "0" && piece !== "Ø")
        .map((piece) => (typeof convertClassicalLettersToNawat === "function"
            ? convertClassicalLettersToNawat(piece, {
...
function buildCnvFormulaRealizationSegmentFrames(paths = []) {
    ...
    return orderedSlots
        .map((slot) => getCnvFormulaSurfacePathSlot(paths, slot))
        .filter(Boolean)
        .map((path) => ({
...
function isCnvFormulaSurfacePathCollapsedDirectional(paths = []) {
    ...
    const base = getCnvFormulaSurfaceSpecificSlotValue(pathList, "base", "", { preferSurface: true });
...
function buildCnvFormulaSurfaceSpecificFormulaEcho({
    ...
    const va1Person = String(va1Path?.formulaFeatures?.person || "")
        || String(va1Path?.formulaMorph || "").split("-")[0]
        || "";
    ...
    const valence = hasDyadicValenceSlots
        ? `${normalizeCnvFormulaSlotToken(va1Person, "0")}-${normalizeCnvFormulaSlotToken(va2Number, "0")}`
        : (vaPath ? normalizeCnvFormulaSlotToken(vaPath.formulaMorph, "0") : "");
    const directionalPath = getCnvFormulaSurfacePathSlot(paths, "directional");
    const directional = directionalPath
        ? normalizeCnvFormulaSlotToken(directionalPath.formulaMorph, "")
        : "";
    ...
    const predicateStem = normalizeCnvFormulaSlotToken(
        getCnvFormulaSurfaceSpecificSlotValue(paths, "base", "STEM", { preferSurface: true }),
        "STEM"
    );
    const tense = normalizeCnvFormulaSlotToken(
        getCnvFormulaSurfaceSpecificSlotValue(paths, "tns", "Ø", { preferSurface: true }),
        "Ø"
    );
    const num1 = normalizeCnvFormulaSlotToken(
        getCnvFormulaSurfaceSpecificSlotValue(paths, "num1", "Ø", { preferSurface: true }),
        "Ø"
    );
    const num2 = normalizeCnvFormulaSlotToken(
        getCnvFormulaSurfaceSpecificSlotValue(paths, "num2", "Ø", { preferSurface: true }),
        "Ø"
    );
...
function buildCnvFormulaSurfacePairs({
    ...
    return Object.freeze((Array.isArray(pathRecords) ? pathRecords : [])
        .map((pathRecord, pathIndex) => {
            ...
            const resolvedSourceFormulaEcho = String(sourceFormulaEcho || pathRecord?.sourceFormulaEcho || "").trim()
                || `CNV(${String(getCnvFormulaSurfacePathSlot(pathRecord.paths, "base")?.formulaMorph || "STEM").trim() || "STEM"})`;
            const segmentFrames = buildCnvFormulaRealizationSegmentFrames(pathRecord.paths);
            const segmentDerivedSurface = segmentFrames
...
function buildGeneratedCnvFormulaSurfacePath({
    ...
    const pathRecordsWithoutAlternatives = orderedRecords
        .map((record, index) => buildCnvFormulaSurfacePathRecord({
            nuclearClauseShell,
...
function buildNuclearClauseSurfaceGrammarFrame({
    ...
    const activeCnvFormulaSurfacePathWithRoute = andrewsRouteActionContract
        ...
        : activeCnvFormulaSurfacePath;
    const activeFormulaSurfacePairs = Array.isArray(output.formulaSurfacePairs) && output.formulaSurfacePairs.length
        ? output.formulaSurfacePairs
        : (Array.isArray(activeCnvFormulaSurfacePathWithRoute?.formulaSurfacePairs)
            ? activeCnvFormulaSurfacePathWithRoute.formulaSurfacePairs
            : []);
    const outputResultFrame = output.grammarFrame?.resultFrame && typeof output.grammarFrame.resultFrame === "object"
        ? output.grammarFrame.resultFrame
        : (output.frames?.resultFrame && typeof output.frames.resultFrame === "object" ? output.frames.resultFrame : null);
    const activeTargetFormulaEchoes = activeFormulaSurfacePairs
        .map((entry) => String(entry?.targetFormulaEcho || "").trim())
        .filter(Boolean);
    const activeSourceFormulaEchoes = Array.from(new Set(activeFormulaSurfacePairs
        .map((entry) => String(entry?.sourceFormulaEcho || "").trim())
        .filter(Boolean)));
    const activeAndrewsFormulaEchoes = Array.from(new Set(activeFormulaSurfacePairs
        .map((entry) => String(entry?.andrewsFormulaEcho || entry?.sourceFormulaEcho || "").trim())
        .filter(Boolean)));
    const activeConjugatorFormulaEchoes = Array.from(new Set(activeFormulaSurfacePairs
        .map((entry) => String(entry?.conjugatorFormulaEcho || entry?.targetFormulaEcho || "").trim())
        .filter(Boolean)));
    const activeSourceToTargetFormulaEcho = activeFormulaSurfacePairs
        .map((entry) => String(entry?.sourceToTargetFormulaEcho || "").trim())
        .filter(Boolean)
        .join(" | ");
    const activeAndrewsToConjugatorFormulaEcho = activeFormulaSurfacePairs
        .map((entry) => String(entry?.andrewsToConjugatorFormulaEcho || entry?.sourceToTargetFormulaEcho || "").trim())
        .filter(Boolean)
        .join(" | ");
    const outputFormulaRecords = Array.isArray(outputResultFrame?.formulaRecords) && outputResultFrame.formulaRecords.length
        ? outputResultFrame.formulaRecords
        : (outputResultFrame?.formulaRecord ? [outputResultFrame.formulaRecord] : []);
    const activeFormulaRecords = (activeFormulaSurfacePairs
        .map((entry) => entry?.formulaRecord)
        .filter(Boolean).length
        ? activeFormulaSurfacePairs.map((entry) => entry?.formulaRecord).filter(Boolean)
        : outputFormulaRecords);
    const outputFormulaRealizationRecords = Array.isArray(outputResultFrame?.formulaRealizationRecords) && outputResultFrame.formulaRealizationRecords.length
        ? outputResultFrame.formulaRealizationRecords
        : (outputResultFrame?.formulaRealizationRecord ? [outputResultFrame.formulaRealizationRecord] : []);
    const activeFormulaRealizationRecords = (activeFormulaSurfacePairs
        .map((entry) => entry?.formulaRealizationRecord)
        .filter(Boolean).length
        ? activeFormulaSurfacePairs.map((entry) => entry?.formulaRealizationRecord).filter(Boolean)
        : outputFormulaRealizationRecords);
...
function executeAdjectivalNncGenerationRoute({
    ...
    const ordinaryAbsolutiveStructuredRouteMissing = shouldUseOrdinaryAbsolutiveRoute
        && adjectivalNnc.requiresStructuredContinuation === true
        && (
            typeof buildStructuredOrdinaryAdjectivalNncFunctionOutput !== "function"
            || !adjectivalSourceFormulaSlots?.predicateStem
            || !adjectivalSourceFormulaSlots?.num1Num2
            || adjectivalNnc.sourceContinuationFrame?.kind !== "generated-output-typed-continuation-frame"
            || adjectivalNnc.targetContinuationFrame?.kind !== "generated-output-typed-continuation-frame"
        );
    const adjectivalSourceFormulaEcho = adjectivalSourceFormulaSlots && typeof buildAdjectivalNncFormulaEchoFromSlots === "function"
        ? buildAdjectivalNncFormulaEchoFromSlots(adjectivalSourceFormulaSlots)
        : "";
    const result = ordinaryAbsolutiveStructuredRouteMissing
        ...
        : shouldUseIntensifiedRoute && typeof buildIntensifiedAdjectivalNncOutput === "function"
        ? buildIntensifiedAdjectivalNncOutput({
            ...
            operationFrame: adjectivalNnc.operationFrame
                || adjectivalNnc.targetContinuationFrame?.operationFrame
                || null,
        ...
        ? buildNominalizedVncAdjectivalNncFunctionOutput({
            ...
            formulaEcho: adjectivalNnc.formulaSlots && typeof buildAdjectivalNncFormulaEchoFromSlots === "function"
                ? buildAdjectivalNncFormulaEchoFromSlots(adjectivalNnc.formulaSlots)
                : "",
            sourceContinuationFrame: adjectivalNnc.sourceContinuationFrame || null,
            targetContinuationFrame: adjectivalNnc.targetContinuationFrame || null,
            operationFrame: adjectivalNnc.operationFrame
        ...
        : shouldUseOrdinaryAbsolutiveRoute
            && adjectivalNnc.requiresStructuredContinuation === true
            && typeof buildStructuredOrdinaryAdjectivalNncFunctionOutput === "function"
        ? buildStructuredOrdinaryAdjectivalNncFunctionOutput({
            sourceContinuationFrame: adjectivalNnc.sourceContinuationFrame || null,
            targetContinuationFrame: adjectivalNnc.targetContinuationFrame || null,
            sourceFormulaSlots: adjectivalSourceFormulaSlots,
            state: adjectivalNnc.state ?? "absolutive",
        ...
        ? (() => {
            ...
            const parsedVerb = typeof resolveAdjectivalNncParsedVerb === "function"
                ? resolveAdjectivalNncParsedVerb(sourceStem)
                : null;
            const sourceFrame = typeof buildRootPlusYaAdjectivalNncSourceFrame === "function"
                ? buildRootPlusYaAdjectivalNncSourceFrame({
                    parsedVerb,
            ...
            const operationFrame = typeof buildRootPlusYaAdjectivalNncOperationFrame === "function"
                ...
                : null;
            return generateRootPlusYaAdjectivalNncOutput({
                stem: sourceStem,
                state: adjectivalNnc.state ?? "absolutive",
                subject: subjectFrame,
                sourceFrame,
...
function getLesson32PilChildNncSideRequestedIds(ordinaryNnc = null) {
    ...
    const source = Array.isArray(ordinaryNnc.ids)
        ? ordinaryNnc.ids
        : (Array.isArray(ordinaryNnc.outputSetIds)
            ? ordinaryNnc.outputSetIds
            : (Array.isArray(ordinaryNnc.lesson32PilChildNncSideIds)
                ? ordinaryNnc.lesson32PilChildNncSideIds
                : null));
...
function buildLesson32PilChildNncSideFormulaSurfacePairs(entries = []) {
    return Object.freeze((Array.isArray(entries) ? entries : [])
        .map((entry, entryIndex) => {
            ...
            const slots = entry?.formulaSlots && typeof entry.formulaSlots === "object"
                ? entry.formulaSlots
                : {};
            const segmentFrames = [
                {
...
function validateLesson32PilChildNncSideResultTextOperationFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "lesson-32-pil-child-nnc-side-result-text-source-frame"
        || String(sourceFrame.sourceSignature || "") !== String(rebuiltSourceFrame.sourceSignature || "")
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "lesson-32-pil-child-nnc-side-result-text-render"
...
function buildLesson32PilChildNncSideOrdinaryNncResult({
    ...
    if (typeof generateLesson32PilChildNncSideOutputs !== "function") {
    ...
    const generatedSet = generateLesson32PilChildNncSideOutputs({
    ...
    const structuralFormulas = entries
        .map((entry) => String(entry?.structuralFormula || entry?.formula || "").trim())
        .filter(Boolean);
    ...
    const lesson32PilChildNncSideResultTextOperationFrame = buildLesson32PilChildNncSideResultTextOperationFrame(
    ...
    const sourceFormulaEchoes = Array.from(new Set(formulaSurfacePairs
        .map((entry) => entry.sourceFormulaEcho)
        .filter(Boolean)));
    const targetFormulaEchoes = formulaSurfacePairs
        .map((entry) => entry.targetFormulaEcho)
        .filter(Boolean);
    const sourceToTargetFormulaEcho = formulaSurfacePairs
        .map((entry) => entry.sourceToTargetFormulaEcho)
        .filter(Boolean)
        .join(" | ");
    const states = Array.from(new Set(entries
    ...
    const diagnosticIds = Array.isArray(generatedSet?.diagnostics)
        ? generatedSet.diagnostics.map((id) => String(id || "").trim()).filter(Boolean)
        : [];
...
function executeOrdinaryNncGenerationRoute({
    ...
    const result = isLesson32PilChildNncSideOrdinaryNncRequest(ordinaryNnc)
        ? buildLesson32PilChildNncSideOrdinaryNncResult({
            ordinaryNnc,
            override,
            verb,
            state,
        })
        : (typeof generateOrdinaryNncParadigm === "function"
    ...
    if (isNominalShell) {
        ...
        const nominalFormulaEcho = (() => {
            ...
            const possessorPrefix = String(
                nominalClauseMetadata?.nominalClauseFrame?.predicate?.stateSlot?.possessorPrefix || ""
            );
...
function buildGeneratedDenominalFamilyProfileMetadata({
    ...
    const andrewsContractRoutePreview = typeof generateNawatDenominalAndrewsContractRoutePreview === "function"
        ...
        : null;
    const andrewsRouteRecordId = generatedDenominalRouteHasAndrewsSuffixContract(spec)
        ? "cnn-nounstem-to-cnv-verbstem-denominal"
        : "";
    const sourceRouteFrame = andrewsRouteRecordId && typeof buildAndrewsCnvCnnBackAndForthRouteCoordinateFrame === "function"
        ? buildAndrewsCnvCnnBackAndForthRouteCoordinateFrame(andrewsRouteRecordId)
        : null;
    return {
        version: 1,
...
function buildGeneratedNominalNum1Num2Metadata({
    ...
    const resolvedFormulaDisplayStem = String(formulaDisplayStem || "")
        || buildGeneratedPreteritAgentiveFormulaDisplayStem({
            nominalKind,
...
function enrichGeneratedNominalClauseMetadataWithAndrewsRendering({
    ...
    const surfaces = Array.isArray(surfaceForms)
        ? surfaceForms.map((entry) => String(entry || "").trim()).filter(Boolean)
        : [];
    ...
    const sourceToTargetFormulaEcho = formulaSurfacePairs.length
        ? formulaSurfacePairs.map((entry) => entry.sourceToTargetFormulaEcho).join(" | ")
        : `${sourceFormulaEcho} -> ${targetFormulaEcho}`;
...
function getGeneratedFormulaPathValue(pathRecord = null, slotKey = "", field = "formulaMorph") {
    const match = (Array.isArray(pathRecord?.paths) ? pathRecord.paths : [])
        .find((path) => String(path?.formulaSlotKey || "") === slotKey);
    return String(match?.[field] || "");
}

function getGeneratedFormulaSegmentValue(pathRecord = null, role = "") {
    const match = (Array.isArray(pathRecord?.segments) ? pathRecord.segments : [])
        .find((segment) => String(segment?.role || segment?.slot || "") === role);
    return String(match?.value || "");
}

function isGeneratedPreteritAgentiveNominalFrame(frame = null) {
    return String(frame?.family || "") === "preterit-agentive"
        && String(frame?.nominalKind || "") === "agentivo-preterito";
}
...
function isGeneratedNominalizerInsideStem(frame = null) {
    ...
    return family === "active-action"
        || operationId === "active-action-z"
        || operationId === "active-action-liz"
        || operationId === "impersonal-action-liz";
}
...
function joinGeneratedNominalStemFormulaPieces({
    ...
    return [
        base,
        ...(Array.isArray(stemSuffixes) ? stemSuffixes : []),
    ]
        .map((piece) => String(piece || "").trim())
        .filter(Boolean)
        .join("-");
...
function normalizeGeneratedNominalFormulaPathSource(pathRecord = null, {
    ...
    const subjectPrefix = getGeneratedFormulaPathValue(pathRecord, "pers1", "formulaMorph")
        || subject.displayPrefix
        || subject.prefix
        || "Ø";
    const subjectCase = getGeneratedFormulaPathValue(pathRecord, "pers2", "formulaMorph")
        || subject.displayCase
        || subject.case
        || "Ø";
    const objectPrefix = getGeneratedFormulaSegmentValue(pathRecord, "obj1");
    const stemCore = getGeneratedFormulaSegmentValue(pathRecord, "tronco")
        || getGeneratedFormulaPathValue(pathRecord, "base", "surfaceValue")
        || getGeneratedFormulaPathValue(pathRecord, "base", "formulaMorph");
    ...
    const possessorPrefix = getGeneratedFormulaSegmentValue(pathRecord, "poseedor")
        || String(formulaSlots.predicateStem?.stateSlot?.possessorPrefix || "");
    const operationId = String(frame?.operationId || "");
    const family = String(frame?.family || "");
    const nominalKind = String(frame?.nominalKind || "");
    return {
...
function buildGeneratedNominalFormulaSourceFrame(pathRecord = null, {
    ...
    return Object.freeze({
        ...
        pathSlotFrames: Object.freeze(source.pathSlotFrames.map((entry) => Object.freeze({ ...entry }))),
        segmentFrames: Object.freeze(source.segmentFrames.map((entry) => Object.freeze({ ...entry }))),
        sourceSignature,
        consumesRenderedInput: false,
        displayStringsAuthorizeGrammar: false,
        displayOnlyFieldsExcluded: Object.freeze(["formulaEcho", "result", "surface", "surfaceForms"]),
...
function buildGeneratedNominalFormulaOperationFrame(sourceFrame = null) {
    ...
    if (sourceFrame.preteritAgentiveNominalFrame === true) {
        ...
        const generalUse = Boolean(possessorPrefix)
            || String(sourceFrame.operationId || "") === "preterit-agentive-general-use-ca";
        const preteritStem = buildGeneratedPreteritAgentiveFormulaStem(stemCore, { generalUse });
        targetStem = joinGeneratedNominalStemFormulaPieces({
            objectPrefix,
            stemCore: preteritStem,
        }) || getGeneratedFormulaPathValue({ paths: sourceFrame.pathSlotFrames }, "base", "formulaMorph") || "STEM";
        connector = getGeneratedPreteritAgentiveConnectorDyad(surfaceSuffix)
            || getGeneratedFormulaPathValue({ paths: sourceFrame.pathSlotFrames }, "num1", "formulaMorph")
            || getGeneratedFormulaPathValue({ paths: sourceFrame.pathSlotFrames }, "num2", "formulaMorph")
            || "Ø-Ø";
        statePrefix = possessorPrefix ? `+${possessorPrefix}` : "";
        branch = "preterit-agentive-nominal-formula";
    } else {
        const activeActionStemCore = `${objectPrefix || ""}${stemCore || ""}`
            || getGeneratedFormulaPathValue({ paths: sourceFrame.pathSlotFrames }, "base", "formulaMorph");
        targetStem = sourceFrame.nominalizerInsideStem === true
            ? [
                activeActionStemCore,
                sourceFrame.suffixInsideStem === true ? surfaceSuffix : "",
                sourceFrame.nominalSuffix,
            ].map((piece) => String(piece || "").trim()).filter(Boolean).join("")
        ...
        targetStem = targetStem || getGeneratedFormulaPathValue({ paths: sourceFrame.pathSlotFrames }, "base", "formulaMorph") || "STEM";
        connector = sourceFrame.suffixInsideStem === true
            ? "Ø"
            : (surfaceSuffix || getGeneratedFormulaPathValue({ paths: sourceFrame.pathSlotFrames }, "num2", "surfaceValue") || "Ø");
    }
...
function getGeneratedNominalFormulaFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "generated-nominal-formula-render"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
    ) {
...
function buildGeneratedNominalFormulaSurfacePairs({
    ...
    const records = Array.isArray(cnvFormulaSurfacePath?.pathsBySurface)
        ? cnvFormulaSurfacePath.pathsBySurface
        : [];
    const pairs = records
        .map((record) => {
...
function normalizeGeneratedOutputResultTextRecords(records = []) {
    return (Array.isArray(records) ? records : [])
        .map((record, index) => ({
            ...
            segmentFrames: normalizeCnvSurfacePathSegments(record?.segments || [])
                .map((segment, segmentIndex) => ({
...
function getGeneratedOutputResultTextSourceSignature(sourceFrame = null) {
    ...
    return JSON.stringify({
        ...
        outputSurfaceRecordFrames: (Array.isArray(sourceFrame.outputSurfaceRecordFrames)
            ? sourceFrame.outputSurfaceRecordFrames
            : []
        ).map((record) => ({
            surface: record.surface || "",
            segmentFrames: record.segmentFrames || [],
...
function buildGeneratedOutputResultTextSourceFrame({
    ...
    const sourceForms = (Array.isArray(surfaceForms) ? surfaceForms : [])
        .map((form) => String(form || "").trim())
        .filter(Boolean);
    const outputSurfaceRecordFrames = normalizeGeneratedOutputResultTextRecords(outputSurfaceRecords);
    const targetSurfaceForms = outputSurfaceRecordFrames
        .map((record) => record.surface)
        .filter((surface, index, list) => surface && list.indexOf(surface) === index);
    const sourceFormsMatchRecords = sourceForms.length === targetSurfaceForms.length
        && sourceForms.every((form, index) => form === targetSurfaceForms[index]);
    const diagnostics = [];
    ...
    const sourceFrame = {
        ...
        supported: sourceForms.length > 0
            && outputSurfaceRecordFrames.length > 0
            && sourceFormsMatchRecords,
        diagnostics,
        consumesRenderedInput: false,
        displayStringsAuthorizeGrammar: false,
...
function buildGeneratedOutputResultTextOperationFrame(sourceFrame = null) {
    ...
    const targetSurfaceForms = (Array.isArray(sourceFrame.outputSurfaceRecordFrames)
        ? sourceFrame.outputSurfaceRecordFrames
        : []
    ).map((record) => String(record?.surface || "").trim()).filter(Boolean);
    const targetFrame = {
...
function validateGeneratedOutputResultTextOperationFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "generated-output-result-text-source-frame"
        || String(sourceFrame.sourceSignature || "") !== String(rebuiltSourceFrame.sourceSignature || "")
        || !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "generated-output-result-text-render"
        || operationFrame.supported !== true
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || String(operationFrame.sourceSignature || "") !== String(sourceFrame.sourceSignature || "")
...
function executeNuclearClauseSurfaceRequest(request = {}) {
    ...
    if (
        ...
        const andrewsCnvTenseLogicAuthorityFrame = getAndrewsCnvTenseLogicAuthorityFrame(tense);
        const andrewsCnvTenseLogicGenerationGate = getAndrewsCnvTenseLogicGenerationGateFrame(
    ...
    const getGeneratedSurfaceTextVariants = (surfaceSource = "") => {
        ...
        const sourceSegmentFrames = surfaceRecord
            ? normalizeCnvSurfacePathSegments(surfaceRecord.segments || [])
            : [];
        const normalizedSurface = String(
            surfaceRecord && sourceSegmentFrames.length
                ? sourceSegmentFrames.map((segment) => String(segment?.value || "")).join("")
                : (surfaceRecord ? surfaceRecord.surface || "" : surfaceSource || "")
        ).trim();
        ...
        if (
            typeof expandOptionalParentheticalForms === "function"
            && typeof buildOptionalParentheticalFormsSourceFrame === "function"
            && typeof buildOptionalParentheticalFormsOperationFrame === "function"
        ) {
            const optionalParentheticalSourceFrame = buildOptionalParentheticalFormsSourceFrame([normalizedSurface], {
                sourceKind: surfaceRecord
                    ? "generated-output-surface-segment-record"
                    : "generated-output-surface-text",
                sourceSegmentFrames,
            });
            const optionalParentheticalOperationFrame = buildOptionalParentheticalFormsOperationFrame(
                optionalParentheticalSourceFrame
            );
            variants = expandOptionalParentheticalForms([normalizedSurface], {
    ...
    const collectGeneratedOutputSurfaceRecord = (record = null) => {
        ...
        const troncoVariants = troncoSegmentIndex >= 0
            ? getGeneratedSurfaceTextVariants({
    ...
    const typedCompoundOperationFrame = override?.typedCompoundOperationFrame
        && typeof override.typedCompoundOperationFrame === "object"
        ? override.typedCompoundOperationFrame
        : null;
    const typedCompoundTargetStem = String(typedCompoundOperationFrame?.targetFrame?.stem || "").trim();
    if (typedCompoundOperationFrame) {
        const typedCompoundExpectedTargetStem = [
            typedCompoundOperationFrame.sourceFrame?.root,
            typedCompoundOperationFrame.matrixFrame?.root,
        ].map((entry) => String(entry || "").trim()).join("");
        const typedCompoundFrameMissing = (
            typedCompoundOperationFrame.operationFrame?.kind !== "andrews-typed-operation-frame"
            || !typedCompoundOperationFrame.sourceFrame?.root
            || !typedCompoundOperationFrame.matrixFrame?.root
            || !typedCompoundTargetStem
        );
        const typedCompoundFrameContradiction = Boolean(
            !typedCompoundFrameMissing
            && typedCompoundExpectedTargetStem
            && typedCompoundTargetStem !== typedCompoundExpectedTargetStem
        );
        if (
            typedCompoundFrameMissing
            || typedCompoundFrameContradiction
        ) {
            const diagnosticId = typedCompoundFrameContradiction
                ? "compound-continuation-contradictory-typed-operation-frame"
                : "compound-continuation-missing-typed-operation-frame";
            return buildNuclearClauseSurfaceBlockedResult({
                result: {
                    result: "—",
                    surfaceForms: [],
                ...
                message: typedCompoundFrameContradiction
                    ? "Compound continuation generation rejected a contradictory Andrews typed operation frame."
                    : "Compound continuation generation requires a complete Andrews typed operation frame.",
                diagnosticId,
    ...
    const classPerfectiveFormulaOperationFrame = buildGeneratedClassPerfectiveFormulaOperationFrame(
        classPerfectiveFormulaSourceFrame
    );
    ...
    const generatedOutputResultTextOperationFrame = buildGeneratedOutputResultTextOperationFrame(
    ...
    let nominalClauseMetadata = isNominalOutputProfile
    ...
    const primaryOutputSurfaceRecord = generatedOutputSurfaceRecords.find((record) => record.surface === forms[0])
        ...
        || null;
    const generatedAndrewsSourceStem = String(
    ...
    const generatedFormulaSurfacePairs = (
        Array.isArray(nominalClauseMetadata?.nominalizationProfile?.operationalSuboperationFrame?.formulaSurfacePairs)
        && nominalClauseMetadata.nominalizationProfile.operationalSuboperationFrame.formulaSurfacePairs.length
    )
        ? nominalClauseMetadata.nominalizationProfile.operationalSuboperationFrame.formulaSurfacePairs
        : (Array.isArray(cnvFormulaSurfacePath?.formulaSurfacePairs) ? cnvFormulaSurfacePath.formulaSurfacePairs : []);
    const generatedTargetFormulaEchoes = generatedFormulaSurfacePairs
        .map((entry) => String(entry?.targetFormulaEcho || "").trim())
        .filter(Boolean);
    const generatedSourceFormulaEchoes = generatedFormulaSurfacePairs
        .map((entry) => String(entry?.sourceFormulaEcho || "").trim())
        .filter(Boolean);
    const generatedAndrewsFormulaEchoes = generatedFormulaSurfacePairs
        .map((entry) => String(entry?.andrewsFormulaEcho || entry?.sourceFormulaEcho || "").trim())
        .filter(Boolean);
    const generatedConjugatorFormulaEchoes = generatedFormulaSurfacePairs
        .map((entry) => String(entry?.conjugatorFormulaEcho || entry?.targetFormulaEcho || "").trim())
        .filter(Boolean);
    const generatedSourceToTargetFormulaEcho = generatedFormulaSurfacePairs
        .map((entry) => String(entry?.sourceToTargetFormulaEcho || "").trim())
        .filter(Boolean)
        .join(" | ");
    const generatedAndrewsToConjugatorFormulaEcho = generatedFormulaSurfacePairs
        .map((entry) => String(entry?.andrewsToConjugatorFormulaEcho || entry?.sourceToTargetFormulaEcho || "").trim())
        .filter(Boolean)
        .join(" | ");
