...
function getLesson6DirectNawatObjectDyadFrame(obj1 = "", {
    ...
    const surfaceScopedPrefix = (
        normalized === "ki"
        && (normalizedSubjectPrefix === "ni" || normalizedSubjectPrefix === "ti")
    )
        ? "k"
        : normalized;
...
function stripCnvFormulaPreteritFoldedBasePrefixesWithTrace(base = "", formulaSlots = null) {
    return getCnvFormulaFoldableBasePrefixes(formulaSlots).reduce(
        (state, prefix) => {
...
function getCnvFormulaPreteritFoldedSurfaceSlots(formulaSlots = null, segments = []) {
    ...
    const hasSurfacePers2 = normalizedSegments.some((segment) => (
        (segment.role === "pers2" || segment.slot === "pers2")
    ...
    const split = splitGeneratedPreteritCnvFoldedConnector(tronco, sourceSubjectSuffix)
        || splitGeneratedPreteritCnvFoldedConnector(tronco, "");
    if (!split) {
        return null;
    }
    ...
    const [linearVa1, linearVa2] = objectMorph.includes("-")
        ? splitCnvFormulaSubslots(objectMorph)
        : [objectMorph, ""];
...
function buildCnvFormulaSurfacePathRecord({
    ...
    const paths = buildCnvFormulaAndrewsPathSlots(formulaSlots).map((pathSlot) => {
        ...
        const foldedSurfaceValue = preteritFoldedSlots
            && Object.prototype.hasOwnProperty.call(preteritFoldedSlots, slotKey)
            ? preteritFoldedSlots[slotKey]
            : null;
        ...
        return {
            ...
            surfaceRealizations: Array.isArray(surfaceRealizationsBySlot[slotKey])
                ...
                : [],
            surfaceCopyRelations: slotKey === "base" && Array.isArray(preteritFoldedSlots?.baseCopyRelations)
                ? preteritFoldedSlots.baseCopyRelations.map((relation) => ({ ...relation }))
                : [],
...
function executeNuclearClauseSurfaceRequest(request = {}) {
    ...
    const formulaStemContext = { stem: formulaStemBeforeInflection, pers1: inputPers1 };
    const realizedFormulaObj1Slot = String(
        isReflexive ? "mu" : (appliedMorphology?.objectPrefix ?? morphologyObj1Slot)
    );
    ...
    const formulaObj1BeforeInflection = isReflexive
        ? formulaReflexiveBeforeInflection
        : getLesson6DirectNawatFormulaObjectPrefix(foldedPreteritFormulaObj1Slot, formulaStemContext);
    ...
    let formulaShellVerb = stripGeneratedVncFormulaTenseSuffix(
        isNonactive ? troncoSlot : formulaStemBeforeInflection,
        tense,
        inputPers2
    );
    ...
    const nominalClauseMetadata = isNominalOutputProfile
        ...
        : {};
    const formulaPers1BeforeInflection = (
        tense === "optativo"
        && (inputPers1 === "ti" || inputPers1 === "an")
        && (appliedMorphology?.pers1 === "shi" || appliedMorphology?.subjectPrefix === "shi")
    )
        ? "shi"
        : inputPers1;
    ...
    const resultPayload = {
        ...
        slotNameBridge: typeof buildNuclearClauseSurfaceSlotNameBridge === "function"
            ? buildNuclearClauseSurfaceSlotNameBridge(posicionesFormula)
            : null,
        soundSpellingFrames: generatedSoundSpellingFrames,
        orthographyFrame: {
