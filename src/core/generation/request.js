...
const CNV_FORMULA_SLOT_KEYS = Object.freeze([
    "pers1",
    "pers2",
    "va1",
    "va2",
...
function normalizeNuclearClauseSurfaceFormulaPositions(posicionesFormula = null, fallbackPosicionesFormula = {}, options = {}) {
    ...
    const normalized = {
        ...
        [NUCLEAR_CLAUSE_SURFACE_ANDREWS_SLOT_KEYS.reflexive]: resolveNuclearClauseSurfaceSlotInputValue(
            source,
            NUCLEAR_CLAUSE_SURFACE_ANDREWS_SLOT_KEYS.reflexive,
            obj1 === "mu" ? "mu" : fallback[NUCLEAR_CLAUSE_SURFACE_ANDREWS_SLOT_KEYS.reflexive]
        ),
        [NUCLEAR_CLAUSE_SURFACE_ANDREWS_SLOT_KEYS.tensePosition]: resolveNuclearClauseSurfaceSlotInputValue(
            source,
...
function resolveNuclearClauseSurfaceFormulaObjectFrame(source = {}) {
    ...
    const dyadFrame = typeof getLesson6DirectNawatObjectDyadFrame === "function"
        ? getLesson6DirectNawatObjectDyadFrame(object, {
            stem,
            pers1: source[NUCLEAR_CLAUSE_SURFACE_ANDREWS_SLOT_KEYS.pers1] || "",
        })
        : null;
    if (dyadFrame?.formulaPosition === "va") {
        return {
            slots: ["va"],
            values: { va: dyadFrame.va || dyadFrame.visibleFormulaPrefix || object },
        };
    }
    ...
    if (va1 || va2) {
        return object.includes("-")
            ? { slots: ["va1", "va2"], values: { va1, va2 } }
            : { slots: ["va"], values: { va: object } };
    }
    return { slots: [], values: {} };
}
...
function resolveNuclearClauseSurfaceFormulaNumberValues(source = {}) {
    const raw = String(
        source[NUCLEAR_CLAUSE_SURFACE_ANDREWS_SLOT_KEYS.num2]
        || source[NUCLEAR_CLAUSE_SURFACE_ANDREWS_SLOT_KEYS.pers2]
        || ""
    ).trim();
    ...
    if (!raw) {
        return { num1: "Ø", num2: "Ø" };
    }
    if (raw.includes("-")) {
        const [num1, num2] = splitNuclearClauseSurfaceFormulaSubslots(raw);
        return { num1: num1 || "Ø", num2: num2 || "Ø" };
    }
    if (raw === "kan") {
        return { num1: "k", num2: "an" };
    }
    return { num1: "Ø", num2: raw };
}
...
function buildNuclearClauseSurfaceAndrewsFormulaPath(source = {}) {
    ...
    return ["pers1", "pers2", ...objectFrame.slots, "base", "tns", "num1", "num2"]
        .map((slot) => ({
            slot,
            value: values[slot] ?? "",
            formulaFeatures: objectFrame.features?.[slot] || null,
            visibleLinearMorph: objectFrame.visibleLinearMorph || "",
            formulaOptions: slot === "num1"
                ? (numberValues.num1Options || [])
                : (slot === "num2" ? (numberValues.num2Options || []) : []),
            formulaDyadOptions: slot === "num1" || slot === "num2"
                ? (numberValues.connectorOptions || [])
                : [],
            andrewsConnectorPattern: slot === "num1" || slot === "num2"
                ? (numberValues.andrewsConnectorPattern || "")
                : "",
            nawatConnectorPattern: slot === "num1" || slot === "num2"
                ? (numberValues.nawatConnectorPattern || "")
                : "",
        }));
}
...
function getNuclearClauseSurfaceCandidateModeFromOverride(override = null) {
    ...
    const activeMode = typeof getActiveTenseMode === "function" ? getActiveTenseMode() : "";
...
function applyNuclearClauseSurfaceEntradaCandidateSlots(posicionesFormula = null, derivedFormulaSlots = null) {
    ...
    const conflictSlots = slotKeys.filter((key) => (
        slots[key]
        && current[key]
        && current[key] !== slots[key]
    ));
...
function deriveNuclearClauseSurfaceFormulaPositionsFromEntradaCandidate({
    ...
    const surfaceCandidateFrame = typeof getAndrewsCnvCnnSurfaceInputCandidateFrame === "function"
        ? getAndrewsCnvCnnSurfaceInputCandidateFrame(rawInput, getAndrewsCnvCnnStemStageForMode(resolvedMode))
        : null;
