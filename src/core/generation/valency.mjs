export function createGenerationValencyApi(targetObject = globalThis) {
    function shouldDropBoundObjectPrefix(parsedVerb) {
        ...
        if ((parsedVerb.derivationValencyDelta || 0) > 0
            || parsedVerb.derivationType === targetObject.DERIVATION_TYPE?.causative
            || parsedVerb.derivationType === targetObject.DERIVATION_TYPE?.applicative) {
            return false;
        }
        ...
        return boundPrefixes.some((prefix) => (
            targetObject.SPECIFIC_VALENCE_PREFIX_SET?.has(prefix)
            || targetObject.OBJECT_MARKERS?.has(prefix)
            || targetObject.FUSION_PREFIXES?.has(prefix)
        ));
    }
    ...
    function applyObjectAllomorphy({
        ...
        const allomorphyObjectPrefix = !isPassiveImpersonalMode
            && targetObject.isSamePersonReflexive(subjectPrefix, subjectSuffix, objectPrefix)
            ? "mu"
            : objectPrefix;
    ...
    function resetSubjectForNounTenses(tense, override, subjectPrefix, subjectSuffix) {
        if (tense === "sustantivo-verbal"
            || targetObject.isPotencialProfileTense(tense)
            || tense === "agentivo"
            || tense === "patientivo") {
    ...
    return {
        shouldDropBoundObjectPrefix,
        shouldDropOccupiedSourceObjectPrefix,
        applyBoundMarkerPrefixOverrides,
        applyObjectAllomorphy,
        applyPassiveImpersonalValencyAdjustments,
        resetSubjectForNounTenses,
        applyPassiveImpersonalOverrides,
