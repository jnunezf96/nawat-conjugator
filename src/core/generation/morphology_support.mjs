...
export function createMorphologySupportModule(targetObject = globalThis) {
    ...
    function buildWalDirectionalPlan({
      ...
      const hasRuntimeSecondValent = Boolean(normalizedObj1Base || normalizedObj2 || normalizedObj3 || isTaFusion);
      const hasRuntimeSpecificObject = [normalizedObj1Base, normalizedObj2, normalizedObj3].some(value => targetObject.SPECIFIC_VALENCE_PREFIX_SET.has(value) || value === "k");
    ...
    function realizeWalDirectionalChain({
      ...
      const directionalPlan = meta.directionalPlan && typeof meta.directionalPlan === "object" ? meta.directionalPlan : buildWalDirectionalPlan({
      ...
      const directionalOutputPrefix = String(directionalPlan.directionalOutputPrefix || meta.directionalOutputPrefix || directionalInputPrefix || "wal");
      const effectiveDirectionalRuleMode = Object.prototype.hasOwnProperty.call(directionalPlan, "effectiveDirectionalRuleMode") ? String(directionalPlan.effectiveDirectionalRuleMode || "") : String(directionalRuleMode || "");
      ...
      const hasThirdPersonObject = [baseObj1, baseObj2, baseObj3].some(value => isWalThirdPersonMarker(value));
      const hasNonspecificObject = [baseObj1, baseObj2, baseObj3].some(value => targetObject.NONSPECIFIC_VALENCE_AFFIX_SET.has(value));
      ...
      if (!shouldUseAl) {
        ...
        realizedObj1 = targetObject.normalizeValenceMarkerOrder(`${directionalOutputPrefix}${markerChain}`);
        return {
          pers1: realizedPers1,
          obj1: realizedObj1,
      ...
      if (isMainlineThirdPersonObject) {
        ...
        realizedObj1 = markerHead.startsWith("k") ? `k${realizedDirectionalPrefix}${markerHead.slice(1)}` : `${realizedDirectionalPrefix}${markerHead}`;
        ...
        const shouldDropLeadK = startsWithK && (isFirstPersonSubject || isSecondPersonSubject || isThirdPersonSubject && effectiveDirectionalRuleMode === "nonspecific");
    ...
    api.isWalThirdPersonMarker = isWalThirdPersonMarker;
    api.buildDirectionalMarkerChain = buildDirectionalMarkerChain;
    api.buildWalDirectionalPlan = buildWalDirectionalPlan;
