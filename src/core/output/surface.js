...
function normalizeSurfaceChainStructuredFrames(frames = []) {
    return (Array.isArray(frames) ? frames : [])
        .map((frame) => cloneSurfaceChainStructuredFrame(frame))
        .filter((frame) => frame && typeof frame === "object" && frame.kind);
}
...
function buildSurfaceChainFinalIAUATrimSourceFrame({
    ...
    const sourceSuffix = requested
        ? (IA_UA_SUFFIXES.find((suffix) => sourceStem.endsWith(suffix)) || "")
        : "";
...
function buildSurfaceChainFinalIAUATrimOperationFrame(sourceFrame = null) {
    ...
    const supported = sourceFrame.supported === true
        && Boolean(sourceStem && sourceSuffix && targetStem)
        && targetStem === sourceStem.slice(0, -1);
    return {
...
function buildSurfaceChainObjectIInitialElisionOperationFrame(sourceFrame = null) {
    ...
    const targetStem = String(sourceFrame.targetStem || "");
    const supported = sourceFrame.supported === true
        && Boolean(objectPrefix && sourceStem && targetStem)
...
function buildSurfaceChainSubjectIInitialReductionSourceFrame({
    ...
    const targetPrefix = subjectPrefix === "ni"
        ? "n"
        : (subjectPrefix === "ti" ? "t" : "");
...
function buildSurfaceChainSubjectIInitialReductionOperationFrame(sourceFrame = null) {
    ...
    const supported = sourceFrame.supported === true
        && (subjectPrefix === "ni" || subjectPrefix === "ti")
        && !objectPrefix
        && sourceStem.startsWith("i")
        && ((subjectPrefix === "ni" && targetPrefix === "n") || (subjectPrefix === "ti" && targetPrefix === "t"));
    return {
...
function buildSurfaceChainOptativeKiReductionSourceFrame({
    ...
    const nextSegmentStartsWithVowel = Boolean(nextSegmentValue && VOWEL_START_RE.test(nextSegmentValue));
    const diagnostics = [];
    if (!requested) {
...
function buildSurfaceChainOptativeKiReductionOperationFrame(sourceFrame = null) {
    ...
    const supported = sourceFrame.supported === true
        ...
        && Boolean(nextSegmentValue)
        && !VOWEL_START_RE.test(nextSegmentValue);
    return {
...
function buildSurfaceChainKContactSourceFrame({
    ...
    const sourceStemInitial = sourceStem.startsWith("kw")
        ? "kw"
        : (sourceStem.startsWith("k") ? "k" : "");
    const kContactKind = sourceObjectFinal && sourceStemInitial
        ? (sourceStemInitial === "kw" ? "k-before-kw" : "k-before-k")
        : "";
    const targetObjectPrefix = kContactKind === "k-before-kw"
        ? objectPrefix.slice(0, -1)
        : objectPrefix;
    const targetStem = kContactKind === "k-before-k"
        ? sourceStem.slice(1)
        : sourceStem;
    const diagnostics = [];
...
function buildSurfaceChainKContactOperationFrame(sourceFrame = null) {
    ...
    const targetStem = String(sourceFrame.targetStem || "");
    const supported = sourceFrame.supported === true
        && objectPrefix.endsWith("k")
        && (sourceStem.startsWith("kw") || sourceStem.startsWith("k"))
        && (
            (kContactKind === "k-before-kw" && targetObjectPrefix === objectPrefix.slice(0, -1) && targetStem === sourceStem)
            || (kContactKind === "k-before-k" && targetObjectPrefix === objectPrefix && targetStem === sourceStem.slice(1))
        );
    return {
...
function buildSurfaceChainKwCodaCoalescenceOperationFrame(sourceFrame = null) {
    ...
    const target = getSurfaceChainKwCodaCoalescenceTarget(sourceSegmentValue);
    const supported = sourceFrame.supported === true
        && Boolean(segmentRole && sourceSegmentValue && targetSegmentValue)
...
function buildSurfaceChainNhBeforeVowelSourceFrame({
    ...
    const nextInitial = sourceNextValue && VOWEL_START_RE.test(sourceNextValue) ? sourceNextValue.slice(0, 1) : "";
    const targetCurrentValue = sourceFinal && previousLetter && VOWEL_RE.test(previousLetter) && nextInitial
        ? `${sourceCurrentValue}h`
        : sourceCurrentValue;
    const diagnostics = [];
    ...
    if (sourceCurrentValue && sourceFinal && !VOWEL_RE.test(previousLetter)) {
    ...
    return {
        ...
        priorOperationFrameKinds: (Array.isArray(priorOperationFrameKinds) ? priorOperationFrameKinds : [])
        ...
        supported: Boolean(
            ...
            && normalizedNextRole
            && sourceFinal
            && VOWEL_RE.test(previousLetter)
            && nextInitial
            && targetCurrentValue !== sourceCurrentValue
        ),
        diagnostics,
        displayOnlyFieldsExcluded: ["formulaEcho", "result", "surface", "surfaceForms"],
    };
...
function buildSurfaceChainNhBeforeVowelOperationFrame(sourceFrame = null) {
    ...
    const previousLetter = sourceCurrentValue.length >= 2 ? sourceCurrentValue[sourceCurrentValue.length - 2] || "" : "";
    const supported = sourceFrame.supported === true
        && Boolean(currentRole && nextRole && sourceCurrentValue && sourceNextValue && targetCurrentValue)
        && sourceCurrentValue.endsWith("n")
        && !sourceCurrentValue.endsWith("nh")
        && VOWEL_RE.test(previousLetter)
        && VOWEL_START_RE.test(sourceNextValue)
        && targetCurrentValue === `${sourceCurrentValue}h`;
    return {
...
function buildSurfaceChainYShiftOperationFrame(sourceFrame = null) {
    ...
    const supported = sourceFrame.supported === true
        && sourceFrame.preserveCodaY !== true
        && Boolean(segmentRole && sourceSegmentValue && targetSegmentValue)
        && target.supported === true
        && targetSegmentValue === target.targetSegmentValue
        && String(sourceFrame.targetCoda || "") === target.replacementTarget;
    return {
...
function getSurfaceChainMCodaAssimilationTarget({
    ...
    const finalMBeforeVowel = sourceValue.endsWith("m") && VOWEL_START_RE.test(nextValue);
...
function buildSurfaceChainOperationFrames(input = {}) {
    ...
    if (subjectIInitialReductionSourceFrame.supported) {
        const subjectIInitialReductionOperationFrame = buildSurfaceChainSubjectIInitialReductionOperationFrame(
            subjectIInitialReductionSourceFrame
        );
    ...
    if (optativeKiReductionSourceFrame.requested) {
        const optativeKiReductionOperationFrame = buildSurfaceChainOptativeKiReductionOperationFrame(
            optativeKiReductionSourceFrame
        );
    ...
    ["pers1", "poseedor", "prefijoExternoFuente", "obj1", "tronco"].forEach((role) => {
        ...
        if (kwCodaCoalescenceSourceFrame.supported) {
            const kwCodaCoalescenceOperationFrame = buildSurfaceChainKwCodaCoalescenceOperationFrame(
                kwCodaCoalescenceSourceFrame
            );
    ...
    const mAssimilationSourceSegments = ["pers1", "poseedor", "prefijoExternoFuente", "obj1", "tronco"]
        .map((role) => ({
            role,
            slot: role,
    ...
    mAssimilationSourceSegments.forEach((segment, index) => {
        ...
        if (mCodaAssimilationSourceFrame.supported) {
            const mCodaAssimilationOperationFrame = buildSurfaceChainMCodaAssimilationOperationFrame(
                mCodaAssimilationSourceFrame
            );
    ...
    const sourceSegments = ["pers1", "poseedor", "prefijoExternoFuente", "obj1", "tronco"]
        .map((role) => ({
            role,
            slot: role,
            value: String(node[role] || ""),
        }));
    const nhStageSegments = sourceSegments.map((segment) => ({ ...segment }));
    ...
    nhStageSegments.forEach((segment, index) => {
        ...
        if (nhBeforeVowelSourceFrame.supported) {
            const nhBeforeVowelOperationFrame = buildSurfaceChainNhBeforeVowelOperationFrame(
                nhBeforeVowelSourceFrame
            );
    ...
    if (objectIInitialElisionSourceFrame.requested) {
        const objectIInitialElisionOperationFrame = buildSurfaceChainObjectIInitialElisionOperationFrame(
            objectIInitialElisionSourceFrame
        );
    ...
    if (muIskaliaReductionSourceFrame.requested) {
        const muIskaliaReductionOperationFrame = buildSurfaceChainMuIskaliaReductionOperationFrame(
            muIskaliaReductionSourceFrame
        );
...
function getSurfaceChainOperationFrame(chain = null, kind = "") {
    ...
    return normalizeSurfaceChainStructuredFrames(chain?.surfaceOperationFrames)
        .find((frame) => frame.kind === requestedKind) || null;
}
...
function validateSurfaceChainRenderOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-render-operation-frame"
    ...
    const segmentsMatch = sourceSegments.every((segment, index) => {
        ...
        return (
            String(segment.role || "") === String(currentSegment.role || "")
            && String(segment.slot || "") === String(currentSegment.slot || "")
            && String(segment.value || "") === String(currentSegment.value || "")
        );
    });
    if (!segmentsMatch) {
        return null;
    ...
    if (
        String(targetFrame.surface || "") !== surface
        || Number(targetFrame.segmentCount || 0) !== currentSegments.length
    ) {
        return null;
    }
    return {
...
function validateSurfaceChainFinalIAUATrimOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-final-ia-ua-trim-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    ...
    if (!IA_UA_SUFFIXES.includes(sourceSuffix) || !sourceStem.endsWith(sourceSuffix)) {
        return null;
    }
...
function validateSurfaceChainObjectIInitialElisionOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-object-i-initial-elision-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
...
function validateSurfaceChainMuIskaliaReductionOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-mu-iskalia-reduction-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
...
function validateSurfaceChainSubjectIInitialReductionOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-subject-i-initial-reduction-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
...
function validateSurfaceChainOptativeKiReductionOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-optative-ki-reduction-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    ...
    const currentNextRole = nextIndex >= 0
        ? normalizeOutputSurfaceRole(segments[nextIndex]?.role || segments[nextIndex]?.slot || "")
        : "";
    ...
    if (VOWEL_START_RE.test(currentNextValue)) {
        return null;
    }
    return {
...
function validateSurfaceChainKContactOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-k-contact-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
...
function validateSurfaceChainKwCodaCoalescenceOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-kw-coda-coalescence-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    ...
    if (
        normalizeOutputSurfaceRole(targetFrame.segmentRole || targetFrame.segmentSlot || "") !== segmentRole
        || String(targetFrame.segmentSlot || segmentSlot || "") !== segmentSlot
        || String(targetFrame.sourceSegmentValue || "") !== sourceSegmentValue
        || String(targetFrame.sourceCoda || "") !== "kw"
        || String(targetFrame.targetCoda || "") !== "k"
    ) {
        return null;
    }
    const target = getSurfaceChainKwCodaCoalescenceTarget(sourceSegmentValue);
    if (
        target.supported !== true
        || targetSegmentValue !== target.targetSegmentValue
        || Number(targetFrame.coalescenceSiteCount || 0) !== target.coalescenceSiteCount
    ) {
        return null;
    }
    return {
...
function validateSurfaceChainNhBeforeVowelOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-nh-before-vowel-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    ...
    const currentIndex = segments.findIndex((segment) => (
        normalizeOutputSurfaceRole(segment?.role || segment?.slot || "") === currentRole
        || segment?.slot === currentSlot
    ));
    if (currentIndex < 0) {
        return null;
    }
    ...
    if (
        currentSegmentValue !== sourceCurrentValue
        || currentNextValue !== sourceNextValue
    ...
    if (
        normalizeOutputSurfaceRole(targetFrame.currentRole || targetFrame.currentSlot || "") !== currentRole
        || normalizeOutputSurfaceRole(targetFrame.nextRole || targetFrame.nextSlot || "") !== nextRole
        || String(targetFrame.currentSlot || currentSlot || "") !== currentSlot
        || String(targetFrame.nextSlot || nextSlot || "") !== nextSlot
        || String(targetFrame.sourceCurrentValue || "") !== sourceCurrentValue
        || String(targetFrame.sourceNextValue || "") !== sourceNextValue
        || String(targetFrame.sourceFinal || "") !== "n"
        || String(targetFrame.targetFinal || "") !== "nh"
    ) {
        return null;
    }
    const previousLetter = sourceCurrentValue.length >= 2 ? sourceCurrentValue[sourceCurrentValue.length - 2] || "" : "";
...
function validateSurfaceChainYShiftOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-y-coda-shift-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    ...
    if (
        ...
        || String(targetFrame.segmentSlot || segmentSlot || "") !== segmentSlot
        || String(targetFrame.sourceSegmentValue || "") !== sourceSegmentValue
        || String(targetFrame.objectSegmentValue || "") !== objectSegmentValue
        || targetFrame.isTransitive !== (objectSegmentValue !== "")
        || String(targetFrame.sourceCoda || "") !== "y"
    ) {
        return null;
    }
    const target = getSurfaceChainYShiftTarget({
    ...
    if (
        target.supported !== true
        || targetSegmentValue !== target.targetSegmentValue
        || String(targetFrame.targetCoda || "") !== target.replacementTarget
        || Number(targetFrame.shiftSiteCount || 0) !== target.shiftSiteCount
    ) {
        return null;
    }
    return {
...
function validateSurfaceChainMCodaAssimilationOperationFrame(chain = null, operationFrame = null) {
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-surface-chain-m-coda-assimilation-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    ...
    const segmentIndex = segments.findIndex((segment) => (
        normalizeOutputSurfaceRole(segment?.role || segment?.slot || "") === segmentRole
        || segment?.slot === segmentSlot
    ));
    if (segmentIndex < 0) {
        return null;
    }
    ...
    const currentNextSlot = nextIndex >= 0 ? String(segments[nextIndex]?.slot || currentNextRole || "") : "";
    if (
        currentSegmentValue !== sourceSegmentValue
        || currentNextValue !== sourceNextValue
        || currentNextRole !== nextRole
        || currentNextSlot !== nextSlot
    ) {
    ...
    if (
        ...
        || String(targetFrame.sourceSegmentValue || "") !== sourceSegmentValue
        || String(targetFrame.sourceNextValue || "") !== sourceNextValue
        || String(targetFrame.sourceCoda || "") !== "m"
        || String(targetFrame.targetCoda || "") !== "n"
    ) {
        return null;
    }
    const target = getSurfaceChainMCodaAssimilationTarget({
    ...
    if (
        target.supported !== true
        || targetSegmentValue !== target.targetSegmentValue
        || Number(targetFrame.assimilationSiteCount || 0) !== target.assimilationSiteCount
    ) {
        return null;
    }
    return {
...
function realizeSurfaceChainSubjectIInitialReduction(chain = null) {
    ...
    const operationFrame = getSurfaceChainOperationFrame(
        nextChain,
        "andrews-surface-chain-subject-i-initial-reduction-operation-frame"
    );
...
function realizeSurfaceChainFinalIAUATrim(chain = null) {
    ...
    const operationFrame = getSurfaceChainOperationFrame(
        nextChain,
        "andrews-surface-chain-final-ia-ua-trim-operation-frame"
    );
...
function realizeSurfaceChainOptativeKiReduction(chain = null) {
    ...
    const operationFrame = getSurfaceChainOperationFrame(
        nextChain,
        "andrews-surface-chain-optative-ki-reduction-operation-frame"
    );
...
function realizeSurfaceChainMuIskaliaReduction(chain = null) {
    ...
    const operationFrame = getSurfaceChainOperationFrame(
        nextChain,
        "andrews-surface-chain-mu-iskalia-reduction-operation-frame"
    );
...
function realizeSurfaceChainObjectIInitialElision(chain = null) {
    ...
    const operationFrame = getSurfaceChainOperationFrame(
        nextChain,
        "andrews-surface-chain-object-i-initial-elision-operation-frame"
    );
...
function realizeSurfaceChainNhBeforeVowel(chain = null) {
    const nextChain = cloneSurfaceChainState(chain);
    const operationFrames = normalizeSurfaceChainStructuredFrames(nextChain.surfaceOperationFrames)
        .filter((frame) => frame.kind === "andrews-surface-chain-nh-before-vowel-operation-frame");
    operationFrames.forEach((operationFrame) => {
        ...
        setSurfaceChainSegmentValue(
            nextChain,
            validatedOperation.currentRole,
            validatedOperation.targetCurrentValue
        );
...
function realizeSurfaceChainKContact(chain = null) {
    ...
    const operationFrame = getSurfaceChainOperationFrame(
        nextChain,
        "andrews-surface-chain-k-contact-operation-frame"
    );
...
function realizeSurfaceChainKwCoalescence(chain = null) {
    const nextChain = cloneSurfaceChainState(chain);
    const operationFrames = normalizeSurfaceChainStructuredFrames(nextChain.surfaceOperationFrames)
        .filter((frame) => frame.kind === "andrews-surface-chain-kw-coda-coalescence-operation-frame");
    operationFrames.forEach((operationFrame) => {
        ...
        setSurfaceChainSegmentValue(
            nextChain,
            validatedOperation.segmentRole,
            validatedOperation.targetSegmentValue
        );
...
function realizeSurfaceChainYShift(chain = null) {
    ...
    const operationFrames = normalizeSurfaceChainStructuredFrames(nextChain.surfaceOperationFrames)
        .filter((frame) => frame.kind === "andrews-surface-chain-y-coda-shift-operation-frame");
    operationFrames.forEach((operationFrame) => {
...
function realizeSurfaceChainMCodaAssimilation(chain = null) {
    const nextChain = cloneSurfaceChainState(chain);
    const operationFrames = normalizeSurfaceChainStructuredFrames(nextChain.surfaceOperationFrames)
        .filter((frame) => frame.kind === "andrews-surface-chain-m-coda-assimilation-operation-frame");
    operationFrames.forEach((operationFrame) => {
...
function joinSurfaceChain(chain = null) {
    const renderFrame = chain?.surfaceRenderOperationFrame && typeof chain.surfaceRenderOperationFrame === "object"
        ? chain.surfaceRenderOperationFrame
        : null;
...
function getOptionalParentheticalFormTargets(form = "") {
    ...
    const expandOne = (value = "") => {
        ...
        const after = match[3] || "";
        return [
            ...expandOne(`${before}${after}`),
            ...expandOne(`${before}${optionalPart}${after}`),
        ];
    };
...
function buildOptionalParentheticalFormsSourceFrame(forms = [], options = {}) {
    const sourceForms = (Array.isArray(forms) ? forms : [])
        .map((form) => String(form || "").trim())
        .filter(Boolean);
    const sourceSegmentFrames = (Array.isArray(options.sourceSegmentFrames) ? options.sourceSegmentFrames : [])
        .map((segment, index) => ({
...
function validateOptionalParentheticalFormsOperationFrame(forms = [], sourceFrame = null, operationFrame = null) {
    if (
        !sourceFrame
        || sourceFrame.kind !== "optional-parenthetical-forms-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-optional-parenthetical-forms-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    const sourceForms = (Array.isArray(forms) ? forms : [])
        ...
        .filter(Boolean);
    if (
        sourceForms.length !== sourceFrame.sourceForms.length
        || sourceForms.some((form, index) => form !== sourceFrame.sourceForms[index])
    ) {
        return null;
    }
    ...
    if (
        targetForms.length !== expectedForms.length
        || targetForms.some((form, index) => form !== expectedForms[index])
    ) {
        return null;
    }
    return targetForms;
...
function expandOptionalParentheticalForms(forms, options = {}) {
    ...
    const targetForms = validateOptionalParentheticalFormsOperationFrame(
        sourceForms,
        options?.sourceFrame || null,
        options?.operationFrame || null
    );
    return targetForms || [];
}
...
function buildOptionalSupportiveMarkedSurfaceSourceFrame({
    ...
    const localPrecedingSurface = markerIndex >= 0
        ? `${String(precedingSurface || "")}${envelopeMarkedSurface.slice(0, markerIndex)}`
        : String(precedingSurface || "");
    const followingSurface = markerIndex >= 0
        ? envelopeMarkedSurface.slice(markerIndex + markerToken.length)
        : "";
    const diagnostics = [];
...
function buildOptionalSupportiveMarkedSurfaceOperationFrame(sourceFrame = null) {
    ...
    const yBehavior = markerLetter === "y"
        ? resolveOptionalSupportiveYBehavior({
            precedingSurface: String(sourceFrame.localPrecedingSurface || ""),
            followingSurface,
        })
        : { deleteMarker: false, deleteReduplicativeY: false };
    const resolvedFollowingSurface = yBehavior.deleteReduplicativeY
        ? dropReduplicativeYAfterVj(followingSurface)
        : followingSurface;
    const resolvedEnvelopeSurface = sourceFrame.supported === true
        ? (
        ...
        : envelopeMarkedSurface;
    const outputSurface = formatResolvedSupportiveMarkedSurface(
        resolvedEnvelopeSurface,
        sourceFrame.outputFormat || SUPPORTIVE_MARKER_FORMAT.envelope,
        preserveMarkers
    );
    const supported = sourceFrame.supported === true
        && markerLetter === "y"
        && Boolean(markerToken && markerIndex >= 0 && envelopeMarkedSurface);
    return {
...
function validateOptionalSupportiveMarkedSurfaceOperationFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "optional-supportive-marked-surface-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-optional-supportive-marked-surface-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    ...
    const sourceMatches = comparableKeys.every((key) => (
        sourceFrame[key] === rebuiltSourceFrame[key]
    ));
    ...
    const targetMatches = [
        ...
        "markerToken",
        "deleteMarker",
        "deleteReduplicativeY",
        "envelopeMarkedSurface",
        "resolvedEnvelopeSurface",
        "plainSurface",
        "outputSurface",
    ].every((key) => targetFrame[key] === expectedTargetFrame[key]);
    ...
    if (markerLetter === "y" && markerMatch) {
        ...
        if (!validatedTargetFrame) {
            return {
                ...
                supported: false,
                diagnostics: [
                    !sourceFrame ? "missing-optional-supportive-marked-surface-source-frame" : "",
                    !operationFrame ? "missing-optional-supportive-marked-surface-operation-frame" : "",
                    sourceFrame && operationFrame ? "invalid-optional-supportive-marked-surface-operation-frame" : "",
                ].filter(Boolean),
            };
        }
        return {
...
function buildOptionalSupportiveOutputVerbSourceFrame({
    ...
    const markedStem = supportiveRequested && supportiveLetter === "y"
    ...
    return {
        ...
        supported: Boolean(
            ...
            && supportiveLetter === "y"
            && sourceStem
            && markedSurfaceSourceFrame.supported
        ),
        diagnostics,
        displayOnlyFieldsExcluded: ["formulaEcho", "result", "surface", "surfaceForms"],
    };
...
function buildOptionalSupportiveOutputVerbOperationFrame(sourceFrame = null) {
    ...
    const markedSurfaceOperationFrame = buildOptionalSupportiveMarkedSurfaceOperationFrame(
    ...
    const supported = sourceFrame.supported === true
        && markedSurfaceOperationFrame?.supported === true
        && Boolean(targetStem);
    return {
...
function validateOptionalSupportiveOutputVerbOperationFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "optional-supportive-output-verb-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-optional-supportive-output-verb-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    ...
    const sourceMatches = [
        ...
        "precedingSurface",
        "markedStem",
    ].every((key) => sourceFrame[key] === rebuiltSourceFrame[key]);
    if (!sourceMatches || rebuiltSourceFrame.supported !== true) {
        return null;
    }
...
function resolveOptionalSupportiveOutputText({
    ...
    return String(value || "")
        ...
        .map((entry) => {
            ...
            const optionalSupportiveOperationFrame = buildOptionalSupportiveOutputVerbOperationFrame(
...
function attachOutputSurfaceGrammarContract(record = null, options = {}) {
    ...
    const surfaceOperationFrames = normalizeSurfaceChainStructuredFrames(
        options.surfaceOperationFrames
        || record.surfaceOperationFrames
        || options.segments?.surfaceOperationFrames
        || record.segments?.surfaceOperationFrames
    );
...
function cloneDerivedMuAlternateForms(alternateForms = []) {
    return (Array.isArray(alternateForms) ? alternateForms : [])
        .map((form) => (form && typeof form === "object" ? { ...form } : form));
}
...
function buildDerivedMuStemInteractionSourceFrame({
    ...
    return {
        ...
        alternateFormFrames: alternates.map((form, index) => ({
            ...
            source: form && typeof form === "object" ? { ...form } : form,
        })),
...
function realizeDerivedMuAlternateVerbForBranch(verb = "", branch = "", embeddedMarker = "") {
    ...
    if (
        ...
        && marker
        && sourceVerb.startsWith("mu")
        && !sourceVerb.startsWith(`mu${marker}`)
    ) {
...
function buildDerivedMuStemInteractionOperationFrame(sourceFrame = null) {
    ...
    const targetAlternateFormFrames = (Array.isArray(sourceFrame.alternateFormFrames)
        ? sourceFrame.alternateFormFrames
        : []
    ).map((frame) => {
    ...
    const changed = targetObjectPrefix !== objectPrefix
        || targetStem !== sourceStem
        || targetAlternateFormFrames.some((frame) => frame.targetVerb !== frame.sourceVerb);
...
function validateDerivedMuStemInteractionOperationFrame({
    ...
    if (
        !sourceFrame
        || sourceFrame.kind !== "derived-mu-stem-interaction-source-frame"
        || !operationFrame
        || operationFrame.kind !== "andrews-derived-mu-stem-interaction-operation-frame"
        || operationFrame.supported !== true
    ) {
        return null;
    }
    ...
    if (
        sourceFrame.enabled !== rebuiltSourceFrame.enabled
        || sourceFrame.objectPrefix !== rebuiltSourceFrame.objectPrefix
        || sourceFrame.sourceStem !== rebuiltSourceFrame.sourceStem
        || sourceFrame.alternateFormFrames.length !== rebuiltSourceFrame.alternateFormFrames.length
    ) {
        return null;
    }
    const alternateSourcesMatch = sourceFrame.alternateFormFrames.every((frame, index) => (
        frame.verb === rebuiltSourceFrame.alternateFormFrames[index]?.verb
        && frame.subjectSuffix === rebuiltSourceFrame.alternateFormFrames[index]?.subjectSuffix
        && frame.nominalSuffix === rebuiltSourceFrame.alternateFormFrames[index]?.nominalSuffix
    ));
    ...
    const expectedAlternates = Array.isArray(expectedTargetFrame.alternateFormFrames)
        ? expectedTargetFrame.alternateFormFrames
        : [];
    const alternatesMatch = targetAlternates.length === expectedAlternates.length
        && targetAlternates.every((frame, index) => (
            frame.sourceVerb === expectedAlternates[index]?.sourceVerb
            && frame.targetVerb === expectedAlternates[index]?.targetVerb
            && frame.subjectSuffix === expectedAlternates[index]?.subjectSuffix
            && frame.nominalSuffix === expectedAlternates[index]?.nominalSuffix
        ));
...
function realizeDerivedMuStemInteraction(input = {}) {
    ...
    if (!targetFrame) {
        return {
            ...
            supported: false,
            diagnostics: [
                !sourceFrame ? "missing-derived-mu-stem-interaction-source-frame" : "",
                !operationFrame ? "missing-derived-mu-stem-interaction-operation-frame" : "",
                sourceFrame && operationFrame ? "invalid-derived-mu-stem-interaction-operation-frame" : "",
            ].filter(Boolean),
        };
    }
    const targetAlternateFrames = Array.isArray(targetFrame.alternateFormFrames)
        ? targetFrame.alternateFormFrames
        : [];
