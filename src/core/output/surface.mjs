...
export function createOutputSurfaceGlobals(targetObject = globalThis) {
    ...
    function buildSurfaceChainState({
      ...
      return {
        ...
        segments: [{
          role: "subject",
          value: String(subjectPrefix || "")
        }, {
          role: "possessive",
          value: String(possessivePrefix || "")
        }, {
          role: "sourceOuter",
          value: sourceOuterPrefix
        }, {
          role: "object",
          value: String(objectPrefix || "")
        }, {
          role: "verb",
          value: String(verb || "")
        }]
    ...
    function realizeSurfaceChainFinalIAUATrim(chain = null) {
      ...
      if (!endsWithAny(verbValue, targetObject.IA_UA_SUFFIXES)) {
        return nextChain;
      }
    ...
    function realizeSurfaceChainImperativeKiReduction(chain = null) {
      ...
      if (!nextValue || targetObject.VOWEL_START_RE.test(nextValue)) {
        return nextChain;
      }
    ...
    function realizeSurfaceChainNhBeforeVowel(chain = null) {
      ...
      for (let index = 0; index < segments.length - 1; index += 1) {
        ...
        if (!targetObject.VOWEL_RE.test(prevChar) || current.endsWith("nh")) {
          continue;
        }
    ...
    function joinSurfaceChain(chain = null) {
      return (Array.isArray(chain?.segments) ? chain.segments : []).map(segment => String(segment?.value || "")).join("");
    }
    ...
    function expandOptionalParentheticalForms(forms) {
      ...
      const expandOne = form => {
        ...
        return [...expandOne(withoutOptional), ...expandOne(withOptional)];
      };
      forms.forEach(form => {
    ...
    function getSupportiveMarkerTokenForLetter(letter = "", format = targetObject.SUPPORTIVE_MARKER_FORMAT.legacy) {
      const normalized = String(letter || "").trim().toLowerCase();
      const resolvedLetter = normalized === "y" ? "y" : "i";
      if (format === targetObject.SUPPORTIVE_MARKER_FORMAT.regex || format === targetObject.SUPPORTIVE_MARKER_FORMAT.screen) {
    ...
    function getSupportiveMarkerLetterFromValue(value = "", format = targetObject.SUPPORTIVE_MARKER_FORMAT.legacy) {
      const source = String(value || "");
      const match = format === targetObject.SUPPORTIVE_MARKER_FORMAT.regex || format === targetObject.SUPPORTIVE_MARKER_FORMAT.screen ? source.match(targetObject.REGEX_OPTIONAL_SUPPORTIVE_MARKER_DETECT_RE) : source.match(targetObject.OPTIONAL_SUPPORTIVE_MARKER_DETECT_RE);
      return match ? String(match[1] || match[2] || "").toLowerCase() : "";
    }
    function replaceSupportiveMarkersWithLetters(value = "", format = targetObject.SUPPORTIVE_MARKER_FORMAT.legacy) {
      const source = String(value || "");
      const pattern = format === targetObject.SUPPORTIVE_MARKER_FORMAT.regex || format === targetObject.SUPPORTIVE_MARKER_FORMAT.screen ? targetObject.REGEX_OPTIONAL_SUPPORTIVE_MARKER_RE : targetObject.OPTIONAL_SUPPORTIVE_MARKER_RE;
    ...
    function getOptionalSupportiveMarkerForLetter(letter = "") {
      return getSupportiveMarkerTokenForLetter(letter, targetObject.SUPPORTIVE_MARKER_FORMAT.legacy);
    }
    function getOptionalSupportiveMarkerLetter(value = "") {
      return getSupportiveMarkerLetterFromValue(value, targetObject.SUPPORTIVE_MARKER_FORMAT.legacy);
    }
    ...
    function replaceOptionalSupportiveMarkersWithLetters(value = "") {
      return replaceSupportiveMarkersWithLetters(value, targetObject.SUPPORTIVE_MARKER_FORMAT.legacy);
    }
    ...
    function markReduplicativeYAfterVj(followingSurface = "", format = targetObject.SUPPORTIVE_MARKER_FORMAT.legacy) {
      const normalized = normalizeSupportiveYContextSurface(followingSurface);
      if (!normalized) {
        return "";
      }
    ...
    function markInitialReduplicativeSupportiveYSurface(surface = "", format = targetObject.SUPPORTIVE_MARKER_FORMAT.legacy) {
    ...
    function normalizeSupportiveMarkedLegacySurface(value = "", format = targetObject.SUPPORTIVE_MARKER_FORMAT.legacy) {
      const source = String(value || "");
      if (format === targetObject.SUPPORTIVE_MARKER_FORMAT.regex || format === targetObject.SUPPORTIVE_MARKER_FORMAT.screen) {
    ...
    function formatResolvedSupportiveMarkedSurface(value = "", format = targetObject.SUPPORTIVE_MARKER_FORMAT.legacy, preserveMarkers = false) {
    ...
    function resolveOptionalSupportiveMarkedSurface({
      ...
      const markerMatch = legacyMarkedSurface.match(targetObject.OPTIONAL_SUPPORTIVE_MARKER_DETECT_RE);
      ...
      if (markerLetter === "y" && markerMatch) {
        ...
        resolvedLegacySurface = preserveMarkers && yBehavior.deleteReduplicativeY ? legacyMarkedSurface : `${legacyMarkedSurface.slice(0, markerIndex)}` + `${yBehavior.deleteMarker ? "" : markerToken}` + `${resolvedFollowingSurface}`;
      }
      return {
        markerLetter,
        legacyMarkedSurface: resolvedLegacySurface,
        plainSurface: replaceOptionalSupportiveMarkersWithLetters(resolvedLegacySurface),
        outputSurface: formatResolvedSupportiveMarkedSurface(resolvedLegacySurface, outputFormat, preserveMarkers)
      };
    }
    function markOptionalSupportiveSurface(value = "", letter = "", format = targetObject.SUPPORTIVE_MARKER_FORMAT.legacy) {
    ...
    function resolveOptionalSupportiveOutputVerb({
      ...
      const markedVerb = markOptionalSupportiveSurface(baseVerb, optionalSupportiveLetter, targetObject.SUPPORTIVE_MARKER_FORMAT.legacy);
    ...
    function getOutputSurfaceSegmentValue(segments = [], role = "") {
      ...
      const match = (Array.isArray(segments) ? segments : []).find(segment => String(segment?.role || "") === normalizedRole);
      return String(match?.value || "");
    }
    ...
    function realizeDerivedMuStemInteraction({
      ...
      if (nextVerb.startsWith("mu")) {
        const embeddedMarker = nextObjectPrefix === "ta" || nextObjectPrefix === "te" ? nextObjectPrefix : nextObjectPrefix.startsWith("al") && (nextObjectPrefix.slice(2) === "ta" || nextObjectPrefix.slice(2) === "te") ? nextObjectPrefix.slice(2) : nextObjectPrefix.startsWith("wal") && (nextObjectPrefix.slice(3) === "ta" || nextObjectPrefix.slice(3) === "te") ? nextObjectPrefix.slice(3) : "";
        if (embeddedMarker && !nextVerb.startsWith(`mu${embeddedMarker}`)) {
          ...
          nextAlternateForms.forEach(form => {
            if (form && form.verb && form.verb.startsWith("mu") && !form.verb.startsWith(`mu${embeddedMarker}`)) {
              form.verb = `mu${embeddedMarker}${form.verb.slice(2)}`;
            }
          });
        }
      } else if (!nextObjectPrefix && (nextVerb.startsWith("tamu") || nextVerb.startsWith("temu"))) {
    ...
    api.realizeSurfaceChainImperativeKiReduction = realizeSurfaceChainImperativeKiReduction;
    ...
    api.replaceLegacyExplicitValenceMarkersWithRegexMarkers = replaceLegacyExplicitValenceMarkersWithRegexMarkers;
    ...
    api.normalizeSupportiveMarkedLegacySurface = normalizeSupportiveMarkedLegacySurface;
...
export function installOutputSurfaceGlobals(targetObject = globalThis) {
    const api = createOutputSurfaceGlobals(targetObject);
