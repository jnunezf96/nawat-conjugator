...
export function createOutputSurfaceApi(targetObject = globalThis) {
    ...
    function realizeSurfaceChainFinalIAUATrim(chain = null) {
      ...
      if (!endsWithAny(troncoValue, targetObject.IA_UA_SUFFIXES)) {
        return nextChain;
      }
    ...
    function realizeSurfaceChainMCodaAssimilation(chain = null) {
      ...
      for (let index = 0; index < segments.length; index += 1) {
        ...
        const finalMBeforeVowel = current.endsWith("m") && targetObject.VOWEL_START_RE.test(nextValueAfterSegment);
    ...
    function resolveOptionalSupportiveMarkedSurface({
      ...
      if (markerLetter === "y" && markerMatch) {
        ...
        resolvedEnvelopeSurface = preserveMarkers && yBehavior.deleteReduplicativeY ? envelopeMarkedSurface : `${envelopeMarkedSurface.slice(0, markerIndex)}` + `${yBehavior.deleteMarker ? "" : markerToken}` + `${resolvedFollowingSurface}`;
      }
      return {
        markerLetter,
    ...
    function resolveOptionalSupportiveOutputVerb(input = {}) {
      ...
      const markedVerb = markOptionalSupportiveSurface(baseVerb, optionalSupportiveLetter, targetObject.SUPPORTIVE_MARKER_FORMAT.envelope);
