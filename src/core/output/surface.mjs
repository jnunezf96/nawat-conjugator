export function createOutputSurfaceApi(targetObject = globalThis) {
    ...
    const isVerbLetterVowel = (...args) => targetObject.isVerbLetterVowel(...args);
    const isVerbLetterConsonant = (...args) => targetObject.isVerbLetterConsonant(...args);
    const resolveDirectionalOutputChain = (...args) => targetObject.resolveDirectionalOutputChain(...args);
    const resolveOptionalSupportiveLetter = (...args) => targetObject.resolveOptionalSupportiveLetter(...args);
    const SUPPORTIVE_MARKER_FORMAT = new Proxy({}, {
        get: (_target, prop) => targetObject.SUPPORTIVE_MARKER_FORMAT?.[prop],
    });
    ...
    function ensureC() {
        if (!C) {
            C = [...(targetObject.VALID_CONSONANTS || [])].join("");
        }
    }
    ...
    function realizeSurfaceChainImperativeKiReduction(chain = null) {
        ...
        if (!nextValue || !targetObject.VOWEL_START_RE || targetObject.VOWEL_START_RE.test(nextValue)) {
            return nextChain;
        }
    ...
    function realizeSurfaceChainNhBeforeVowel(chain = null) {
        ...
        for (let index = 0; index < segments.length - 1; index += 1) {
            ...
            if (!targetObject.VOWEL_RE || !targetObject.VOWEL_RE.test(prevChar) || current.endsWith("nh")) {
                continue;
            }
    ...
    function realizeSurfaceChainYShift(chain = null) {
        ...
        for (let index = 0; index < segments.length; index += 1) {
            ...
            segments[index].value = current.replace(_codaReY, (_match, offset) => {
                if (!isTransitive) {
    ...
    function isWjAlternationPair(left, right) {
        ...
        for (let index = 0; index < left.length; index += 1) {
            ...
            diffIndex = index;
            const isWj =
                (left[index] === "w" && right[index] === "j")
                || (left[index] === "j" && right[index] === "w");
            if (!isWj) {
                return false;
            }
        }
    ...
    function formatConjugationDisplay(value) {
        ...
        const lines = [];
        for (let index = 0; index < expandedForms.length; index += 1) {
            if (used[index]) {
                continue;
            }
            const nominalFamily = resolveNominalMarkerFamily(expandedForms, used, index);
            if (nominalFamily) {
                lines.push(nominalFamily.text);
                nominalFamily.indices.forEach((familyIndex) => {
                    used[familyIndex] = true;
                });
                continue;
            }
            let pairedIndex = -1;
            let pairText = "";
            for (let candidateIndex = index + 1; candidateIndex < expandedForms.length; candidateIndex += 1) {
                if (used[candidateIndex]) {
                    continue;
                }
                const left = expandedForms[index];
                const right = expandedForms[candidateIndex];
            ...
            if (pairedIndex !== -1) {
                lines.push(pairText || `${expandedForms[index]} / ${expandedForms[pairedIndex]}`);
                used[index] = true;
                used[pairedIndex] = true;
            } else {
                lines.push(expandedForms[index]);
                used[index] = true;
            }
        }
        return lines.join("\n");
    ...
    function getSupportiveMarkerTokenForLetter(letter = "", format = SUPPORTIVE_MARKER_FORMAT.legacy) {
        ...
        if (format === SUPPORTIVE_MARKER_FORMAT.regex || format === SUPPORTIVE_MARKER_FORMAT.screen) {
            return resolvedLetter === "y"
                ? targetObject.REGEX_OPTIONAL_SUPPORTIVE_Y_MARKER
                : targetObject.REGEX_OPTIONAL_SUPPORTIVE_I_MARKER;
        }
        return resolvedLetter === "y"
            ? targetObject.OPTIONAL_SUPPORTIVE_Y_MARKER
            : targetObject.OPTIONAL_SUPPORTIVE_I_MARKER;
    }
    ...
    function getSupportiveMarkerLetterFromValue(value = "", format = SUPPORTIVE_MARKER_FORMAT.legacy) {
        ...
        const match = (
            ...
            ? source.match(targetObject.REGEX_OPTIONAL_SUPPORTIVE_MARKER_DETECT_RE)
            : source.match(targetObject.OPTIONAL_SUPPORTIVE_MARKER_DETECT_RE);
    ...
    function replaceSupportiveMarkersWithLetters(value = "", format = SUPPORTIVE_MARKER_FORMAT.legacy) {
        ...
        const pattern = (
            ...
            ? targetObject.REGEX_OPTIONAL_SUPPORTIVE_MARKER_RE
            : targetObject.OPTIONAL_SUPPORTIVE_MARKER_RE;
    ...
    function convertEnvelopeSupportiveMarkersToRegexInput(value = "") {
        return String(value || "").replace(targetObject.REGEX_OPTIONAL_SUPPORTIVE_MARKER_RE, (_match, letter) => (
            getOptionalSupportiveMarkerForLetter(letter)
        ));
    }

    function convertRegexInputSupportiveMarkersToEnvelope(value = "") {
        return String(value || "").replace(targetObject.OPTIONAL_SUPPORTIVE_MARKER_RE, (_match, letterA, letterB) => (
    ...
    return {
        ...
        buildCodaRe,
        endsWithAny,
        buildSurfaceChainState,
        cloneSurfaceChainState,
        getSurfaceChainSegmentValue,
        setSurfaceChainSegmentValue,
        getSurfaceChainNextNonEmptyIndex,
        realizeSurfaceChainSubjectIInitialReduction,
        realizeSurfaceChainFinalIAUATrim,
        realizeSurfaceChainImperativeKiReduction,
        realizeSurfaceChainMuIskaliaReduction,
        realizeSurfaceChainObjectIInitialElision,
        realizeSurfaceChainNhBeforeVowel,
        realizeSurfaceChainKContact,
        realizeSurfaceChainKwCoalescence,
        realizeSurfaceChainYShift,
        realizeSurfaceChainMCodaAssimilation,
        realizeSurfaceChain,
        joinSurfaceChain,
        buildPrefixedChain,
        isWjAlternationPair,
        getExpectedAbsolutiveSuffix,
        isAbsolutivePair,
        isPatientivoInPair,
        getNominalMarkerFamilyBases,
        resolveNominalMarkerFamily,
        expandOptionalParentheticalForms,
        hasOptionalParentheticalForm,
        formatConjugationDisplay,
        normalizeSupportiveMarkerValue,
        hasSupportiveMarkerValue,
        getSupportiveMarkerTokenForLetter,
        getSupportiveMarkerLetterFromValue,
        replaceSupportiveMarkersWithLetters,
        getOptionalSupportiveMarkerForLetter,
        getOptionalSupportiveMarkerLetter,
        ...
        getSupportiveYFollowingSurfaceLetters,
        getSupportiveYFollowingVowel,
        hasInitialReduplicativeSupportiveYPattern,
        dropReduplicativeYAfterVj,
        markReduplicativeYAfterVj,
        markInitialReduplicativeSupportiveYSurface,
        resolveOptionalSupportiveYBehavior,
        normalizeSupportiveMarkedLegacySurface,
        formatResolvedSupportiveMarkedSurface,
