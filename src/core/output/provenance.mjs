...
export function createOutputProvenanceGlobals(targetObject = globalThis) {
    ...
    var OUTPUT_PROVENANCE_CAUSATIVE_ANDREWS_REFS = Object.freeze(["Andrews Lesson 24 24.1"]);
    function getOutputProvenanceAndrewsRefs({
      derivationType = ""
    } = {}) {
    ...
    function splitOutputProvenanceContractSurfaceText(value = "") {
      return String(value || "").split(/\s*\/\s*/g).map(entry => normalizeOutputProvenanceContractSurfaceValue(entry)).filter(Boolean);
    }
    ...
    function buildProvenanceVariantEntry({
      ...
      const resolvedBaseSpec = baseSpec && typeof baseSpec === "object" ? baseSpec : null;
      const realizedStem = targetObject.normalizeDerivationStemValue(surfaceStem || (resolvedStemSpec ? targetObject.realizeMorphStemSpec(resolvedStemSpec, `${base || ""}${suffix || ""}`) : `${base || ""}${suffix || ""}`));
      const variant = {
        base: String(base || ""),
    ...
    function getProvenancePrimaryStemSurface(provenance = null) {
      ...
      if (primaryVariant) {
        ...
        const resolvedSurface = targetObject.normalizeDerivationStemValue(primaryVariant.surfaceStem || (primaryVariant.stemSpec ? targetObject.realizeMorphStemSpec(primaryVariant.stemSpec, `${primaryVariant.base || ""}${primaryVariant.suffix || ""}`) : `${primaryVariant.base || ""}${primaryVariant.suffix || ""}`));
        if (resolvedSurface) {
      ...
      return getOutputProvenancePrimarySurface(provenance) || targetObject.normalizeDerivationStemValue(provenance?.surfaceStem || "");
    }
    function buildForwardDerivationProvenance({
      ...
      const selectedStem = targetObject.normalizeDerivationStemValue(meta.surfaceStem || targetObject.realizeMorphStemSpec(meta.stemSpec || null, meta.stem || "") || meta.stem || "");
    ...
    const api = {};
    Object.defineProperty(api, "OUTPUT_PROVENANCE_STEM_ANDREWS_REFS", {
        configurable: true,
        enumerable: true,
        get() { return OUTPUT_PROVENANCE_STEM_ANDREWS_REFS; },
        set(value) { OUTPUT_PROVENANCE_STEM_ANDREWS_REFS = value; },
    });
    Object.defineProperty(api, "OUTPUT_PROVENANCE_CAUSATIVE_ANDREWS_REFS", {
        configurable: true,
        enumerable: true,
        get() { return OUTPUT_PROVENANCE_CAUSATIVE_ANDREWS_REFS; },
        set(value) { OUTPUT_PROVENANCE_CAUSATIVE_ANDREWS_REFS = value; },
    });
    api.getOutputProvenanceAndrewsRefs = getOutputProvenanceAndrewsRefs;
    api.buildOutputProvenanceMissingStemDiagnostic = buildOutputProvenanceMissingStemDiagnostic;
    api.normalizeOutputProvenanceContractSurfaceValue = normalizeOutputProvenanceContractSurfaceValue;
    ...
    api.buildProvenanceVariantEntry = buildProvenanceVariantEntry;
    api.getProvenancePrimaryStemSurface = getProvenancePrimaryStemSurface;
    api.buildForwardDerivationProvenance = buildForwardDerivationProvenance;
    return api;
}

export function installOutputProvenanceGlobals(targetObject = globalThis) {
    const api = createOutputProvenanceGlobals(targetObject);
