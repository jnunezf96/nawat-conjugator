...
// Orthography-profile and bridge metadata. Arbitrary input classification remains
// diagnostic-only; grammar-rule spellings may be realized in Nawat/Pipil letters.
...
const LESSON2_SOUND_SPELLING_RULES = Object.freeze([
    Object.freeze({
        id: "h-num2-t",
        source: "h",
        sourceDisplay: "-h",
...
const LESSON2_COVERAGE_INVENTORY = Object.freeze([
    Object.freeze({
...
function buildLesson2SoundSpellingFrame(input = {}) {
    ...
    return {
        ...
        reason,
        scopeNote: node.scopeNote || rule?.scopeNote || (
            rule
                ? "Correspondence is conditioned by grammar slot and phonological position; it is not a global letter replacement."
                : "Orthography conversion only; not a generated Nawat/Pipil lexical form."
        ),
        slotSensitive: grammarSlot !== "unknown",
        generationAllowed: false,
        evidenceStatus: targetCandidates.length && !target
            ? "nawat-evidence-required"
            : (rule ? "slot-sensitive-spelling" : "orthography-conversion-only"),
        diagnostics,
    };
}
...
function buildLesson2CoverageFrame() {
    ...
    const remainingGaps = inventory
        .filter((entry) => entry.coverageStatus !== "implemented-foundation")
        .map((entry) => ({
            id: entry.id,
            pdfRef: entry.pdfRef,
    ...
    const frame = {
        ...
        generationPolicy: [
            "diagnostic coverage is not generation permission",
            "Classical spelling is not imported as Nawat output",
            "lossy or evidence-sensitive categories remain blocked or diagnostic",
        ],
    };
