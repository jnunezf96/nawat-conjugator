const FULL_SOURCE_CHAIN_REALIZATION_POLICY = Object.freeze({
...
export function createDerivationSourceModelApi(targetObject = globalThis) {
    ...
    function normalizeDerivationSourceOuterPiece(piece = null) {
        ...
        if (piece.type === "valence") {
            const valenceValue = targetObject.normalizeComposerSecondaryValenceSurfaceToken(piece.value)
                || targetObject.normalizeComposerValenceToken(piece.value)
                || targetObject.normalizeComposerStem(piece.value);
    ...
    function buildCurrentRegexDerivationSourceModel(rawValue = "") {
        ...
        const outerPieces = (Array.isArray(movingTargetParsed.outerPieces) ? movingTargetParsed.outerPieces : [])
            ...
            .filter(Boolean);
        const markedCore = targetObject.convertEnvelopeSupportiveMarkersToRegexInput(
            targetObject.normalizeRegexCoreTokenCase(String(movingTargetParsed.coreText || "").trim())
        );
    ...
    function buildFallbackDerivationSourceModel(verbMeta = {}, verb = "", analysisVerb = "") {
            ...
            : (() => {
                const lexicalPieces = (
                    Array.isArray(meta?.lexicalBoundPrefixes) && meta.lexicalBoundPrefixes.length
                        ...
                        : targetObject.getLexicalBoundPrefixes(boundPrefixes, boundExplicitFlags)
                )
                    .map((value) => targetObject.normalizeRuleBase(value))
                    .filter(Boolean)
                    .map((value) => ({ type: "lexical", value }));
                const valencePieces = targetObject.getExplicitBoundNonspecificPrefixes(boundPrefixes, boundExplicitFlags)
                    .map((value) => targetObject.normalizeComposerSecondaryValenceSurfaceToken(value) || targetObject.normalizeComposerValenceToken(value))
                    .filter(Boolean)
                    .map((value) => ({ type: "valence", value }));
                const directionalPrefix = targetObject.normalizeComposerStem(
        ...
        const sourcePrefix = targetObject.normalizeRuleBase(
            meta?.sourcePrefix
            ...
            transitivity: meta?.isMarkedTransitive
                ? (
                    Number.isFinite(meta?.semanticObjectSlotCount) && Number(meta.semanticObjectSlotCount) >= 2
                        ? targetObject.COMPOSER_TRANSITIVITY.bitransitive
                        : targetObject.COMPOSER_TRANSITIVITY.transitive
                )
                : targetObject.COMPOSER_TRANSITIVITY.intransitive,
            outerPieces,
            corePrefixParts,
            supportiveMarker,
            adjacentCoreEmbed,
            matrixBase: targetObject.normalizeRuleBase(
                meta?.exactBaseVerb
                || meta?.sourceBase
    ...
    function buildSupportivePrecedingSurfaceFromMorphologyInput({
        ...
        const explicitPrefixes = targetObject.getExplicitBoundNonspecificPrefixes(boundPrefixes, boundExplicitFlags)
            .map((value) => targetObject.normalizeComposerSecondaryValenceSurfaceToken(value) || targetObject.normalizeComposerValenceToken(value))
            .filter(Boolean);
        return [
            targetObject.normalizeComposerStem(directionalPrefix || ""),
            lexicalPrefix,
            ...explicitPrefixes,
    ...
    function applySourceChainStemSpecByPolicy(stemSpec = null, fallbackStem = "", chain = null, {
        ...
        return prefixParts.reduceRight((current, prefixPart) => (
            targetObject.buildPrependMorphStemSpec(current, prefixPart, {
    ...
    function resolvePatientivoImperfectiveBaseStemSpec(chain = null) {
        ...
        if (targetObject.endsWithAny(baseVerb, targetObject.IA_UA_SUFFIXES)) {
            const spec = targetObject.buildReplaceSuffixMorphStemSpec(baseVerb, "a", "", {
                sourceBase: normalizedSourceBase,
                sourceSuffix: "a",
            });
            if (spec) {
        ...
        return targetObject.buildLiteralMorphStemSpec(baseVerb, {
            sourceBase: normalizedSourceBase,
        });
    }
    ...
    return {
        FULL_SOURCE_CHAIN_REALIZATION_POLICY,
        PATIENTIVO_IMPERFECTIVE_SOURCE_CHAIN_POLICY,
