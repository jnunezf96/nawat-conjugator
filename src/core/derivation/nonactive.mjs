...
export function createNonactiveDerivationApi(targetObject = globalThis) {
    ...
    function buildPrefixedNonactiveSelectionEntry({
      ...
      const withPrefixSpec = (spec = null, fallbackStem = "") => {
        const resolvedSpec = spec && typeof spec === "object" && spec.kind ? spec : fallbackStem ? targetObject.buildLiteralMorphStemSpec(fallbackStem) : null;
        if (!resolvedSpec) {
          return null;
        }
        if (!prefix) {
      ...
      const selectedStemSpec = withPrefixSpec(selection?.selectedStemSpec || null, selection?.selectedStem || "");
      const selectedStemSpecs = targetObject.getUniqueMorphStemSpecs((Array.isArray(selection?.selectedStemSpecs) ? selection.selectedStemSpecs : []).map((spec, index) => withPrefixSpec(spec, Array.isArray(selection?.selectedStems) ? selection.selectedStems[index] || "" : "")));
      const allStemSpecs = targetObject.getUniqueMorphStemSpecs((Array.isArray(selection?.allStemSpecs) ? selection.allStemSpecs : []).map((spec, index) => withPrefixSpec(spec, Array.isArray(selection?.allStems) ? selection.allStems[index] || "" : "")));
      return {
    ...
    function resolveDerivedNonactiveSelectionEntry({
      ...
      const matchSource = parsedMatch ? targetObject.getNonactiveDerivationSource(sourceMeta, sourceVerb, sourceAnalysisVerb) : {
      ...
      const matchSelection = targetObject.resolveNonactiveStemSelection(matchBaseVerb, matchBaseVerb, {
        ...
        rootPlusYaBase: parsedMatch ? parsedMatch.rootPlusYaBase : parsedVerb?.rootPlusYaBase
      });
    ...
    function resolveNonactiveAllStems({
      ...
      const selectedStems = Array.isArray(selection?.selectedStems) ? selection.selectedStems.filter(Boolean) : [];
      const selectedStemVariants = selectionHasPrefixedStems ? syncedSelectedStems && syncedSelectedStems.length ? syncedSelectedStems : selectedStems : selectedStems;
    ...
    function resolveNonactiveAllStemSpecs({
      ...
      const selectedStemSpecs = Array.isArray(selection?.selectedStemSpecs) ? selection.selectedStemSpecs.filter(Boolean) : [];
      const selectedStemSpecVariants = selectionHasPrefixedStems ? syncedSelectedStemSpecs && syncedSelectedStemSpecs.length ? syncedSelectedStemSpecs : selectedStemSpecs : selectedStemSpecs;
      if (selectionHasPrefixedStems && selectedStemSpecVariants.length > 1) {
        ...
        if (selectionHasPrefixedStems) {
          const specs = syncedAllStemSpecs && syncedAllStemSpecs.length ? syncedAllStemSpecs : selection.allStemSpecs;
          nonactiveAllStemSpecs = specs;
        } else {
          nonactiveAllStemSpecs = selection.allStemSpecs.map(spec => applyPrefixSpec(spec)).filter(Boolean);
        }
      } else if (selection?.selectedSuffix && selectedStemSpecVariants.length > 1) {
        if (selectionHasPrefixedStems) {
          nonactiveAllStemSpecs = selectedStemSpecVariants;
        } else {
          nonactiveAllStemSpecs = selectedStemSpecVariants.map(spec => applyPrefixSpec(spec)).filter(Boolean);
        }
      }
    ...
    function applyNonactiveDerivation({
      ...
      const nonactiveIsTransitive = targetObject.isNonactiveTransitiveByObj1(objectPrefix, parsedVerb);
      ...
      const nonactiveSource = preferredNonactiveBaseVerb ? {
        ...
        prefix: typeof preferredNonactiveSourcePrefix === "string" ? preferredNonactiveSourcePrefix : "",
        chain: {
          baseVerb: preferredNonactiveBaseVerb,
          prefix: typeof preferredNonactiveSourcePrefix === "string" ? preferredNonactiveSourcePrefix : "",
      ...
      const nonactiveRuleSource = targetObject.buildNonactiveRuleSourceContext(verb, analysisVerb, {
      ...
      const selectedStemMatchesBareNonactiveBase = targetObject.normalizeDerivationStemValue(selection?.selectedStem || "") === targetObject.normalizeDerivationStemValue(nonactiveBaseVerb || "");
      const shouldRetrySupportiveSurfaceSelection = Boolean(parsedVerb?.hasOptionalSupportiveI === true && !selection?.selectedSuffix && selectedStemMatchesBareNonactiveBase && (verb || analysisVerb));
      if (shouldRetrySupportiveSurfaceSelection) {
        const supportiveSurfaceSelection = targetObject.resolveNonactiveStemSelection(verb || analysisVerb || nonactiveBaseVerb, analysisVerb || verb || nonactiveBaseVerb, {
          isTransitive: nonactiveIsTransitive,
          isYawi,
          forceAll: forceAllNonactiveOptions,
          ruleBase: nonactiveRuleBase,
        ...
        const supportiveSelectedStem = targetObject.normalizeDerivationStemValue(supportiveSurfaceSelection?.selectedStem || "");
        if (supportiveSelectedStem && supportiveSelectedStem !== targetObject.normalizeDerivationStemValue(nonactiveBaseVerb || "")) {
          selection = supportiveSurfaceSelection;
          selectionHasPrefixedStems = true;
        }
      }
      if (!selectionHasPrefixedStems && selection?.selectedRealizedStemSpec) {
        selection = {
          ...selection,
          ...
          selectedStems: Array.isArray(selection.selectedRealizedStems) && selection.selectedRealizedStems.length ? selection.selectedRealizedStems : selection.selectedStems,
          selectedStemSpecs: Array.isArray(selection.selectedRealizedStemSpecs) && selection.selectedRealizedStemSpecs.length ? selection.selectedRealizedStemSpecs : selection.selectedStemSpecs,
          allStems: Array.isArray(selection.allRealizedStems) && selection.allRealizedStems.length ? selection.allRealizedStems : selection.allStems,
          allStemSpecs: Array.isArray(selection.allRealizedStemSpecs) && selection.allRealizedStemSpecs.length ? selection.allRealizedStemSpecs : selection.allStemSpecs
        };
        selectionHasPrefixedStems = true;
      }
      const isDerivedSyncType = !!forwardDerivationConfig;
      const derivedAllStems = isDerivedSyncType ? targetObject.getDerivedStemPoolValue(forwardDerivationConfig.resultField, derivedStemPoolByField) : null;
      if (isDerivedSyncType && targetObject.BASIC_DATA_CANONICAL_MAP.size && Array.isArray(derivedAllStems) && derivedAllStems.length) {
        ...
        const orderedOneToOneStems = unique(derivedAllStems.map(stem => {
        ...
        const orderedOneToOneStemSpecs = targetObject.getUniqueMorphStemSpecs(derivedAllStems.map(stem => {
          const entry = getNonactiveSelectionEntry(selectionByStem, stem);
        ...
        if (matchedSelection) {
          ...
          const shouldForceZeroNonactiveObjects = targetObject.getDerivationValencyDelta(derivationType) <= 0 && Number.isFinite(matchedSelection.nonactiveObjectSlots) && matchedSelection.nonactiveObjectSlots <= 0;
      ...
      const applyPrefixSpec = (stemSpec = null, fallbackStem = "") => {
        ...
        return targetObject.applyNonactiveSourceChainStemSpec(resolvedSpec, fallbackStem, nonactiveSourceChain, {
          sourceSuffix: selection?.selectedSuffix || "",
          policy: nonactiveChainPolicy
        });
      };
      const applyPrefix = stem => selectionHasPrefixedStems ? stem : targetObject.realizeNonactiveSourceChainStem(stem, nonactiveSourceChain, nonactiveChainPolicy);
      const selectedStemSpec = applyPrefixSpec(selection.selectedStemSpec, selection.selectedStem || "");
      verb = selectedStemSpec ? targetObject.realizeMorphStemSpec(selectedStemSpec, "") : selection.selectedStem ? applyPrefix(selection.selectedStem) : "";
      analysisVerb = verb;
    ...
    api.resolveNonactiveAllStems = resolveNonactiveAllStems;
    api.resolveNonactiveAllStemSpecs = resolveNonactiveAllStemSpecs;
    api.applyNonactiveDerivation = applyNonactiveDerivation;
    return api;
}
