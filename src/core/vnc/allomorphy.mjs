...
export function createVncAllomorphyModule(targetObject = globalThis) {
    ...
    function splitVncAllomorphyContractSurfaceText(value = "") {
      return String(value || "").split(/\s*\/\s*/g).map(entry => normalizeVncAllomorphyContractSurfaceValue(entry)).filter(Boolean);
    }
    ...
    function getVncAllomorphyContractSurfaceForms(record = null, options = {}) {
      ...
      const orthographyFrame = options?.orthographyFrame && typeof options.orthographyFrame === "object" ? options.orthographyFrame : null;
      const targetContract = options?.targetContract && typeof options.targetContract === "object" ? options.targetContract : null;
      const forms = [];
      ...
      if (hasResultFrame) {
        return forms.flatMap(entry => splitVncAllomorphyContractSurfaceText(entry)).filter((entry, index, list) => entry && list.indexOf(entry) === index);
      }
    ...
    function getVncAllomorphyContractSourceInput(record = null, options = {}) {
      ...
      const resultFrame = getVncAllomorphyContractResultFrame(node, options);
      if (resultFrame) {
        return normalizeVncAllomorphyContractSurfaceValue(resultFrame.sourceInput || getVncAllomorphyContractSurface(node, options) || "");
      }
      return normalizeVncAllomorphyContractSurfaceValue(options.sourceInput || node.sourceStem || node.stem || node.sourceSuffix || node.selectedOutputSurface || "");
    }
    function getVncAllomorphySourceSurfaceForms(record = null, options = {}) {
      ...
      return (Array.isArray(node.forms) ? node.forms : []).flatMap(entry => splitVncAllomorphyContractSurfaceText(entry)).filter((entry, index, list) => entry && list.indexOf(entry) === index);
    }
    function attachVncAllomorphyGrammarContract(record = null, options = {}) {
      ...
      const node = record && typeof record === "object" ? record : {};
      const curriculumRef = node.curriculumRef && typeof node.curriculumRef === "object" ? node.curriculumRef : {};
      ...
      const supported = Object.prototype.hasOwnProperty.call(options, "supported") ? options.supported === true : node.supported === true || node.allowed === true;
      const diagnostics = [...(Array.isArray(node.diagnostics) ? node.diagnostics : []), ...(Array.isArray(options.diagnostics) ? options.diagnostics : [])];
      ...
      const resolvedOrthographyFrame = {
        ...orthographyFrame,
        surface: hasResultFrame ? surface : normalizeVncAllomorphyContractSurfaceValue(orthographyFrame.surface || surface),
        surfaceForms: hasResultFrame ? surfaceForms : Array.isArray(orthographyFrame.surfaceForms) && orthographyFrame.surfaceForms.length ? getVncAllomorphyContractSurfaceForms({
      ...
      const resolvedTargetContract = {
        ...
        surface: hasResultFrame ? surface : normalizeVncAllomorphyContractSurfaceValue(rawTargetContract.surface || ""),
        surfaceForms: hasResultFrame ? surfaceForms : Array.isArray(rawTargetContract.surfaceForms) ? getVncAllomorphyContractSurfaceForms({
          surfaceForms: rawTargetContract.surfaceForms
        }) : []
      };
    ...
    function getActiveActionNominalizerContract() {
      ...
      const shortConversion = typeof targetObject.convertClassicalLettersToNawat === "function" ? targetObject.convertClassicalLettersToNawat("z", {
        contract: "active-action-nominalizer"
      }) : null;
      const contract = {
    ...
    function getActiveActionNominalizerSuffixes() {
      ...
      return Object.freeze([contract.nawatSuffixes.long, contract.nawatSuffixes.short].filter(Boolean));
    }
    ...
    function getPatientivoNonactiveSourceSuffixContract(sourceSuffix = "") {
      ...
      const orthographyConversion = typeof targetObject.convertClassicalLettersToNawat === "function" ? targetObject.convertClassicalLettersToNawat(contract.classicalSuffix, {
    ...
    function buildPatientivoSourceStageFrame({
      ...
      const nonactiveSuffixContract = normalizedSourceType === "nonactive" ? getPatientivoNonactiveSourceSuffixContract(normalizedSourceSuffix) : null;
      const perfectiveEndingContract = normalizedSourceType === "perfectivo" ? sourceEndingContract && typeof sourceEndingContract === "object" && sourceEndingContract.kind === "patientive-perfective-source-ending-contract" ? sourceEndingContract : getPatientivoPerfectiveSourceStemContract(outputStem || sourceStem) : null;
      const imperfectiveStemContract = normalizedSourceType === "imperfectivo" ? sourceEndingContract && typeof sourceEndingContract === "object" && sourceEndingContract.kind === "patientive-imperfective-source-stem-contract" ? sourceEndingContract : getPatientivoImperfectiveSourceStemContract({
        sourceStem,
        outputStem,
        outputConnector
      }) : null;
      const rootStockContract = normalizedSourceType === "tronco-verbal" ? sourceStockContract && typeof sourceStockContract === "object" && sourceStockContract.kind === "patientive-root-stock-source-contract" ? sourceStockContract : getPatientivoRootStockSourceContract({
        sourceStem,
        outputStem,
        outputConnector
      }) : null;
      const sourceCore = nonactiveSuffixContract ? "nonactive-core" : normalizedSourceType === "perfectivo" ? "perfective-active-core" : normalizedSourceType === "imperfectivo" ? "imperfective-active-core" : normalizedSourceType === "tronco-verbal" ? "root-or-stock-stem" : normalizedSourceType;
      const operation = nonactiveSuffixContract?.sourceOperation || (normalizedSourceType === "perfectivo" ? "derive-from-perfective-active-core" : normalizedSourceType === "imperfectivo" ? "derive-from-imperfective-active-core" : normalizedSourceType === "tronco-verbal" ? "derive-from-root-or-stock" : "unknown-patientive-source-operation");
      const frameRecord = {
    ...
    function buildVerbDerivedNominalizationProfile({
      ...
      const isImpersonalActionNominal = kind === "sustantivo-verbal" && String(sourceModel?.combinedMode || "") === "nonactive";
      const isGeneralUseActionNominal = kind === "calificativo-instrumentivo" && String(sourceModel?.actionNounStemUse || "") === "general-use";
      const isGeneralUsePassiveActionNominal = isGeneralUseActionNominal && String(sourceModel?.combinedMode || "") === "nonactive";
      const isGeneralUseActiveActionNominal = isGeneralUseActionNominal && !isGeneralUsePassiveActionNominal;
      ...
      const profile = {
        ...
        patientiveMultipleDerivationContract: patientiveMultipleDerivationContract && typeof patientiveMultipleDerivationContract === "object" && patientiveMultipleDerivationContract.kind === "patientive-multiple-derivation-contract" ? patientiveMultipleDerivationContract : null,
    ...
    function derivePatientivoStemFromNonactiveEntries(sourceFrame = null) {
      ...
      const buildVariantEntry = (stemSpec, fallbackStem, subjectSuffix, metadata = null) => {
        ...
        const patientiveSourceStageFrame = metadata?.patientiveSourceStageFrame && typeof metadata.patientiveSourceStageFrame === "object" ? metadata.patientiveSourceStageFrame : buildPatientivoSourceStageFrame({
      ...
      const buildFallbackBaseEntry = () => {
        ...
        if (!sourceSuffix || !deletedSegment || !normalizedStem.endsWith(sourceSuffix) || !normalizedStem.endsWith(deletedSegment)) {
          return null;
        }
        return {
      ...
      const buildRouteBaseEntry = () => {
        ...
        if (!routeSourceBase || !routeSourceSuffix || routeSourceSuffix !== normalizedSuffix || routeSurface !== normalizedStem) {
          return null;
        }
        return {
          stem: `${routeSourceBase}${sourceSuffixContract.retainedSegment || ""}`,
          stemSpec: buildBaseSuffixRouteSpec(routeSourceBase, routeSourceSuffix || sourceSuffixContract.sourceSuffix, sourceSuffixContract.retainedSegment || "")
        };
      };
    ...
    function buildPatientivoDerivationEntry({
      ...
      const patientiveSourceStageFrame = metadata?.patientiveSourceStageFrame && typeof metadata.patientiveSourceStageFrame === "object" ? metadata.patientiveSourceStageFrame : buildPatientivoSourceStageFrame({
        ...
        sourceStem: metadata?.nonactiveSourceStem || metadata?.stem || metadata?.verb || sourceModel?.matrixBase || "",
        outputStem: realizedStem,
        outputConnector: subjectSuffix,
        sourceEndingContract: metadata?.perfectiveSourceEndingContract || metadata?.imperfectiveSourceStemContract || null,
    ...
    const PATIENTIVO_PERFECTIVE_SOURCE_ENDING_CONTRACTS = Object.freeze([Object.freeze({
      id: "w",
      classicalEnding: "w",
    ...
    function buildPatientivoPerfectiveEndingOrthographyConversions(contract = null) {
      const spellings = Array.isArray(contract?.classicalSpellings) ? contract.classicalSpellings : [];
      return Object.freeze(spellings.map(spelling => typeof targetObject.convertClassicalLettersToNawat === "function" ? targetObject.convertClassicalLettersToNawat(spelling, {
    ...
    function getPatientivoPerfectiveSourceStemContract(stem = "") {
      ...
      const contract = PATIENTIVO_PERFECTIVE_SOURCE_ENDING_CONTRACTS.find(entry => Array.isArray(entry.acceptedEndings) && entry.acceptedEndings.includes(matchedEnding)) || null;
      ...
      const supportedContract = {
        ...
        diagnostics: Object.freeze(["perfective-patientive-source-ending-allowed", "orthography-conversion-is-not-lexical-evidence"]),
        grammarAuthority: "Andrews PDF",
        orthographyAuthority: "Nawat/Pipil output spelling",
        generationAllowedByOrthography: false
      };
    ...
    function buildPasadoRemotoStemEntries({
      ...
      classKeys.forEach(classKey => {
        ...
        if (!provenanceVariants.length) {
          getVncAllomorphySourceSurfaceForms(pasadoRemotoOutput).map(entry => normalizeDerivationStemValue(entry)).filter(entry => entry && entry !== "—").forEach(surfaceForm => {
    ...
    function buildSustantivoVerbalStemCandidates({
      ...
      const pushCandidate = (stemSpec = null, fallbackStem = "", {
        ...
        const resolvedStemSpec = isImpersonalActionSource ? stemSpec || buildLiteralMorphStemSpec(fallbackStem) : applySustantivoVerbalSourceChainStemSpec(stemSpec, fallbackStem, sourceChain);
      ...
      const actionNominalRootPlusYaRules = targetObject.DERIVATIONAL_RULES?.actionNominal?.sustantivoVerbal?.rootPlusYa || {};
      const deleteYaOnlyVerbs = Array.isArray(actionNominalRootPlusYaRules.deleteYaOnlyVerbs) ? actionNominalRootPlusYaRules.deleteYaOnlyVerbs : [];
      const deleteYaOnlySuffixes = Array.isArray(actionNominalRootPlusYaRules.deleteYaOnlySuffixes) ? actionNominalRootPlusYaRules.deleteYaOnlySuffixes : [];
      const allowStemVariantVerbs = Array.isArray(actionNominalRootPlusYaRules.allowStemVariantVerbs) ? actionNominalRootPlusYaRules.allowStemVariantVerbs : [];
      const allowStemVariantSuffixes = Array.isArray(actionNominalRootPlusYaRules.allowStemVariantSuffixes) ? actionNominalRootPlusYaRules.allowStemVariantSuffixes : [];
      const sourceRuleBase = matrixSourceBase || baseStem || "";
      const sourceFullRuleBase = sourceRawVerb || rawAnalysisVerb || analysisVerb || verb || "";
      const matchesActionNominalSuffix = suffixes => suffixes.some(suffix => suffix && (sourceRuleBase.endsWith(suffix) || sourceFullRuleBase && sourceFullRuleBase.endsWith(suffix)));
      const actionNominalRules = targetObject.DERIVATIONAL_RULES?.actionNominal?.sustantivoVerbal || {};
      const replaciveImperfectiveRules = actionNominalRules.replaciveImperfective || {};
      const finalKaToKiOnly = Array.isArray(replaciveImperfectiveRules.finalKaToKiOnly) ? replaciveImperfectiveRules.finalKaToKiOnly : [];
      const finalSiToShiOnlySuffixes = Array.isArray(replaciveImperfectiveRules.finalSiToShiOnlySuffixes) ? replaciveImperfectiveRules.finalSiToShiOnlySuffixes : [];
      const finalTiToChiOptional = Array.isArray(replaciveImperfectiveRules.finalTiToChiOptional) ? replaciveImperfectiveRules.finalTiToChiOptional : [];
      if (baseStem.endsWith("ka") && targetObject.matchesDerivationRuleBaseList(finalKaToKiOnly, sourceRuleBase, sourceFullRuleBase)) {
      ...
      if (baseStem.endsWith("si") && matchesActionNominalSuffix(finalSiToShiOnlySuffixes)) {
      ...
      const rootPlusYaStemVariantAllowed = targetObject.matchesDerivationRuleBaseList(allowStemVariantVerbs, sourceRuleBase, sourceFullRuleBase) || matchesActionNominalSuffix(allowStemVariantSuffixes);
      const rootPlusYaDeleteOnly = (targetObject.matchesDerivationRuleBaseList(deleteYaOnlyVerbs, sourceRuleBase, sourceFullRuleBase) || matchesActionNominalSuffix(deleteYaOnlySuffixes)) && !rootPlusYaStemVariantAllowed;
      ...
      pushCandidate(baseStemSpec, baseStem);
      if (baseStem.endsWith("ti") && targetObject.matchesDerivationRuleBaseList(finalTiToChiOptional, sourceRuleBase, sourceFullRuleBase)) {
    ...
    function buildPatientivoPerfectivoStemEntries({
      ...
      if (shouldUseClassABStem) {
        ...
        if (!perfectiveStemEntries.length) {
          const fallbackPerfectiveStem = normalizeDerivationStemValue(getVncAllomorphyContractSurface(preteriteOutput));
    ...
    function buildPatientivoRootStockVariantOrthographyConversions() {
      return Object.freeze(PATIENTIVO_ROOT_STOCK_VARIANT_CONSONANTS.map(entry => typeof targetObject.convertClassicalLettersToNawat === "function" ? targetObject.convertClassicalLettersToNawat(entry.classical, {
    ...
    function buildPatientivoRootStockSourceContractRecord(options = {}) {
      ...
      const routeStemOnly = options.routeStemOnly === true || hasOutputConnector && String(options.outputConnector ?? "") === "";
      const resolvedConnector = routeStemOnly ? "" : hasOutputConnector ? String(options.outputConnector ?? "") : getTClassSuffixForStem(normalizedOutputStem);
      ...
      const contract = {
        ...
        nawatVariantConsonants: Object.freeze(PATIENTIVO_ROOT_STOCK_VARIANT_CONSONANTS.map(entry => entry.nawat)),
        orthographyConversions: buildPatientivoRootStockVariantOrthographyConversions(),
        routeStemOnly,
        grammarAuthority: "Andrews PDF",
        orthographyAuthority: "Nawat/Pipil output spelling",
        diagnostics: Object.freeze(supported ? ["root-stock-patientive-source-supported"] : ["unsupported-root-stock-patientive-source"]),
        boundaries: Object.freeze({
          noClassicalSurfaceImport: true,
          noNewFixtureEvidence: true,
          preservesGeneratedSurfaceForms: true,
    ...
    function getPatientivoImperfectiveSourceStemContract({
      ...
      const contract = {
        ...
        grammarAuthority: "Andrews PDF",
        orthographyAuthority: "Nawat/Pipil output spelling",
        diagnostics: Object.freeze(supported ? ["imperfective-patientive-source-stem-supported"] : ["unsupported-imperfective-patientive-source-stem"]),
        boundaries: Object.freeze({
          noClassicalSurfaceImport: true,
          noNewFixtureEvidence: true,
          preservesGeneratedSurfaceForms: true
    ...
    function buildPatientivoTroncoDerivations({
      ...
      const addRawResult = (stem, suffix, options = {}) => {
        ...
        const rootStockSourceContract = options.rootStockSourceContract || getPatientivoRootStockSourceContract({
          sourceStem: base,
    ...
    const PATIENTIVO_MULTIPLE_DERIVATION_SOURCE_FAMILIES = Object.freeze([VERB_DERIVED_PATIENTIVE_FAMILY.passive, VERB_DERIVED_PATIENTIVE_FAMILY.impersonal, VERB_DERIVED_PATIENTIVE_FAMILY.perfectivo, VERB_DERIVED_PATIENTIVE_FAMILY.imperfectivo, VERB_DERIVED_PATIENTIVE_FAMILY.troncoVerbal]);
    function buildPatientivoMultipleDerivationContract({
      ...
      const families = Array.isArray(sourceFamilies) && sourceFamilies.length ? sourceFamilies : PATIENTIVO_MULTIPLE_DERIVATION_SOURCE_FAMILIES;
      const sourceOptions = families.map(familyValue => {
        ...
        const sourceCores = Array.from(new Set(entries.map(entry => String(entry?.patientiveSourceStageFrame?.sourceCore || entry?.sourceType || "")).filter(Boolean)));
        const outputSurfaces = Array.from(new Set(entries.map(entry => `${String(entry?.verb || entry?.stem || "")}${String(entry?.subjectSuffix || "")}`).filter(Boolean)));
        return Object.freeze({
          sourceFamily,
          ...
          diagnostics: Object.freeze(entries.length ? ["patientive-derivation-procedure-available"] : ["patientive-derivation-procedure-unavailable"])
        });
      });
      ...
      const contract = {
        ...
        grammarAuthority: "Andrews PDF",
        orthographyAuthority: "Nawat/Pipil output spelling",
        diagnostics: Object.freeze(availableOptions.length > 1 ? ["multiple-patientive-derivation-procedures-available"] : ["single-patientive-derivation-procedure-available"]),
        boundaries: Object.freeze({
          noClassicalSurfaceImport: true,
          noNewFixtureEvidence: true,
          noNewSurfaceForms: true,
    ...
    const api = {};
    api.normalizeVncAllomorphyContractSurfaceValue = normalizeVncAllomorphyContractSurfaceValue;
    api.splitVncAllomorphyContractSurfaceText = splitVncAllomorphyContractSurfaceText;
    api.getVncAllomorphyContractGrammarFrame = getVncAllomorphyContractGrammarFrame;
    api.getVncAllomorphyContractResultFrame = getVncAllomorphyContractResultFrame;
    api.getVncAllomorphyContractSurfaceForms = getVncAllomorphyContractSurfaceForms;
    api.getVncAllomorphyContractSurface = getVncAllomorphyContractSurface;
    ...
    Object.defineProperty(api, "PATIENTIVO_NONACTIVE_SOURCE_SUFFIX_CONTRACTS", {
        configurable: true,
        enumerable: true,
        get() { return PATIENTIVO_NONACTIVE_SOURCE_SUFFIX_CONTRACTS; },
    });
    api.normalizePatientivoSourceSuffix = normalizePatientivoSourceSuffix;
    api.getPatientivoNonactiveSourceSuffixContract = getPatientivoNonactiveSourceSuffixContract;
    api.buildPatientivoSourceStageFrame = buildPatientivoSourceStageFrame;
    ...
    Object.defineProperty(api, "PATIENTIVO_PERFECTIVE_SOURCE_ENDING_CONTRACTS", {
        configurable: true,
        enumerable: true,
        get() { return PATIENTIVO_PERFECTIVE_SOURCE_ENDING_CONTRACTS; },
    });
    api.buildPatientivoPerfectiveEndingOrthographyConversions = buildPatientivoPerfectiveEndingOrthographyConversions;
    api.getPatientivoPerfectiveSourceStemContract = getPatientivoPerfectiveSourceStemContract;
    ...
    Object.defineProperty(api, "PATIENTIVO_ROOT_STOCK_VARIANT_CONSONANTS", {
        configurable: true,
        enumerable: true,
        get() { return PATIENTIVO_ROOT_STOCK_VARIANT_CONSONANTS; },
    });
    ...
    Object.defineProperty(api, "PATIENTIVO_MULTIPLE_DERIVATION_SOURCE_FAMILIES", {
        configurable: true,
        enumerable: true,
        get() { return PATIENTIVO_MULTIPLE_DERIVATION_SOURCE_FAMILIES; },
    });
    api.buildPatientivoMultipleDerivationContract = buildPatientivoMultipleDerivationContract;
