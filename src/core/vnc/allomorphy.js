...
function buildLesson24FirstTypeCausativePursuitFrame() {
    ...
    const remainingGaps = [
        "Final-vowel valence tendencies are not a complete lexical source model; valence-neutral stems still need Andrews source models plus orthography-bridge support and source-specific gates.",
...
const LESSON25_SECOND_TYPE_CAUSATIVE_SUBSECTION_INVENTORY = Object.freeze([
    ...
    Object.freeze({ id: "lesson25-hua-source", andrewsSection: "25.2", category: "hua-nonactive-source", directiveEs: "Tia reemplaza hua y frecuentemente muestra i final de base; hay formaciones inesperadas por terminacion de fuente.", engineSurface: "type-two causative source selection", implementationState: "partial", redirectAction: "source-gated" }),
    ...
    Object.freeze({ id: "lesson25-type-one-type-two-same-source", andrewsSection: "25.8", category: "type-one-type-two-parallel", directiveEs: "Una misma fuente puede tener causativo tipo 1 y tipo 2, con significado igual o diferente.", engineSurface: "parallel causative stem pool", implementationState: "partial", redirectAction: "source-gated" }),
...
const LESSON26_APPLICATIVE_SUBSECTION_INVENTORY = Object.freeze([
    ...
    Object.freeze({ id: "lesson26-final-i-sources", andrewsSection: "26.4", category: "final-i-source", directiveEs: "Fuentes en i agregan lia y pueden mostrar alargamiento o cambios si/ti/tz.", engineSurface: "source-shape allomorphy", implementationState: "partial", redirectAction: "source-gated" }),
    ...
    Object.freeze({ id: "lesson26-final-consonant-a-sources", andrewsSection: "26.7", category: "final-consonant-a-source", directiveEs: "Fuentes consonante+a cambian a por i y agregan lia, con rutas xi/ch/ti.", engineSurface: "source-shape allomorphy", implementationState: "partial", redirectAction: "source-gated" }),
    Object.freeze({ id: "lesson26-exceptions", andrewsSection: "26.8", category: "applicative-source-exceptions", directiveEs: "Clase D, clase B iya/eya y oya tienen excepciones propias antes de lia.", engineSurface: "exception stem pool", implementationState: "partial", redirectAction: "source-gated" }),
    ...
    Object.freeze({ id: "lesson26-two-types-same-source", andrewsSection: "26.12", category: "type-one-type-two-parallel", directiveEs: "Una misma fuente puede tener aplicativo tipo 1 y tipo 2, con significado igual o diferente.", engineSurface: "parallel applicative stem pool", implementationState: "partial", redirectAction: "source-gated" }),
...
function buildNonspecificObjectAllomorphySourceFrame({
    ...
    return Object.freeze({
        ...
        sourceSignature: [
            normalizedVerb,
            normalizedAnalysisVerb,
            normalizedObj1,
            normalizedObj2,
            normalizedObj3,
            hasOptionalSupportiveI === true ? "optional-supportive" : "no-optional-supportive",
            String(optionalSupportiveLetter || "").trim().toLowerCase(),
            normalizedSupportivePrecedingSurface,
            hasNonspecificValence === true ? "nonspecific-valence" : "specific-valence",
            hasSlashMarker === true ? "slash" : "no-slash",
            hasBoundMarker === true ? "bound" : "unbound",
            normalizedDirectionalPrefix,
        ].join("|"),
...
function buildNonspecificObjectAllomorphyOperationFrame(sourceFrame = null) {
    ...
    const soundSpellingFrames = Array.isArray(targetFrame.soundSpellingFrames)
        ? targetFrame.soundSpellingFrames
        : [];
    return Object.freeze({
        ...
        targetFrame: Object.freeze({
            ...
            soundSpellingFrames: Object.freeze(
                soundSpellingFrames.map((frame) => Object.freeze({ ...frame }))
            ),
        }),
        targetSignature: [
            sourceFrame.sourceSignature,
            String(targetFrame.verb || ""),
            String(targetFrame.analysisVerb || ""),
            String(targetFrame.obj1 || ""),
            soundSpellingFrames.map((frame) => [
                frame.ruleId || "",
                frame.grammarSlot || "",
                frame.sourceSegmentValue || "",
                frame.targetSegmentValue || "",
...
function getNonspecificObjectAllomorphyFrameMismatch({
    ...
    if (
        ...
        || operationFrame.operationId !== "andrews-nonspecific-object-allomorphy-realization"
        || operationFrame.routeFamily !== "nonspecific-object-allomorphy"
        || operationFrame.operationApplied !== "realize-object-prefix-supportive-marker-allomorphy"
    ...
    if (
        ...
        || expectedSourceFrame.sourceSignature !== sourceFrame.sourceSignature
    ) {
    ...
    const expectedFrames = Array.isArray(expectedTarget?.soundSpellingFrames)
        ? expectedTarget.soundSpellingFrames
        : [];
    const targetFrames = Array.isArray(targetFrame.soundSpellingFrames)
        ? targetFrame.soundSpellingFrames
        : [];
    const frameSignature = (frames) => frames.map((frame) => [
        frame.ruleId || "",
        frame.grammarSlot || "",
        frame.sourceSegmentValue || "",
        frame.targetSegmentValue || "",
    ...
    if (
        String(targetFrame.verb || "") !== String(expectedTarget?.verb || "")
        || String(targetFrame.analysisVerb || "") !== String(expectedTarget?.analysisVerb || "")
        || String(targetFrame.obj1 || "") !== String(expectedTarget?.obj1 || "")
        || frameSignature(targetFrames) !== frameSignature(expectedFrames)
    ) {
...
function applyNonspecificObjectAllomorphy({
    ...
    if (getNonspecificObjectAllomorphyFrameMismatch({ sourceFrame, operationFrame, request })) {
        return {
            verb: "",
            analysisVerb: "",
            obj1: "",
    ...
    return {
        ...
        soundSpellingFrames: Array.isArray(operationFrame.targetFrame.soundSpellingFrames)
            ? operationFrame.targetFrame.soundSpellingFrames.map((frame) => ({ ...frame }))
            : [],
    };
}
...
function collapseSerialStemDashInput(baseValue = "") {
    ...
    const sourceFrame = (
        ...
        && arguments[1].serialStemDashSourceFrame
    ) ? arguments[1].serialStemDashSourceFrame : null;
    const operationFrame = (
        arguments.length > 1
        && arguments[1]
        && typeof arguments[1] === "object"
        && arguments[1].serialStemDashOperationFrame
    ) ? arguments[1].serialStemDashOperationFrame : null;
    if (getSerialStemDashFrameMismatch({ sourceFrame, operationFrame })) {
        return baseValue;
    }
...
function buildSerialStemDashSourceFrame(baseValue = "") {
    ...
    const route = (
        collapseByPattern(/^([a-z]+)-ti$/i, "serial-ti-stem-dash-collapse", "ti", ["ti"])
        || collapseByPattern(/^([a-z]+)-ya$/i, "serial-ya-stem-dash-collapse", "ya", ["ya"])
        || collapseByPattern(/^([a-z]+)-u-a$/i, "serial-ua-stem-dash-collapse", "ua", ["u", "a"])
        || collapseByPattern(/^([a-z]+)-a-wi$/i, "serial-awi-stem-dash-collapse", "awi", ["a", "wi"])
        || collapseByPattern(/^([a-z]+)-i-wi$/i, "serial-iwi-stem-dash-collapse", "iwi", ["i", "wi"])
    );
    if (!route) {
        return null;
    }
...
function getSerialStemDashFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-serial-stem-dash-collapse"
        || operationFrame.routeFamily !== "serial-stem-dash-source"
        || operationFrame.sourceFrameKind !== sourceFrame.kind
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
    ...
    if (
        operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || (requestFrame && requestFrame.sourceSignature !== sourceFrame.sourceSignature)
    ) {
        return "contradictory-source-frame";
    }
    const expectedTargetSignature = [
        sourceFrame.sourceSignature,
        sourceFrame.targetInput,
    ].join("|");
    if (
        operationFrame.targetInput !== sourceFrame.targetInput
        || operationFrame.targetSignature !== expectedTargetSignature
    ) {
        return "contradictory-target-frame";
    }
    return "";
...
function getMorphStemAppendFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-morph-stem-append-realization"
        || operationFrame.routeFamily !== "morph-stem-transform"
        || operationFrame.operationApplied !== "append-morph-stem-segment"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
    ...
    if (
        normalizedSourceStem !== sourceFrame.sourceStem
        || specAppendText !== sourceFrame.appendText
        || operationFrame.sourceFrameKind !== sourceFrame.kind
        || operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || operationFrame.sourceStem !== sourceFrame.sourceStem
        || operationFrame.appendText !== sourceFrame.appendText
    ) {
        return "contradictory-source-frame";
    }
...
function getMorphStemPrependFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-morph-stem-prepend-realization"
        || operationFrame.routeFamily !== "morph-stem-transform"
        || operationFrame.operationApplied !== "prepend-morph-stem-segment"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
    ...
    if (
        normalizedSourceStem !== sourceFrame.sourceStem
        || specPrependText !== sourceFrame.prependText
        || operationFrame.sourceFrameKind !== sourceFrame.kind
        || operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || operationFrame.sourceStem !== sourceFrame.sourceStem
        || operationFrame.prependText !== sourceFrame.prependText
    ) {
        return "contradictory-source-frame";
    }
    const targetSurface = Array.isArray(operationFrame.targetSegmentFrames)
...
function buildMorphStemReplaceSuffixOperationFrame(sourceFrame = null) {
    ...
    const targetStem = sourceSuffix && sourceStem.endsWith(sourceSuffix)
        ? `${sourceStem.slice(0, -sourceSuffix.length)}${replacement}`
        : `${sourceStem}${replacement}`;
    if (!targetStem) {
        return null;
    }
    const baseSurface = sourceSuffix && sourceStem.endsWith(sourceSuffix)
        ? sourceStem.slice(0, -sourceSuffix.length)
        : sourceStem;
    return Object.freeze({
        kind: "andrews-typed-operation-frame",
...
function getMorphStemReplaceSuffixFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-morph-stem-replace-suffix-realization"
        || operationFrame.routeFamily !== "morph-stem-transform"
        || operationFrame.operationApplied !== "replace-morph-stem-suffix-segment"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
    ...
    if (
        normalizedSourceStem !== sourceFrame.sourceStem
        || specSourceSuffix !== sourceFrame.sourceSuffix
        || specReplacement !== sourceFrame.replacement
        || operationFrame.sourceFrameKind !== sourceFrame.kind
        || operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || operationFrame.sourceStem !== sourceFrame.sourceStem
        || operationFrame.sourceSuffix !== sourceFrame.sourceSuffix
        || operationFrame.replacement !== sourceFrame.replacement
    ) {
        return "contradictory-source-frame";
    }
    const targetSurface = Array.isArray(operationFrame.targetSegmentFrames)
...
function buildMorphStemStripPrefixOperationFrame(sourceFrame = null) {
    ...
    const targetStem = sourceStem.startsWith(prefix)
        ? sourceStem.slice(prefix.length)
        : sourceStem;
    if (!targetStem) {
        return null;
    }
...
function getMorphStemStripPrefixFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-morph-stem-strip-prefix-realization"
        || operationFrame.routeFamily !== "morph-stem-transform"
        || operationFrame.operationApplied !== "strip-morph-stem-prefix-segment"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
    ...
    if (
        normalizedSourceStem !== sourceFrame.sourceStem
        || specPrefix !== sourceFrame.prefix
        || operationFrame.sourceFrameKind !== sourceFrame.kind
        || operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || operationFrame.sourceStem !== sourceFrame.sourceStem
        || operationFrame.prefix !== sourceFrame.prefix
    ) {
        return "contradictory-source-frame";
    }
    const targetSurface = Array.isArray(operationFrame.targetSegmentFrames)
...
function buildMorphStemDeletionShiftSourceFrame({
    ...
    return Object.freeze({
        ...
        sourceSignature: [
            normalizedSourceStem,
            normalizedDeletionVariant,
            isTransitive === true ? "transitive" : "intransitive",
            normalizeRuleBase(String(sourceBase || "").trim().toLowerCase()),
            normalizeRuleBase(String(sourceSuffix || "").trim().toLowerCase()),
        ].join("|"),
        consumesRenderedInput: false,
...
function getMorphStemDeletionShiftFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-morph-stem-deletion-shift-realization"
        || operationFrame.routeFamily !== "morph-stem-transform"
        || operationFrame.operationApplied !== "realize-morph-stem-deletion-shift"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
    ...
    if (
        normalizedSourceStem !== sourceFrame.sourceStem
        || specDeletionVariant !== sourceFrame.deletionVariant
        || specIsTransitive !== (sourceFrame.isTransitive === true)
        || operationFrame.sourceFrameKind !== sourceFrame.kind
        || operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || operationFrame.sourceStem !== sourceFrame.sourceStem
        || operationFrame.deletionVariant !== sourceFrame.deletionVariant
        || (operationFrame.isTransitive === true) !== (sourceFrame.isTransitive === true)
    ) {
        return "contradictory-source-frame";
    }
    const targetSurface = Array.isArray(operationFrame.targetSegmentFrames)
...
function buildMorphStemTruncateNonactiveBaseSourceFrame({
    ...
    return Object.freeze({
        ...
        sourceSignature: [
            normalizedSourceStem,
            dropFinalW === true ? "drop-final-w" : "keep-final-w",
            tzToCh === true ? "tz-to-ch" : "keep-tz",
            normalizeRuleBase(String(sourceBase || "").trim().toLowerCase()),
            normalizeRuleBase(String(sourceSuffix || "").trim().toLowerCase()),
        ].join("|"),
        consumesRenderedInput: false,
...
function getMorphStemTruncateNonactiveBaseFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-morph-stem-truncate-nonactive-base-realization"
        || operationFrame.routeFamily !== "morph-stem-transform"
        || operationFrame.operationApplied !== "realize-morph-stem-truncate-nonactive-base"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
    ...
    if (
        normalizedSourceStem !== sourceFrame.sourceStem
        || specDropFinalW !== (sourceFrame.dropFinalW === true)
        || specTzToCh !== (sourceFrame.tzToCh === true)
        || operationFrame.sourceFrameKind !== sourceFrame.kind
        || operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || operationFrame.sourceStem !== sourceFrame.sourceStem
        || (operationFrame.dropFinalW === true) !== (sourceFrame.dropFinalW === true)
        || (operationFrame.tzToCh === true) !== (sourceFrame.tzToCh === true)
    ) {
        return "contradictory-source-frame";
    }
    const targetSurface = Array.isArray(operationFrame.targetSegmentFrames)
...
function buildMorphStemWaOnsetVariantSourceFrame({
    ...
    return Object.freeze({
        ...
        sourceSignature: [
            normalizedSourceStem,
            blockCh === true ? "block-ch" : "allow-ch",
            blockOnsetReplacement === true ? "block-onset-replacement" : "allow-onset-replacement",
            normalizeRuleBase(String(sourceBase || "").trim().toLowerCase()),
            normalizeRuleBase(String(sourceSuffix || "").trim().toLowerCase()),
        ].join("|"),
        consumesRenderedInput: false,
...
function getMorphStemWaOnsetVariantFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-morph-stem-wa-onset-variant-realization"
        || operationFrame.routeFamily !== "morph-stem-transform"
        || operationFrame.operationApplied !== "realize-morph-stem-wa-onset-variant"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
    ...
    if (
        normalizedSourceStem !== sourceFrame.sourceStem
        || specBlockCh !== (sourceFrame.blockCh === true)
        || specBlockOnsetReplacement !== (sourceFrame.blockOnsetReplacement === true)
        || operationFrame.sourceFrameKind !== sourceFrame.kind
        || operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || operationFrame.sourceStem !== sourceFrame.sourceStem
        || (operationFrame.blockCh === true) !== (sourceFrame.blockCh === true)
        || (operationFrame.blockOnsetReplacement === true) !== (sourceFrame.blockOnsetReplacement === true)
    ) {
        return "contradictory-source-frame";
    }
    const targetSurface = Array.isArray(operationFrame.targetSegmentFrames)
...
function deriveMorphStemNonactiveUTarget({
    ...
    if (
        ...
        && (
            nucleus === "i"
            || (nucleus === "a" && allowFinalTaReplacement === true)
        )
    ) {
        if (blockCh !== true && blockOnsetReplacement !== true) {
            letters[onsetIndex] = "ch";
...
function buildMorphStemNonactiveUSourceFrame({
    ...
    return Object.freeze({
        ...
        sourceSignature: [
            normalizedSourceStem,
            normalizedLastOnset,
            normalizedLastNucleus,
            blockCh === true ? "block-ch" : "allow-ch",
            blockOnsetReplacement === true ? "block-onset-replacement" : "allow-onset-replacement",
            allowFinalTaReplacement === true ? "allow-final-ta-replacement" : "block-final-ta-replacement",
            normalizeRuleBase(String(sourceBase || "").trim().toLowerCase()),
            normalizeRuleBase(String(sourceSuffix || "").trim().toLowerCase()),
        ].join("|"),
        consumesRenderedInput: false,
...
function getMorphStemNonactiveUFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-morph-stem-nonactive-u-realization"
        || operationFrame.routeFamily !== "morph-stem-transform"
        || operationFrame.operationApplied !== "realize-morph-stem-nonactive-u"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
    ...
    if (
        normalizedSourceStem !== sourceFrame.sourceStem
        || specLastOnset !== sourceFrame.lastOnset
        || specLastNucleus !== sourceFrame.lastNucleus
        || specBlockCh !== (sourceFrame.blockCh === true)
        || specBlockOnsetReplacement !== (sourceFrame.blockOnsetReplacement === true)
        || specAllowFinalTaReplacement !== (sourceFrame.allowFinalTaReplacement === true)
        || operationFrame.sourceFrameKind !== sourceFrame.kind
        || operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || operationFrame.sourceStem !== sourceFrame.sourceStem
        || operationFrame.lastOnset !== sourceFrame.lastOnset
        || operationFrame.lastNucleus !== sourceFrame.lastNucleus
        || (operationFrame.blockCh === true) !== (sourceFrame.blockCh === true)
        || (operationFrame.blockOnsetReplacement === true) !== (sourceFrame.blockOnsetReplacement === true)
        || (operationFrame.allowFinalTaReplacement === true) !== (sourceFrame.allowFinalTaReplacement === true)
    ) {
        return "contradictory-source-frame";
    }
    const targetSurface = Array.isArray(operationFrame.targetSegmentFrames)
...
function getMorphStemNonactiveUwaFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-morph-stem-nonactive-uwa-realization"
        || operationFrame.routeFamily !== "morph-stem-transform"
        || operationFrame.operationApplied !== "realize-morph-stem-nonactive-uwa"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
...
function buildTransformMorphStemSpec({
    ...
    return Object.freeze({
        ...
        tzToCh: tzToCh === true,
        isTransitive: isTransitive === true,
        ...(typeOneSourceFrame ? { typeOneSourceFrame } : {}),
        ...(typeOneOperationFrame ? { typeOneOperationFrame } : {}),
        ...(morphStemSourceFrame ? { morphStemSourceFrame } : {}),
        ...(morphStemOperationFrame ? { morphStemOperationFrame } : {}),
    });
}
...
function buildAppendMorphStemSpec(sourceStem = "", appendText = "", options = {}) {
    ...
    const resolvedSourceStem = sourceStemSpec
...
function getIntransitiveTypeOneFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-intransitive-type-one-stem-realization"
        || operationFrame.routeFamily !== "intransitive-type-one"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
    ) {
        return "operation-frame-required";
    ...
    if (
        ...
        || operationFrame.sourceSignature !== sourceFrame.sourceSignature
        || operationFrame.sourceStem !== sourceFrame.sourceStem
        || operationFrame.typeOneTarget !== sourceFrame.typeOneTarget
        || operationFrame.rightEdgeClass !== sourceFrame.rightEdgeClass
    ) {
        return "contradictory-source-frame";
    }
    const targetSurface = Array.isArray(operationFrame.targetSegmentFrames)
...
function buildIntransitiveTypeOneStem(baseStem = "", typeOneTarget = null, options = {}) {
    ...
    if (getIntransitiveTypeOneFrameMismatch({ sourceFrame, operationFrame })) {
        return "";
    }
    ...
    if (
        normalizedBaseStem
        && normalizedBaseStem !== sourceFrame.sourceStem
    ) {
        return "";
    }
    if (
        normalizedTarget
        && normalizedTarget !== sourceFrame.typeOneTarget
    ) {
        return "";
    }
...
function buildIntransitiveTypeOneMorphStemSpec(baseStem = "", typeOneTarget = null, options = {}) {
    const sourceFrame = buildIntransitiveTypeOneSourceFrame({ baseStem, typeOneTarget });
...
function getPreteritAgentiveNominalConnectorFrame(subjectSuffix = "") {
    ...
    const frames = {
        ki: { num1: "ki", num2: "0", dyad: "ki-0" },
        k: { num1: "k", num2: "0", dyad: "k-0" },
        ket: { num1: "k", num2: "et", dyad: "k-et" },
        w: { num1: "w", num2: "0", dyad: "w-0" },
        wan: { num1: "w", num2: "an", dyad: "w-an" },
    };
...
function buildNominalNum1Num2({
    ...
    const preteritAgentiveConnector = normalizedNominalKind === "agentivo-preterito"
        ? getPreteritAgentiveNominalConnectorFrame(surface)
        : null;
    return Object.freeze({
        version: 1,
...
function buildVerbDerivedNominalizationProfile({
    ...
    const operationalSourceFrame = typeof buildAndrewsCnvCnnOperationalSourceFrame === "function"
        ? buildAndrewsCnvCnnOperationalSourceFrame({
            nominalKind: kind,
        ...
        : null;
    const operationalOperationFrame = typeof buildAndrewsCnvCnnOperationalOperationFrame === "function"
        ? buildAndrewsCnvCnnOperationalOperationFrame(operationalSourceFrame)
        : null;
    const operationalSuboperationFrame = typeof buildAndrewsCnvCnnOperationalSuboperationFrame === "function"
...
function truncateNonactiveBase(stem, options = {}) {
    ...
    const operationFrame = options?.operationFrame || null;
    if (
        getMorphStemTruncateNonactiveBaseFrameMismatch({
            spec: {
...
function buildNonactiveUStem(stem, lastOnset, lastNucleus, options = {}) {
    ...
    const operationFrame = options?.operationFrame || null;
    if (
        getMorphStemNonactiveUFrameMismatch({
            spec: {
                lastOnset,
                lastNucleus,
...
function buildNonactiveUwaStem(stem, lastOnset, lastNucleus, options = {}) {
    ...
    const operationFrame = options?.operationFrame || null;
    if (
        getMorphStemNonactiveUwaFrameMismatch({
            spec: {
                lastOnset,
                lastNucleus,
                blockCh: options.blockCh === true,
...
function buildNonactiveStemSelectionSourceFrame({
    ...
    const sourceContext = (
        nonactiveRuleSource
        && typeof nonactiveRuleSource === "object"
    ) ? nonactiveRuleSource : buildNonactiveRuleSourceContext(verb, analysisVerb, options || {});
    ...
    const analysisStem = normalizeDerivationStemValue(
        sourceContext.analysisStem || analysisVerb || verb || sourceStem
    );
    ...
    return Object.freeze({
        ...
        sourceSignature: [
            ...
            analysisStem,
            ruleBase,
            options?.isTransitive === true ? "transitive" : "intransitive",
            options?.isYawi === true ? "yawi" : "not-yawi",
            normalizeDerivationStemValue(options?.rootPlusYaBase || ""),
        ].join("|"),
        consumesRenderedInput: false,
        displayStringsAuthorizeGrammar: false,
    });
...
function deriveNonactiveStemSelectionTargetFromSourceFrame(sourceFrame = null) {
    ...
    const fallbackStem = fallbackSpec
        ? realizeMorphStemSpec(fallbackSpec, "")
        : normalizeDerivationStemValue(optionsList[0]?.stem || source);
    return {
...
function getNonactiveStemSelectionFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-nonactive-stem-selection-realization"
        || operationFrame.routeFamily !== "nonactive-stem-selection"
        || operationFrame.operationApplied !== "select-nonactive-stem-from-source-frame"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || !operationFrame.targetFrame
    ) {
    ...
    const expectedTargetSignature = [
        sourceFrame.sourceSignature,
        expectedTarget?.targetStem || "",
        expectedTarget?.selectedSuffix || "",
    ].join("|");
    const targetFrame = operationFrame.targetFrame || {};
    if (
        operationFrame.targetSignature !== expectedTargetSignature
        || String(operationFrame.targetStem || "") !== String(expectedTarget?.targetStem || "")
        || String(targetFrame.stem || "") !== String(expectedTarget?.targetStem || "")
        || String(targetFrame.selectedSuffix || "") !== String(expectedTarget?.selectedSuffix || "")
    ) {
        return "contradictory-target-frame";
    }
    return "";
...
function deriveNonactiveStem(verb, analysisVerb, options = {}) {
    ...
    if (getNonactiveStemSelectionFrameMismatch({ sourceFrame, operationFrame, requestFrame })) {
        return "";
    }
    return String(operationFrame.targetStem || "");
}
...
function derivePatientivoStemFromNonactiveEntries(sourceFrame = null) {
    ...
    const options = {
        ...
        stemSpec: (
            sourceFrame?.stemSpec
            && typeof sourceFrame.stemSpec === "object"
            && sourceFrame.stemSpec.kind
        ) ? sourceFrame.stemSpec : null,
    };
    if (!stem || !suffix) {
        return [];
    }
...
function getPatientivoNonactiveSourceFrameSignature({
    ...
    return [
        ...
        normalizedSuffix,
        isTransitive === true ? "transitive" : "intransitive",
        String(baseInfo?.lastOnset || ""),
        String(baseInfo?.lastNucleus || ""),
        normalizeDerivationStemValue(stemSpec?.sourceBase || ""),
        normalizePatientivoSourceSuffix(stemSpec?.sourceSuffix || ""),
        String(stemSpec?.kind || ""),
    ].join("|");
}
...
function buildPatientivoNonactiveStemSourceFrame({
    ...
    const normalizedStem = normalizeDerivationStemValue(
        resolvedStemSpec
            ? realizeMorphStemSpec(resolvedStemSpec, stem || "")
            : stem
    );
    const normalizedSuffix = normalizePatientivoSourceSuffix(suffix);
    const sourceSuffixContract = getPatientivoNonactiveSourceSuffixContract(normalizedSuffix);
    ...
    const normalizedBaseInfo = (baseInfo && typeof baseInfo === "object")
        ? Object.freeze({
...
function getPatientivoNonactiveEntrySignature(entries = []) {
    return (Array.isArray(entries) ? entries : []).map((entry) => [
        normalizeDerivationStemValue(entry?.stem || ""),
        String(entry?.suffix || ""),
        Array.isArray(entry?.allowedSuffixes) ? entry.allowedSuffixes.join("/") : "",
        entry?.blocksAbsolutiveZeroNominalMarker === true ? "blocks-zero" : "allows-zero",
        entry?.patientiveSourceStageFrame?.operation || "",
        entry?.patientiveSourceStageFrame?.sourceSuffix || "",
    ].join(":")).join("|");
}
...
function buildPatientivoNonactiveStemOperationFrame(sourceFrame = null) {
    ...
    return Object.freeze({
        ...
        targetSignature: [
            sourceFrame.sourceSignature,
            getPatientivoNonactiveEntrySignature(targetEntries),
        ].join("|"),
        consumesRenderedInput: false,
        displayStringsAuthorizeGrammar: false,
    });
...
function getPatientivoNonactiveStemFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-patientivo-nonactive-stem-realization"
        || operationFrame.routeFamily !== "patientivo-nonactive-source"
        || operationFrame.operationApplied !== "realize-patientive-stems-from-nonactive-source-frame"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || !Array.isArray(operationFrame.targetEntries)
    ) {
        return "operation-frame-required";
    }
    if (
    ...
    const expectedTargetSignature = [
        sourceFrame.sourceSignature,
        getPatientivoNonactiveEntrySignature(expectedEntries),
    ].join("|");
    const actualTargetSignature = [
        sourceFrame.sourceSignature,
        getPatientivoNonactiveEntrySignature(operationFrame.targetEntries),
    ].join("|");
    if (
        operationFrame.targetSignature !== expectedTargetSignature
        || actualTargetSignature !== expectedTargetSignature
    ) {
        return "contradictory-target-frame";
    }
    return "";
...
function getPatientivoStemFromNonactive(stem, suffix, options = {}) {
    ...
    if (getPatientivoNonactiveStemFrameMismatch({ sourceFrame, operationFrame, requestFrame })) {
        return [];
    }
    return operationFrame.targetEntries.map((entry) => ({ ...entry }));
}
...
function buildPatientivoDerivations({
    ...
    options.forEach((option) => {
        ...
        const patientivoNonactiveOperationFrame = buildPatientivoNonactiveStemOperationFrame(
...
function getPatientivoRootStockStemSpecSignature(stemSpec = null) {
    ...
    return [
        String(stemSpec.kind || ""),
        normalizeDerivationStemValue(stemSpec.sourceBase || stemSpec.sourceStem || ""),
        normalizeDerivationStemValue(stemSpec.targetStem || stemSpec.stem || ""),
        String(stemSpec.sourceSuffix || ""),
        String(stemSpec.resultSuffix || stemSpec.replacement || stemSpec.appendText || ""),
        String(stemSpec.operationId || ""),
    ].join(":");
}
...
function buildPatientivoRootStockStemSourceFrame({
    ...
    const sourceSyllables = getSyllables(normalizedSourceStem, {
        analysis: true,
        assumeFinalV: true,
    });
    const gateSyllables = getSyllables(normalizedGateStem, {
        analysis: true,
        assumeFinalV: true,
    });
    const endsWithLua = normalizedSourceStem.endsWith("lua");
    const gateEndsWithLua = normalizedGateStem.endsWith("lua");
    const endsWithProductiveNi = endsWithOpenSyllableSuffix(normalizedSourceStem, "n", "i");
    const endsWithProductiveNa = endsWithOpenSyllableSuffix(normalizedSourceStem, "n", "a");
    const gateEndsWithProductiveNi = endsWithOpenSyllableSuffix(normalizedGateStem, "n", "i");
    const gateEndsWithProductiveNa = endsWithOpenSyllableSuffix(normalizedGateStem, "n", "a");
    const endsWithPlainWa = endsWithOpenSyllableSuffix(normalizedSourceStem, "w", "a");
    const endsWithPlainWi = endsWithOpenSyllableSuffix(normalizedSourceStem, "w", "i");
    const gateEndsWithPlainWa = endsWithOpenSyllableSuffix(normalizedGateStem, "w", "a");
    const gateEndsWithPlainWi = endsWithOpenSyllableSuffix(normalizedGateStem, "w", "i");
    const endsWithProductiveKi = endsWithOpenSyllableSuffix(normalizedSourceStem, "k", "i");
    const gateEndsWithProductiveKi = endsWithOpenSyllableSuffix(normalizedGateStem, "k", "i");
    const endsWithProductiveKa = endsWithOpenSyllableSuffix(normalizedSourceStem, "k", "a");
    const gateEndsWithProductiveKa = endsWithOpenSyllableSuffix(normalizedGateStem, "k", "a");
    ...
    const endsWithProductiveYa = endsWithOpenSyllableSuffix(normalizedSourceStem, "y", "a");
    const gateEndsWithProductiveYa = endsWithOpenSyllableSuffix(normalizedGateStem, "y", "a");
    const sourceIwiAwiUaSuffix = normalizedSourceStem.endsWith("iwi")
        ? "iwi"
        : (normalizedSourceStem.endsWith("awi")
            ? "awi"
            : (normalizedSourceStem.endsWith("ua") ? "ua" : ""));
    const gateIwiAwiUaSuffix = normalizedGateStem.endsWith("iwi")
        ? "iwi"
        : (normalizedGateStem.endsWith("awi")
            ? "awi"
            : (normalizedGateStem.endsWith("ua") ? "ua" : ""));
    const getCoreBeforeSuffixSyllablesForStem = (suffix = "", sourceBase = "") => {
        ...
        return getSyllables(sourceBase.slice(0, -suffix.length), {
            analysis: true,
            assumeFinalV: true,
        });
    };
    const hasShortOpenCoreBeforeSuffix = (suffix = "", sourceBase = "", coreSyllables = null) => {
        ...
        const resolvedCoreSyllables = Array.isArray(coreSyllables)
            ? coreSyllables
            : getCoreBeforeSuffixSyllablesForStem(suffix, sourceBase);
    ...
    const isShortWiSuperpositionCandidate = endsWithPlainWi
        && gateEndsWithPlainWi
        && hasShortOpenCoreBeforeSuffix("wi", normalizedGateStem);
    const useIwiFamilySource = !isShortWiSuperpositionCandidate
        && sourceIwiAwiUaSuffix === "iwi"
        && gateIwiAwiUaSuffix === "iwi";
    const useAwiFamilySource = !isShortWiSuperpositionCandidate
        && sourceIwiAwiUaSuffix === "awi"
        && gateIwiAwiUaSuffix === "awi";
    const useUaFamilySource = isTransitive !== true
        && !isShortWiSuperpositionCandidate
        && sourceIwiAwiUaSuffix === "ua"
        && gateIwiAwiUaSuffix === "ua";
    const isCvkiShape = (
        sourceSyllables.length === 2
        && sourceSyllables[0]?.form === "CV"
        && sourceSyllables[1]?.form === "CV"
        && sourceSyllables[1]?.onset === "k"
        && sourceSyllables[1]?.nucleus === "i"
    );
    const gateIsCvkiShape = (
        gateSyllables.length === 2
        && gateSyllables[0]?.form === "CV"
        && gateSyllables[1]?.form === "CV"
        && gateSyllables[1]?.onset === "k"
        && gateSyllables[1]?.nucleus === "i"
    );
    const sourceEndingFamily = endsWithLua && isTransitive === true
        ? "lua"
        : (
            endsWithProductiveNi
                ? "productive-ni"
                : (
                                                            ...
                                                            useUaFamilySource
                                                                ...
                                                                : (
                                                                    endsWithPlainWi
                                                                        ? "plain-wi"
                                                                        : (
                                                                            endsWithProductiveKi
                                                                                ? "productive-ki"
                                                                                : (
                                                                                    endsWithProductiveKa
                                                                                        ? "productive-ka"
                                                                                        : (endsWithProductiveTiya ? "productive-tiya" : (endsWithProductiveYa ? "productive-ya" : ""))
                                                                                )
                                                                        )
                                                                )
                                                        )
    ...
    const gateEndingFamily = gateEndsWithLua && isTransitive === true
        ? "lua"
        : (
            gateEndsWithProductiveNi
                ? "productive-ni"
                : (
                    gateEndsWithProductiveNa
                        ? "productive-na"
                        : (
                            gateEndsWithPlainWa
                                ? "plain-wa"
                                                            ...
                                                            useUaFamilySource
                                                                ...
                                                                : (
                                                                    gateEndsWithPlainWi
                                                                        ? "plain-wi"
                                                                        : (
                                                                            gateEndsWithProductiveKi
                                                                                ? "productive-ki"
                                                                                : (
                                                                                    gateEndsWithProductiveKa
                                                                                        ? "productive-ka"
                                                                                        : (gateEndsWithProductiveTiya ? "productive-tiya" : (gateEndsWithProductiveYa ? "productive-ya" : ""))
                                                                                )
                                                                        )
                                                                )
                                                        )
    ...
    const sourceSuffix = endsWithLua && isTransitive === true
        ? "ua"
        : (
            endsWithProductiveNi
                ? "ni"
                                                                ...
                                                                : (
                                                                    endsWithPlainWi
                                                                        ...
                                                                        : (
                                                                            endsWithProductiveKi
                                                                                ? "ki"
                                                                                : (endsWithProductiveKa ? "ka" : (endsWithProductiveTiya ? "tiya" : (endsWithProductiveYa ? "ya" : "")))
                                                                        )
                                                                )
                                                        )
                                                )
    ...
    const gateSuffix = gateEndsWithLua && isTransitive === true
        ? "ua"
        : (
            gateEndsWithProductiveNi
                ? "ni"
                                                                ...
                                                                : (
                                                                    gateEndsWithPlainWi
                                                                        ...
                                                                        : (
                                                                            gateEndsWithProductiveKi
                                                                                ? "ki"
                                                                                : (gateEndsWithProductiveKa ? "ka" : (gateEndsWithProductiveTiya ? "tiya" : (gateEndsWithProductiveYa ? "ya" : "")))
                                                                        )
                                                                )
                                                        )
                                                )
    ...
    const sourceCoreSyllables = sourceCore
        ? getSyllables(sourceCore, { analysis: true, assumeFinalV: true })
        : [];
    const gateCore = gateSuffix ? normalizedGateStem.slice(0, -gateSuffix.length) : "";
    const gateCoreSyllables = gateCore
        ? getSyllables(gateCore, { analysis: true, assumeFinalV: true })
        : [];
    ...
    const isShortWiSuperpositionSource = endsWithPlainWi
        && gateEndsWithPlainWi
        && sourceCore
        && sourceEndingFamily === "plain-wi"
        && gateEndingFamily === "plain-wi"
        && hasShortOpenCoreBeforeSuffix("wi", normalizedGateStem, gateCoreSyllables);
    const routeOperation = (() => {
        if (
            isTransitive === true
            && sourceEndingFamily === "lua"
            && gateEndingFamily === "lua"
            && sourceCore
            && sourceSyllables.length > 1
        ) {
        ...
        if (
            isTransitive !== true
            && sourceEndingFamily
            && sourceEndingFamily === gateEndingFamily
            && (sourceEndingFamily === "productive-ni" || sourceEndingFamily === "productive-na")
            && sourceCore
            && sourceSyllables.length > 1
        ) {
        ...
        if (
            ...
            && gateEndingFamily === "plain-wi"
            && sourceCore
            && gateCoreSyllables.length > 1
        ) {
            return "plain-wi-root-stock-stem";
        }
        if (
            isTransitive !== true
            && sourceEndingFamily === "iwi-family"
            && gateEndingFamily === "iwi-family"
            && sourceSuffix === "iwi"
            && gateSuffix === "iwi"
            && sourceCore
            && (
                ...
                || (
                    gateCoreSyllables.length === 1
                    && isSyllableSequencePronounceable(sourceCore)
                )
            )
        ) {
            return hasFinalConsonantCluster(sourceCore)
                ? "iwi-final-cluster-root-stock-stem"
                : "iwi-family-root-stock-stem";
        }
        if (
            isTransitive !== true
            && sourceEndingFamily === "awi-family"
            && gateEndingFamily === "awi-family"
            && sourceSuffix === "awi"
            && gateSuffix === "awi"
            && sourceCore
            && (
                gateCoreSyllables.length > 1
                || normalizedGateStem === "awi"
            )
        ) {
            return hasFinalConsonantCluster(sourceCore)
                ? "awi-final-cluster-root-stock-stem"
                : "awi-family-root-stock-stem";
        }
        if (
            isTransitive !== true
            && sourceEndingFamily === "ua-family"
            && gateEndingFamily === "ua-family"
            && sourceSuffix === "ua"
            && gateSuffix === "ua"
            && sourceCore
            && gateCoreSyllables.length > 1
            ...
            return hasFinalConsonantCluster(sourceCore)
                ...
                : "ua-family-root-stock-stem";
        }
        if (
            isTransitive !== true
            && sourceEndingFamily === "plain-wa"
            && gateEndingFamily === "plain-wa"
            && sourceCore
            && gateCoreSyllables.length > 1
        ) {
        ...
        if (
            isTransitive !== true
            && sourceEndingFamily === "productive-ki"
            && gateEndingFamily === "productive-ki"
            && isCvkiShape
            && gateIsCvkiShape
            && sourceCore
        ) {
        ...
        if (
            isTransitive !== true
            && sourceEndingFamily === "productive-ka"
            && gateEndingFamily === "productive-ka"
            && sourceCore
            && sourceCoreSyllables.length > 1
        ...
        if (
            isTransitive !== true
            && sourceEndingFamily === "productive-tiya"
            && gateEndingFamily === "productive-tiya"
            && sourceCore
        ) {
        ...
        if (
            isTransitive !== true
            && sourceEndingFamily === "productive-ya"
            && gateEndingFamily === "productive-ya"
            && sourceCore
        ) {
    ...
    const rightEdgeFrame = Object.freeze({
        ...
        sourceCore,
        targetStem: routeOperation === "productive-ya-l-root-stock-stem"
            ? `${sourceCore}l`
            : (
                routeOperation === "iwi-final-cluster-root-stock-stem"
                    ? `${sourceCore}i`
                    : (
                        routeOperation === "awi-final-cluster-root-stock-stem"
                            ? `${sourceCore}a`
                            : (
                                routeOperation === "ua-final-cluster-root-stock-stem"
                                    ? `${sourceCore}u`
                                    : sourceCore
                            )
                    )
            ),
        replacementSegment: routeOperation === "productive-ya-l-root-stock-stem"
            ? "l"
            : (
                routeOperation === "iwi-final-cluster-root-stock-stem"
                    ? "i"
                    : (
                        routeOperation === "awi-final-cluster-root-stock-stem"
                            ? "a"
                            : (routeOperation === "ua-final-cluster-root-stock-stem" ? "u" : "")
                    )
            ),
        ...
        startsWithVj,
        variantConsonants: routeOperation === "productive-ni-root-stock-stem"
            || routeOperation === "productive-na-root-stock-stem"
            || routeOperation === "plain-wa-root-stock-stem"
            || routeOperation === "plain-wi-root-stock-stem"
            || routeOperation === "productive-ki-root-stock-stem"
            ? Object.freeze(["k", "ch", "s", "sh"])
            : (
                routeOperation === "productive-ka-root-stock-stem"
                    ? Object.freeze(["k", "ch", "j"])
                    : (
                        routeOperation === "short-wi-superposition-root-stock-stem"
                            ? Object.freeze(["k"])
                            : (
                                routeOperation === "awi-family-root-stock-stem"
                                    || routeOperation === "awi-final-cluster-root-stock-stem"
                                    ? Object.freeze(["k", "ch", "s", "sh"])
                                    : Object.freeze([])
                            )
        ...
        additionalVariantConsonants: routeOperation === "productive-ki-root-stock-stem"
            ? Object.freeze(["j"])
            : Object.freeze([]),
        hasProductiveKiSVariant: routeOperation === "productive-ki-root-stock-stem"
            && (normalizedSourceStem === "waki" || normalizedGateStem === "waki"),
        routeOperation,
        routeSupported,
    });
    return Object.freeze({
        ...
        sourceSignature: [
            ...
            normalizedGateStem,
            isTransitive === true ? "transitive" : "intransitive",
            rightEdgeFrame.sourceEndingFamily,
            rightEdgeFrame.gateEndingFamily,
            rightEdgeFrame.sourceCore,
            rightEdgeFrame.routeOperation,
            rightEdgeFrame.routeSupported === true ? "supported" : "blocked",
            getPatientivoRootStockStemSpecSignature(resolvedStemSpec),
        ].join("|"),
        consumesRenderedInput: false,
        displayStringsAuthorizeGrammar: false,
    });
...
function getPatientivoRootStockEntrySignature(entries = []) {
    return (Array.isArray(entries) ? entries : []).map((entry) => [
        normalizeDerivationStemValue(entry?.stem || entry?.targetStem || ""),
        String(entry?.suffix || entry?.outputConnector || ""),
        entry?.lockNominalMarker === true ? "locked" : "unlocked",
        entry?.rootStockSourceContract?.outputSurface || "",
        entry?.rootStockSourceContract?.routeStemOnly === true ? "stem-only" : "with-connector",
        getPatientivoRootStockStemSpecSignature(entry?.stemSpec || entry?.targetStemSpec || null),
    ].join(":")).join("|");
}
...
function derivePatientivoRootStockEntriesFromSourceFrame(sourceFrame = null) {
    ...
    const targetStem = normalizeDerivationStemValue(
        rightEdgeFrame.targetStem || rightEdgeFrame.sourceCore || ""
    );
    if (!targetStem) {
        return [];
    }
    const sourceSuffix = rightEdgeFrame.sourceSuffix || "ua";
    const targetStemSpec = buildReplaceSuffixMorphStemSpec(
        (
            sourceFrame.sourceStemSpec
            && typeof sourceFrame.sourceStemSpec === "object"
            && sourceFrame.sourceStemSpec.kind
        ) ? sourceFrame.sourceStemSpec : sourceFrame.sourceStem,
        sourceSuffix,
        String(rightEdgeFrame.replacementSegment || ""),
        {
    ...
    if (
        rightEdgeFrame.routeOperation !== "productive-ni-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "productive-na-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "plain-wa-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "plain-wi-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "productive-ki-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "productive-ka-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "short-wi-superposition-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "iwi-family-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "iwi-final-cluster-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "awi-family-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "awi-final-cluster-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "ua-family-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "ua-final-cluster-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "productive-tiya-root-stock-stem"
        && rightEdgeFrame.routeOperation !== "productive-ya-l-root-stock-stem"
    ) {
        return [];
    }
    const entries = [];
    ...
    const variantConsonants = rightEdgeFrame.routeOperation === "productive-ki-root-stock-stem"
        ? [
            "k",
            "ch",
            "j",
            ...(rightEdgeFrame.hasProductiveKiSVariant === true ? ["s"] : []),
        ]
        : (
            rightEdgeFrame.routeOperation === "awi-family-root-stock-stem"
                || rightEdgeFrame.routeOperation === "awi-final-cluster-root-stock-stem"
                ? []
                : (rightEdgeFrame.variantConsonants || [])
        );
    ...
    if (rightEdgeFrame.routeOperation === "plain-wi-root-stock-stem") {
        const targetLetters = splitVerbLetters(targetStem);
        const lastTarget = targetLetters[targetLetters.length - 1] || "";
        const reducedTargetStem = isVerbLetterVowel(lastTarget)
            ? targetStem.slice(0, -1)
            : targetStem;
        const reducedTargetStemSpec = isVerbLetterVowel(lastTarget)
            ? buildReplaceSuffixMorphStemSpec(targetStemSpec || targetStem, lastTarget, "", {
    ...
    if (
        ...
        if (lastTarget === "y" && isVerbLetterVowel(prevTarget)) {
...
function buildPatientivoRootStockStemOperationFrame(sourceFrame = null) {
    ...
    return Object.freeze({
        ...
        sourceFrameKind: sourceFrame.kind,
        sourceSignature: sourceFrame.sourceSignature,
        targetEntries: Object.freeze(targetEntries.map((entry) => Object.freeze({ ...entry }))),
        targetSignature: [
            sourceFrame.sourceSignature,
            getPatientivoRootStockEntrySignature(targetEntries),
        ].join("|"),
        consumesRenderedInput: false,
        displayStringsAuthorizeGrammar: false,
    });
...
function getPatientivoRootStockStemFrameMismatch({
    ...
    if (
        !operationFrame
        || operationFrame.kind !== "andrews-typed-operation-frame"
        || operationFrame.operationId !== "andrews-patientivo-root-stock-stem-realization"
        || operationFrame.routeFamily !== "patientivo-root-stock-source"
        || operationFrame.operationApplied !== "realize-patientive-root-stock-stems-from-source-frame"
        || operationFrame.consumesRenderedInput !== false
        || operationFrame.displayStringsAuthorizeGrammar !== false
        || !Array.isArray(operationFrame.targetEntries)
    ) {
    ...
    const expectedTargetSignature = [
        sourceFrame.sourceSignature,
        getPatientivoRootStockEntrySignature(expectedEntries),
    ].join("|");
    const actualTargetSignature = [
        sourceFrame.sourceSignature,
        getPatientivoRootStockEntrySignature(operationFrame.targetEntries),
    ].join("|");
    if (
        operationFrame.targetSignature !== expectedTargetSignature
        || actualTargetSignature !== expectedTargetSignature
...
function buildPatientivoTroncoDerivations({
    ...
    const addTypedRootStockEntry = (targetEntry = null, operationFrame = null, sourceFrame = null) => {
        if (
            !targetEntry
            || getPatientivoRootStockStemFrameMismatch({ sourceFrame, operationFrame })
        ) {
            return;
        }
    ...
    if (
        ...
        if (
            rootStockSourceFrame?.rightEdgeFrame?.routeOperation === "iwi-family-root-stock-stem"
            || rootStockSourceFrame?.rightEdgeFrame?.routeOperation === "iwi-final-cluster-root-stock-stem"
            || rootStockSourceFrame?.rightEdgeFrame?.routeOperation === "awi-family-root-stock-stem"
            || rootStockSourceFrame?.rightEdgeFrame?.routeOperation === "awi-final-cluster-root-stock-stem"
            || rootStockSourceFrame?.rightEdgeFrame?.routeOperation === "ua-family-root-stock-stem"
            || rootStockSourceFrame?.rightEdgeFrame?.routeOperation === "ua-final-cluster-root-stock-stem"
        ) {
            const rootStockOperationFrame = buildPatientivoRootStockStemOperationFrame(rootStockSourceFrame);
            if (!getPatientivoRootStockStemFrameMismatch({
                sourceFrame: rootStockSourceFrame,
...
function resolveNonactiveStemSelection(verb, analysisVerb, options = {}) {
    ...
    if (selectedSuffix && optionMap.has(selectedSuffix)) {
        ...
        const nonactiveStemSelectionOperationFrame = buildNonactiveStemSelectionOperationFrame(
        ...
        selectedStemSpec = selectedStem
            ? (nonactiveStemSelectionOperationFrame?.targetStemSpec || buildLiteralMorphStemSpec(selectedStem))
            : null;
