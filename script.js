...
function applyStaticPhonology(data) {
    ...
    if (typeof data.vowels === "string") {
        ...
        VOWEL_GLOBAL_RE = new RegExp(`[${VOWELS}]`, "g");
...
function applyStaticSuppletives(data) {
    ...
    if (witziData) {
        ...
        if (Array.isArray(witziData.activeTenses)) {
            SUPPLETIVE_WITZI_TENSES = new Set(witziData.activeTenses);
        }
        ...
        const witziImperative = witziTenses.imperativo && typeof witziTenses.imperativo === "object"
            ...
            : {};
        const witziRemoto = witziTenses["pasado-remoto"] && typeof witziTenses["pasado-remoto"] === "object"
            ? witziTenses["pasado-remoto"]
            : {};
        if (typeof witziImperative.verb === "string") {
            SUPPLETIVE_WITZI_IMPERATIVE = witziImperative.verb;
        }
        if (witziRemoto.suffixOverrides && typeof witziRemoto.suffixOverrides === "object") {
            SUPPLETIVE_WITZI_TENSE_SUFFIX_OVERRIDES = { "pasado-remoto": { ...witziRemoto.suffixOverrides } };
        }
    ...
    SUPPLETIVE_STEM_PATHS = data.paths.map((entry) => {
        ...
        if (matchConfig.type === "yawi") {
            match = (parsedVerb) => {
                ...
                if (parsedVerb.directionalPrefix) {
                    return false;
                }
...
function applyNonspecificObjectAllomorphy({
    ...
    if (hasOptionalSupportiveI) {
        if (!hasNonspecificMarker && !nextAnalysis.startsWith("i")) {
            const extended = `i${nextAnalysis}`;
            const updated = replaceAnalysisSuffix(nextVerb, nextAnalysis, extended);
            nextVerb = updated.verb;
            nextAnalysis = updated.analysisVerb;
        } else if (hasNonspecificMarker && nextAnalysis.startsWith("i")) {
...
function getNonactiveBaseInfo(ruleBase) {
    ...
    const endsWithConsonantCluster = isVerbLetterVowel(last)
        && isVerbLetterConsonant(prev)
...
function deriveNonactiveStem(verb, analysisVerb, options = {}) {
    ...
    const luCandidate = (endsWithA || endsWithU || (isTransitive && endsWithTV))
        && (!uCandidate
            || allowLuVariantTzV
            || allowLuVariantTV
            || allowLuVariantIntransA
            || allowLuVariantIntransU)
        && (!uwaCandidate || allowLuVariantIntransA || allowLuVariantIntransU);
    ...
    const allowWaluVariant = ruleBase === "kwi"
        ...
        || (!isTransitive && (syllableCount === 1 || nonRedupSyllableCount === 1))
        || (isTransitive && isVowelMonosyllable);
    if (allowWaluVariant) {
        return `${source}walu`;
    }
...
function buildPatientivoDerivations({
    ...
    const source = analysisVerb || rawAnalysisVerb || verb || "";
    if (!source) {
        return [];
    }
    let base = source;
    ...
    if (!prefix && !hasImpersonalTaPrefix && verb) {
        const prefixCandidates = [rawAnalysisVerb, analysisVerb].filter(Boolean);
...
function getNonactiveDerivationOptions(verb, analysisVerb, options = {}) {
    ...
    const allowSVariant = prev === "s";
    const allowTzVariant = prev === "tz";
    const allowIntransitiveMiVariant = !isTransitive && endsWithMi;
    ...
    const allowLuVariantIntransA = !isTransitive && endsWithA;
    const allowLuVariantIntransU = !isTransitive && endsWithU;
    const luCandidate = (endsWithA || endsWithU)
        && (!uCandidate
            || allowLuVariantTzV
            || allowLuVariantTV
            || allowLuVariantIntransA
    ...
    const replaceFinalConsonantVariant = (stem, suffix, from, to) => {
        const tail = suffix === "uwa" ? "uwa" : "u";
        const target = `${from}${tail}`;
        if (!stem.endsWith(target)) {
            return "";
        }
        return `${stem.slice(0, -target.length)}${to}${tail}`;
    };
    const pushWithVariants = (suffix, stem) => {
        push(suffix, stem);
        if (!stem || (suffix !== "u" && suffix !== "uwa")) {
            return;
        }
        if (allowSVariant) {
            const sVariant = replaceFinalConsonantVariant(stem, suffix, "sh", "s");
            if (sVariant) {
                push(suffix, sVariant);
            }
        }
        if (allowTzVariant) {
            const tzVariant = replaceFinalConsonantVariant(stem, suffix, "ch", "tz");
            if (tzVariant) {
                push(suffix, tzVariant);
            }
        }
    };
    ...
    const allowLuVariant = isTransitive
        && (endsWithNa || endsWithNi || endsWithSa || endsWithTa || endsWithTi || endsWithTV);
...
function getInvalidVerbStructure(rawValue) {
    const raw = String(rawValue || "").toLowerCase();
...
function isVerbValueAllowed(rawValue) {
    return (
        ...
        && !getInvalidVerbStructure(rawValue)
    );
}
...
function adjustPatientivoPossessiveSuffix(suffix, isPossessed) {
    if (!isPossessed || !suffix) {
        return suffix || "";
    }
    if (suffix.endsWith("met")) {
        return `${suffix.slice(0, -3)}wan`;
    }
...
function buildPretUniversalContext(verb, analysisVerb, isTransitive, options = {}) {
    ...
    const rootPlusYaBase = getRootPlusYaBase(rootPlusYaSource, { isTransitive, isYawi, isWeya });
    ...
    const allowUnpronounceableStems = verbOverride?.allowUnpronounceableStems === true;
    ...
    const endsWithNaOrNi = lastNucleus === "a" || lastNucleus === "i";
    const isExactVnV = isExactVnVRaw && !isExactVna && !endsWithNaOrNi;
    const isExactCVnV = isExactCVnVRaw && !isExactCVna && !isExactCVni && !endsWithNaOrNi;
...
function getPretUniversalClassCandidates(context, trace = null) {
    ...
    const isLiEndingCandidate = !context.isTransitive
        ...
        && (context.isExactVlV || context.isExactCVlV || context.endsWithLV);
...
function buildPretUniversalClassA(context) {
    ...
    const allowVtVStartClassA = context.allowVtVStartClassA === true;
...
function buildPretUniversalClassB(context) {
    ...
    const rootPlusYaBase = disallowRootPlusYa
        ? null
        : getRootPlusYaBase(context.verb, {
            isTransitive: context.isTransitive,
...
let SUPPLETIVE_WITZI_TENSE_SUFFIX_OVERRIDES = {};
...
function getVerbInputMeta() {
    ...
    return getParsedVerbForTab("verb-input", raw);
}
...
function prepareSearchToggleState({ mode, group, tenseValue }) {
    ...
    if (mode === TENSE_MODE.sustantivo) {
        ...
        const allowedPrefixes = getAllowedNounObjectPrefixesFromMeta(verbMeta, tenseValue);
        const objectStateKey = getObjectStateKey({ groupKey, tenseValue, mode: "noun" });
        const fallbackObject = getPreferredNounObjectPrefix(allowedPrefixes);
...
function buildPretUniversalFlagsForContext(context, flags) {
    ...
    if (candidates.has("A")) {
        ...
        if (variants) {
            ...
            if (context.isTransitive && context.isItaVerb && context.analysisVerb === "ita") {
...
function buildBulkExportRow({ verb, isTransitive }) {
    const objectPrefix = isTransitive ? "ki" : "";
    const preterit3s = getBulkExportConjugation({
        verb,
        tense: "preterito",
        subjectSuffix: "",
        objectPrefix,
    });
    const preterit3pl = getBulkExportConjugation({
        verb,
        tense: "preterito",
        subjectSuffix: "t",
        objectPrefix,
    });
    const perfect3s = getBulkExportConjugation({
        verb,
        tense: "perfecto",
        subjectSuffix: "",
        objectPrefix,
    });
    const perfect3pl = getBulkExportConjugation({
        verb,
        tense: "perfecto",
        subjectSuffix: "t",
        objectPrefix,
...
function updateBulkExportStatusSummary() {
    ...
    const missingRows = rows.filter((row) => (
        [row.preterit3s, row.preterit3pl, row.perfect3s, row.perfect3pl]
...
function updateBulkExportRowData(index, rawValue) {
    ...
    row.preterit3s = updated.preterit3s;
    row.preterit3pl = updated.preterit3pl;
    row.perfect3s = updated.perfect3s;
    row.perfect3pl = updated.perfect3pl;
    updateBulkExportControlState();
    return normalized;
}
...
function renderBulkExportTable(container, rows) {
    ...
    rows.forEach((row, index) => {
        ...
        const preterit3sCell = document.createElement("td");
        preterit3sCell.textContent = row.preterit3s;
        tr.appendChild(preterit3sCell);
        const preterit3plCell = document.createElement("td");
        preterit3plCell.textContent = row.preterit3pl;
        tr.appendChild(preterit3plCell);
        const perfect3sCell = document.createElement("td");
        perfect3sCell.textContent = row.perfect3s;
        tr.appendChild(perfect3sCell);
        const perfect3plCell = document.createElement("td");
        perfect3plCell.textContent = row.perfect3pl;
        tr.appendChild(perfect3plCell);
        const syncCells = () => {
            preterit3sCell.textContent = row.preterit3s;
            preterit3plCell.textContent = row.preterit3pl;
            perfect3sCell.textContent = row.perfect3s;
...
function getLocalizedLabel(entry, isNawat, fallback = "") {
    ...
    if (typeof entry === "object") {
        return isNawat
            ? (entry.labelNa || entry.labelEs || fallback)
            : (entry.labelEs || entry.labelNa || fallback);
    }
    return fallback;
}
    ...
    if (selectedLanguage === "nawat") {
      labelElementIds.forEach(function(elementId) {
        ...
        if (labelElement) {
          ...
          labelElement.textContent = localized;
        }
      });
    } else {
      labelElementIds.forEach(function(elementId) {
...
function applyMorphologyRules({
    ...
    if (tense === "sustantivo-verbal") {
        ...
        if (isIntransitive && verb.endsWith("ya") && sustantivoLetterCount > 2) {
            verb = verb.slice(0, -2);
        }
        const nounBase = verb;
    ...
    if (tense === "patientivo") {
        ...
        const derivations = buildPatientivoDerivations({
            verb,
            analysisVerb,
            rawAnalysisVerb,
            isTransitive,
...
/* GRAMATICAL RULES */
    ...
    const rootPlusYaBase = isPerfectiveTense(tense) && !disallowRootPlusYa
...
function applyNonactiveDerivation({
    ...
    if (!selection.selectedSuffix && selection.allStems.length > 1) {
        ...
        nonactiveAllStems = nonactiveSource.prefix
            ? selectedStems.map((stem) => `${nonactiveSource.prefix}${stem}`)
            : selectedStems;
    }
...
function generateWord(options = {}) {
    ...
    if (isPatientivoPossessed) {
        subjectSuffix = adjustPatientivoPossessiveSuffix(subjectSuffix, true);
    }
    ...
    if (
        ...
        const collectFormsForStem = (stem) => {
            ...
            if (tense === "patientivo" && Boolean(possessivePrefix)) {
                localSubjectSuffix = adjustPatientivoPossessiveSuffix(localSubjectSuffix, true);
            }
            ...
            localAlternates.forEach((form) => {
                ...
                const altSuffix = (tense === "patientivo" && Boolean(possessivePrefix))
                    ? adjustPatientivoPossessiveSuffix(form.subjectSuffix ?? localSubjectSuffix, true)
